...
description: Create a new Claude Code skill tailored to the Citadel Terminal project. Use when the user wants to formalize a repeated workflow into an invocable `/<name>` skill — e.g. "make a skill for profiling algo turn time", "add a skill that pulls my latest ranked replay", "create a skill that runs opponent-specific regression matches".
---

# skill-creator-terminal

Creates a new skill under `.claude/skills/<name>/SKILL.md` with the right shape for this project. Keeps skill definitions consistent and well-integrated with the existing tooling (`tools/`, `docs/`, `algos/`).
...
- User is making the repo more Claude-friendly ("add a skill for X").

## Skill anatomy

Every skill in this project is a directory under `.claude/skills/<kebab-name>/` containing at minimum `SKILL.md` with YAML frontmatter:

```yaml
---
name: kebab-name
description: One sentence explaining what the skill does AND when Claude should invoke it. Good descriptions start with a verb and name common trigger phrases ("Use when the user says X, Y, or Z").
---
```

The body of `SKILL.md` is a **runbook**: concrete steps Claude should follow when the skill is invoked. Write it like an SOP for a colleague.
...
3. **Check for overlap** with existing skills under `.claude/skills/`. If there's significant overlap, suggest extending the existing skill instead.
...
5. **Write `SKILL.md`** using the template above. Key quality bars:
   - The description must be specific enough for Claude to know *when* to invoke (listing trigger phrases is the single biggest lever).
...
7. **Add a permission entry** in `.claude/settings.json` under `permissions.allow` if the skill runs a new Bash command pattern. Example: `"Bash(python3 tools/new_helper.py:*)"`.

8. **Update `README.md`**'s skill table to include the new skill.

9. **Test it** by writing a trigger phrase and confirming Claude would pick this skill over others (mentally; no runtime test).
...
- **Reference, don't duplicate.** Link to `docs/<file>.md` instead of inlining rules. The docs are the source of truth; skills are how Claude finds and uses them.
...
- [ ] If new permissions are needed, `.claude/settings.json` is updated.
