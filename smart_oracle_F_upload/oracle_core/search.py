"""Search loop for smart_oracle_F — IS6's Variant F + funnel-aware filtering.

IS6 (Variant F) combines:
...
- IS6: probe-driven smart-refund template generator (in enumerator.py)

VC was REMOVED based on live ladder evidence (caused regression on
pipmy/Redemption). smart_oracle_F preserves that decision — no VC.

smart_oracle_F additionally adds a funnel detector:
  When the algo_strategy passes a `funnel_target` ∈ {"left","right"},
  search restricts the opponent samples it evaluates against to those
  whose spawn launcher routes to that flank, and forwards `funnel_target`
  to the enumerator so flank-corridor defense templates appear in the
  candidate pool. The filter has a fallback — if it would zero out the
  sample set we revert to the unfiltered list. The search algorithm
  itself is unchanged.
"""
...
class SearchTelemetry:
    ...
    # smart_oracle_F: which flank zone (if any) the funnel detector
...
def search(game_state, config, opp_model: OpponentModel, *,
    ...
    Variant F = VA + VB + IS6 smart-refund (no VC coverage term).

    smart_oracle_F additions:
      funnel_target: when not None, search restricts opp samples to those
        whose spawn launcher routes to the funnel flank, and forwards
        funnel_target to the enumerator so flank-corridor templates
        appear in the candidate pool. Filter has a fallback — if it
        zeros the sample set we revert to unfiltered.
    """
    t0 = time.time()
    tel = SearchTelemetry()
    ...
    # smart_oracle_F: opp-sample filter — fires only when funnel_target is
    # set, which (as of algo_strategy gating) requires the OpponentClassifier
    # to have determined the opp is single_archetype. So this filter only
    # narrows samples when we have positive evidence the opponent IS doing
    # the funnel attack the filter assumes. Multi-archetype opps never get
    # filtered (algo_strategy passes funnel_target=None for them).
    if funnel_target:
