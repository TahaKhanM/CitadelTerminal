...
# but never reaches MIN_BREACHES=10, so this gate is redundant against
# that pattern; included for principled defense against future opps.
...
def detect_funnel_target(recent_breaches: Sequence[Tuple[int, int]]
    ...
    A funnel is declared when ALL of:
      ...
      - at least FUNNEL_DOMINANCE_FRAC of the last FUNNEL_LOOKBACK breaches
        fall in the same flank zone (LEFT or RIGHT), AND
    ...
    CENTER is intentionally excluded — center breaches are consistent with
    a balanced rush, not a funnel.
    """
