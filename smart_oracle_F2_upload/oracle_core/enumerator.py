...
# ---------------------------------------------------------------------------
# Deep-clamp atoms (smart_oracle_F2) — added based on live-ladder loss
# analysis. The existing FLANK_CORRIDOR templates cover the mid-flank
# breach tiles (3,10/4,9/5,8 left, 23,9/24,10 right) but MISS:
#   - Far-LEFT corner (1,12)/(2,11)/(0,13): hit by funnel-rush-v6 ×20
#   - Far-RIGHT corner (25,11)/(26,12): hit by fluffffy ×15+
#   - Deep-CENTER tile (16,2) and similar: hit by funnel-rush-v6 ×20
# These are the tiles where 3 of 4 live losses landed. Templates below
# cluster turrets+walls around those tiles. Only added when funnel
# detector returns the matching zone AND classifier says single_archetype.
# ---------------------------------------------------------------------------

# Far-LEFT clamp — covers (0,13)/(1,12)/(2,11) corner
LEFT_DEEP_CLAMP = [
    (TURRET_IDX, 1, 12),
    (TURRET_IDX, 2, 11),
    (TURRET_IDX, 1, 13),
...
LEFT_DEEP_CLAMP_UPGRADES = [(1, 12), (2, 11)]

# Far-RIGHT clamp — covers (25,11)/(26,12)/(27,13) corner
RIGHT_DEEP_CLAMP = [
    (TURRET_IDX, 26, 12),
    (TURRET_IDX, 25, 11),
    (TURRET_IDX, 26, 13),
...
RIGHT_DEEP_CLAMP_UPGRADES = [(26, 12), (25, 11)]

# Deep-CENTER clamp — covers (16,2) area (deep penetration tile that
# opp's mobiles reach after walking through our front lines).
CENTER_DEEP_CLAMP = [
    (TURRET_IDX, 16, 3),
    (TURRET_IDX, 12, 3),
    (TURRET_IDX, 14, 3),
...
CENTER_DEEP_CLAMP_UPGRADES = [(16, 3), (12, 3), (14, 3)]
...
def _enumerate_defense_templates(game_state, config, sp_budget: float,
    ...
    if funnel_target == "left":
        ...
        if upgrade_candidates:
            ...
            if p:
                plans.append(p)
        # smart_F2: deep-LEFT clamp (covers far-corner (0,13)/(1,12)/(2,11)
        # — where funnel-rush-v6 landed 20+ breaches that LEFT_FLANK_CORRIDOR
        # missed)
        p = _build_defense_plan(
            "defense:left_deep_clamp",
            LEFT_DEEP_CLAMP + ANCHOR_TURRETS,
            game_state, config, sp_budget,
            upgrades=LEFT_DEEP_CLAMP_UPGRADES,
            archetype="left_deep_clamp",
        )
        if p:
            plans.append(p)
        # Combined corridor + deep-clamp (most expensive but fully covers
        # entire LEFT side from corner to mid-flank)
        p = _build_defense_plan(
            "defense:left_full_clamp",
            LEFT_DEEP_CLAMP + LEFT_FLANK_CORRIDOR + ANCHOR_TURRETS,
            game_state, config, sp_budget,
            upgrades=LEFT_DEEP_CLAMP_UPGRADES + LEFT_FLANK_UPGRADES,
            archetype="left_full_clamp",
        )
        if p:
            plans.append(p)
        ...
        if upgrade_candidates:
            ...
            if p:
                plans.append(p)
        # smart_F2: deep-RIGHT clamp (covers far-corner (25,11)/(26,12)/
        # (27,13) — where fluffffy landed 15+ breaches per top tile)
        p = _build_defense_plan(
            "defense:right_deep_clamp",
            RIGHT_DEEP_CLAMP + ANCHOR_TURRETS,
            game_state, config, sp_budget,
            upgrades=RIGHT_DEEP_CLAMP_UPGRADES,
            archetype="right_deep_clamp",
        )
        if p:
            plans.append(p)
        p = _build_defense_plan(
            "defense:right_full_clamp",
            RIGHT_DEEP_CLAMP + RIGHT_FLANK_CORRIDOR + ANCHOR_TURRETS,
            game_state, config, sp_budget,
            upgrades=RIGHT_DEEP_CLAMP_UPGRADES + RIGHT_FLANK_UPGRADES,
            archetype="right_full_clamp",
        )
        if p:
            plans.append(p)
    elif funnel_target == "center":
        # smart_F2: CENTER funnel response (e.g. ameyg/v6 drilling (16,2)).
        # Cluster turrets+walls around the deep penetration tile.
        p = _build_defense_plan(
            "defense:center_deep_clamp",
            CENTER_DEEP_CLAMP + ANCHOR_TURRETS,
            game_state, config, sp_budget,
            upgrades=CENTER_DEEP_CLAMP_UPGRADES,
            archetype="center_deep_clamp",
        )
        if p:
            plans.append(p)
        # Lighter variant (budget-friendly mid-game)
        p = _build_defense_plan(
            "defense:center_deep_clamp_min",
            [(TURRET_IDX, 16, 3), (WALL_IDX, 16, 2), (TURRET_IDX, 14, 3)] + ANCHOR_TURRETS,
            game_state, config, sp_budget,
            upgrades=[(16, 3)],
            archetype="center_deep_clamp_min",
        )
        if p:
            plans.append(p)
