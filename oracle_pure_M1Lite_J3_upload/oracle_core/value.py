"""Value function for oracle_pure - VARIANT J3 (D + Path-Prediction).

Builds on VD (Hybrid). Additions over VD:
- J3: Path-interception score via `path_intercept_score` param. The
      score is a sim-probe result: spawn opp scouts at their RECENT
      observed launcher positions, count interceptions, return
      kill_ratio in [0, 1]. Higher = our defense intercepts opp's
...
All optional params default to None — back-compat with M1Lite behavior.
"""
...
VALUE_WEIGHTS = {
    ...
    # J3 — path-interception bonus (per kill_ratio unit in [0,1])
    # Weight 50.0 = at perfect interception (1.0), the term contributes
    # +50 to the score; at total failure (0.0), it contributes 0.
    # The score is centered on 0.5 (neutral, no data) so the effective
    # range is roughly +/-25 in practice.
    "path_intercept_weight": 50.0,
}
...
def evaluate(state_dict: Dict[str, Any], my_player: int = 1,
             *,
             ...
             path_intercept_score: Optional[float] = None,
             ) -> float:
    ...
    Args:
        ...
        path_intercept_score: J3 — kill ratio in [0, 1] from path-prediction
            probe (1.0 = our defense intercepts ALL opp probe scouts;
            0.0 = none intercepted; None = no data, term skipped).
    """
    if weights is None:
    ...
    # J3: Path-interception bonus.
    # Center the score on 0.5 so neutral data (no launcher info) contributes
    # nothing, perfect interception (1.0) adds +0.5*weight, total failure
    # (0.0) subtracts -0.5*weight. This produces a SIGNED reward — defenses
    # that fail vs opp's pattern are penalized, defenses that succeed are
    # boosted.
    path_intercept_term = 0.0
    if path_intercept_score is not None:
        weight = weights.get("path_intercept_weight", 50.0)
        # Subtract 0.5 to center: 0.5 (neutral) → 0; 1.0 (perfect) → +0.5;
        # 0.0 (failure) → -0.5. Multiply by weight.
        path_intercept_term = weight * (float(path_intercept_score) - 0.5)
