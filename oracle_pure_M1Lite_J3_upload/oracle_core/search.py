"""Search loop for oracle_pure - VARIANT J3 (D + Path-Prediction).

Builds on VD. Adds:
- J3: Path-interception probe per phase-2 unique defense template.
      Spawns opp scouts at recently-observed launcher positions,
      counts kills, passes kill_ratio to evaluate(). Cached per
      defense template (top ~15 templates) so the cost is bounded.

Carried over from VD:
...
- VC (I4): breach-pressure coverage (passes breach_pressure to evaluate)
"""
...
from .path_intercept_probe import (
    PROBE_NEUTRAL_SCORE, probe_path_interception,
)
...
def search(game_state, config, opp_model: OpponentModel, *,
           ...
           opp_recent_launchers: Optional[Sequence[Tuple[Tuple[int, int], float]]] = None,
           config_path: Optional[str] = None,
           debug_log=None,
           ) -> SearchResult:
    """Pick the maximum-EU plan for the current turn.

    Variant J3 additions vs VD:
      opp_recent_launchers: Sequence of ((x, y), weight) describing opp's
        recent observed launcher positions. Used by the path-prediction
        probe in phase 2: spawn opp scouts at top-k launchers, count
        breaches, pass kill_ratio to evaluate() as path_intercept_score.
        None or empty = probe is skipped (signal degenerates to neutral).

    Variant D additions vs M1Lite:
      breach_pressure: VC — per-tile soft pressure map for coverage scoring.
    """
    t0 = time.time()
    tel = SearchTelemetry()
    ...
    # J3: Path-interception probe per unique defense template that
    # survived phase-1 into phase-2. Cached so each unique defense layout
    # runs the probe at most once. Skipped entirely if opp_recent_launchers
    # is empty (early game / no observation data).
    path_intercept_probes: Dict[str, float] = {}
    j3_active = bool(opp_recent_launchers)
    if j3_active:
        for _, cand in top_for_phase2:
            if time.time() > deadline:
                break
            def_name = cand.name.split("|", 1)[0]
            if def_name in path_intercept_probes:
                continue
            defense_only = ActionPlan(name=def_name, archetype=cand.archetype)
            for op in cand.ops:
                if op.kind != KIND_SPAWN_MOBILE:
                    defense_only.ops.append(op)
            try:
                kill_ratio = probe_path_interception(
                    base_state, defense_only, opp_recent_launchers,
                    config, _probe_runner, my_player=1,
                )
                tel.sims_run += 1
            except Exception as e:
                debug_log(f"[search] j3 path-intercept probe failed for {def_name}: {e!r}")
                kill_ratio = PROBE_NEUTRAL_SCORE
            path_intercept_probes[def_name] = kill_ratio
    ...
    for score1, cand in top_for_phase2:
        if time.time() > deadline:
            ...
            break
        sample_scores = []
        # Look up J3 probe result for this candidate's defense template.
        def_name = cand.name.split("|", 1)[0]
        path_intercept_score = (path_intercept_probes.get(def_name)
                                if j3_active else None)
        for opp_sample in opp_samples_phase2:
            ...
            try:
                ...
                score = evaluate(post, my_player=1,
                                 ...
                                 defense_viability=cand.defense_viability,
                                 breach_pressure=breach_pressure,
                                 path_intercept_score=path_intercept_score)
    ...
    if do_depth2 and time.time() < deadline:
        ...
        for score, cand in depth2_top:
            ...
            try:
                ...
                tel.sims_run += 1
                # J3 path-intercept score for depth-2 candidate
                d2_def_name = cand.name.split("|", 1)[0]
                d2_path_score = (path_intercept_probes.get(d2_def_name)
                                 if j3_active else None)
                s1 = evaluate(post1, my_player=1,
                              pre_state_dict=base_state,
                              defense_viability=cand.defense_viability,
                              breach_pressure=breach_pressure,
                              path_intercept_score=d2_path_score)
