...
VARIANT J3: D + Path-Prediction.

Adds opp recent launcher tracking. on_action_frame records each opp
mobile spawn's tile with a fresh weight = 1.0; the per-turn decay
rate (LAUNCHER_DECAY = 0.85) fades old launchers. Top-3 launchers
by weight are passed to search() and used by the path-prediction
probe to score how well each candidate defense intercepts opp's
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    # Variant J3: per-tile opp launcher tracking. Each opp mobile spawn
    # observed via on_action_frame adds +1.0 to that tile's weight. Decay
    # runs once per turn. Top-K launchers (by weight) are passed to search
    # and used by the path-prediction probe to score how well candidate
    # defenses intercept opp's actual attack pattern.
    #
    # Decay rate 0.85 → after 5 turns of no observation, weight drops to
    # 0.85^5 = 0.444 (about half-life ~ 4.3 turns). Slightly faster than
    # breach_pressure (0.9) because launcher choice can shift turn-to-turn
    # while breach success is more sticky.
    LAUNCHER_DECAY = 0.85
    LAUNCHER_PRUNE_BELOW = 0.05  # drop tiles whose weight faded out
    LAUNCHER_TOP_K = 3            # number of launchers to pass to search
    ...
    def __init__(self):
        ...
        # Variant J3: opp launcher tracking
        self.opp_launcher_weights: dict = {}  # {(x,y): float}
        self._last_launcher_decayed_turn = -1  # idempotent decay per turn
        self._turn_count = 0
    ...
    def on_turn(self, turn_state):
        ...
        try:
            ...
            # Variant J3 — decay opp launcher weights once per turn.
            if cur_turn != self._last_launcher_decayed_turn:
                if self.opp_launcher_weights:
                    decayed = {}
                    for xy, w in self.opp_launcher_weights.items():
                        new_w = w * self.LAUNCHER_DECAY
                        if new_w >= self.LAUNCHER_PRUNE_BELOW:
                            decayed[xy] = new_w
                    self.opp_launcher_weights = decayed
                self._last_launcher_decayed_turn = cur_turn

            # Variant J3 — top-K launchers passed to search. Sort by weight
            # descending; ties broken by tile position (deterministic).
            if self.opp_launcher_weights:
                top_launchers = sorted(
                    self.opp_launcher_weights.items(),
            ...
            result = search(
                ...
                opp_recent_launchers=top_launchers,
            ...
            # Variant J3 telemetry: log top launchers
            if top_launchers:
                gamelib.debug_write(
                    f"[oracle_pure] j3_launchers n={len(self.opp_launcher_weights)} "
                    f"top={top_launchers}"
                )
            gamelib.debug_write(
    ...
    def on_action_frame(self, turn_string):
        ...
        # Variant J3 — track opp mobile spawn launchers across turns.
        # Each opp mobile spawn (Scout/Demolisher/Interceptor) increments
        # that tile's weight by +1.0. Decay runs once per turn at the
        # start of on_turn (separate from this hot path).
        try:
            for ev in (frame.get("events", {}).get("spawn", []) or []):
                if not ev or len(ev) < 4:
                    continue
                loc, utype, _uid, owner = ev[:4]
                # Action-frame convention: 1=us, 2=opp
                if int(owner) != 2:
                    continue
                ut = int(utype)
                # Only track MOBILE units (Scout=3, Demolisher=4, Interceptor=5)
                if ut not in (3, 4, 5):
                    continue
                try:
                    ...
                    continue
                # Only valid spawn-edge tiles in the top half (y >= 14)
                if y < 14:
                    continue
                xy = (x, y)
                self.opp_launcher_weights[xy] = \
                    self.opp_launcher_weights.get(xy, 0.0) + 1.0
        except Exception:
            pass
