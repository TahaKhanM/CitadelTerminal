"""Value function for oracle_pure - VARIANT J1 (Transit-Pressure Spatial).

Combines four independent improvements over M1Lite:
- VA (I3): Wasted-MP penalty via `pre_state_dict` param
- VB (I2): Trap penalty via `defense_viability` param (sim-based probe result)
- VC (I4): Breach-pressure coverage via `breach_pressure` param (legacy fallback)
- J1: Transit-pressure spatial value with adaptive weight. The caller
  (algo_strategy.py:_build_adaptive_pressure) passes a PRE-WEIGHTED dict
  via the existing `breach_pressure` param — so this file is structurally
  unchanged. The weight is computed FROM observed data: when opp's
  mobile transit is concentrated on a single path (peak/mean is high),
  the dict values are scaled up; when uniform, they stay at baseline.

All extra params default to None — back-compat with M1Lite behavior.
"""
