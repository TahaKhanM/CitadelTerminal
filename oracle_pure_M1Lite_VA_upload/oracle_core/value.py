...
VALUE_WEIGHTS = {
    ...
    # I3: wasted-MP penalty (Variant A).
    # Fires when this turn's mobiles spent >= waste_mp_threshold MP but
    # caused minimal damage to opponent. Specifically targets the "trap"
    # bug where supports at (12,11)/(15,11) seal the wall row gaps,
    # causing all our mobiles to walk to (2,11)/(25,11) and self-destruct
    # in our own territory. See replay 15314226 (587/782 mobile deaths
    # in own territory; 1018 MP spent for 3 breaches).
    #
    # damage_caused = opp_hp_lost + struct_dmg_coef * opp_struct_value_lost
    # waste_unjustified = max(0, mp_spent - waste_k * damage_caused)
    # waste_term = -waste_w * waste_unjustified
    #
    # Calibration (start point):
    # - Trap (22 MP, 0 dmg): waste_term = -waste_w * 22  (kills the trap
    #   plan IF -waste_w*22 > +40 struct_term gain → waste_w >= 2)
    # - Successful demo rush (10 MP, 5 HP breach): mp - 1*5 = 5 unjustified
    #   → waste_term = -25 (small enough that hp_term=+500 still wins)
    # - Defensive interceptor screen that absorbs 5 scouts (10 MP spent,
    #   0 damage to opp HP, 5 HP saved by absorbing): waste fires (-50)
    #   but the +500 hp_term from the saved HP DOMINATES.
    "waste_w": 5.0,         # penalty weight per unjustified MP
    "waste_k": 1.0,         # 1 HP-damage-equivalent justifies 1 MP
    "waste_mp_threshold": 5.0,  # don't penalize tiny exploratory probes (< 5 MP)
    "waste_struct_dmg_coef": 0.5,  # struct value loss → damage units (less than HP)
}
...
def evaluate(state_dict: Dict[str, Any], my_player: int = 1,
             *,
             ...
             pre_state_dict: Dict[str, Any] = None,
             ) -> float:
    ...
    Args:
        ...
        pre_state_dict: optional pre-sim (post-deploy) state. Required for
            the I3 wasted-MP penalty. When provided we compute MP spent
            and damage caused to detect "trap" plans where mobiles spawn
    ...
    # Detects "trap" plans where we spawn many mobiles that fail to deal
    # damage. The trap signature: mobiles SD in own territory because
    # supports at (12,11)/(15,11) seal the wall row gaps. We can't see the
    # SD events from post-state alone, but we CAN see "lots of MP spent +
    # zero damage caused" which is the same signal.
    #
    # Important calibration considerations:
    # 1. Threshold (waste_mp_threshold) prevents penalizing exploratory probes.
    # 2. damage_caused includes opp HP loss AND opp structure loss, so
    #    plans that destroy opp turrets without breaching still get credit.
    # 3. Defensive interceptors that ABSORB opp scouts technically cause
    #    no opp damage. These would be penalized, BUT the HP they SAVE
    #    is reflected in hp_term (+100 per HP) which is FAR larger than
    #    the waste penalty (~5 per MP). The penalty fires but doesn't
    #    dominate the legitimate defensive value.
    # 4. By averaging over k_opp samples in phase 2, the search naturally
    #    finds plans where waste fires occasionally but is outweighed by
    #    the average HP saved.
    waste_term = 0.0
    if pre_state_dict is not None:
        pre_my_mp = float(pre_state_dict.get(side_key, {}).get("mp", 0))
        post_my_mp = my_mp  # already extracted above
        mp_spent_self = max(0.0, pre_my_mp - post_my_mp)
        if mp_spent_self >= weights.get("waste_mp_threshold", 5.0):
            # Damage caused to opponent
            pre_opp_hp = float(pre_state_dict.get(opp_key, {}).get("hp", opp_hp))
            opp_hp_lost = max(0.0, pre_opp_hp - opp_hp)
            # Opp structure value lost (structures destroyed by our mobiles)
