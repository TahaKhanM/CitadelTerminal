...
# Variant A — I7: variance-aware ranking constant.
# Plans are ranked by (mean_score - VARIANCE_LAMBDA * std_score) to
# penalize plans whose value depends on a single fortunate opp sample.
# Higher lambda = more risk-averse. lambda=0 reproduces M1Lite behavior.
# Calibration: started at 0.3, but combined with I3 caused a Tier A
# regression on lostkids-aet (0/2 vs M1Lite's 2/2) — the search picked
# different defensive priorities late-game and let the right-flank funnel
# through. Tuned down to 0.1 which restores 10/10 Tier A. Sweep tested:
# - 0.0 (I7 disabled): wins lostkids 2/2 (I3-only baseline)
# - 0.1 (current): wins all Tier A 10/10
# - 0.3 (initial): regressed lostkids 0/2
VARIANCE_LAMBDA = 0.1
...
def search(game_state, config, opp_model: OpponentModel, *,
    ...
    #
    # Variant A (I3): we pass `pre_state_dict=base_state` so evaluate()
    # can compute the wasted-MP penalty (detects "trap" plans where
    # mobiles spawn but accomplish nothing).
        ...
        try:
            ...
            for opp_sample in opp_samples_phase1:
                ...
                sample_scores.append(
                    evaluate(post, my_player=1, pre_state_dict=base_state)
                )
            if sample_scores:
    ...
    #
    # Variant A (I7): rank by `mean - VARIANCE_LAMBDA * std` instead of
    # mean alone. Plans whose value depends on a single fortunate opp
    # sample (high variance) are penalized vs plans with consistent
    # outcomes across opp samples. Sample std uses ddof=1.
    phase1_scores.sort(key=lambda t: -t[0])
    top_for_phase2 = phase1_scores[:phase2_top_n]

    # phase2_scores stores (risk_adjusted_score, mean, std, plan)
    ...
    for score1, cand in top_for_phase2:
        ...
        for opp_sample in opp_samples_phase2:
            ...
            try:
                ...
                post = _run_sim(sd, config_path)
                tel.sims_run += 1
                score = evaluate(post, my_player=1, pre_state_dict=base_state)
        ...
        if sample_scores:
            ...
            # Sample std (ddof=1) — only meaningful with 2+ samples.
    ...
    # Re-pack to (score, plan) for compatibility with downstream code.
    # Use the risk-adjusted score for ranking.
    ...
    if do_depth2 and time.time() < deadline:
        ...
        for score, cand in depth2_top:
            ...
            try:
                ...
                # Variant A (I3): pass base_state as pre_state for waste penalty.
                # post1 is post-this-turn; pre is base_state (start of this turn).
                # post2 is post-next-turn projection; we don't have a clean
                # "pre-next-turn" so we use post1 (close enough — captures
                # MP spent on next-turn cheap-scout heuristic).
                s1 = evaluate(post1, my_player=1, pre_state_dict=base_state)
                s2 = evaluate(post2, my_player=1, pre_state_dict=post1)
