...
def _gen_path_block_templates(
    ...
    for tile, pressure in sorted_tiles:
        ...
        for cand in oracle_path[:K2_PATH_SCAN]:
            ...
            if len(candidates) >= K2_TURRETS_PER_TEMPLATE + 1:
                break
        ...
        path_atoms = [(TURRET_IDX, x, y)
                      for x, y in candidates[:K2_TURRETS_PER_TEMPLATE]]
        atoms = (path_atoms +
                 list(ANCHOR_TURRETS) +
                 list(INNER_CORNER_TURRETS) +
                 list(WALL_ROW_Y12))
        ...
        plans.append(plan)
        debug_log(
            f"[oracle_pure M2v3 K2] path_block@({bx},{by}) "
            f"pressure={pressure:.2f} path_len={len(oracle_path)} "
            f"placed_turrets={[c for c in candidates[:K2_TURRETS_PER_TEMPLATE] if c in placed_xys]}"
        )
