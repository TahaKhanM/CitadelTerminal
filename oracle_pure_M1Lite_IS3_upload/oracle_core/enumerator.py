...
# IS3: SUPPORTS_BACK constant DELETED (was the trap-forming list).
# Original positions (12,11)/(15,11) sealed the only y=11 cells adjacent to
# the wall-row launch gaps at (12,12)/(15,12), causing oracle's own scouts
# to self-destruct in own territory at (2,11)/(25,11). Documented in
# CONTEXT_HANDOFF.md §5.2 (suchir-g, not-tnb 100-turn HP-tiebreak losses)
# and §16's recommendation Option A. This deletion is the surgical fix
# the user/prior-session iteratively converged on after M2 (BFS path-check
# + ALT-OUTSIDE swap) regressed live on ameyg matches.
#
# Net effect: oracle loses the support-shield-amplification template, but
# the defense:upg{k} template can still upgrade existing supports, and the
# VF viability_probe + value.py trap-penalty still soft-penalize any other
# trap-creating structures the search proposes.
...
def _enumerate_defense_templates(game_state, config, sp_budget: float,
    ...
    # IS3: defense:supports template DELETED.
    # Was: "Anchor + support (offense-amplifier — supports give shield to scouts)"
    # The SUPPORTS_BACK positions (12,11)/(15,11) sealed the y=12 wall-row
    # launch gaps, trapping oracle's own scouts. See enumerator.py header
    # comment near the deleted SUPPORTS_BACK constant for full rationale.
    ...
    # 6. IS3: defense:supports_only template DELETED.
    # Was: "Support-back-row + minimal defense"
    # Same trap-bug rationale — SUPPORTS_BACK positions seal the wall-row
    # launch gaps. See enumerator.py header comment near the deleted
    # SUPPORTS_BACK constant.
