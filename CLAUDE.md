# Citadel Terminal — Claude Code Project Context

> **Status: competition complete (2026) — Top 5 of 1,000+.** This is now an archived
> portfolio repository. Start with the [**README**](README.md) for the result and the
> engineering story; the shipped agent is [`algos/smart_oracle_F2/`](algos/smart_oracle_F2),
> the search engine is [`algos/oracle_pure/`](algos/oracle_pure), and the Rust simulator is
> [`algos/athena/sim_rs/`](algos/athena/sim_rs). This file remains the build-time orientation
> used while iterating on algos.
...
```
CitadelTerminal/
├── README.md                      ← START HERE: result + engineering story (recruiter-facing)
├── CLAUDE.md                      ← you are here (build-time orientation)
├── docs/                          ← deep reference material (READ THESE)
│   ├── game/                      ← rules & unit math (COMPETITION values)
│   │   ├── GAME_RULES.md          ← turn structure, resources, win conditions, competition deltas
│   │   ├── UNITS_REFERENCE.md     ← exact stats for every unit
...
│   ├── workflow/                  ← how to iterate: CLAUDE_WORKFLOW, LOCAL_TESTING, OBTAINING_REPLAYS, RUST_MIGRATION, BACKTEST_CONTEXT
│   └── devlog/                    ← the iteration story: handoffs + variant reports
├── algos/                         ← algorithms (one folder per algo)
│   ├── smart_oracle_F2/           ← ★ the shipped agent (Top-5)
│   ├── oracle_pure/               ← the search engine (M1Lite)
│   ├── athena/                    ← the simulator: Python sim + sim_rs/ (Rust port) + SIM_PARITY.md
│   └── baselines/                 ← earlier heuristic algos (v13, heuristic_v1/v2)
├── tools/                         ← helper scripts (match runners, replay analysis, eval harness)
├── research/                      ← distilled engine/opponent research (GOTCHAS, PATHFINDER_SPEC, audits)
├── data/                          ← leaderboard snapshots + pairwise analyses (result evidence)
...
1. Re-read [`docs/game/UNITS_REFERENCE.md`](docs/game/UNITS_REFERENCE.md) so unit math is correct.
2. Skim [`docs/game/STRATEGY_GUIDE.md`](docs/game/STRATEGY_GUIDE.md) for relevant archetypes.
3. If pathing/targeting logic matters, consult [`docs/game/TARGETING_AND_PATHING.md`](docs/game/TARGETING_AND_PATHING.md) — most strategic bugs trace back to misunderstanding these rules.
...
- For fast syntax/sanity checks without a full engine run, use the `test_algo_mac` / `test_algo_linux` binary against a `.replay` file (see [`docs/workflow/LOCAL_TESTING.md`](docs/workflow/LOCAL_TESTING.md)).
...
Full numerics in [`docs/game/GAME_RULES.md`](docs/game/GAME_RULES.md).
...
Skills live under `.agents/skills/` and each has its own SKILL.md with invocation details.

---

## Important "do not confuse this" notes

1. **Starter-kit config is outdated.** `C1GamesStarterKit-master/game-configs.json` uses pre-rename shorthands (FF/EF/DF/PI/EI/SI → Wall/Support/Turret/Scout/Demolisher/Interceptor) AND its numeric values predate the competition changes. Treat it as engine-plumbing reference only — quote competition numbers from `docs/game/UNITS_REFERENCE.md`.
