"""Search loop for oracle_pure - VARIANT K1 (= VD + first-breach reaction).
...
- K1:      first-breach reaction defense templates
           (passes breach_pressure to enumerate_plans for tile selection,
           less gated than J2 — fires from the first observed breach)
"""
...
def search(game_state, config, opp_model: OpponentModel, *,
    ...
    Variant K1 additions vs Variant D:
      breach_pressure: ALSO passed to enumerate_plans so the enumerator
        can generate `defense:first_breach_*` templates whose turret
        positions are derived from observed pressure (less gated than
        J2 — fires from the first observed breach).
    """
    t0 = time.time()
    tel = SearchTelemetry()
    ...
    candidates = enumerate_plans(
        ...
        max_plans=max_plans,
        breach_pressure=breach_pressure,  # K1 — drives first_breach templates
        debug_log=debug_log,
    )
