...
============================================
Adds `_enumerate_first_breach_response_templates`: produces NEW defense
templates each turn whose turret positions are derived from the
observed `breach_pressure` map maintained by algo_strategy.

Unlike J2's lane_block templates (which require sustained, concentrated
pressure: turn>12 AND peak>=2.5 AND peak/total>=40%), K1 fires after
turn 4 once any tile crosses pressure>=0.5. The only gates are:
  - turn >= 4 (skip early-game edge-ping noise from MP=1 scouts that
    naturally land at our spawn corners)
  - pressure >= 0.5 (signal-noise floor)
  - tile in our half (y < 14)
Per qualifying tile, generate ONE defense template placing 2 turrets
within Chebyshev-2 of that tile, capped at top-5 tiles.

Rationale: opponents like snorkeldink-v3-2 deterministically attack the
same 1-2 tiles slowly (29 breaches over 31 turns). J2's gates often fail
to fire before the match resolves; K1 reacts on turn 1 if a breach is
observed. The cost is more candidate plans early-game, capped at top-5
unique tiles per turn so the candidate space stays bounded.

The mechanism (place turrets near tiles where the opponent is breaching)
is principled — the threshold (>=0.5) is signal-noise filtering against
decay-only artifacts (a 1.0 breach decays to 0.9 -> 0.81 -> ... so the
0.5 threshold lets a single breach drive ~6 turns of templates). Tile
coordinates are NEVER hardcoded; they come from observation.
...
# ===========================================================================
# K1: First-breach reaction defense templates
# ===========================================================================
#
# DESIGN
# ------
# For each unique tile in `breach_pressure` whose pressure >= K1_PRESSURE_MIN,
# generate ONE defense template that places turrets within Chebyshev-`K1_RADIUS`
# of the tile. The template includes the standard ANCHOR_TURRETS skeleton so
# the template is a complete defense, not just a patch.
#
# UNLIKE J2:
#   - No turn-number gate (fires at turn 1 if a breach is observed)
#   - No peak-pressure threshold beyond signal-noise (0.5)
#   - No concentration ratio gate (handles diffuse multi-tile attacks)
#   - One template PER tile (top-5 capped) — J2 emits at most 2 variants total
#     for a single primary tile.
#
# WHY THIS HELPS SLOW DRIP-ATTACKERS
# ----------------------------------
# Opponents like snorkeldink-v3-2 generate sustained but slow concentrated
# pressure: 29 breaches over 31 turns at the same 2 tiles. The accumulated
# pressure from decay never grows large enough to cross J2's peak>=2.5 OR
# turn>12 gate before the opponent has already exploited the breach pattern.
# K1 reacts on the very first breach, generating a template that covers the
# tile in subsequent turns so the search picks it once SP allows.
#
# COST CAP
# --------
# Top-5 unique tiles per turn × 1 template per tile = at most 5 extra defense
# templates. With ~30 base defense × ~50 offense = ~1500 candidates, K1 adds
# ~250 more (5 × 50 offense), well within max_plans=2500.

K1_PRESSURE_MIN = 0.5     # signal-noise floor
K1_TURN_MIN = 4           # avoid early-game edge-ping noise (turns 0..3)
K1_TOP_TILES = 5          # cap unique tiles per turn
K1_RADIUS = 2             # Chebyshev radius for turret placement
K1_TURRETS_PER_TEMPLATE = 2  # 2 turrets per first-breach template
...
def _find_valid_turret_positions_near(
    ...
    Tiles are sorted by:
      1. depth: prefer tiles ~2 cells inside our territory (so the turret
         threatens the approach path, not the edge breach destination).
      2. Chebyshev distance to the breach tile (closer = more relevant).
    ...
    for dy in range(-distance, distance + 1):
        for dx in range(-distance, distance + 1):
            ...
            if not _is_legal_struct_tile(game_state, tx, ty):
                continue
            # Skip our own spawn edges. Building there blocks our offense;
            # the breach is AT the edge, not approach-able by a turret
            # placed on the same edge.
    ...
    def _score(t):
        ...
        # We want edge_d ~ 2-3 (deep enough to threaten approach, close
        # enough to actually be near the breach).
...
def _enumerate_first_breach_response_templates(
    game_state, config, sp_budget: float,
    breach_pressure: Optional[dict] = None,
    debug_log=None,
) -> List[ActionPlan]:
    """K1 — Generate defense templates targeting tiles with observed
    breach pressure. One template per top-K tile (top-5).

    Unlike J2's gated lane_block, K1 fires from the very first breach
    and from each of the top-5 tiles independently. This catches:
      - drip-attackers (pressure builds slowly but signal is real)
      - diffuse multi-tile attackers (J2's concentration gate would block)
      - early-game pressure (J2's turn>12 gate would block)

    Returns [] if no breach_pressure or no tile crosses K1_PRESSURE_MIN.
    """
    ...
    # Stability gate: turn count > K1_TURN_MIN. Many opponents' first MP=1
    # scout naturally lands at our edge tiles (e.g., (12,1), (0,13)) for a
    # benign +1 breach that decays out by turn 4-5. Without this gate, K1
    # spends SP on defending these phantom tiles in early-game banking
    # turns and falls behind in mirror matchups vs the un-gated VD baseline.
    # snorkeldink-v3-2's drip-attack pattern still kicks in turn 5+, so this
    # gate doesn't compromise the critical use case.
    try:
    ...
    # Sort tiles by pressure desc, take top-K.
    sorted_tiles = sorted(breach_pressure.items(), key=lambda kv: -kv[1])
    sorted_tiles = sorted_tiles[:K1_TOP_TILES]
    if not sorted_tiles:
        return []
    ...
    for tile, pressure in sorted_tiles:
        try:
            ...
            continue
        # Signal-noise gate.
        if pressure < K1_PRESSURE_MIN:
            continue
        # Tile must be in our half (we can only defend tiles in y<ARENA_HALF_Y).
        if not (0 <= ty < ARENA_HALF_Y):
            continue
        if (tx, ty) in seen_tiles:
            continue
        seen_tiles.add((tx, ty))

        # Find Chebyshev-2 turret positions near the breach tile.
        positions = _find_valid_turret_positions_near(
            (tx, ty), game_state,
            max_count=K1_TURRETS_PER_TEMPLATE,
            distance=K1_RADIUS,
        )
        if len(positions) < K1_TURRETS_PER_TEMPLATE:
            # Not enough valid tiles near this breach to make a useful patch.
            continue

        # Atom priority order:
        #   1. K1 turrets (the K1 contribution — derived from observation)
        #   2. ANCHOR_TURRETS (standard skeleton — always relevant)
        #   3. INNER_CORNER_TURRETS (corner-breach stop — common pattern)
        #   4. WALL_ROW_Y12 (deflector row)
        # Greedy SP placement preserves K1 turrets first when SP is tight.
        k1_atoms = [(TURRET_IDX, x, y) for x, y in positions]
        atoms = (k1_atoms +
                 list(ANCHOR_TURRETS) +
        ...
        plan = _build_defense_plan(
            f"defense:first_breach_{tx}_{ty}",
            atoms, game_state, config, sp_budget,
            archetype="first_breach",
        )
        if plan is None:
            continue

        # Only accept the plan if at least one K1 turret was actually placed
        # (otherwise the template collapses into the standard skeleton).
        placed_xys = {(op.xy[0], op.xy[1]) for op in plan.ops
                      if op.kind == KIND_SPAWN_STRUCT}
        if not any((x, y) in placed_xys for x, y in positions):
            continue

        plans.append(plan)
        debug_log(
            f"[oracle_pure K1] first_breach template emitted "
            f"tile=({tx},{ty}) pressure={pressure:.2f} "
            f"placed_turrets={[p for p in positions if p in placed_xys]}"
        )
...
def enumerate_plans(game_state, config, *,
    ...
    K1 — when `breach_pressure` is provided AND any tile crosses the
    signal-noise floor (>=0.5), prepend pressure-derived
    `defense:first_breach_*` templates to the defense pool. Up to 5
    templates (one per top-5 unique pressure tile) compete in the search
    alongside the existing 13. Empty when no pressure observed.
    """
    ...
    # K1 — first-breach reaction templates.
    # PREPEND (not append): the per-defense×offense expansion below caps
    # at max_plans, so trailing defenses can be silently dropped. When a
    # breach has been observed, first_breach templates are likely the
    # most relevant defense — evaluate them first so they are not dropped.
    first_breach_plans = _enumerate_first_breach_response_templates(
        game_state, config, sp_budget=sp,
        breach_pressure=breach_pressure, debug_log=debug_log,
    )
    if first_breach_plans:
        defenses = first_breach_plans + list(defenses)
...
__all__ = [
    ...
    "_enumerate_first_breach_response_templates",
