...
# Edge constants — MUST MATCH vendored_sim/map.py constants
# (so pysim's pathfinder lookups are correct). Per map.py:
#   EDGE_TOP_RIGHT     = 0   (x+y==41)
#   EDGE_TOP_LEFT      = 1   (y-x==14)
#   EDGE_BOTTOM_LEFT   = 2   (x+y==13)
#   EDGE_BOTTOM_RIGHT  = 3   (x-y==14)
# A mobile's target_edge is the edge OPPOSITE the spawn corner (engine
# picks the diagonally-opposite edge as the goal).
_EDGE_TR = 0
_EDGE_TL = 1
_EDGE_BL = 2
_EDGE_BR = 3
...
def _spawn_to_target_edge(x: int, y: int, player: int) -> int:
    ...
    Following vendored_sim/map.py spawn_tile_target_edge:
      bottom-left  spawn (x<14, y<14)   -> target TOP_RIGHT     (0)
      bottom-right spawn (x>=14, y<14)  -> target TOP_LEFT      (1)
      top-left     spawn (x<14, y>=14)  -> target BOTTOM_RIGHT  (3)
      top-right    spawn (x>=14, y>=14) -> target BOTTOM_LEFT   (2)
    """
    HALF = 14
    if y < HALF:
        return _EDGE_TR if x < HALF else _EDGE_TL
    return _EDGE_BR if x < HALF else _EDGE_BL
