"""Search loop for oracle_pure - VARIANT M2v2 (VD + K1 + K2).
...
- M2v2:    breach_pressure ALSO passed to enumerate_plans so the
           enumerator can generate `defense:first_breach_*` (K1) and
           `defense:path_block_*` (K2) templates whose turret positions
           are derived from observed pressure.
"""
...
def search(game_state, config, opp_model: OpponentModel, *,
    ...
    Variant M2v2 additions vs Variant D:
      breach_pressure: ALSO passed to enumerate_plans so the enumerator
        can generate `defense:first_breach_*` (K1) and
        `defense:path_block_*` (K2) templates whose turret positions
        are derived from observed pressure.
    """
    t0 = time.time()
    ...
    candidates = enumerate_plans(
        ...
        max_plans=max_plans,
        breach_pressure=breach_pressure,  # M2v2 — drives K1+K2 templates
        debug_log=debug_log,
    )
