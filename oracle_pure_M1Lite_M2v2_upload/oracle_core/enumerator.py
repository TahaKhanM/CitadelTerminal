...
============================================================
Adds two pressure-derived defense generators that fire from the FIRST
observed breach against us. Both derive their tile choices from the
runtime `breach_pressure` map; nothing is hardcoded.

  K1 (`_gen_first_breach_response`)
    For each tile in `breach_pressure` whose pressure >= 0.5, generate a
    defense template that places turrets within Chebyshev-2 of the tile.
    Top-5 unique tiles per turn. Aggressive, low-threshold; reacts on
    turn 1 of the first breach. Mirrors K1 from
    oracle_pure_M1Lite_K1_upload (which never wired through to the
    enumerator).

  K2 (`_gen_path_block_templates`)
    For each top breach tile, infer the opponent's likely approach path
    via `game_state.find_path_to_edge` and place turrets ALONG that
    path. Mirrors snorkeldink-v3-2's own reactive-defence mechanism
    (its `build_reactive_defense` in `reactive_defence.py`). Pressure
    threshold 1.0 (one full breach observed); top-3 tiles per turn.

Both run BEFORE existing defense templates so they survive the
max_plans=2500 cap. Each emits a complete defense template (K1/K2
turrets + ANCHOR_TURRETS skeleton) so it is a viable candidate the
search can pick directly.

This is the second iteration of "M2." The PRIOR M2 (BFS path-viability
check + ALT-OUTSIDE supports) regressed live on ameyg/funnel-rush-v6/v7/v8
and was reverted (see oracle_pure_M1Lite_VD_upload/CONTEXT_HANDOFF.md
§16). M2v2 has a DIFFERENT mechanism (pressure-derived templates, not
path validation), but the cautionary tale applies: local pass != live pass.
...
# ===========================================================================
# M2v2 — Pressure-derived defense generators (K1 + K2)
# ===========================================================================
#
# Both generators derive tile choices from the live `breach_pressure`
# map maintained by algo_strategy. Nothing is hardcoded.
#
# K1 (`_gen_first_breach_response`)
#   - Threshold: pressure >= 0.5 (signal-noise floor; a single breach
#     decays from 1.0 -> 0.9 -> 0.81 ... so >=0.5 lasts ~7 turns)
#   - Top-5 unique tiles per turn -> at most 5 templates
#   - Each template: ANCHOR_TURRETS + 2 turrets within Chebyshev-2 of
#     the breach tile (via `_find_valid_turret_positions_near`)
#
# K2 (`_gen_path_block_templates`)
#   - Threshold: top tile pressure >= 1.0 (one full breach observed)
#   - Top-3 unique tiles per turn -> at most 3 templates
#   - For each tile: call game_state.find_path_to_edge(tile) to get the
#     path the opponent's mobile WOULD take to reach that tile, then
#     place turrets at up to 2 path tiles closest to opp territory (the
#     idea: stop the unit BEFORE it reaches the breach tile)
#   - Mirrors snorkeldink-v3-2's `build_reactive_defense` mechanism
...
K2_PRESSURE_MIN = 1.0
K2_TOP_TILES = 3
K2_PATH_SCAN = 6              # how many path cells to consider
...
def _find_valid_turret_positions_near(
    ...
    Tiles are ranked: depth ~2 from the diamond edge first, then by
    Chebyshev distance to the breach tile, then preferring y=11 row,
    then preferring centerline.
    """
...
def _gen_first_breach_response(
    game_state, config, sp_budget: float,
    ...
    For each top-5 tile in `breach_pressure` whose pressure >= 0.5,
    generate a defense template that places 2 turrets within Chebyshev-2
    of the tile.
    """
    ...
    for tile, pressure in sorted_tiles:
        ...
        if pressure < K1_PRESSURE_MIN:
            continue
        # Tile must be on our side (we can only defend tiles in our half).
        ...
        # Atom priority: K1 turrets first (the M2v2 contribution),
        # then standard skeleton (ANCHOR + INNER_CORNER + WALL_ROW).
        ...
        # Reject if greedy SP placement dropped ALL K1 turrets — the
        # template would collapse to plain skeleton (already a candidate).
        ...
        plans.append(plan)
        debug_log(
            f"[oracle_pure M2v2 K1] first_breach@({tx},{ty}) "
...
def _gen_path_block_templates(
    ...
    and place turrets at the path cells closest to opp territory (the
    idea: stop the mobile BEFORE it reaches the breach tile).

    Mirrors snorkeldink-v3-2's reactive_defence.build_reactive_defense.
    """
    if not breach_pressure:
        return []
    if debug_log is None:
    ...
    if not sorted_tiles:
        return []
    # Threshold gate: the top tile must have a real breach (>= 1.0).
    ...
    for tile, pressure in sorted_tiles:
        ...
        # Pathing must start from a legal start (no stationary unit).
        # If something is on the breach tile, find_path_to_edge would
        # warn & return None.
        try:
        ...
        # Keep only path cells in our half. Sort closest to opp territory
        # first (highest y) so we block the EARLIEST step of the
        # approach into our territory.
        oracle_path = []
        for p in path:
            try:
        ...
        if not oracle_path:
            continue
        # Highest y first (closest to the y=14 boundary).
        oracle_path.sort(key=lambda pp: -pp[1])

        # Pick up to 2 candidate turret tiles along the path.
        candidates: List[Tuple[int, int]] = []
        ...
        plans.append(plan)
        debug_log(
            f"[oracle_pure M2v2 K2] path_block@({bx},{by}) "
...
def enumerate_plans(game_state, config, *,
    ...
    M2v2 (K1+K2) — when `breach_pressure` is provided AND signal gates
    pass, prepend pressure-derived `defense:first_breach_*` (K1) and
    `defense:path_block_*` (K2) templates to the defense pool.

    K1 fires whenever any tile crosses the 0.5 signal-noise floor (so
    on the turn AFTER the first observed breach).
    K2 fires whenever the top tile crosses 1.0 (one full breach
    observed).
    Both empty when no pressure observed -> graceful no-op.
    """
    ...
    # M2v2 — pressure-derived templates. PREPEND so they survive the
    # max_plans cap (the per-defense×offense expansion below caps at
    # max_plans, so trailing defenses can be silently dropped).
    extra_defenses: List[ActionPlan] = []
    extra_defenses.extend(
        _gen_first_breach_response(
            game_state, config, sp_budget=sp,
            breach_pressure=breach_pressure, debug_log=debug_log,
        )
    )
    extra_defenses.extend(
        _gen_path_block_templates(
            game_state, config, sp_budget=sp,
            breach_pressure=breach_pressure, debug_log=debug_log,
        )
    )
