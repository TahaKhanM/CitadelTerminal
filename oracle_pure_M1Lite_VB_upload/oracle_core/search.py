...
def search(game_state, config, opp_model: OpponentModel, *,
    ...
    # 2b. VARIANT B: Sim-based offensive viability probe per defense template.
    #
    # For each unique defense template, run ONE probe sim that applies
    # the defense's structure ops (no offense) and adds 5 synthetic
    # probe scouts at center launcher (14, 0). Count breaches via SP
    # delta. If 0 breaches, the defense is "trap-prone" — applying it
    # would seal off our offensive paths. Soft-penalize trap-prone
    # plans in evaluation (see value.evaluate's defense_viability arg).
    #
    # Why not BFS (M2 approach)? §16 of CONTEXT_HANDOFF.md documents
    # how M2's BFS-based check rejected M1Lite's working SUPPORTS_BACK
    # config (0/10 launchers viable per BFS yet wins matches). Sim-based
    # probe uses the actual engine pathfinder via sim_rs, so it cannot
    # be more conservative than the engine.
    #
    # Cost: ~13 unique defenses × ~0.15 ms/probe ≈ 2 ms/turn.
    ...
    for cand in candidates:
        # Defense name is everything before the first "|" (combo format)
        def_name = cand.name.split("|", 1)[0]
        if def_name in defense_probes:
            cand.defense_viability = defense_probes[def_name]
            continue
        # Build a defense-only ActionPlan from this candidate's structure ops
        # (we strip the offense ops). Calling probe_offense_viability with
        # this plan reproduces the post-deploy structure layout.
        ...
        try:
            ...
            breach = PROBE_SCOUT_COUNT  # default to "safe" if probe fails
