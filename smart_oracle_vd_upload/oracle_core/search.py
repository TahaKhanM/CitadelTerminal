"""Search loop for smart_oracle_vd — VD baseline + funnel-aware filtering.

Inherits from VD (variance ranking, wasted-MP penalty, viability probe,
breach-pressure coverage) and adds one targeted change for funnel
strategies:

  When the algo_strategy passes a `funnel_target` ∈ {"left", "right"},
  search filters the opponent samples it evaluates each candidate against
  to those whose spawn launcher routes to that flank. The enumerator is
  also told the funnel target so it adds zone-focused defense templates
  to the candidate pool. Net effect: the search is still the sole
  decision-maker and still picks the maximum-EU plan, but the EU is now
  computed against a sample distribution that matches the threat the
  opponent has been showing — instead of being averaged across center
  rushes / two-prong attacks the opponent isn't actually launching.
"""
...
from .funnel_detector import plan_attacks_zone
...
class SearchTelemetry:
    ...
    # smart_oracle_vd: which flank zone (if any) the funnel detector
    # classified the opponent into for THIS turn, and how many opp samples
    # survived the funnel filter at each phase.
    funnel_target: Optional[str] = None
    opp_samples_after_filter_phase1: int = 0
    opp_samples_after_filter_phase2: int = 0
...
def search(game_state, config, opp_model: OpponentModel, *,
    ...
    smart_oracle_vd additions vs VD:
      funnel_target: when not None, search restricts the opponent samples
        it evaluates against to those whose spawn launcher routes to that
        flank (LEFT/RIGHT), and forwards the same flag to the enumerator
        so flank-corridor defense templates appear in the candidate pool.
        The filter is conservative — if it would zero-out the sample set
        we revert to the unfiltered list so the search never starves.
    """
    t0 = time.time()
    tel = SearchTelemetry()
    tel.funnel_target = funnel_target
    ...
    # smart_oracle_vd: when the funnel detector has classified the opponent
    # as funneling one flank, restrict the samples to those whose spawn
    # launcher historically routes to that flank. The base VD scores each
    # candidate against an averaged sample distribution that includes
    # opposite-flank attacks the opponent isn't actually launching, which
    # dilutes the value of focused-flank defense plans. Filtering removes
    # that dilution. We always keep at least one sample (fall back to the
    # unfiltered list) so the EU evaluation stays well-defined.
    if funnel_target:
        filtered_p1 = [s for s in opp_samples_phase1
                       if plan_attacks_zone(s, funnel_target)]
        filtered_p2 = [s for s in opp_samples_phase2
                       if plan_attacks_zone(s, funnel_target)]
        if filtered_p1:
            opp_samples_phase1 = filtered_p1
        if filtered_p2:
            opp_samples_phase2 = filtered_p2
        debug_log(
            f"[search] funnel={funnel_target} "
            f"opp_filtered p1={len(opp_samples_phase1)} "
            f"p2={len(opp_samples_phase2)}"
        )
    tel.opp_samples_after_filter_phase1 = len(opp_samples_phase1)
    tel.opp_samples_after_filter_phase2 = len(opp_samples_phase2)
