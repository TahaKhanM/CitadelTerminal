...
# ---------------------------------------------------------------------------
# Flank-corridor atoms (smart_oracle_vd) — only proposed when the funnel
...
def _enumerate_defense_templates(game_state, config, sp_budget: float,
    ...
    When `funnel_target` ∈ {"left", "right"}, smart_oracle_vd injects
    flank-corridor templates focused on that side. The base templates are
    still returned so the search can pick a non-funnel plan if sim_rs
    evaluation says so — we ADD options, we don't remove them.
    """
    ...
    # 12. Flank-corridor templates (smart_oracle_vd) — only when the funnel
    # detector has classified the opponent as consistently attacking one
    # flank. These shift defense priority into the targeted zone, but the
    # search still picks among them via sim_rs evaluation; if a baseline
    # template scores higher it still wins.
    if funnel_target == "left":
        ...
        if p:
            plans.append(p)
        # Flank corridor + selective upgrades on existing damaged structures
        if upgrade_candidates:
...
def enumerate_plans(game_state, config, *,
    ...
    `funnel_target` ∈ {None, "left", "right"} forwards to the defense
    enumerator so flank-corridor templates are added when the funnel
    detector has fired (smart_oracle_vd extension over VD).
    """
