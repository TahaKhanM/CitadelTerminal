...
================================================
Adds `_gen_path_block_templates`: when breach pressure is observed,
runs `game_state.find_path_to_edge(breach_tile)` to derive the path
opp's mobiles took (the engine's own pathfinder, not heuristic BFS),
then generates defense templates that place turrets along that path
in oracle's territory.

Mirrors snorkeldink-v3-2's `reactive_defence.py` mechanism, which
deterministically beats VD/M1Lite. Unlike v3-2 (early-game only) we
fire whenever observed `breach_pressure[tile] >= 1.0`.

Principle: the path is derived from `game_state` (engine pathfinder),
not hardcoded. Block tiles are those engine paths actually use, so
defenses are placed exactly where opp would walk. Per §16 lessons of
CONTEXT_HANDOFF.md (BFS-based path checks are over-conservative vs
the engine), this uses the engine itself.
...
# ===========================================================================
# K2: Path-block template generator (uses engine's pathfinder)
# ===========================================================================

def _is_valid_turret_position(xy, game_state) -> bool:
    """Return True iff (x, y) is a legal turret-placement tile.

    Rules:
      - in arena bounds
      - in oracle's territory (y < 14)
      - not occupied by any stationary unit
      - not on a spawn edge (would block our own deploys)
    """
    try:
        x = int(xy[0]); y = int(xy[1])
    except Exception:
        return False
    if y < 0 or y >= ARENA_HALF_Y or x < 0 or x >= ARENA_DIM:
        return False
    if not game_state.game_map.in_arena_bounds([x, y]):
        return False
    if game_state.contains_stationary_unit([x, y]):
        return False
    # Don't place on spawn edges (BL: y == 13 - x, BR: y == x - 14)
    if y == 13 - x or y == x - 14:
        return False
    return True
...
def _gen_path_block_templates(game_state, breach_pressure, config):
    """Yield defense templates that block paths leading to observed breach tiles.

    For each high-pressure breach tile, calls find_path_to_edge(tile)
    to derive the engine's path through oracle's territory, then places
    anchor turrets + 2 turrets along the highest-y path tiles (closest
    to opp's territory).

    Mirrors snorkeldink-v3-2's reactive_defence.build_reactive_defense.
    Uses the engine's pathfinder, NOT a custom BFS — so it cannot be
    more conservative than the engine itself.

    Only yields atoms; the caller (enumerator) does budget gating
    via _build_defense_plan or equivalent.
    """
    if not breach_pressure:
        return

    # Top-3 high-pressure tiles, ignore decay-only artifacts (<1.0).
    ...
    seen_tiles = set()
    template_idx = 0
    for breach_tile, _pressure in top_tiles:
        if breach_tile in seen_tiles:
            continue
        seen_tiles.add(breach_tile)

        # find_path_to_edge needs an UNBLOCKED start location. If the
        # breach tile is itself blocked (a structure landed on top of
        # it post-breach), the call would warn + return None. The API
        # also expects a list, not a tuple.
        start_xy = [int(breach_tile[0]), int(breach_tile[1])]
        try:
            if game_state.contains_stationary_unit(start_xy):
                # Try to find an unblocked neighbor on oracle's side
                # at the same row or one row deeper.
                shifted = None
                for dx, dy in ((0, -1), (1, 0), (-1, 0), (1, -1), (-1, -1)):
                    nx, ny = start_xy[0] + dx, start_xy[1] + dy
                    if 0 <= nx < ARENA_DIM and 0 <= ny < ARENA_HALF_Y:
                        if not game_state.game_map.in_arena_bounds([nx, ny]):
                            continue
                        if not game_state.contains_stationary_unit([nx, ny]):
                            shifted = [nx, ny]
                            break
                if shifted is None:
                    continue
                start_xy = shifted
            path = game_state.find_path_to_edge(start_xy)
        except Exception:
            continue
        if path is None or len(path) == 0:
            continue

        # Filter to oracle's territory tiles (y < 14).
        oracle_path_tiles = []
        for p in path:
            try:
                ...
                continue
            if py < ARENA_HALF_Y:
                oracle_path_tiles.append((px, py))
        if not oracle_path_tiles:
            continue

        # Sort by descending y — block CLOSE to where opp enters our territory
        # (y=12-13 is the "opp side" of oracle's half).
        oracle_path_tiles.sort(key=lambda p: -p[1])

        # Pick top-K path tiles that are valid for turret placement.
        candidate_positions = []
        seen_candidate = set()
        for tile in oracle_path_tiles[:6]:  # check first 6 path tiles
            if tile in seen_candidate:
                continue
            seen_candidate.add(tile)
            if _is_valid_turret_position(tile, game_state):
                candidate_positions.append(tile)
        ...
        # Build the atom list: anchor turrets first (only legal ones),
        # then 2 path-block turrets. _build_defense_plan handles cost
        # gating greedily — the path-block atoms come AFTER anchors,
        # so anchors fill first and path-blocks come if SP allows.
        # We want the OPPOSITE here: path-block tiles are the K2
        # signal, anchors are nice-to-have. Put path-block turrets
        # FIRST so SP exhaustion drops anchors, not the K2 signal.
        atoms = []
        for x, y in candidate_positions[:2]:
            atoms.append((TURRET_IDX, x, y))
        atoms.extend(ANCHOR_TURRETS)

        template_idx += 1
        yield (
            f"defense:path_block_{breach_tile[0]}_{breach_tile[1]}",
            atoms,
        )
        # Cap at 3 distinct templates per turn — bound search cost.
        if template_idx >= 3:
            return
...
def _enumerate_defense_templates(game_state, config, sp_budget: float,
    ...
    # 12. K2: Path-block templates (engine-pathfinder-derived).
    #
    # When breach pressure is observed, ask the engine where opp's
    # mobile would walk through oracle's territory, and place turrets
    # along that path. Mirrors snorkeldink-v3-2's reactive_defence.
    #
    # Gated by turn > 8: early-game placements would fire on stochastic
    # opening breaches (e.g., a single demolisher poke at T2) and add
    # noise. After T8 a breach is a real signal of an exploitable hole.
    if breach_pressure and turn > 8:
        for tpl_name, tpl_atoms in _gen_path_block_templates(
            game_state, breach_pressure, config,
        ):
            p = _build_defense_plan(
                tpl_name, tpl_atoms, game_state, config, sp_budget,
...
def enumerate_plans(game_state, config, *,
    ...
    K2 addition: ``breach_pressure`` (dict {(x,y): float}) drives the
    path-block templates that mirror snorkeldink-v3-2's reactive
    defence. None / empty disables K2 templates (back-compat with VD).
    """
