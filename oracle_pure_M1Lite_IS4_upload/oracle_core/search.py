...
def search(game_state, config, opp_model: OpponentModel, *,
    ...
    # IS4: dump probe results for the 5-way support variants so we can
    # verify the probe IS differentiating between trap-forming and
    # non-trap layouts (the diversity-effect we're testing).
    sup_probes = sorted(
        (k, v) for k, v in defense_probes.items()
        if k.startswith("defense:supports")
    )
    if sup_probes:
        probes_str = " ".join(
            f"{k.replace('defense:', '')}={v}" for k, v in sup_probes
        )
        debug_log(f"[IS4] support_probes: {probes_str}")
