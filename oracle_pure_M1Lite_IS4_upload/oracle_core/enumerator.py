...
# Back-row support positions — multiple candidate sets so the search
# (with VB's viability probe) can pick the best one for the current
# board state.
#
# Why multiple sets? The original single set [(12,11),(15,11),(13,10),(14,10)]
# is known to occasionally seal the wall-row gaps at (12,12)/(15,12) and
# trap our offense (suchir-g/not-tnb losses, see CONTEXT_HANDOFF.md §16).
# VB's probe CAN detect this trap but provides no signal when ALL defense
# templates are trap-prone (uniform penalty = no signal).
#
# By offering multiple geometrically-distinct support layouts, the probe
# can differentiate: trap-forming sets get -250 penalty, non-trap sets
# don't. The search picks based on probe + simulation outcomes.
#
# IMPORTANT: We do NOT pre-rank these or bias toward any specific set.
# The search owns the choice. M2's lesson (§16): hardcoded tile swaps
# can cascade into launcher-selection regressions. Here we ADD options
# rather than REPLACE.
...
def _supports_atoms(positions):
    """Convert (x, y) tuples into (SUPPORT_IDX, x, y) atoms."""
    return [(SUPPORT_IDX, x, y) for (x, y) in positions]


# Back-compat alias: keep SUPPORTS_BACK pointing at v0 so any external
# import still resolves. New code uses SUPPORT_POSITION_SETS directly.
SUPPORTS_BACK = _supports_atoms(SUPPORT_POSITION_SETS[0])
...
def _enumerate_defense_templates(game_state, config, sp_budget: float,
    ...
    # IS4: enumerate all SUPPORT_POSITION_SETS so VB's probe can differentiate
    # trap-forming layouts from viable ones. Each variant gets its own template
    # name (defense:supports_v0, ..._v4) so the probe-cache treats them as
    # distinct.
    for set_idx, positions in enumerate(SUPPORT_POSITION_SETS):
        atoms = _supports_atoms(positions) + ANCHOR_TURRETS
        p = _build_defense_plan(
            f"defense:supports_v{set_idx}",
            atoms,
    ...
    # 6. Support-back-row + minimal defense (also one per SUPPORT_POSITION_SET).
    # IS4: same logic as defense:supports — enumerate variants so the probe
    # can differentiate trap vs non-trap layouts.
    for set_idx, positions in enumerate(SUPPORT_POSITION_SETS):
        atoms = _supports_atoms(positions)
        p = _build_defense_plan(
            f"defense:supports_only_v{set_idx}",
            atoms,
            game_state, config, sp_budget, archetype="supports",
        )
        if p:
