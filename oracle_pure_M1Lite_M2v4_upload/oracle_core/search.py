"""Search loop for oracle_pure - VARIANT J2 (= VD + lane-block templates).
...
- J2:    pressure-derived lane-block defense templates
...
def search(game_state, config, opp_model: OpponentModel, *,
    ...
    Variant J2 additions vs Variant D:
      breach_pressure: ALSO passed to enumerate_plans so the enumerator
        can generate `defense:lane_block_*` templates whose turret
        positions are derived from observed pressure.
    """
    t0 = time.time()
    tel = SearchTelemetry()
    ...
    candidates = enumerate_plans(
        ...
        max_plans=max_plans,
        breach_pressure=breach_pressure,  # J2 — drives lane_block templates
        debug_log=debug_log,
    )
