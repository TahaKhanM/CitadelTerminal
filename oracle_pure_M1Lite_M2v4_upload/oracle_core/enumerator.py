...
================================================
Adds `_enumerate_lane_block_templates`: produces NEW defense templates
each turn whose turret positions are DERIVED from observed
`breach_pressure` (per-tile pressure map maintained from
on_action_frame breaches in algo_strategy). Tile choices come from
observation, NOT hardcoded.

Activation gate (signal stability):
  - Top pressure tile peak > 1.0 (one breach survives one decay step)
  - turn_count > 8 (avoid early-game noise)
  - Top pressure tile is on oracle's side (y < ARENA_HALF_Y)

When all gates pass, emits up to 3 lane_block variants per turn, each
placing 2-4 turrets within Chebyshev-2 of the highest-pressure tile,
combined with the standard ANCHOR_TURRETS skeleton. The mechanism
("place turrets near high-pressure tiles") is principled. The threshold
is an activation gate against noise — derived from signal
characteristics, not from opponent identity.
...
# ===========================================================================
# J2: Lane-block defense templates (gated by breach_pressure signal)
# ===========================================================================

# Activation thresholds for the pressure-derived lane-block generator.
# These are SIGNAL CHARACTERISTICS, not opponent identity.
# - 2.5 = 3+ breaches at the same tile within ~3 turns of decay (0.9/turn).
#   2 breaches in one turn → 2.0 (just under), so 2.5 requires SUSTAINED
#   concentrated pressure rather than a single multi-breach spike.
#   Egil's 35-breach pattern would yield peak ≥ 9 after sustained pressure.
# - turn_count > 12 ensures decay window has filtered transient noise.
# - Concentration ratio: top tile must hold ≥40% of total tracked pressure
#   to prevent diffuse-pressure firing (e.g., breaches at 5 different
#   spawn edges).
J2_PRESSURE_PEAK_MIN = 2.5
J2_TURN_MIN = 12
J2_CONCENTRATION_MIN = 0.40
# Cap on lane_block variants per turn so candidate space doesn't explode.
J2_MAX_VARIANTS = 2


def _find_valid_turret_positions_near(
    top_xy: Tuple[int, int], game_state, max_count: int,
    distance: int = 2,
    extra_top_xys: Sequence[Tuple[int, int]] = (),
) -> List[Tuple[int, int]]:
    """Return up to `max_count` legal turret tiles within Chebyshev-`distance`
    of `top_xy` (and any `extra_top_xys`).

    A tile is "valid" iff:
      - in arena bounds
      - in our half (y < ARENA_HALF_Y)
      - NOT currently occupied by any stationary unit
      - NOT on our spawn edges (placing a turret on a spawn tile blocks
        our own offensive options at that tile, which is strictly bad —
        the breach is at the EDGE, not on the path leading to it)

    Tiles are sorted by:
      1. Position INSIDE/BEHIND the breach (further from the edge): the
         goal is to threaten the unit's APPROACH path, not the breach
         destination itself. Prefer tiles with greater perpendicular
         distance from the diamond edge.
      2. Closest Chebyshev distance to the primary anchor.
      3. Distance from y=11 (preferred turret row).
      4. Distance from x=13.5 (centerline).
    """
    spawn_edges = set(BOTTOM_LEFT_EDGE) | set(BOTTOM_RIGHT_EDGE)
    candidates: List[Tuple[int, int]] = []
    seen: set = set()
    # Use ONLY the primary anchor for distance-based sort to avoid
    # scattered placements when secondary anchors are far apart.
    primary = top_xy
    cx, cy = primary
    anchor_xys = [top_xy] + list(extra_top_xys)
    for ax, ay in anchor_xys:
        for dy in range(-distance, distance + 1):
            for dx in range(-distance, distance + 1):
                tx, ty = ax + dx, ay + dy
                if (tx, ty) in seen:
                    continue
                if not _is_legal_struct_tile(game_state, tx, ty):
                    continue
                # CRITICAL: Skip our own spawn edges. Building there blocks
                # our offense; the breach is AT the edge, not approach-able
                # by a turret placed on the same edge.
                if (tx, ty) in spawn_edges:
                    continue
                seen.add((tx, ty))
                candidates.append((tx, ty))

    def _edge_dist(tx, ty):
        """Perpendicular distance from the bottom diamond edge.
        Greater = further inside our territory (better turret position).
        For y=13 (back of half), edge_dist=0 to back row.
        For y=0 (apex), edge_dist=13.
        """
        # Diamond edge: bottom-left is x+y=13, bottom-right is y=27-x.
        # Distance from the LOSEST edge.
        d_left = abs(tx + ty - 13)
        d_right = abs((27 - tx) - ty)
        return min(d_left, d_right)

    def _score(t):
        tx, ty = t
        # 1. Bias TOWARD inside-territory (further from edge), but not too far
        #    away — prefer 2-3 cells deep so the turret threatens the
        #    final approach. Score is "negative edge_dist" so smaller =
        #    closer to edge = LATER preferred.
        edge_d = _edge_dist(tx, ty)
        # We want edge_d ~ 2-3 (deep enough to be inside, close enough to
        # actually be near the breach). Penalize 0 (edge) and very high.
        depth_score = abs(edge_d - 2)
        cheb_primary = max(abs(tx - cx), abs(ty - cy))
        row_dist = abs(ty - 11)
        center_dist = abs(tx - 13.5)
        return (depth_score, cheb_primary, row_dist, center_dist)

    candidates.sort(key=_score)
    return candidates[:max_count]


def _enumerate_lane_block_templates(game_state, config, sp_budget: float,
                                    breach_pressure: Optional[dict] = None,
                                    debug_log=None
                                    ) -> List[ActionPlan]:
    """J2 — Generate up to J2_MAX_VARIANTS defense templates whose turret
    positions are derived from observed concentrated breach_pressure.
    Returns [] if no signal or gates fail.
    """
    ...
    # Stability gate 1: turn count > J2_TURN_MIN (avoid early-game noise)
    try:
    ...
    # Stability gate 2: peak pressure must exceed threshold
    top_pressure = sorted(breach_pressure.items(), key=lambda kv: -kv[1])[:5]
    if not top_pressure:
        return []
    peak = float(top_pressure[0][1])
    if peak < J2_PRESSURE_PEAK_MIN:
        return []

    # Stability gate 3: top pressure tile must be on oracle's side
    (tpx, tpy), _ = top_pressure[0]
    if not (0 <= tpy < ARENA_HALF_Y):
        # Pressure tracked elsewhere is meaningless for our defense.
        return []

    # Stability gate 4: pressure must be CONCENTRATED.
    # If the top tile holds < J2_CONCENTRATION_MIN of total pressure,
    # the signal is diffuse (multiple breach points) — turret placement
    # can't address all of them and we'd just over-fit one tile.
    total_pressure = sum(float(p) for _, p in breach_pressure.items())
    if total_pressure <= 0:
        return []
    concentration = peak / total_pressure
    if concentration < J2_CONCENTRATION_MIN:
        return []

    primary = (int(top_pressure[0][0][0]), int(top_pressure[0][0][1]))

    plans: List[ActionPlan] = []

    # We emit variants with 2/3 lane turrets. _build_defense_plan
    # will skip later atoms once SP runs out, so the priority ordering is:
    #   1. lane_turrets (the J2 contribution — derived from observation)
    #   2. ANCHOR_TURRETS (standard skeleton — always relevant)
    #   3. INNER_CORNER_TURRETS (corner-breach stop)
    #   4. WALL_ROW_Y12 (wall row deflector)
    #
    # NOTE: tiles are filtered to:
    #   - exclude our spawn edges (placement there blocks our offense)
    #   - prefer tiles INSIDE territory (depth ≈ 2 from edge)
    # See _find_valid_turret_positions_near.
    for n_turrets in (2, 3):
        if len(plans) >= J2_MAX_VARIANTS:
            break
        lane_positions = _find_valid_turret_positions_near(
            primary, game_state, max_count=n_turrets,
            distance=2,
        )
        if len(lane_positions) < n_turrets:
            # Skip this variant — not enough valid lane tiles
            continue

        lane_atoms = [(TURRET_IDX, x, y) for x, y in lane_positions]
        atoms = (lane_atoms +
                 list(ANCHOR_TURRETS) +
                 list(INNER_CORNER_TURRETS) +
                 list(WALL_ROW_Y12))
        p = _build_defense_plan(
            f"defense:lane_block_{n_turrets}t",
            atoms, game_state, config, sp_budget,
            archetype="lane_block",
        )
        if p is None:
            continue
        # Only accept the plan if at least one lane turret was actually
        # placed — otherwise the J2 contribution is empty and the plan
        # collapses into the standard skeleton (which already exists as
        # defense:t0_full).
        placed = {(op.xy[0], op.xy[1]) for op in p.ops if op.kind == KIND_SPAWN_STRUCT}
        if not any((x, y) in placed for x, y in lane_positions):
            continue
        plans.append(p)
        debug_log(
            f"[oracle_pure J2] lane_block_{n_turrets}t emitted "
            f"primary={primary} peak={peak:.2f} conc={concentration:.2f} "
            f"lane_tiles={lane_positions[:n_turrets]}"
        )

    return plans[:J2_MAX_VARIANTS]


# ---------------------------------------------------------------------------
# M2v3 — K2 path-blocking templates (from M2v2)
# ---------------------------------------------------------------------------
# K2: when a tile shows breach pressure, call game_state.find_path_to_edge
# to infer opp's likely approach path, then place turrets at the path cells
# closest to opp territory. Mirrors snorkeldink-v3-2's reactive_defence
# mechanism. Required for the v3-2 fix (J2's lane_block alone fails 0/2).
K2_PRESSURE_MIN = 1.0
K2_TOP_TILES = 3
K2_PATH_SCAN = 6
K2_TURRETS_PER_TEMPLATE = 2
K2_STRONG_PRESSURE = 2.0
K2_TURRETS_STRONG = 3


def _gen_path_block_templates(
    game_state, config, sp_budget: float,
    breach_pressure: Optional[dict] = None,
    debug_log=None,
) -> List[ActionPlan]:
    """K2 — path-block templates derived from `find_path_to_edge`.

    For each top-3 tile in `breach_pressure` whose pressure >= 1.0,
    infer the opp's likely approach path via game_state.find_path_to_edge
    and place turrets at the path cells closest to opp territory.
    """
    if not breach_pressure:
        return []
    if debug_log is None:
    ...
    sorted_tiles = sorted(breach_pressure.items(), key=lambda kv: -kv[1])
    sorted_tiles = sorted_tiles[:K2_TOP_TILES]
    if not sorted_tiles:
        return []
    if float(sorted_tiles[0][1]) < K2_PRESSURE_MIN:
        return []
    ...
    for tile, pressure in sorted_tiles:
        try:
            bx, by = int(tile[0]), int(tile[1])
        except Exception:
            continue
        if (bx, by) in seen_tiles:
            continue
        seen_tiles.add((bx, by))

        try:
            if game_state.contains_stationary_unit([bx, by]):
                continue
        except Exception:
            continue

        try:
            path = game_state.find_path_to_edge([bx, by])
        except Exception:
            continue
        if path is None:
            continue
        ...
        for p in path:
            try:
                ...
                continue
            if 0 <= py < ARENA_HALF_Y:
                oracle_path.append((px, py))
        if not oracle_path:
            continue
        oracle_path.sort(key=lambda pp: -pp[1])

        candidates: List[Tuple[int, int]] = []
        for cand in oracle_path[:K2_PATH_SCAN]:
            cx, cy = cand
            if not _is_legal_struct_tile(game_state, cx, cy):
                continue
            if (cx, cy) in spawn_edges:
                continue
            candidates.append((cx, cy))
            if len(candidates) >= K2_TURRETS_STRONG + 1:
                break

        if len(candidates) < K2_TURRETS_PER_TEMPLATE:
            continue

        # M2v4: emit a 2-turret variant always; emit an additional
        # 3-turret variant when pressure is sustained.
        variants = [(K2_TURRETS_PER_TEMPLATE, "")]
        if (float(pressure) >= K2_STRONG_PRESSURE and
                len(candidates) >= K2_TURRETS_STRONG):
            variants.append((K2_TURRETS_STRONG, "_strong"))

        for n_turrets, suffix in variants:
            path_atoms = [(TURRET_IDX, x, y)
                          for x, y in candidates[:n_turrets]]
            atoms = (path_atoms +
                     list(ANCHOR_TURRETS) +
                     list(INNER_CORNER_TURRETS) +
                     list(WALL_ROW_Y12))
            ...
            plan = _build_defense_plan(
                f"defense:path_block_{bx}_{by}{suffix}",
                atoms, game_state, config, sp_budget,
                archetype="path_block",
            )
            if plan is None:
                continue

            placed_xys = {(op.xy[0], op.xy[1]) for op in plan.ops
                          if op.kind == KIND_SPAWN_STRUCT}
            if not any((x, y) in placed_xys
                       for x, y in candidates[:n_turrets]):
                continue

            plans.append(plan)
            debug_log(
                f"[oracle_pure M2v4 K2] path_block@({bx},{by}){suffix} "
                f"pressure={pressure:.2f} path_len={len(oracle_path)} "
                f"placed_turrets={[c for c in candidates[:n_turrets] if c in placed_xys]}"
            )
...
def enumerate_plans(game_state, config, *,
    ...
    J2: when `breach_pressure` is provided AND signal gates pass, append
    pressure-derived `defense:lane_block_*` templates to the defense pool.
    These compete in the search alongside the existing 13. Empty when no
    signal or gates fail — graceful no-op.
    """
    ...
    # J2 — pressure-derived lane-block templates.
    # PREPEND (not append): the per-defense×offense expansion below caps
    # at max_plans, so trailing defenses can be silently dropped. When
    # the pressure signal is real, lane_block templates are likely the
    # most relevant defense — evaluate them first. The standard
    # "defense:t0_full" etc. still appear in the search via the main
    # defense pool.
    lane_block = _enumerate_lane_block_templates(
        game_state, config, sp_budget=sp,
        breach_pressure=breach_pressure, debug_log=debug_log,
    )
    # M2v3 — K2 path-block templates (sim_rs path-derived defense).
    # Mirrors snorkeldink-v3-2's reactive_defence: when a tile is being
    # repeatedly breached, infer opp's likely path via find_path_to_edge
    # and place turrets along it. Necessary for the snorkeldink-v3-2 fix
    # (J2's lane_block alone doesn't break that loss).
    path_block = _gen_path_block_templates(
        game_state, config, sp_budget=sp,
        breach_pressure=breach_pressure, debug_log=debug_log,
    )
    extra_defenses = list(lane_block) + list(path_block)
    if extra_defenses:
        defenses = extra_defenses + list(defenses)
...
__all__ = [
    ...
    "_enumerate_lane_block_templates",
    "_find_valid_turret_positions_near",
    "ALL_LAUNCHERS",
]
