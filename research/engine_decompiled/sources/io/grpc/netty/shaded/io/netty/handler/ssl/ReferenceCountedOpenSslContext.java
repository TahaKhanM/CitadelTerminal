...
import io.grpc.netty.shaded.io.netty.handler.ssl.OpenSslCertificateException;
...
implements ReferenceCounted {
    ...
    private static final int DEFAULT_BIO_NON_APPLICATION_BUFFER_SIZE = AccessController.doPrivileged(new PrivilegedAction<Integer>(){
        ...
        @Override
        public Integer run() {
            return Math.max(1, SystemPropertyUtil.getInt("io.grpc.netty.shaded.io.netty.handler.ssl.openssl.bioNonApplicationBufferSize", 2048));
        }
    });
    ...
    protected static X509Certificate[] certificates(byte[][] chain2) {
        X509Certificate[] peerCerts = new X509Certificate[chain2.length];
        for (int i = 0; i < peerCerts.length; ++i) {
            peerCerts[i] = new OpenSslX509Certificate(chain2[i]);
        }
        return peerCerts;
    }
    ...
    extends CertificateVerifier {
        ...
        @Override
        public final int verify(long ssl, byte[][] chain2, String auth) {
            X509Certificate[] peerCerts = ReferenceCountedOpenSslContext.certificates(chain2);
