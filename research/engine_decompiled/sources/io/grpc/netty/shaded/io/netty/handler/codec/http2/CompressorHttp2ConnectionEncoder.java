...
extends DecoratingHttp2ConnectionEncoder {
        ...
        try {
            ...
            if (buf == null) {
                if (endOfStream) {
                    ...
                    ChannelFuture channelFuture = super.writeData(ctx, streamId, buf == null ? Unpooled.EMPTY_BUFFER : buf, padding2, true, promise);
            ...
            while (true) {
                ...
                super.writeData(ctx, streamId, buf, padding2, compressedEndOfStream, bufPromise);
    ...
    public ChannelFuture writeHeaders(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int padding2, boolean endStream, ChannelPromise promise) {
        try {
            ...
            ChannelFuture future = super.writeHeaders(ctx, streamId, headers, padding2, endStream, promise);
    ...
    public ChannelFuture writeHeaders(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding2, boolean endOfStream, ChannelPromise promise) {
        try {
            ...
            ChannelFuture future = super.writeHeaders(ctx, streamId, headers, streamDependency, weight, exclusive, padding2, endOfStream, promise);
            this.bindCompressorToStream(compressor, streamId);
            return future;
        }
        catch (Throwable e) {
