...
extends Random {
    ...
    private static volatile long initialSeedUniquifier = SystemPropertyUtil.getLong("io.grpc.netty.shaded.io.netty.initialSeedUniquifier", 0L);
    ...
    private static long newSeed() {
        ...
        long next2;
        long current;
        while (!seedUniquifier.compareAndSet(current, next2 = (actualCurrent = (current = seedUniquifier.get()) != 0L ? current : ThreadLocalRandom.getInitialSeedUniquifier()) * 181783497276652981L)) {
        }
    ...
    @Override
    protected int next(int bits2) {
        this.rnd = this.rnd * 25214903917L + 11L & 0xFFFFFFFFFFFFL;
        return (int)(this.rnd >>> 48 - bits2);
    }
    ...
    public long nextLong(long n) {
        ...
        while (n >= Integer.MAX_VALUE) {
            long nextn;
            int bits2 = this.next(2);
            long half = n >>> 1;
            long l = nextn = (bits2 & 2) == 0 ? half : n - half;
            if ((bits2 & 1) == 0) {
                offset += n - nextn;
            }
            n = nextn;
            ...
            if (secureRandom) {
                ...
                seedGeneratorThread = new Thread("initialSeedUniquifierGenerator"){
                    ...
                    public void run() {
                        ...
                        seedGeneratorEndTime = System.nanoTime();
                        long s2 = ((long)seed[0] & 0xFFL) << 56 | ((long)seed[1] & 0xFFL) << 48 | ((long)seed[2] & 0xFFL) << 40 | ((long)seed[3] & 0xFFL) << 32 | ((long)seed[4] & 0xFFL) << 24 | ((long)seed[5] & 0xFFL) << 16 | ((long)seed[6] & 0xFFL) << 8 | (long)seed[7] & 0xFFL;
                        seedQueue.add(s2);
                    }
                };
