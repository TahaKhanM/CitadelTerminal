...
import io.grpc.netty.shaded.io.netty.channel.FailedChannelFuture;
import io.grpc.netty.shaded.io.netty.channel.SingleThreadEventLoop;
import io.grpc.netty.shaded.io.netty.channel.SucceededChannelFuture;
...
ResourceLeakHint {
    ...
    static void invokeChannelRegistered(final AbstractChannelHandlerContext next2) {
    ...
    static void invokeChannelUnregistered(final AbstractChannelHandlerContext next2) {
        EventExecutor executor = next2.executor();
        if (executor.inEventLoop()) {
    ...
    static void invokeChannelActive(final AbstractChannelHandlerContext next2) {
        EventExecutor executor = next2.executor();
        if (executor.inEventLoop()) {
    ...
    static void invokeChannelInactive(final AbstractChannelHandlerContext next2) {
        EventExecutor executor = next2.executor();
        if (executor.inEventLoop()) {
    ...
    static void invokeExceptionCaught(final AbstractChannelHandlerContext next2, final Throwable cause) {
        block4: {
        ...
        if (this.invokeHandler()) {
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("An exception {}was thrown by a user handler's exceptionCaught() method while handling the following exception:", (Object)ThrowableUtil.stackTraceToString(error2), (Object)cause);
    ...
    static void invokeUserEventTriggered(final AbstractChannelHandlerContext next2, final Object event) {
    ...
    static void invokeChannelRead(final AbstractChannelHandlerContext next2, Object msg) {
        final Object m = next2.pipeline.touch(ObjectUtil.checkNotNull(msg, "msg"), next2);
        EventExecutor executor = next2.executor();
        if (executor.inEventLoop()) {
            next2.invokeChannelRead(m);
        } else {
            executor.execute(new Runnable(){
    ...
    static void invokeChannelReadComplete(final AbstractChannelHandlerContext next2) {
        EventExecutor executor = next2.executor();
        if (executor.inEventLoop()) {
            ...
            if (task == null) {
                next2.invokeChannelReadCompleteTask = task = new Runnable(){
    ...
    static void invokeChannelWritabilityChanged(final AbstractChannelHandlerContext next2) {
        EventExecutor executor = next2.executor();
        if (executor.inEventLoop()) {
            ...
            if (task == null) {
                next2.invokeChannelWritableStateChangedTask = task = new Runnable(){
    ...
    public ChannelFuture bind(final SocketAddress localAddress, final ChannelPromise promise) {
        ...
        final AbstractChannelHandlerContext next2 = this.findContextOutbound();
        EventExecutor executor = next2.executor();
        if (executor.inEventLoop()) {
    ...
    public ChannelHandlerContext read() {
        ...
        if (executor.inEventLoop()) {
            ...
            if (task == null) {
                next2.invokeReadTask = task = new Runnable(){
    ...
    public ChannelHandlerContext flush() {
        ...
        if (executor.inEventLoop()) {
            ...
            if (task == null) {
                next2.invokeFlushTask = task = new Runnable(){
    ...
    private void write(Object msg, boolean flush2, ChannelPromise promise) {
        AbstractChannelHandlerContext next2 = this.findContextOutbound();
        Object m = this.pipeline.touch(msg, next2);
        EventExecutor executor = next2.executor();
        if (executor.inEventLoop()) {
            if (flush2) {
                next2.invokeWriteAndFlush(m, promise);
            } else {
                next2.invokeWrite(m, promise);
            }
        } else {
            AbstractWriteTask task = flush2 ? WriteAndFlushTask.newInstance(next2, m, promise) : WriteTask.newInstance(next2, m, promise);
    ...
    private static boolean inExceptionCaught(Throwable cause) {
        do {
            ...
            if ((trace2 = cause.getStackTrace()) == null) continue;
            for (StackTraceElement t : trace2) {
                if (t == null) break;
    ...
    final void setAddPending() {
        boolean updated2 = HANDLER_STATE_UPDATER.compareAndSet(this, 0, 1);
        assert (updated2);
    }
