...
implements PoolChunkMetric {
    ...
    PoolChunk(PoolArena<T> arena, T memory, int size2, int offset) {
        this.unpooled = true;
        this.arena = arena;
        this.memory = memory;
        this.offset = offset;
    ...
    private PoolSubpage<T>[] newSubpageArray(int size2) {
    ...
    private long allocateSubpage(int normCapacity) {
        PoolSubpage<T> head2;
        PoolSubpage<T> poolSubpage = head2 = this.arena.findSubpagePoolHead(normCapacity);
        synchronized (poolSubpage) {
            ...
            if (subpage == null) {
                subpage = new PoolSubpage<T>(head2, this, id, this.runOffset(id), pageSize, normCapacity);
                subpages[subpageIdx] = subpage;
            } else {
                subpage.init(head2, normCapacity);
            }
    ...
    void free(long handle) {
        ...
        if (bitmapIdx != 0) {
            ...
            PoolSubpage<T> poolSubpage = head2 = this.arena.findSubpagePoolHead(subpage.elemSize);
            synchronized (poolSubpage) {
                if (subpage.free(head2, bitmapIdx & 0x3FFFFFFF)) {
                    return;
                }
            }
        }
    ...
    private void initBufWithSubpage(PooledByteBuf<T> buf, long handle, int bitmapIdx, int reqCapacity) {
        ...
        buf.init(this, handle, this.runOffset(memoryMapIdx) + (bitmapIdx & 0x3FFFFFFF) * subpage.elemSize + this.offset, reqCapacity, subpage.elemSize, this.arena.parent.threadCache());
    }
