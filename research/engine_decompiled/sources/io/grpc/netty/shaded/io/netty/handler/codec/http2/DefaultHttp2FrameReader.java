...
Http2FrameReader.Configuration {
    ...
    public void maxFrameSize(int max2) throws Http2Exception {
        if (!Http2CodecUtil.isMaxFrameSizeValid(max2)) {
            throw Http2Exception.streamError(this.streamId, Http2Error.FRAME_SIZE_ERROR, "Invalid MAX_FRAME_SIZE specified in sent settings: %d", max2);
        }
    ...
    @Override
    public void readFrame(ChannelHandlerContext ctx, ByteBuf input2, Http2FrameListener listener) throws Http2Exception {
        if (this.readError) {
            input2.skipBytes(input2.readableBytes());
            return;
        }
        try {
            do {
                if (this.readingHeaders) {
                    this.processHeaderState(input2);
                    if (this.readingHeaders) {
                        return;
                    }
                }
                this.processPayloadState(ctx, input2, listener);
                if (this.readingHeaders) continue;
                return;
            } while (input2.isReadable());
        }
    ...
    private void readDataFrame(ChannelHandlerContext ctx, ByteBuf payload, Http2FrameListener listener) throws Http2Exception {
        int padding2 = this.readPadding(payload);
        this.verifyPadding(padding2);
        int dataLength = DefaultHttp2FrameReader.lengthWithoutTrailingPadding(payload.readableBytes(), padding2);
        ByteBuf data = payload.readSlice(dataLength);
        listener.onDataRead(ctx, this.streamId, data, padding2, this.flags.endOfStream());
    ...
    private void readHeadersFrame(final ChannelHandlerContext ctx, ByteBuf payload, Http2FrameListener listener) throws Http2Exception {
        ...
        final int padding2 = this.readPadding(payload);
        this.verifyPadding(padding2);
        if (this.flags.priorityPresent()) {
            ...
            ByteBuf fragment = payload.readSlice(DefaultHttp2FrameReader.lengthWithoutTrailingPadding(payload.readableBytes(), padding2));
            this.headersContinuation = new HeadersContinuation(){
                ...
                public void processFragment(boolean endOfHeaders, ByteBuf fragment, Http2FrameListener listener) throws Http2Exception {
                    ...
                    if (endOfHeaders) {
                        listener.onHeadersRead(ctx, headersStreamId, hdrBlockBuilder.headers(), streamDependency, weight, exclusive, padding2, headersFlags.endOfStream());
                    }
                }
            };
        ...
        this.headersContinuation = new HeadersContinuation(){
            ...
            public void processFragment(boolean endOfHeaders, ByteBuf fragment, Http2FrameListener listener) throws Http2Exception {
                ...
                hdrBlockBuilder.addFragment(fragment, ctx.alloc(), endOfHeaders);
                if (endOfHeaders) {
                    listener.onHeadersRead(ctx, headersStreamId, hdrBlockBuilder.headers(), padding2, headersFlags.endOfStream());
                }
            }
        };
        ByteBuf fragment = payload.readSlice(DefaultHttp2FrameReader.lengthWithoutTrailingPadding(payload.readableBytes(), padding2));
    ...
    private void readPushPromiseFrame(final ChannelHandlerContext ctx, ByteBuf payload, Http2FrameListener listener) throws Http2Exception {
        ...
        this.headersContinuation = new HeadersContinuation(){
            ...
            public void processFragment(boolean endOfHeaders, ByteBuf fragment, Http2FrameListener listener) throws Http2Exception {
                ...
                if (endOfHeaders) {
                    listener.onPushPromiseRead(ctx, pushPromiseStreamId, promisedStreamId, this.headersBlockBuilder().headers(), padding2);
                }
            }
        };
        ByteBuf fragment = payload.readSlice(DefaultHttp2FrameReader.lengthWithoutTrailingPadding(payload.readableBytes(), padding2));
    ...
    private void verifyPadding(int padding2) throws Http2Exception {
        int len = DefaultHttp2FrameReader.lengthWithoutTrailingPadding(this.payloadLength, padding2);
        if (len < 0) {
            throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "Frame payload too small for padding.", new Object[0]);
        }
    }

    private static int lengthWithoutTrailingPadding(int readableBytes, int padding2) {
        return padding2 == 0 ? readableBytes : readableBytes - (padding2 - 1);
    }
