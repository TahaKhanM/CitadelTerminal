...
extends ChannelDuplexHandler {
    ...
    private static final class PendingWrite {
        ...
        void success(long total2) {
            if (this.promise.isDone()) {
                return;
            }
            this.progress(total2, total2);
            this.promise.trySuccess();
        }

        void progress(long progress, long total2) {
            if (this.promise instanceof ChannelProgressivePromise) {
                ((ChannelProgressivePromise)this.promise).tryProgress(progress, total2);
            }
        }
    }
}
