...
implements ChannelGroupFuture {
        ...
        public void operationComplete(ChannelFuture future) throws Exception {
            ...
            if (callSetDone) {
                if (DefaultChannelGroupFuture.this.failureCount > 0) {
                    ArrayList<Map.Entry<Channel, Throwable>> failed2 = new ArrayList<Map.Entry<Channel, Throwable>>(DefaultChannelGroupFuture.this.failureCount);
                    for (ChannelFuture f : DefaultChannelGroupFuture.this.futures.values()) {
                        if (f.isSuccess()) continue;
                        failed2.add(new DefaultEntry<Channel, Throwable>(f.channel(), f.cause()));
                    }
                    DefaultChannelGroupFuture.this.setFailure0(new ChannelGroupException(failed2));
                } else {
    ...
    public DefaultChannelGroupFuture setSuccess(Void result2) {
