...
implements EventLoopGroup {
    ...
    private EventLoop nextChild() throws Exception {
        ...
        EventLoop loop2 = this.idleChildren.poll();
        if (loop2 == null) {
            if (this.maxChannels > 0 && this.activeChildren.size() >= this.maxChannels) {
                throw this.tooManyChannels;
            }
            loop2 = this.newChild(this.childArgs);
            loop2.terminationFuture().addListener(this.childTerminationListener);
        }
        this.activeChildren.add(loop2);
        return loop2;
    }
}
