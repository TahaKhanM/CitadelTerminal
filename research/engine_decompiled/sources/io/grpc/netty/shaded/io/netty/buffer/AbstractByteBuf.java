...
extends ByteBuf {
    ...
    private static final String PROP_MODE = "io.grpc.netty.shaded.io.netty.buffer.bytebuf.checkAccessible";
    private static final boolean checkAccessible = SystemPropertyUtil.getBoolean("io.grpc.netty.shaded.io.netty.buffer.bytebuf.checkAccessible", true);
    ...
    public CharSequence readCharSequence(int length, Charset charset) {
        CharSequence sequence2 = this.getCharSequence(this.readerIndex, length, charset);
    ...
    public int setCharSequence(int index, CharSequence sequence2, Charset charset) {
        return this.setCharSequence0(index, sequence2, charset, false);
    }

    private int setCharSequence0(int index, CharSequence sequence2, Charset charset, boolean expand) {
        if (charset.equals(CharsetUtil.UTF_8)) {
            int length = ByteBufUtil.utf8MaxBytes(sequence2);
            if (expand) {
            ...
            return ByteBufUtil.writeUtf8(this, index, sequence2, sequence2.length());
        }
        if (charset.equals(CharsetUtil.US_ASCII) || charset.equals(CharsetUtil.ISO_8859_1)) {
            ...
            return ByteBufUtil.writeAscii(this, index, sequence2, length);
        }
        byte[] bytes2 = sequence2.toString().getBytes(charset);
        if (expand) {
            this.ensureWritable0(bytes2.length);
        }
        this.setBytes(index, bytes2);
    ...
    public ByteBuf readSlice(int length) {
        this.checkReadableBytes(length);
        ByteBuf slice2 = this.slice(this.readerIndex, length);
        this.readerIndex += length;
        return slice2;
    }
    ...
    public ByteBuf readRetainedSlice(int length) {
        this.checkReadableBytes(length);
        ByteBuf slice2 = this.retainedSlice(this.readerIndex, length);
        this.readerIndex += length;
        return slice2;
    }
    ...
    public int writeCharSequence(CharSequence sequence2, Charset charset) {
        int written = this.setCharSequence0(this.writerIndex, sequence2, charset, true);
