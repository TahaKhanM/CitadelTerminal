...
implements Http2FrameReader {
    ...
    @Override
    public void readFrame(ChannelHandlerContext ctx, ByteBuf input2, final Http2FrameListener listener) throws Http2Exception {
        this.reader.readFrame(ctx, input2, new Http2FrameListener(){
            ...
            public int onDataRead(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding2, boolean endOfStream) throws Http2Exception {
                Http2InboundFrameLogger.this.logger.logData(Http2FrameLogger.Direction.INBOUND, ctx, streamId, data, padding2, endOfStream);
                return listener.onDataRead(ctx, streamId, data, padding2, endOfStream);
            }

            @Override
            public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int padding2, boolean endStream) throws Http2Exception {
                Http2InboundFrameLogger.this.logger.logHeaders(Http2FrameLogger.Direction.INBOUND, ctx, streamId, headers, padding2, endStream);
                listener.onHeadersRead(ctx, streamId, headers, padding2, endStream);
            }
            ...
            public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding2, boolean endStream) throws Http2Exception {
                Http2InboundFrameLogger.this.logger.logHeaders(Http2FrameLogger.Direction.INBOUND, ctx, streamId, headers, streamDependency, weight, exclusive, padding2, endStream);
                listener.onHeadersRead(ctx, streamId, headers, streamDependency, weight, exclusive, padding2, endStream);
            }
            ...
            public void onPushPromiseRead(ChannelHandlerContext ctx, int streamId, int promisedStreamId, Http2Headers headers, int padding2) throws Http2Exception {
                Http2InboundFrameLogger.this.logger.logPushPromise(Http2FrameLogger.Direction.INBOUND, ctx, streamId, promisedStreamId, headers, padding2);
                listener.onPushPromiseRead(ctx, streamId, promisedStreamId, headers, padding2);
            }
