...
implements Http2DataFrame {
    ...
    public DefaultHttp2DataFrame(ByteBuf content, boolean endStream, int padding2) {
        this.content = ObjectUtil.checkNotNull(content, "content");
        this.endStream = endStream;
        Http2CodecUtil.verifyPadding(padding2);
        this.padding = padding2;
        if ((long)this.content().readableBytes() + (long)padding2 > Integer.MAX_VALUE) {
            throw new IllegalArgumentException("content + padding must be <= Integer.MAX_VALUE");
        }
        this.initialFlowControlledBytes = this.content().readableBytes() + padding2;
    }
