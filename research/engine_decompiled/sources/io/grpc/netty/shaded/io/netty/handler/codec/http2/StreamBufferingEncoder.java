...
extends DecoratingHttp2ConnectionEncoder {
    ...
    public ChannelFuture writeHeaders(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding2, boolean endOfStream, ChannelPromise promise) {
        ...
        if (this.isExistingStream(streamId) || this.connection().goAwayReceived()) {
            return super.writeHeaders(ctx, streamId, headers, streamDependency, weight, exclusive, padding2, endOfStream, promise);
        }
        ...
        pendingStream.frames.add(new HeadersFrame(headers, streamDependency, weight, exclusive, padding2, endOfStream, promise));
        return promise;
    }
    ...
    public ChannelFuture writeData(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding2, boolean endOfStream, ChannelPromise promise) {
        if (this.isExistingStream(streamId)) {
            return super.writeData(ctx, streamId, data, padding2, endOfStream, promise);
        }
        PendingStream pendingStream = this.pendingStreams.get(streamId);
        if (pendingStream != null) {
            pendingStream.frames.add(new DataFrame(data, padding2, endOfStream, promise));
        } else {
    ...
    extends Frame {
        ...
        DataFrame(ByteBuf data, int padding2, boolean endOfStream, ChannelPromise promise) {
            super(promise);
            this.data = data;
    ...
    extends Frame {
        ...
        HeadersFrame(Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding2, boolean endOfStream, ChannelPromise promise) {
            super(promise);
