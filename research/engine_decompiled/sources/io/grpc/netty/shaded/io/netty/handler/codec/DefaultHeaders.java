...
implements Headers<K, V, T> {
    ...
    public final int hashCode(HashingStrategy<V> valueHashingStrategy) {
        ...
        for (K name : this.names()) {
            result2 = 31 * result2 + this.hashingStrategy.hashCode(name);
            List<V> values = this.getAll(name);
            for (int i = 0; i < values.size(); ++i) {
                result2 = 31 * result2 + valueHashingStrategy.hashCode(values.get(i));
            }
        }
        return result2;
    }
    ...
    protected HeaderEntry<K, V> newHeaderEntry(int h, K name, V value, HeaderEntry<K, V> next2) {
        return new HeaderEntry<K, V>(h, name, value, next2, this.head);
    }
    ...
    private V remove0(int h, int i, K name) {
        ...
        while (next2 != null) {
            if (next2.hash == h && this.hashingStrategy.equals(name, next2.key)) {
                value = next2.value;
                e.next = next2.next;
                next2.remove();
                --this.size;
    ...
    implements Map.Entry<K, V> {
        ...
        HeaderEntry(int hash, K key, V value, HeaderEntry<K, V> next2, HeaderEntry<K, V> head2) {
            this.hash = hash;
            this.key = key;
            this.value = value;
            this.next = next2;
            this.after = head2;
            this.before = head2.before;
