...
import io.grpc.netty.shaded.io.netty.handler.ssl.PemValue;
...
implements PemEncoded {
     ...
     */
    static PemEncoded toPEM(ByteBufAllocator allocator, boolean useDirect, X509Certificate ... chain2) throws CertificateEncodingException {
        X509Certificate first;
        if (chain2 == null || chain2.length == 0) {
            throw new IllegalArgumentException("X.509 certificate chain can't be null or empty");
        }
        if (chain2.length == 1 && (first = chain2[0]) instanceof PemEncoded) {
            return ((PemEncoded)((Object)first)).retain();
        }
        boolean success = false;
        ReferenceCounted pem = null;
        try {
            for (X509Certificate cert : chain2) {
                if (cert == null) {
                    throw new IllegalArgumentException("Null element in chain: " + Arrays.toString(chain2));
                }
                pem = cert instanceof PemEncoded ? PemX509Certificate.append(allocator, useDirect, (PemEncoded)((Object)cert), chain2.length, (ByteBuf)pem) : PemX509Certificate.append(allocator, useDirect, cert, chain2.length, (ByteBuf)pem);
            }
    ...
    private static ByteBuf append(ByteBufAllocator allocator, boolean useDirect, PemEncoded encoded, int count2, ByteBuf pem) {
        ByteBuf content = encoded.content();
        if (pem == null) {
            pem = PemX509Certificate.newBuffer(allocator, useDirect, content.readableBytes() * count2);
        }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private static ByteBuf append(ByteBufAllocator allocator, boolean useDirect, X509Certificate cert, int count2, ByteBuf pem) throws CertificateEncodingException {
        ByteBuf encoded = Unpooled.wrappedBuffer(cert.getEncoded());
        try {
            ByteBuf base64 = SslUtils.toBase64(allocator, encoded);
            try {
                if (pem == null) {
                    pem = PemX509Certificate.newBuffer(allocator, useDirect, (BEGIN_CERT.length + base64.readableBytes() + END_CERT.length) * count2);
                }
    ...
    public ByteBuf content() {
        int count2 = this.refCnt();
        if (count2 <= 0) {
            throw new IllegalReferenceCountException(count2);
        }
        return this.content;
    }
