...
extends AbstractTrafficShapingHandler {
            ...
            synchronized (perChannel2) {
                if (channel.isActive()) {
                    for (ToSend toSend : perChannel.messagesQueue) {
                        ...
                        perChannel.channelTrafficCounter.bytesRealWriteFlowControl(size2);
                        perChannel.queueSize -= size2;
                        this.queuesSize.addAndGet(-size2);
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        long size2 = this.calculateSize(msg);
        long now = TrafficCounter.milliSecondFromNano();
        if (size2 > 0L) {
            long waitGlobal = this.trafficCounter.readTimeToWait(size2, this.getReadLimit(), this.maxTime, now);
            Integer key = ctx.channel().hashCode();
            PerChannel perChannel = (PerChannel)this.channelQueues.get(key);
            long wait = 0L;
            if (perChannel != null) {
                wait = perChannel.channelTrafficCounter.readTimeToWait(size2, this.readChannelLimit, this.maxTime, now);
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
        ...
        long now = TrafficCounter.milliSecondFromNano();
        if (size2 > 0L) {
            long waitGlobal = this.trafficCounter.writeTimeToWait(size2, this.getWriteLimit(), this.maxTime, now);
            Integer key = ctx.channel().hashCode();
            PerChannel perChannel = (PerChannel)this.channelQueues.get(key);
            long wait = 0L;
            if (perChannel != null) {
                wait = perChannel.channelTrafficCounter.writeTimeToWait(size2, this.writeChannelLimit, this.maxTime, now);
            ...
            if (wait >= 10L) {
                ...
                this.submitWrite(ctx, msg, size2, wait, now, promise);
                return;
            }
        }
        this.submitWrite(ctx, msg, size2, 0L, now, promise);
    }
     ...
     */
    @Override
    protected void submitWrite(final ChannelHandlerContext ctx, Object msg, long size2, long writedelay, long now, ChannelPromise promise) {
        ToSend newToSend;
        Channel channel = ctx.channel();
        Integer key = channel.hashCode();
        PerChannel perChannel = (PerChannel)this.channelQueues.get(key);
        ...
        synchronized (perChannel2) {
            ...
            newToSend = new ToSend(delay + now, msg, size2, promise);
    ...
    private static final class ToSend {
        ...
        private ToSend(long delay, Object toSend, long size2, ChannelPromise promise) {
