...
extends ChannelDuplexHandler {
    ...
    private boolean acceptStream(int streamId, byte priority, boolean remoteSideClosed, boolean localSideClosed) {
        ...
        boolean remote2 = this.isRemoteInitiatedId(streamId);
        int n = maxConcurrentStreams = remote2 ? this.localConcurrentStreams : this.remoteConcurrentStreams;
        if (this.spdySession.numActiveStreams(remote2) >= maxConcurrentStreams) {
            return false;
        }
        this.spdySession.acceptStream(streamId, priority, remoteSideClosed, localSideClosed, this.initialSendWindowSize, this.initialReceiveWindowSize, remote2);
        if (remote2) {
    ...
    private void halfCloseStream(int streamId, boolean remote2, ChannelFuture future) {
        if (remote2) {
