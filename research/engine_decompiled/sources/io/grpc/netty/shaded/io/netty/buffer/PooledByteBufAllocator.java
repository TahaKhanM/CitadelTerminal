...
implements ByteBufAllocatorMetricProvider {
    ...
    private static <T> PoolArena<T>[] newArenaArray(int size2) {
        return new PoolArena[size2];
    }
    ...
    public int numThreadLocalCaches() {
        ...
        for (PoolArena<Object> arena : arenas) {
            total2 += arena.numThreadCaches.get();
        }
        return total2;
    }
    ...
    static {
        int defaultPageSize = SystemPropertyUtil.getInt("io.grpc.netty.shaded.io.netty.allocator.pageSize", 8192);
        ...
        int defaultMaxOrder = SystemPropertyUtil.getInt("io.grpc.netty.shaded.io.netty.allocator.maxOrder", 11);
        ...
        DEFAULT_NUM_HEAP_ARENA = Math.max(0, SystemPropertyUtil.getInt("io.grpc.netty.shaded.io.netty.allocator.numHeapArenas", (int)Math.min((long)defaultMinNumArena, runtime.maxMemory() / (long)defaultChunkSize / 2L / 3L)));
        DEFAULT_NUM_DIRECT_ARENA = Math.max(0, SystemPropertyUtil.getInt("io.grpc.netty.shaded.io.netty.allocator.numDirectArenas", (int)Math.min((long)defaultMinNumArena, PlatformDependent.maxDirectMemory() / (long)defaultChunkSize / 2L / 3L)));
