...
import io.grpc.netty.shaded.io.netty.handler.ssl.AbstractSniHandler;
...
extends AbstractSniHandler<SslContext> {
    ...
    implements AsyncMapping<String, SslContext> {
        ...
        @Override
        public Future<SslContext> map(String input2, Promise<SslContext> promise) {
            SslContext context;
            try {
                context = this.mapping.map(input2);
            }
            catch (Throwable cause) {
