...
implements Comparable<HttpResponseStatus> {
    ...
    private HttpResponseStatus(int code, String reasonPhrase, boolean bytes2) {
        if (code < 0) {
        ...
        this.bytes = (byte[])(bytes2 ? (codeString + ' ' + reasonPhrase).getBytes(CharsetUtil.US_ASCII) : null);
    }
