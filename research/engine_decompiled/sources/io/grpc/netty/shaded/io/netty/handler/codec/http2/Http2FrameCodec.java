...
import io.grpc.netty.shaded.io.netty.handler.codec.http2.DefaultHttp2PingFrame;
import io.grpc.netty.shaded.io.netty.handler.codec.http2.DefaultHttp2ResetFrame;
import io.grpc.netty.shaded.io.netty.handler.codec.http2.DefaultHttp2SettingsFrame;
import io.grpc.netty.shaded.io.netty.handler.codec.http2.DefaultHttp2UnknownFrame;
...
extends Http2ConnectionHandler {
    ...
    final boolean consumeBytes(int streamId, int bytes2) throws Http2Exception {
        Boolean upgraded;
        Http2Stream stream = this.connection().stream(streamId);
        if (stream != null && streamId == 1 && Boolean.TRUE.equals(upgraded = (Boolean)stream.getProperty(this.upgradeKey))) {
            return false;
        }
        return this.connection().local().flowController().consumeBytes(stream, bytes2);
    }
    ...
    final boolean isWritable(DefaultHttp2FrameStream stream) {
        Http2Stream s2 = stream.stream;
        return s2 != null && this.connection().remote().flowController().isWritable(s2);
    }
    ...
    implements Http2FrameListener {
        ...
        @Override
        public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding2, boolean endStream) {
            this.onHeadersRead(ctx, streamId, headers, padding2, endStream);
        }

        @Override
        public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int padding2, boolean endOfStream) {
            Http2FrameCodec.this.onHttp2Frame(ctx, new DefaultHttp2HeadersFrame(headers, endOfStream, padding2).stream(this.requireStream(streamId)));
        }

        @Override
        public int onDataRead(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding2, boolean endOfStream) {
            Http2FrameCodec.this.onHttp2Frame(ctx, new DefaultHttp2DataFrame(data, endOfStream, padding2).stream(this.requireStream(streamId)).retain());
            return 0;
        }
        ...
        @Override
        public void onPushPromiseRead(ChannelHandlerContext ctx, int streamId, int promisedStreamId, Http2Headers headers, int padding2) {
        }
