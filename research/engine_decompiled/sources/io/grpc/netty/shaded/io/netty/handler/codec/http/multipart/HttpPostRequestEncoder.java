...
import io.grpc.netty.shaded.io.netty.handler.codec.http.multipart.InternalAttribute;
...
implements ChunkedInput<HttpContent> {
    ...
    private String encodeAttribute(String s2, Charset charset) throws ErrorDataEncoderException {
        if (s2 == null) {
            return "";
        }
        try {
            String encoded = URLEncoder.encode(s2, charset.name());
    ...
    private ByteBuf fillByteBuf() {
        ...
        ByteBuf slice2 = this.currentBuffer;
    ...
    private HttpContent encodeNextChunkUrlEncoded(int sizeleft) throws ErrorDataEncoderException {
        ...
        if (this.isKey) {
            ...
            this.isKey = false;
            this.currentBuffer = this.currentBuffer == null ? Unpooled.wrappedBuffer(buffer, Unpooled.wrappedBuffer("=".getBytes())) : Unpooled.wrappedBuffer(this.currentBuffer, buffer, Unpooled.wrappedBuffer("=".getBytes()));
            size2 -= buffer.readableBytes() + 1;
            if (this.currentBuffer.readableBytes() >= 8096) {
                buffer = this.fillByteBuf();
                return new DefaultHttpContent(buffer);
            }
        ...
        try {
            buffer = ((HttpData)this.currentData).getChunk(size2);
        }
        catch (IOException e) {
            throw new ErrorDataEncoderException(e);
        }
        ByteBuf delimiter = null;
        if (buffer.readableBytes() < size2) {
            this.isKey = true;
    ...
    private HttpContent nextChunk() throws ErrorDataEncoderException {
        ...
        int size2 = this.calculateRemainingSize();
        if (size2 <= 0) {
        ...
        if (this.currentData != null) {
            ...
            size2 = this.calculateRemainingSize();
        }
    ...
    implements HttpRequest {
        ...
        public void setDecoderResult(DecoderResult result2) {
            this.request.setDecoderResult(result2);
        }
    }
