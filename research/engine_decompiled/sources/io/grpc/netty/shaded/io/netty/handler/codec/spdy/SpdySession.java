...
final class SpdySession {
    ...
    int numActiveStreams(boolean remote2) {
        if (remote2) {
    ...
    void acceptStream(int streamId, byte priority, boolean remoteSideClosed, boolean localSideClosed, int sendWindowSize, int receiveWindowSize, boolean remote2) {
        StreamState state;
    ...
    private StreamState removeActiveStream(int streamId, boolean remote2) {
    ...
    void removeStream(int streamId, Throwable cause, boolean remote2) {
        StreamState state = this.removeActiveStream(streamId, remote2);
        if (state != null) {
    ...
    void closeRemoteSide(int streamId, boolean remote2) {
        StreamState state = this.activeStreams.get(streamId);
        if (state != null) {
            state.closeRemoteSide();
            if (state.isLocalSideClosed()) {
                this.removeActiveStream(streamId, remote2);
            }
        }
    }
    ...
    void closeLocalSide(int streamId, boolean remote2) {
        StreamState state = this.activeStreams.get(streamId);
        if (state != null) {
    ...
    implements Comparator<Integer> {
        ...
        public int compare(Integer id1, Integer id2) {
            ...
            int result2 = state1.getPriority() - state2.getPriority();
            if (result2 != 0) {
                return result2;
            }
            return id1 - id2;
