...
public class TrafficCounter {
    ...
    @Deprecated
    public long readTimeToWait(long size2, long limitTraffic, long maxTime) {
        return this.readTimeToWait(size2, limitTraffic, maxTime, TrafficCounter.milliSecondFromNano());
    }

    public long readTimeToWait(long size2, long limitTraffic, long maxTime, long now) {
        this.bytesRecvFlowControl(size2);
        if (size2 == 0L || limitTraffic == 0L) {
            return 0L;
        }
        long lastTimeCheck = this.lastTime.get();
        long sum2 = this.currentReadBytes.get();
        ...
        if (interval > 10L) {
            long time = sum2 * 1000L / limitTraffic - interval + pastDelay;
            if (time > 10L) {
                if (logger.isDebugEnabled()) {
                    logger.debug("Time: " + time + ':' + sum2 + ':' + interval + ':' + pastDelay);
                }
    ...
    @Deprecated
    public long writeTimeToWait(long size2, long limitTraffic, long maxTime) {
        return this.writeTimeToWait(size2, limitTraffic, maxTime, TrafficCounter.milliSecondFromNano());
    }

    public long writeTimeToWait(long size2, long limitTraffic, long maxTime, long now) {
        this.bytesWriteFlowControl(size2);
        if (size2 == 0L || limitTraffic == 0L) {
            return 0L;
        }
        long lastTimeCheck = this.lastTime.get();
        long sum2 = this.currentWrittenBytes.get();
