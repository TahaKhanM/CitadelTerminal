...
import io.grpc.netty.shaded.io.netty.handler.codec.http2.EmptyHttp2Headers;
...
public final class HttpConversionUtil {
    ...
    public static HttpResponseStatus parseStatus(CharSequence status) throws Http2Exception {
        ...
        try {
            result2 = HttpResponseStatus.parseLine(status);
        ...
        while (valuesIter.hasNext()) {
            ...
            try {
                ...
                if (index != -1) {
                    int start = 0;
                    do {
                        result2.add(lowerCased.subSequence(start, index, false).trim(), AsciiString.EMPTY_STRING);
                    ...
                    result2.add(lowerCased.subSequence(start, lowerCased.length(), false).trim(), AsciiString.EMPTY_STRING);
                    continue;
                }
