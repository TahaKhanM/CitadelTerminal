...
public final class ChannelOutboundBuffer {
    ...
    public void addMessage(Object msg, int size2, ChannelPromise promise) {
        Entry entry = Entry.newInstance(msg, size2, ChannelOutboundBuffer.total(msg), promise);
    ...
    private void clearNioBuffers() {
        ...
        if (count2 > 0) {
            ...
            Arrays.fill(NIO_BUFFERS.get(), 0, count2, null);
        }
    }
        ...
        while (this.isFlushedEntry(entry) && entry.msg instanceof ByteBuf) {
            if (!entry.cancelled) {
                ...
                if (readableBytes > 0) {
                    ...
                    int count2 = entry.count;
                    if (count2 == -1) {
                        entry.count = count2 = buf.nioBufferCount();
                    }
                    if ((neededSpace = Math.min(maxCount, nioBufferCount + count2)) > nioBuffers.length) {
    ...
    private static ByteBuffer[] expandNioBufferArray(ByteBuffer[] array, int neededSpace, int size2) {
    ...
    static final class Entry {
        ...
        static Entry newInstance(Object msg, int size2, long total2, ChannelPromise promise) {
            ...
            entry.msg = msg;
            entry.pendingSize = size2 + CHANNEL_OUTBOUND_BUFFER_ENTRY_OVERHEAD;
            entry.total = total2;
