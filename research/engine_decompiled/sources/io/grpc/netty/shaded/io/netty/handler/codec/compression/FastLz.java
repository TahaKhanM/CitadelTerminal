...
final class FastLz {
    ...
    static int compress(byte[] input2, int inOffset, int inLength, byte[] output, int outOffset, int proposedLevel) {
        int hslot;
        ...
        if (inLength < 4) {
            if (inLength != 0) {
                ...
                ++ipBound;
                while (ip <= ipBound) {
                    output[outOffset + op++] = input2[inOffset + ip++];
                }
                return inLength + 1;
            }
            return 0;
        ...
        while (ip < ipLimit) {
            ...
            block37: {
                ...
                block39: {
                    block38: {
                        ...
                        if (level == 2 && input2[inOffset + ip] == input2[inOffset + ip - 1] && FastLz.readU16(input2, inOffset + ip - 1) == FastLz.readU16(input2, inOffset + ip + 1)) {
                            distance = 1L;
                            ip += 3;
                            ref = anchor + 2;
                            matchLabel = true;
                        ...
                        if (input2[inOffset + ref++] == input2[inOffset + ip++] && input2[inOffset + ref++] == input2[inOffset + ip++] && input2[inOffset + ref++] == input2[inOffset + ip++]) break block39;
                    }
                    output[outOffset + op++] = input2[inOffset + anchor++];
                    ip = anchor;
                ...
                if (level == 2 && distance >= 8191L) {
                    if (input2[inOffset + ip++] != input2[inOffset + ref++] || input2[inOffset + ip++] != input2[inOffset + ref++]) {
                        output[outOffset + op++] = input2[inOffset + anchor++];
                        ip = anchor;
                        if (++copy2 != 32) continue;
                        copy2 = 0;
            ...
            if (--distance == 0L) {
                byte x = input2[inOffset + ip - 1];
                for (ip = anchor + len; ip < ipBound && input2[inOffset + ref++] == x; ++ip) {
                }
            } else if (input2[inOffset + ref++] == input2[inOffset + ip++] && input2[inOffset + ref++] == input2[inOffset + ip++] && input2[inOffset + ref++] == input2[inOffset + ip++] && input2[inOffset + ref++] == input2[inOffset + ip++] && input2[inOffset + ref++] == input2[inOffset + ip++] && input2[inOffset + ref++] == input2[inOffset + ip++] && input2[inOffset + ref++] == input2[inOffset + ip++] && input2[inOffset + ref++] == input2[inOffset + ip++]) {
                while (ip < ipBound && input2[inOffset + ref++] == input2[inOffset + ip++]) {
                }
            }
            if (copy2 != 0) {
                output[outOffset + op - copy2 - 1] = (byte)(copy2 - 1);
            } else {
                --op;
            }
            copy2 = 0;
        ...
        if (level == 2) {
            int n = outOffset;
            output[n] = (byte)(output[n] | 0x20);
        }
        return op;
    }

    static int decompress(byte[] input2, int inOffset, int inLength, byte[] output, int outOffset, int outLength) {
        int level = (input2[inOffset] >> 5) + 1;
        if (level != 1 && level != 2) {
            throw new DecompressionException(String.format("invalid level: %d (expected: %d or %d)", level, 1, 2));
        }
        int ip = 0;
        int op = 0;
        long ctrl = input2[inOffset + ip++] & 0x1F;
        boolean loop2 = true;
        do {
            int ref = op;
            long len = ctrl >> 5;
            ...
            if (ctrl >= 32L) {
                ...
                if (--len == 6L) {
                    if (level == 1) {
                        len += (long)(input2[inOffset + ip++] & 0xFF);
                    } else {
                        do {
                            code = input2[inOffset + ip++] & 0xFF;
                            len += (long)code;
                        } while (code == 255);
                    }
                }
                if (level == 1) {
                    ref -= input2[inOffset + ip++] & 0xFF;
                } else {
                    code = input2[inOffset + ip++] & 0xFF;
                    ref -= code;
                    if (code == 255 && ofs == 7936L) {
                        ofs = (input2[inOffset + ip++] & 0xFF) << 8;
                        ref = (int)((long)op - (ofs += (long)(input2[inOffset + ip++] & 0xFF)) - 8191L);
                    }
                }
