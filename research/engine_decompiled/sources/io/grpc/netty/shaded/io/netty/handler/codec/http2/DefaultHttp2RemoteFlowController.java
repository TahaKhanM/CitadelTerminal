...
implements Http2RemoteFlowController {
    ...
    implements StreamByteDistributor.StreamState {
        ...
        void enqueueFrame(Http2RemoteFlowController.FlowControlled frame) {
            Http2RemoteFlowController.FlowControlled last2 = this.pendingWriteQueue.peekLast();
            if (last2 == null) {
                this.enqueueFrameWithoutMerge(frame);
                return;
            }
            int lastSize = last2.size();
            if (last2.merge(DefaultHttp2RemoteFlowController.this.ctx, frame)) {
                this.incrementPendingBytes(last2.size() - lastSize, true);
                return;
            }
            this.enqueueFrameWithoutMerge(frame);
        }
        ...
        void cancel(Http2Error error2, Throwable cause) {
            this.cancelled = true;
            if (this.writing) {
                return;
            }
            ...
            if (frame != null) {
                Http2Exception exception = Http2Exception.streamError(this.stream.id(), error2, cause, "Stream closed before write could take place", new Object[0]);
                do {
        ...
        private void decrementPendingBytes(int bytes2, boolean updateStreamableBytes) {
            this.incrementPendingBytes(-bytes2, updateStreamableBytes);
        }

        private void decrementFlowControlWindow(int bytes2) {
            try {
                int negativeBytes = -bytes2;
