...
implements Iterable<ByteBuf> {
    ...
    private int addComponents0(boolean increaseWriterIndex, int cIndex, ByteBuf[] buffers, int offset, int len) {
        ...
        try {
            ...
            while (i < len && (b = buffers[i++]) != null) {
                int size2;
                if ((cIndex = this.addComponent0(increaseWriterIndex, cIndex, b) + 1) <= (size2 = this.components.size())) continue;
                cIndex = size2;
            }
            int n = cIndex;
            return n;
    ...
    private int addComponents0(boolean increaseIndex, int cIndex, Iterable<ByteBuf> buffers) {
        ...
        if (!(buffers instanceof Collection)) {
            ArrayList<ByteBuf> list2 = new ArrayList<ByteBuf>();
            try {
    ...
    public List<ByteBuf> decompose(int offset, int length) {
        ...
        ArrayList<ByteBuf> slice2 = new ArrayList<ByteBuf>(this.components.size());
        ...
        for (int i = 0; i < slice2.size(); ++i) {
            slice2.set(i, ((ByteBuf)slice2.get(i)).slice());
        }
        return slice2;
    }
    ...
    public CompositeByteBuf capacity(int newCapacity) {
        ...
        if (newCapacity > oldCapacity) {
            ...
            if (nComponents < this.maxNumComponents) {
                ByteBuf padding2 = this.allocBuffer(paddingLength);
                padding2.setIndex(0, paddingLength);
                this.addComponent0(false, this.components.size(), padding2);
            } else {
                ByteBuf padding3 = this.allocBuffer(paddingLength);
                padding3.setIndex(0, paddingLength);
                this.addComponent0(false, this.components.size(), padding3);
                this.consolidateIfNeeded();
            }
    ...
    public int setBytes(int index, InputStream in, int length) throws IOException {
        ...
        do {
            ...
            int localReadBytes = s2.setBytes(index - adjustment, in, localLength);
            if (localReadBytes < 0) {
    ...
    public int setBytes(int index, FileChannel in, long position, int length) throws IOException {
        ...
        do {
            ...
            int localReadBytes = s2.setBytes(index - adjustment, in, position + (long)readBytes, localLength);
            if (localReadBytes == 0) break;
            if (localReadBytes < 0) {
                if (readBytes != 0) break;
                return -1;
    ...
    public CompositeByteBuf consolidate() {
        ...
        Component last2 = (Component)this.components.get(numComponents - 1);
        int capacity = last2.endOffset;
        ByteBuf consolidated = this.allocBuffer(capacity);
        for (int i = 0; i < numComponents; ++i) {
            Component c = (Component)this.components.get(i);
            ByteBuf b = c.buf;
    ...
    public CompositeByteBuf consolidate(int cIndex, int numComponents) {
        ...
        Component last2 = (Component)this.components.get(endCIndex - 1);
        int capacity = last2.endOffset - ((Component)this.components.get((int)cIndex)).offset;
        ByteBuf consolidated = this.allocBuffer(capacity);
