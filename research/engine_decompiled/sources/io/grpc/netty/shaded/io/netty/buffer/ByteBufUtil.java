...
public final class ByteBufUtil {
    ...
    public static byte decodeHexByte(CharSequence s2, int pos) {
        return StringUtil.decodeHexByte(s2, pos);
    }
    ...
    public static int writeUtf8(ByteBuf buf, CharSequence seq) {
        return ByteBufUtil.reserveAndWriteUtf8(buf, seq, ByteBufUtil.utf8MaxBytes(seq));
    }
    ...
    public static int reserveAndWriteUtf8(ByteBuf buf, CharSequence seq, int reserveBytes) {
        ...
        byte[] bytes2 = seq.toString().getBytes(CharsetUtil.UTF_8);
    ...
    public static int utf8Bytes(CharSequence seq) {
        ...
        for (i = 0; i < seqLength && seq.charAt(i) < '\u0080'; ++i) {
        }
        return i < seqLength ? i + ByteBufUtil.utf8Bytes(seq, i, seqLength) : i;
    }
    ...
    public static int writeAscii(ByteBuf buf, CharSequence seq) {
        ...
        if (!(seq instanceof AsciiString)) {
            ...
            byte[] bytes2 = seq.toString().getBytes(CharsetUtil.US_ASCII);
            buf.writeBytes(bytes2);
            return bytes2.length;
        }
    ...
    static String decodeString(ByteBuf src, int readerIndex, int len, Charset charset) {
        ...
        if (src.nioBufferCount() == 1) {
            ByteBufUtil.decodeString(decoder, src.nioBuffer(readerIndex, len), dst);
        } else {
    ...
    public static byte[] getBytes(ByteBuf buf, int start, int length, boolean copy2) {
        ...
        if (buf.hasArray()) {
            if (copy2 || start != 0 || length != buf.capacity()) {
    ...
    static void readBytes(ByteBufAllocator allocator, ByteBuffer buffer, int position, int length, OutputStream out) throws IOException {
        if (buffer.hasArray()) {
            ...
            if (allocator.isDirectBufferPooled()) {
                ...
                ByteBufUtil.getBytes(buffer, new byte[chunkLen], 0, chunkLen, out, length);
            }
        }
    }
    ...
    static {
        ...
        String allocType = SystemPropertyUtil.get("io.grpc.netty.shaded.io.netty.allocator.type", PlatformDependent.isAndroid() ? "unpooled" : "pooled");
    ...
    private static final class HexUtil {
        ...
        static {
            ...
            for (i = 0; i < HEXPADDING.length; ++i) {
                int padding2 = HEXPADDING.length - i;
                buf = new StringBuilder(padding2 * 3);
                for (j = 0; j < padding2; ++j) {
                    buf.append("   ");
                }
            ...
            for (i = 0; i < BYTEPADDING.length; ++i) {
                int padding3 = BYTEPADDING.length - i;
                buf = new StringBuilder(padding3);
                for (j = 0; j < padding3; ++j) {
                    buf.append(' ');
                }
