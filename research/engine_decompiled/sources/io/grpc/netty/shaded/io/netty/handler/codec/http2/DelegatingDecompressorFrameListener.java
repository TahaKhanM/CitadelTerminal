...
extends Http2FrameListenerDecorator {
    ...
    public int onDataRead(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding2, boolean endOfStream) throws Http2Exception {
        ...
        if (decompressor == null) {
            return this.listener.onDataRead(ctx, streamId, data, padding2, endOfStream);
        }
        ...
        if (buf == null) {
            if (endOfStream) {
                this.listener.onDataRead(ctx, streamId, Unpooled.EMPTY_BUFFER, padding2, true);
            }
        ...
        try {
            ...
            while (true) {
                ...
                flowController.consumeBytes(stream, this.listener.onDataRead(ctx, streamId, buf, padding2, decompressedEndOfStream));
    ...
    public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int padding2, boolean endStream) throws Http2Exception {
        ...
        this.listener.onHeadersRead(ctx, streamId, headers, padding2, endStream);
    }
    ...
    public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding2, boolean endStream) throws Http2Exception {
        this.initDecompressor(ctx, streamId, headers, endStream);
        this.listener.onHeadersRead(ctx, streamId, headers, streamDependency, weight, exclusive, padding2, endStream);
    }
    ...
    implements Http2LocalFlowController {
        ...
        @Override
        public void receiveFlowControlledFrame(Http2Stream stream, ByteBuf data, int padding2, boolean endOfStream) throws Http2Exception {
            this.flowController.receiveFlowControlledFrame(stream, data, padding2, endOfStream);
        }
