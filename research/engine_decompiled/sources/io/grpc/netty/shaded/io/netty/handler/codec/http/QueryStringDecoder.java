...
public class QueryStringDecoder {
     ...
     */
    private static Map<String, List<String>> decodeParams(String s2, int from2, Charset charset, int paramsLimit) {
        int i;
        int len = s2.length();
        if (from2 >= len) {
        ...
        block5: for (i = from2; i < len; ++i) {
            switch (s2.charAt(i)) {
                ...
                case '&': 
                case ';': {
                    if (QueryStringDecoder.addParam(s2, nameStart, valueStart, i, params2, charset) && --paramsLimit == 0) {
                        return params2;
                    }
                    nameStart = i + 1;
                    break;
        ...
        QueryStringDecoder.addParam(s2, nameStart, valueStart, i, params2, charset);
        return params2;
    }

    private static boolean addParam(String s2, int nameStart, int valueStart, int valueEnd, Map<String, List<String>> params2, Charset charset) {
    ...
    private static String decodeComponent(String s2, int from2, int toExcluded, Charset charset, boolean isPath) {
        int len = toExcluded - from2;
        if (len <= 0) {
            return "";
        }
        int firstEscaped = -1;
        for (int i = from2; i < toExcluded; ++i) {
            char c = s2.charAt(i);
        ...
        for (int i = firstEscaped; i < toExcluded; ++i) {
            ...
            do {
                ...
                byteBuf.put(StringUtil.decodeHexByte(s2, i + 1));
            } while ((i += 3) < toExcluded && s2.charAt(i) == '%');
            --i;
            byteBuf.flip();
            charBuf.clear();
            CoderResult result2 = decoder.reset().decode(byteBuf, charBuf, true);
            try {
                if (!result2.isUnderflow()) {
                    result2.throwException();
                }
                if (!(result2 = decoder.flush(charBuf)).isUnderflow()) {
                    result2.throwException();
                }
            }
