...
final class PoolThreadCache {
    ...
    private static <T> MemoryRegionCache<T>[] createNormalCaches(int cacheSize, int maxCachedBufferCapacity, PoolArena<T> area) {
        if (cacheSize > 0 && maxCachedBufferCapacity > 0) {
            int max2 = Math.min(area.chunkSize, maxCachedBufferCapacity);
            int arraySize = Math.max(1, PoolThreadCache.log2(max2 / area.pageSize) + 1);
    ...
    private static abstract class MemoryRegionCache<T> {
        ...
        MemoryRegionCache(int size2, PoolArena.SizeClass sizeClass) {
            this.size = MathUtil.safeFindNextPositivePowerOfTwo(size2);
        ...
        private int free(int max2) {
            int numFreed;
            for (numFreed = 0; numFreed < max2; ++numFreed) {
                Entry<T> entry = this.queue.poll();
                if (entry == null) {
                    return numFreed;
                }
    ...
    extends MemoryRegionCache<T> {
        NormalMemoryRegionCache(int size2) {
            super(size2, PoolArena.SizeClass.Normal);
        }
    ...
    extends MemoryRegionCache<T> {
        SubPageMemoryRegionCache(int size2, PoolArena.SizeClass sizeClass) {
            super(size2, sizeClass);
        }
