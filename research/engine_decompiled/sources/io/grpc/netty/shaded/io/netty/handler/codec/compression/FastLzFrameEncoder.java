        ...
        while (in.isReadable()) {
            ...
            if (length < 32) {
                ...
                if (checksum != null) {
                    ...
                    checksum.reset();
                    checksum.update(input2, inputPtr, length);
                    out.setInt(outputIdx + 4, (int)checksum.getValue());
                    System.arraycopy(input2, inputPtr, output, outputPtr, length);
                } else {
                ...
                if (in.hasArray()) {
                    ...
                    input3 = new byte[length];
                    in.getBytes(idx, input3);
                    inputPtr = 0;
                }
                if (checksum != null) {
                    checksum.reset();
                    checksum.update(input3, inputPtr, length);
                    out.setInt(outputIdx + 4, (int)checksum.getValue());
                }
                ...
                int compressedLength = FastLz.compress(input3, inputPtr, length, output, outputPtr, this.level);
                if (compressedLength < length) {
                    ...
                    blockType = 0;
                    System.arraycopy(input3, inputPtr, output, outputPtr - 2, length);
                    chunkLength = length;
                }
            }
