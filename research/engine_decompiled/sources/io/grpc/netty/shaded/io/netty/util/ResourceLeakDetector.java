...
public class ResourceLeakDetector<T> {
    ...
    static {
        ...
        if (SystemPropertyUtil.get("io.grpc.netty.shaded.io.netty.noResourceLeakDetection") != null) {
            disabled = SystemPropertyUtil.getBoolean("io.grpc.netty.shaded.io.netty.noResourceLeakDetection", false);
    ...
    ResourceLeak {
        ...
        public String toString() {
            ...
            while (oldHead != Record.BOTTOM) {
                String s2 = oldHead.toString();
                if (seen.add(s2)) {
                    if (oldHead.next == Record.BOTTOM) {
                        buf.append("Created at:").append(StringUtil.NEWLINE).append(s2);
                    } else {
                        buf.append('#').append(i++).append(':').append(StringUtil.NEWLINE).append(s2);
                    }
                } else {
                    ++duped;
                }
