...
implements InterfaceHttpPostRequestDecoder {
        ...
        switch (state) {
            ...
            case FIELD: {
                ...
                if (this.currentAttribute == null) {
                    ...
                    try {
                        this.currentAttribute = size2 > 0L ? this.factory.createAttribute(this.request, HttpPostMultipartRequestDecoder.cleanString(nameAttribute.getValue()), size2) : this.factory.createAttribute(this.request, HttpPostMultipartRequestDecoder.cleanString(nameAttribute.getValue()));
                    }
    ...
    private Attribute getContentDispositionAttribute(String ... values) {
        ...
        if (HttpHeaderValues.FILENAME.contentEquals(name)) {
            ...
            if (last2 > 0 && value.charAt(0) == '\"' && value.charAt(last2) == '\"') {
                value = value.substring(1, last2);
            }
        } else if (FILENAME_ENCODED.equals(name)) {
            try {
                name = HttpHeaderValues.FILENAME.toString();
                String[] split2 = value.split("'", 3);
                value = QueryStringDecoder.decodeComponent(split2[2], Charset.forName(split2[0]));
            }
    ...
    protected InterfaceHttpData getFileUpload(String delimiter) {
        ...
        if (this.currentFileUpload == null) {
            ...
            try {
                ...
                this.currentFileUpload = this.factory.createFileUpload(this.request, HttpPostMultipartRequestDecoder.cleanString(nameAttribute.getValue()), HttpPostMultipartRequestDecoder.cleanString(filenameAttribute.getValue()), contentType, mechanism.value(), localCharset, size2);
            }
            catch (NullPointerException e) {
                throw new HttpPostRequestDecoder.ErrorDataDecoderException(e);
            }
