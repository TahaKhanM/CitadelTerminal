...
public final class Unpooled {
    ...
    public static ByteBuf copiedBuffer(byte[] array, int offset, int length) {
        ...
        System.arraycopy(array, offset, copy2, 0, length);
        return Unpooled.wrappedBuffer(copy2);
    }
    ...
    public static ByteBuf copiedBuffer(ByteBuffer buffer) {
        ...
        duplicate.get(copy2);
        return Unpooled.wrappedBuffer(copy2).order(duplicate.order());
    }

    public static ByteBuf copiedBuffer(ByteBuf buffer) {
        int readable = buffer.readableBytes();
        if (readable > 0) {
            ByteBuf copy2 = Unpooled.buffer(readable);
            copy2.writeBytes(buffer, buffer.readerIndex(), readable);
            return copy2;
        }
        return EMPTY_BUFFER;
    }
    ...
    public static ByteBuf copiedBuffer(CharSequence string2, int offset, int length, Charset charset) {
        if (string2 == null) {
            throw new NullPointerException("string");
        }
        if (length == 0) {
        ...
        return Unpooled.copiedBuffer(CharBuffer.wrap(string2, offset, offset + length), charset);
    }
