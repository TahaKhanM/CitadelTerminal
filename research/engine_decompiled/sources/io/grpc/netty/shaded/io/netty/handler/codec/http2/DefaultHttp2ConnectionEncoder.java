...
implements Http2ConnectionEncoder {
    ...
    @Override
    public ChannelFuture writeData(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding2, boolean endOfStream, ChannelPromise promise) {
        Http2Stream stream;
        try {
        ...
        this.flowController().addFlowControlled(stream, new FlowControlledData(stream, data, padding2, endOfStream, promise));
        return promise;
    }
    ...
    public ChannelFuture writeHeaders(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int padding2, boolean endStream, ChannelPromise promise) {
        return this.writeHeaders(ctx, streamId, headers, 0, (short)16, false, padding2, endStream, promise);
    }
    ...
    @Override
    public ChannelFuture writeHeaders(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding2, boolean endOfStream, ChannelPromise promise) {
        try {
            ...
            if (!endOfStream || !flowController.hasFlowControlled(stream)) {
                ...
                if ((failureCause = (future = this.frameWriter.writeHeaders(ctx, streamId, headers, streamDependency, weight, exclusive, padding2, endOfStream, promise)).cause()) == null) {
            ...
            flowController.addFlowControlled(stream, new FlowControlledHeaders(stream, headers, streamDependency, weight, exclusive, padding2, true, promise));
            return promise;
        }
        catch (Throwable t) {
    ...
    public ChannelFuture writePushPromise(ChannelHandlerContext ctx, int streamId, int promisedStreamId, Http2Headers headers, int padding2, ChannelPromise promise) {
        try {
            ...
            ChannelFuture future = this.frameWriter.writePushPromise(ctx, streamId, promisedStreamId, headers, padding2, promise);
    ...
    ChannelFutureListener {
        ...
        FlowControlledBase(Http2Stream stream, int padding2, boolean endOfStream, ChannelPromise promise) {
            if (padding2 < 0) {
    ...
    extends FlowControlledBase {
        ...
        FlowControlledHeaders(Http2Stream stream, Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding2, boolean endOfStream, ChannelPromise promise) {
            super(stream, padding2, endOfStream, promise);
        ...
        @Override
        public boolean merge(ChannelHandlerContext ctx, Http2RemoteFlowController.FlowControlled next2) {
            return false;
        }
    }
    ...
    extends FlowControlledBase {
        ...
        FlowControlledData(Http2Stream stream, ByteBuf buf, int padding2, boolean endOfStream, ChannelPromise promise) {
            super(stream, padding2, endOfStream, promise);
        ...
        public boolean merge(ChannelHandlerContext ctx, Http2RemoteFlowController.FlowControlled next2) {
            ...
            if (FlowControlledData.class != next2.getClass() || Integer.MAX_VALUE - (nextData = (FlowControlledData)next2).size() < this.size()) {
                return false;
            }
