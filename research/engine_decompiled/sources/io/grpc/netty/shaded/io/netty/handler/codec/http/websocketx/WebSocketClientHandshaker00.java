...
extends WebSocketClientHandshaker {
    ...
    private static String insertRandomCharacters(String key) {
        ...
        for (int i = 0; i < count2; ++i) {
            int split2 = WebSocketUtil.randomNumber(0, key.length());
            String part1 = key.substring(0, split2);
            String part2 = key.substring(split2);
            key = part1 + randomChars[i] + part2;
        }
        return key;
    }

    private static String insertSpaces(String key, int spaces2) {
        for (int i = 0; i < spaces2; ++i) {
            int split2 = WebSocketUtil.randomNumber(1, key.length() - 1);
            String part1 = key.substring(0, split2);
            String part2 = key.substring(split2);
            key = part1 + ' ' + part2;
        }
