...
Http2FrameWriter.Configuration {
    ...
    @Override
    public void maxFrameSize(int max2) throws Http2Exception {
        if (!Http2CodecUtil.isMaxFrameSizeValid(max2)) {
            throw Http2Exception.connectionError(Http2Error.FRAME_SIZE_ERROR, "Invalid MAX_FRAME_SIZE specified in sent settings: %d", max2);
        }
        this.maxFrameSize = max2;
    }
    ...
    public ChannelFuture writeData(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding2, boolean endStream, ChannelPromise promise) {
        ...
        try {
            ...
            Http2CodecUtil.verifyPadding(padding2);
            ...
            if (padding2 == 0) {
                ...
                do {
                    ...
                    int framePaddingBytes = Math.min(padding2, Math.max(0, this.maxFrameSize - 1 - frameDataBytes));
                    ByteBuf frameHeader2 = ctx.alloc().buffer(10);
                    flags.endOfStream(endStream && (remainingData -= frameDataBytes) == 0 && (padding2 -= framePaddingBytes) == 0);
                ...
                } while (remainingData != 0 || padding2 != 0);
            }
        }
    ...
    public ChannelFuture writeHeaders(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int padding2, boolean endStream, ChannelPromise promise) {
        return this.writeHeadersInternal(ctx, streamId, headers, padding2, endStream, false, 0, (short)0, false, promise);
    }
    ...
    public ChannelFuture writeHeaders(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding2, boolean endStream, ChannelPromise promise) {
        return this.writeHeadersInternal(ctx, streamId, headers, padding2, endStream, true, streamDependency, weight, exclusive, promise);
    }
    ...
    public ChannelFuture writePushPromise(ChannelHandlerContext ctx, int streamId, int promisedStreamId, Http2Headers headers, int padding2, ChannelPromise promise) {
        ...
        try {
            ...
            Http2Flags flags = new Http2Flags().paddingPresent(padding2 > 0);
            int nonFragmentLength = 4 + padding2;
            ...
            DefaultHttp2FrameWriter.writePaddingLength(buf, padding2);
            buf.writeInt(promisedStreamId);
            ctx.write(buf, promiseAggregator.newPromise());
            ctx.write(fragment, promiseAggregator.newPromise());
            if (DefaultHttp2FrameWriter.paddingBytes(padding2) > 0) {
                ctx.write(ZERO_BUFFER.slice(0, DefaultHttp2FrameWriter.paddingBytes(padding2)), promiseAggregator.newPromise());
            }
            if (!flags.endOfHeaders()) {
                this.writeContinuationFrames(ctx, streamId, (ByteBuf)headerBlock, padding2, promiseAggregator);
            }
        }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private ChannelFuture writeHeadersInternal(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int padding2, boolean endStream, boolean hasPriority, int streamDependency, short weight, boolean exclusive, ChannelPromise promise) {
        ReferenceCounted headerBlock = null;
        Http2CodecUtil.SimpleChannelPromiseAggregator promiseAggregator = new Http2CodecUtil.SimpleChannelPromiseAggregator(promise, ctx.channel(), ctx.executor());
        try {
            DefaultHttp2FrameWriter.verifyStreamId(streamId, STREAM_ID);
            ...
            headerBlock = ctx.alloc().buffer();
            this.headersEncoder.encodeHeaders(streamId, headers, (ByteBuf)headerBlock);
            Http2Flags flags = new Http2Flags().endOfStream(endStream).priorityPresent(hasPriority).paddingPresent(padding2 > 0);
            int nonFragmentBytes = padding2 + flags.getNumPriorityBytes();
    ...
    private ChannelFuture writeContinuationFrames(ChannelHandlerContext ctx, int streamId, ByteBuf headerBlock, int padding2, Http2CodecUtil.SimpleChannelPromiseAggregator promiseAggregator) {
        Http2Flags flags = new Http2Flags().paddingPresent(padding2 > 0);
        int maxFragmentLength = this.maxFrameSize - padding2;
        if (maxFragmentLength <= 0) {
            return promiseAggregator.setFailure(new IllegalArgumentException("Padding [" + padding2 + "] is too large for max frame size [" + this.maxFrameSize + "]"));
        }
        if (headerBlock.isReadable()) {
            int fragmentReadableBytes = Math.min(headerBlock.readableBytes(), maxFragmentLength);
            int payloadLength = fragmentReadableBytes + padding2;
            ...
            do {
                ...
                payloadLength = fragmentReadableBytes + padding2;
                if (headerBlock.isReadable()) {
                ...
                ctx.write(fragment, promiseAggregator.newPromise());
                if (DefaultHttp2FrameWriter.paddingBytes(padding2) <= 0) continue;
                ctx.write(ZERO_BUFFER.slice(0, DefaultHttp2FrameWriter.paddingBytes(padding2)), promiseAggregator.newPromise());
    ...
    private static int paddingBytes(int padding2) {
        return padding2 - 1;
    }

    private static void writePaddingLength(ByteBuf buf, int padding2) {
        if (padding2 > 0) {
            buf.writeByte(padding2 - 1);
        }
    }
