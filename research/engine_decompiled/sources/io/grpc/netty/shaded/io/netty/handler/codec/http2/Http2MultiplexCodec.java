...
extends Http2FrameCodec {
    ...
    final void onHttp2StreamStateChanged(ChannelHandlerContext ctx, Http2FrameStream stream) {
        Http2MultiplexCodecStream s2 = (Http2MultiplexCodecStream)stream;
        switch (stream.state()) {
            case HALF_CLOSED_REMOTE: 
            case OPEN: {
                if (s2.channel != null) break;
                ChannelFuture future = ctx.channel().eventLoop().register(new DefaultHttp2StreamChannel(s2, false));
                if (future.isDone()) {
                    Http2MultiplexCodec.registerDone(future);
                    break;
                }
            ...
            case CLOSED: {
                DefaultHttp2StreamChannel channel = s2.channel;
    ...
    boolean onBytesConsumed(ChannelHandlerContext ctx, Http2FrameStream stream, int bytes2) throws Http2Exception {
        return this.consumeBytes(stream.id(), bytes2);
    }
        ...
        implements Channel.Unsafe {
            ...
            private void writeComplete(ChannelFuture future, ChannelPromise promise) {
                ...
                if (cause == null) {
                    ...
                    Throwable error2 = this.wrapStreamClosedError(cause);
                    promise.setFailure(error2);
