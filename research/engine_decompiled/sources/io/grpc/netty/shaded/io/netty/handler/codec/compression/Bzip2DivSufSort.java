...
final class Bzip2DivSufSort {
    ...
    private int ssCompareLast(int pa, int p1, int p2, int depth, int size2) {
        int U2;
        int[] SA = this.SA;
        byte[] T = this.T;
        int U1 = depth + SA[p1];
    ...
    private void ssInsertionSort(int pa, int first, int last2, int depth) {
        int[] SA = this.SA;
        for (int i = last2 - 2; first <= i; --i) {
            int r;
            int t = SA[i];
            int j = i + 1;
    ...
    private void ssFixdown(int td, int pa, int sa, int i, int size2) {
        int j;
        int[] SA = this.SA;
        byte[] T = this.T;
        int v = SA[sa + i];
    ...
    private void ssHeapSort(int td, int pa, int sa, int size2) {
        int i;
        int[] SA = this.SA;
        byte[] T = this.T;
        int m = size2;
        if (size2 % 2 == 0 && (T[td + SA[pa + SA[sa + --m / 2]]] & 0xFF) < (T[td + SA[pa + SA[sa + m]]] & 0xFF)) {
    ...
    private int ssPivot(int td, int pa, int first, int last2) {
        int t = last2 - first;
        int middle = first + t / 2;
        if (t <= 512) {
            if (t <= 32) {
                return this.ssMedian3(td, pa, first, middle, last2 - 1);
            }
            return this.ssMedian5(td, pa, first, first + (t >>= 2), middle, last2 - 1 - t, last2 - 1);
        }
        return this.ssMedian3(td, pa, this.ssMedian3(td, pa, first, first + (t >>= 3), first + (t << 1)), this.ssMedian3(td, pa, middle - t, middle, middle + t), this.ssMedian3(td, pa, last2 - 1 - (t << 1), last2 - 1 - t, last2 - 1));
    }
    ...
    private int ssSubstringPartition(int pa, int first, int last2, int depth) {
        int[] SA = this.SA;
        int a = first - 1;
        int b = last2;
        while (true) {
    ...
    private void ssMultiKeyIntroSort(int pa, int first, int last2, int depth) {
        int[] SA = this.SA;
        byte[] T = this.T;
        StackEntry[] stack = new StackEntry[64];
        int x = 0;
        int ssize = 0;
        int limit = Bzip2DivSufSort.ssLog(last2 - first);
        while (true) {
            int c;
            int b;
            int a;
            ...
            if (last2 - first <= 8) {
                if (1 < last2 - first) {
                    this.ssInsertionSort(pa, first, last2, depth);
                }
                if (ssize == 0) {
                    return;
                }
            ...
            int Td = depth;
            if (limit-- == 0) {
                this.ssHeapSort(Td, pa, first, last2 - first);
            }
            if (limit < 0) {
                v = T[Td + SA[pa + SA[first]]] & 0xFF;
                for (a = first + 1; a < last2; ++a) {
            ...
            a = this.ssPivot(Td, pa, first, last2);
            v = T[Td + SA[pa + SA[a]]] & 0xFF;
            Bzip2DivSufSort.swapElements(SA, first, SA, a);
            for (b = first + 1; b < last2 && (x = T[Td + SA[pa + SA[b]]] & 0xFF) == v; ++b) {
            }
            a = b;
            if (a < last2 && x < v) {
            ...
            if (a <= d) {
                ...
                if (a - first <= last2 - c) {
                    if (last2 - c <= c - b) {
                        ...
                        stack[ssize++] = new StackEntry(c, last2, depth, limit);
                        last2 = a;
                        continue;
                    }
                    if (a - first <= c - b) {
            ...
            ++limit;
            if ((T[Td + SA[pa + SA[first]] - 1] & 0xFF) < v) {
                first = this.ssSubstringPartition(pa, first, last2, depth);
                limit = Bzip2DivSufSort.ssLog(last2 - first);
            }
            ++depth;
        }
    }

    private static void ssBlockSwap(int[] array1, int first1, int[] array2, int first2, int size2) {
        int i = size2;
        int a = first1;
        int b = first2;
        while (0 < i) {
    ...
    private void ssMergeForward(int pa, int[] buf, int bufoffset, int first, int middle, int last2, int depth) {
        int[] SA = this.SA;
    ...
    private void ssMergeBackward(int pa, int[] buf, int bufoffset, int first, int middle, int last2, int depth) {
        int p2;
        int p1;
        int[] SA = this.SA;
    ...
    private void ssMerge(int pa, int first, int middle, int last2, int[] buf, int bufoffset, int bufsize, int depth) {
        int[] SA = this.SA;
        StackEntry[] stack = new StackEntry[64];
        int check2 = 0;
        int ssize = 0;
        while (true) {
            ...
            if (last2 - middle <= bufsize) {
                if (first < middle && middle < last2) {
                    this.ssMergeBackward(pa, buf, bufoffset, first, middle, last2, depth);
                }
                if (check2 & true) {
            ...
            if (middle - first <= bufsize) {
                if (first < middle) {
                    this.ssMergeForward(pa, buf, bufoffset, first, middle, last2, depth);
                }
                if ((check2 & 1) != 0) {
                    this.ssMergeCheckEqual(pa, depth, first);
                }
            ...
            int m = 0;
            int len = Math.min(middle - first, last2 - middle);
            int half = len >> 1;
            while (0 < len) {
            ...
            if (0 < m) {
                ...
                if (i - first <= last2 - j) {
                    stack[ssize++] = new StackEntry(j, middle + m, last2, check2 & 2 | next2 & 1);
                    middle -= m;
                    last2 = i;
                    check2 &= 1;
                    continue;
                ...
                stack[ssize++] = new StackEntry(first, middle - m, i, check2 & 1 | next2 & 2);
                first = j;
                middle += m;
                check2 = check2 & 2 | next2 & 1;
                continue;
            }
            if ((check2 & 1) != 0) {
                this.ssMergeCheckEqual(pa, depth, first);
    ...
    private void subStringSort(int pa, int first, int last2, int[] buf, int bufoffset, int bufsize, int depth, boolean lastsuffix, int size2) {
        int k;
        int[] SA = this.SA;
        if (lastsuffix) {
            ++first;
        ...
        if (lastsuffix) {
            i = SA[first - 1];
            int r = 1;
            for (a = first; a < last2 && (SA[a] < 0 || 0 < (r = this.ssCompareLast(pa, pa + i, pa + SA[a], depth, size2))); ++a) {
                SA[a - 1] = SA[a];
            }
            if (r == 0) {
                SA[a] = ~SA[a];
    ...
    private void trFixdown(int isa, int isaD, int isaN, int sa, int i, int size2) {
        int j;
        int[] SA = this.SA;
        int v = SA[sa + i];
    ...
    private void trHeapSort(int isa, int isaD, int isaN, int sa, int size2) {
        int i;
        int[] SA = this.SA;
        int m = size2;
        if (size2 % 2 == 0 && this.trGetC(isa, isaD, isaN, SA[sa + --m / 2]) < this.trGetC(isa, isaD, isaN, SA[sa + m])) {
            Bzip2DivSufSort.swapElements(SA, sa + m, SA, sa + m / 2);
        }
        for (i = m / 2 - 1; 0 <= i; --i) {
    ...
    private void trInsertionSort(int isa, int isaD, int isaN, int first, int last2) {
        int[] SA = this.SA;
        for (int a = first + 1; a < last2; ++a) {
            int r;
            int t = SA[a];
            int b = a - 1;
    ...
    private int trPivot(int isa, int isaD, int isaN, int first, int last2) {
        int t = last2 - first;
        int middle = first + t / 2;
        if (t <= 512) {
            if (t <= 32) {
                return this.trMedian3(isa, isaD, isaN, first, middle, last2 - 1);
            }
            return this.trMedian5(isa, isaD, isaN, first, first + (t >>= 2), middle, last2 - 1 - t, last2 - 1);
        }
        return this.trMedian3(isa, isaD, isaN, this.trMedian3(isa, isaD, isaN, first, first + (t >>= 3), first + (t << 1)), this.trMedian3(isa, isaD, isaN, middle - t, middle, middle + t), this.trMedian3(isa, isaD, isaN, last2 - 1 - (t << 1), last2 - 1 - t, last2 - 1));
    }

    private void lsUpdateGroup(int isa, int first, int last2) {
        int[] SA = this.SA;
        for (int a = first; a < last2; ++a) {
            int b;
            if (0 <= SA[a]) {
                b = a;
                do {
    ...
    private void lsIntroSort(int isa, int isaD, int isaN, int first, int last2) {
        int[] SA = this.SA;
        StackEntry[] stack = new StackEntry[64];
        int x = 0;
        int ssize = 0;
        int limit = Bzip2DivSufSort.trLog(last2 - first);
        while (true) {
            int c;
            int b;
            int a;
            ...
            if (last2 - first <= 8) {
                if (1 < last2 - first) {
                    this.trInsertionSort(isa, isaD, isaN, first, last2);
                    this.lsUpdateGroup(isa, first, last2);
                } else if (last2 - first == 1) {
                    SA[first] = -1;
                }
                if (ssize == 0) {
                    return;
            ...
            if (limit-- == 0) {
                this.trHeapSort(isa, isaD, isaN, first, last2 - first);
                a = last2 - 1;
                while (first < a) {
            ...
            a = this.trPivot(isa, isaD, isaN, first, last2);
            Bzip2DivSufSort.swapElements(SA, first, SA, a);
            int v = this.trGetC(isa, isaD, isaN, SA[first]);
            for (b = first + 1; b < last2 && (x = this.trGetC(isa, isaD, isaN, SA[b])) == v; ++b) {
            }
            a = b;
            if (a < last2 && x < v) {
            ...
            if (a <= d) {
                ...
                if (a - first <= last2 - b) {
                    if (first < a) {
                        stack[ssize++] = new StackEntry(b, last2, limit, 0);
                        last2 = a;
                        continue;
                    }
                    first = b;
    ...
    private void lsSort(int isa, int n, int depth) {
        ...
        while (-n < SA[0]) {
            ...
            do {
                ...
                last2 = SA[isa + t] + 1;
                this.lsIntroSort(isa, isaD, isa + n, first, last2);
                first = last2;
            } while (first < n);
            if (skip != 0) {
                SA[first + skip] = skip;
    ...
    private PartitionResult trPartition(int isa, int isaD, int isaN, int first, int last2, int v) {
        int c;
        int b;
        int[] SA = this.SA;
        int x = 0;
        for (b = first; b < last2 && (x = this.trGetC(isa, isaD, isaN, SA[b])) == v; ++b) {
        }
        int a = b;
        if (a < last2 && x < v) {
            while (++b < last2 && (x = this.trGetC(isa, isaD, isaN, SA[b])) <= v) {
        ...
        return new PartitionResult(first, last2);
    }

    private void trCopy(int isa, int isaN, int first, int a, int b, int last2, int depth) {
        int s2;
        int c;
        int[] SA = this.SA;
        int v = b - 1;
    ...
    private void trIntroSort(int isa, int isaD, int isaN, int first, int last2, TRBudget budget, int size2) {
        int s2;
        int[] SA = this.SA;
        StackEntry[] stack = new StackEntry[64];
        int x = 0;
        ...
        while (true) {
            ...
            if (limit < 0) {
                if (limit == -1) {
                    StackEntry entry2;
                    if (!budget.update(size2, last2 - first)) break;
                    PartitionResult result2 = this.trPartition(isa, isaD - 1, isaN, first, last2, last2 - 1);
                    a = result2.first;
                    b = result2.last;
                    if (first < a || b < last2) {
                        if (a < last2) {
                        ...
                        stack[ssize++] = new StackEntry(isaD - 1, first, last2, -2);
                        if (a - first <= last2 - b) {
                            if (1 < a - first) {
                            ...
                            if (1 < last2 - b) {
                                first = b;
                                limit = Bzip2DivSufSort.trLog(last2 - b);
                                continue;
                            }
                            if (ssize == 0) {
                                return;
                ...
                if (limit == -2) {
                    a = stack[--ssize].b;
                    b = stack[ssize].c;
                    this.trCopy(isa, isaN, first, a, b, last2, isaD - isa);
                    if (ssize == 0) {
                        return;
                    }
                    entry = stack[--ssize];
                ...
                if (first < last2) {
                    ...
                    int n = next2 = SA[isa + SA[a]] != SA[isaD + SA[a]] ? Bzip2DivSufSort.trLog(a - first + 1) : -1;
                    if (++a < last2) {
                        v = a - 1;
                        for (b = first; b < a; ++b) {
                            SA[isa + SA[b]] = v;
                    ...
                    if (1 < last2 - a) {
                        stack[ssize++] = new StackEntry(isaD + 1, first, a, next2);
                        first = a;
                        limit = -3;
                        continue;
                    }
            ...
            if (a <= d) {
                ...
                if (a - first <= last2 - b) {
                    if (last2 - b <= b - a) {
                        if (1 < a - first) {
                            ...
                            stack[ssize++] = new StackEntry(isaD, b, last2, limit);
                            last2 = a;
                            continue;
                        }
                        if (1 < last2 - b) {
                ...
                if (a - first <= b - a) {
                    ...
                    stack[ssize++] = new StackEntry(isaD, first, last2, limit);
                    continue;
                }
                if (last2 - b <= b - a) {
                    if (1 < last2 - b) {
    ...
    private void trSort(int isa, int n, int depth) {
        ...
        if (-n < SA[0]) {
            ...
            do {
                ...
                if (1 < last2 - first) {
                    this.trIntroSort(isa, isa + depth, isa + n, first, last2, budget, n);
                    if (budget.chance == 0) {
                        if (0 < first) {
                            SA[0] = -first;
                        }
    ...
    private static class TRBudget {
        ...
        boolean update(int size2, int n) {
            this.budget -= n;
            if (this.budget <= 0) {
                if (--this.chance == 0) {
                    return false;
    ...
    private static class PartitionResult {
        ...
        PartitionResult(int first, int last2) {
            this.first = first;
            this.last = last2;
        }
    }
