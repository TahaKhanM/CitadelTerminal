...
implements StreamByteDistributor {
        ...
        connection.addListener(new Http2ConnectionAdapter(){
            ...
            public void onStreamAdded(Http2Stream stream) {
                ...
                if (state == null) {
                    ...
                    ArrayList<ParentChangedEvent> events2 = new ArrayList<ParentChangedEvent>(1);
                    WeightedFairQueueByteDistributor.this.connectionState.takeChild(state, false, events2);
                    WeightedFairQueueByteDistributor.this.notifyParentChanged(events2);
                } else {
    ...
    public void updateDependencyTree(int childStreamId, int parentStreamId, short weight, boolean exclusive) {
        ArrayList<ParentChangedEvent> events2;
        State newParent;
        ...
        if ((newParent = this.state(parentStreamId)) == null) {
            ...
            events2 = new ArrayList(1);
            this.connectionState.takeChild(newParent, false, events2);
            this.notifyParentChanged(events2);
        }
        ...
        if (newParent != state.parent || exclusive && newParent.children.size() != 1) {
            if (newParent.isDescendantOf(state)) {
                events2 = new ArrayList<ParentChangedEvent>(2 + (exclusive ? newParent.children.size() : 0));
                state.parent.takeChild(newParent, false, events2);
            } else {
                events2 = new ArrayList(1 + (exclusive ? newParent.children.size() : 0));
            }
            newParent.takeChild(state, exclusive, events2);
            this.notifyParentChanged(events2);
        }
    ...
    void notifyParentChanged(List<ParentChangedEvent> events2) {
        for (int i = 0; i < events2.size(); ++i) {
            ParentChangedEvent event = events2.get(i);
    ...
    implements PriorityQueueNode {
        ...
        void takeChild(State child, boolean exclusive, List<ParentChangedEvent> events2) {
            this.takeChild(null, child, exclusive, events2);
        }

        void takeChild(Iterator<IntObjectMap.PrimitiveEntry<State>> childItr, State child, boolean exclusive, List<ParentChangedEvent> events2) {
            State oldParent = child.parent;
            if (oldParent != this) {
                events2.add(new ParentChangedEvent(child, oldParent));
            ...
            if (exclusive && !this.children.isEmpty()) {
                ...
                while (itr.hasNext()) {
                    child.takeChild(itr, itr.next().value(), false, events2);
                }
            }
        }
        ...
        void removeChild(State child) {
            if (this.children.remove(child.streamId) != null) {
                ArrayList<ParentChangedEvent> events2 = new ArrayList<ParentChangedEvent>(1 + child.children.size());
                events2.add(new ParentChangedEvent(child, child.parent));
                child.setParent(null);
                Iterator<IntObjectMap.PrimitiveEntry<State>> itr = child.children.entries().iterator();
                while (itr.hasNext()) {
                    this.takeChild(itr, itr.next().value(), false, events2);
                }
                WeightedFairQueueByteDistributor.this.notifyParentChanged(events2);
            }
        }
        ...
        private void toString(StringBuilder sb) {
            ...
            if (!this.pseudoTimeQueue.isEmpty()) {
                for (State s2 : this.pseudoTimeQueue) {
                    s2.toString(sb);
                    sb.append(", ");
                }
