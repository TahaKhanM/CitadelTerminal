...
public class ComparisonCompactor {
    ...
    private String compactString(String source) {
        String result2 = DELTA_START + source.substring(this.fPrefix, source.length() - this.fSuffix + 1) + DELTA_END;
        if (this.fPrefix > 0) {
            result2 = this.computeCommonPrefix() + result2;
        }
        if (this.fSuffix > 0) {
            result2 = result2 + this.computeCommonSuffix();
        }
        return result2;
    }
