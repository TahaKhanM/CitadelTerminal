...
extends PriorityQueue<A> {
    ...
    public SynchronizedPriorityQueue<A> $plus$eq(A elem) {
        SynchronizedPriorityQueue synchronizedPriorityQueue = this;
        synchronized (synchronizedPriorityQueue) {
            super.$plus$eq((Object)elem);
    ...
    public SynchronizedPriorityQueue<A> $plus$plus$eq(TraversableOnce<A> xs) {
        SynchronizedPriorityQueue synchronizedPriorityQueue = this;
        synchronized (synchronizedPriorityQueue) {
            Growable$class.$plus$plus$eq(this, xs);
            // ** MonitorExit[this] (shouldn't be in output)
            return this;
        }
    }
    ...
    public void enqueue(Seq<A> elems) {
        SynchronizedPriorityQueue synchronizedPriorityQueue = this;
        synchronized (synchronizedPriorityQueue) {
            Growable$class.$plus$plus$eq(this, elems);
            // ** MonitorExit[this] (shouldn't be in output)
            return;
        }
    }
