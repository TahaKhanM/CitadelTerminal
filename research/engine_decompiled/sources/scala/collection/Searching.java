...
public final class Searching {
    ...
    Serializable {
        ...
        public boolean equals(Object x$1) {
            ...
            if (!(x$1 instanceof Found)) return false;
            boolean bl = true;
            if (!bl) return false;
            Found found = (Found)x$1;
            if (this.foundIndex() != found.foundIndex()) return false;
            if (!found.canEqual(this)) return false;
            return true;
        }
    ...
    public static class SearchImpl<A, Repr> {
        ...
        public final <B> SearchResult search(B elem, Ordering<B> ord) {
            SeqLike<A, Repr> seqLike = this.coll();
            SearchResult searchResult = seqLike instanceof IndexedSeqLike ? this.binarySearch(elem, 0, this.coll().length(), ord) : this.linearSearch((SeqView<A, Repr>)this.coll().view(), elem, 0, ord);
            return searchResult;
        }

        public final <B> SearchResult search(B elem, int from2, int to2, Ordering<B> ord) {
            SeqLike<A, Repr> seqLike = this.coll();
            SearchResult searchResult = seqLike instanceof IndexedSeqLike ? this.binarySearch(elem, from2, to2, ord) : this.linearSearch(this.coll().view(from2, to2), elem, from2, ord);
            return searchResult;
        }
         ...
         * Lifted jumps to return sites
         */
        private <B> SearchResult binarySearch(B elem, int from2, int to2, Ordering<B> ord) {
            block4: while (true) {
                SearchResult searchResult;
                if (to2 == from2) {
                    searchResult = new InsertionPoint(from2);
                    return searchResult;
                }
                int idx = from2 + (to2 - from2 - 1) / 2;
                int n = package$.MODULE$.signum(ord.compare(elem, this.coll().apply(idx)));
                switch (n) {
                    default: {
                        searchResult = new Found(idx);
                        return searchResult;
                    }
                    case 1: {
                        from2 = idx + 1;
    ...
    Serializable {
        ...
        public boolean equals(Object x$1) {
            if (this == x$1) return true;
            if (!(x$1 instanceof InsertionPoint)) return false;
            boolean bl = true;
            if (!bl) return false;
            InsertionPoint insertionPoint = (InsertionPoint)x$1;
            if (this.insertionPoint() != insertionPoint.insertionPoint()) return false;
            if (!insertionPoint.canEqual(this)) return false;
            return true;
        }
