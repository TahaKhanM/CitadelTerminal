...
implements Serializable {
    ...
    @Override
    public <B1> ListMap<A, B1> $plus(Tuple2<A, B1> elem1, Tuple2<A, B1> elem2, Seq<Tuple2<A, B1>> elems) {
        return ((ListMap)((ListMap)this.$plus((Tuple2)elem1)).$plus((Tuple2)elem2)).$plus$plus(elems);
    }

    @Override
    public <B1> ListMap<A, B1> $plus$plus(GenTraversableOnce<Tuple2<A, B1>> xs) {
        ListMap listMap = (ListMap)this.repr();
        return xs.seq().$div$colon(listMap, new Serializable(this){
            public static final long serialVersionUID = 0L;

            public final ListMap<A, B1> apply(ListMap<A, B1> x$2, Tuple2<A, B1> x$3) {
    ...
    public Iterator<Tuple2<A, B>> iterator() {
        return new AbstractIterator<Tuple2<A, B>>(this){
            ...
            public Tuple2<A, B> next() {
                if (this.hasNext()) {
                    void var1_1;
                    Tuple2<A, B> res = new Tuple2<A, B>(this.self().key(), this.self().value());
                    this.self_$eq(this.self().next());
                    return var1_1;
                }
    ...
    extends ListMap<A, B1> {
        ...
        private int size0(ListMap<A, B1> cur, int acc) {
            while (!cur.isEmpty()) {
                ++acc;
                cur = cur.next();
            }
        ...
        @Override
        public B1 apply(A k) {
            return this.apply0(this, k);
        }

        private B1 apply0(ListMap<A, B1> cur, A k) {
            while (true) {
                if (cur.isEmpty()) {
                    throw new NoSuchElementException(new StringBuilder().append((Object)"key not found: ").append(k).toString());
                }
                Object a = cur.key();
                if (k == a ? true : (k == null ? false : (k instanceof Number ? BoxesRunTime.equalsNumObject((Number)k, a) : (k instanceof Character ? BoxesRunTime.equalsCharObject((Character)k, a) : k.equals(a))))) {
                    return cur.value();
                }
                cur = cur.next();
            }
        ...
        public Option<B1> get(A k) {
            return this.get0(this, k);
        }

        private Option<B1> get0(ListMap<A, B1> cur, A k) {
            Option option;
            block2: {
                while (true) {
                    Object a;
                    if (k == (a = cur.key()) ? true : (k == null ? false : (k instanceof Number ? BoxesRunTime.equalsNumObject((Number)k, a) : (k instanceof Character ? BoxesRunTime.equalsCharObject((Character)k, a) : k.equals(a))))) {
                        option = new Some<B1>(cur.value());
                        break block2;
                    }
                    if (!cur.next().nonEmpty()) break;
                    cur = cur.next();
                }
                option = None$.MODULE$;
            }
        ...
        public <B2> ListMap<A, B2> updated(A k, B2 v) {
            ...
            return new Node<B2>(m, k, v);
        }

        @Override
        public ListMap<A, B1> $minus(A k) {
            return this.remove0(k, this, Nil$.MODULE$);
        }

        private ListMap<A, B1> remove0(A k, ListMap<A, B1> cur, List<ListMap<A, B1>> acc) {
            while (true) {
                block5: {
                    ListMap listMap;
                    block4: {
                        block3: {
                            if (!cur.isEmpty()) break block3;
                            listMap = acc.last();
                            break block4;
                        }
                        Object a = cur.key();
                        if (!(k == a ? true : (k == null ? false : (k instanceof Number ? BoxesRunTime.equalsNumObject((Number)k, a) : (k instanceof Character ? BoxesRunTime.equalsCharObject((Character)k, a) : k.equals(a)))))) break block5;
                        ListMap listMap2 = cur.next();
                        listMap = acc.$div$colon(listMap2, new Serializable(this){
                            public static final long serialVersionUID = 0L;

                            public final ListMap<A, B1> apply(ListMap<A, B1> x0$1, ListMap<A, B1> x1$1) {
                                Tuple2<ListMap<A, B1>, ListMap<A, B1>> tuple2 = new Tuple2<ListMap<A, B1>, ListMap<A, B1>>(x0$1, x1$1);
                                ListMap<A, B1> tt = tuple2._1();
                                return new Node<B1>(tt, tuple2._2().key(), tuple2._2().value());
                            }
                        });
                    }
                    return listMap;
        ...
        public Node(ListMap<A, B> $outer, A key, B1 value) {
            this.key = key;
            this.value = value;
            if ($outer == null) {
                throw null;
