/*
 * Decompiled with CFR 0.152.
 */
package scala.collection.immutable;

import java.io.ObjectInputStream;
import java.io.ObjectOutputStream;
import scala.Array$;
import scala.Function1;
import scala.MatchError;
import scala.Predef$;
import scala.Serializable;
import scala.collection.AbstractSet;
import scala.collection.CustomParallelizable;
import scala.collection.CustomParallelizable$class;
import scala.collection.GenSet;
import scala.collection.GenSetLike$class;
import scala.collection.GenTraversableOnce;
import scala.collection.Iterator;
import scala.collection.Iterator$;
import scala.collection.Seq;
import scala.collection.Set;
import scala.collection.SetLike$class;
import scala.collection.generic.CanBuildFrom;
import scala.collection.generic.GenericCompanion;
import scala.collection.immutable.HashSet$;
import scala.collection.immutable.HashSet$EmptyHashSet$;
import scala.collection.immutable.Iterable$class;
import scala.collection.immutable.ListSet;
import scala.collection.immutable.ListSet$;
import scala.collection.immutable.Range;
import scala.collection.immutable.Range$;
import scala.collection.immutable.Set$class;
import scala.collection.immutable.Traversable$class;
import scala.collection.immutable.TrieIterator;
import scala.collection.parallel.Combiner;
import scala.collection.parallel.immutable.ParHashSet;
import scala.collection.parallel.immutable.ParHashSet$;
import scala.reflect.ScalaSignature;
import scala.runtime.BoxesRunTime;
import scala.runtime.RichInt$;
import scala.runtime.ScalaRunTime$;
import scala.sys.package$;

@ScalaSignature(bytes="\u0006\u0001\u0015=c\u0001B\u0001\u0003\u0001%\u0011q\u0001S1tQN+GO\u0003\u0002\u0004\t\u0005I\u0011.\\7vi\u0006\u0014G.\u001a\u0006\u0003\u000b\u0019\t!bY8mY\u0016\u001cG/[8o\u0015\u00059\u0011!B:dC2\f7\u0001A\u000b\u0003\u0015E\u0019r\u0001A\u0006\u001c?\u0019RC\u0007E\u0002\r\u001b=i\u0011\u0001B\u0005\u0003\u001d\u0011\u00111\"\u00112tiJ\f7\r^*fiB\u0011\u0001#\u0005\u0007\u0001\t\u0015\u0011\u0002A1\u0001\u0014\u0005\u0005\t\u0015C\u0001\u000b\u0019!\t)b#D\u0001\u0007\u0013\t9bAA\u0004O_RD\u0017N\\4\u0011\u0005UI\u0012B\u0001\u000e\u0007\u0005\r\te.\u001f\t\u00049uyQ\"\u0001\u0002\n\u0005y\u0011!aA*fiB!\u0001eI\b&\u001b\u0005\t#B\u0001\u0012\u0005\u0003\u001d9WM\\3sS\u000eL!\u0001J\u0011\u0003%\u001d+g.\u001a:jGN+G\u000fV3na2\fG/\u001a\t\u00039\u0001\u0001B\u0001D\u0014\u0010S%\u0011\u0001\u0006\u0002\u0002\b'\u0016$H*[6f!\ra\u0002a\u0004\t\u0005\u0019-zQ&\u0003\u0002-\t\t!2)^:u_6\u0004\u0016M]1mY\u0016d\u0017N_1cY\u0016\u00042A\f\u001a\u0010\u001b\u0005y#BA\u00021\u0015\t\tD!\u0001\u0005qCJ\fG\u000e\\3m\u0013\t\u0019tF\u0001\u0006QCJD\u0015m\u001d5TKR\u0004\"!F\u001b\n\u0005Y2!\u0001D*fe&\fG.\u001b>bE2,\u0007\"\u0002\u001d\u0001\t\u0003I\u0014A\u0002\u001fj]&$h\bF\u0001*\u0011\u0015Y\u0004\u0001\"\u0011=\u0003%\u0019w.\u001c9b]&|g.F\u0001>!\r\u0001c(J\u0005\u0003\u007f\u0005\u0012\u0001cR3oKJL7mQ8na\u0006t\u0017n\u001c8\t\u000b\u0005\u0003A\u0011\t\"\u0002\u0007A\f'/F\u0001.\u0011\u0015!\u0005\u0001\"\u0011F\u0003\u0011\u0019\u0018N_3\u0016\u0003\u0019\u0003\"!F$\n\u0005!3!aA%oi\")!\n\u0001C!\u0017\u0006)Q-\u001c9usV\t\u0011\u0006C\u0003N\u0001\u0011\u0005a*\u0001\u0005ji\u0016\u0014\u0018\r^8s+\u0005y\u0005c\u0001\u0007Q\u001f%\u0011\u0011\u000b\u0002\u0002\t\u0013R,'/\u0019;pe\")1\u000b\u0001C!)\u00069am\u001c:fC\u000eDWCA+`)\t1\u0016\f\u0005\u0002\u0016/&\u0011\u0001L\u0002\u0002\u0005+:LG\u000fC\u0003[%\u0002\u00071,A\u0001g!\u0011)Bl\u00040\n\u0005u3!!\u0003$v]\u000e$\u0018n\u001c82!\t\u0001r\fB\u0003a%\n\u00071CA\u0001V\u0011\u0015\u0011\u0007\u0001\"\u0001d\u0003!\u0019wN\u001c;bS:\u001cHC\u00013h!\t)R-\u0003\u0002g\r\t9!i\\8mK\u0006t\u0007\"\u00025b\u0001\u0004y\u0011!A3\t\u000b)\u0004A\u0011I6\u0002\u0011M,(m]3u\u001f\u001a$\"\u0001\u001a7\t\u000b5L\u0007\u0019\u00018\u0002\tQD\u0017\r\u001e\t\u0004\u0019=|\u0011B\u00019\u0005\u0005\u00199UM\\*fi\")!\u000f\u0001C\tg\u0006I1/\u001e2tKR|e\r\r\u000b\u0004IR,\b\"B7r\u0001\u0004I\u0003\"\u0002<r\u0001\u00041\u0015!\u00027fm\u0016d\u0007\"\u0002=\u0001\t\u0003J\u0018!\u0002\u0013qYV\u001cHCA\u0015{\u0011\u0015Aw\u000f1\u0001\u0010\u0011\u0015A\b\u0001\"\u0011})\u0015ISp`A\u0002\u0011\u0015q8\u00101\u0001\u0010\u0003\u0015)G.Z72\u0011\u0019\t\ta\u001fa\u0001\u001f\u0005)Q\r\\3ne!9\u0011QA>A\u0002\u0005\u001d\u0011!B3mK6\u001c\b\u0003B\u000b\u0002\n=I1!a\u0003\u0007\u0005)a$/\u001a9fCR,GM\u0010\u0005\b\u0003\u001f\u0001A\u0011IA\t\u0003\u0015)h.[8o)\rI\u00131\u0003\u0005\u0007[\u00065\u0001\u0019\u00018\t\u000f\u0005]\u0001\u0001\"\u0011\u0002\u001a\u0005I\u0011N\u001c;feN,7\r\u001e\u000b\u0004S\u0005m\u0001BB7\u0002\u0016\u0001\u0007a\u000eC\u0004\u0002 \u0001!\t%!\t\u0002\t\u0011LgM\u001a\u000b\u0004S\u0005\r\u0002BB7\u0002\u001e\u0001\u0007a\u000e\u0003\u0005\u0002(\u0001!\tAAA\u0015\u0003\u0019)h.[8oaQ)\u0011&a\u000b\u0005\"\"9Q.!\nA\u0002\u00055\u0002#BA\u0018\u0003o|ab\u0001\u000f\u00022\u001d9\u00111\u0007\u0002\t\u0002\u0005U\u0012a\u0002%bg\"\u001cV\r\u001e\t\u00049\u0005]bAB\u0001\u0003\u0011\u0003\tIdE\u0003\u00028\u0005mB\u0007\u0005\u0003!\u0003{)\u0013bAA C\t\u0019\u0012*\\7vi\u0006\u0014G.Z*fi\u001a\u000b7\r^8ss\"9\u0001(a\u000e\u0005\u0002\u0005\rCCAA\u001b\u0011!\t9%a\u000e\u0005\u0004\u0005%\u0013\u0001D2b]\n+\u0018\u000e\u001c3Ge>lW\u0003BA&\u0003;*\"!!\u0014\u0011\u0013\u0001\ny%a\u0015\u0002\\\u0005}\u0013bAA)C\ta1)\u00198Ck&dGM\u0012:p[B!\u0011QKA,\u001b\t\t9$C\u0002\u0002Zy\u0012AaQ8mYB\u0019\u0001#!\u0018\u0005\rI\t)E1\u0001\u0014!\u0011a\u0002!a\u0017\b\u0011\u0005\r\u0014q\u0007E\u0005\u0003K\nA\"R7qifD\u0015m\u001d5TKR\u0004B!!\u0016\u0002h\u0019A\u0011\u0011NA\u001c\u0011\u0013\tYG\u0001\u0007F[B$\u0018\u0010S1tQN+Go\u0005\u0003\u0002h\u00055\u0004c\u0001\u000f\u00011!9\u0001(a\u001a\u0005\u0002\u0005EDCAA3\u0011)\t)(a\u001a\u0002\u0002\u0013%\u0011qO\u0001\fe\u0016\fGMU3t_24X\r\u0006\u0002\u0002zA!\u00111PAC\u001b\t\tiH\u0003\u0003\u0002\u0000\u0005\u0005\u0015\u0001\u00027b]\u001eT!!a!\u0002\t)\fg/Y\u0005\u0005\u0003\u000f\u000biH\u0001\u0004PE*,7\r\u001e\u0005\n\u0003\u0017\u000b9\u0004\"\u0001\u0005\u0003\u001b\u000bQ\"Z7qifLen\u001d;b]\u000e,WCAA7\u0011!\t\t*a\u000e\u0005\n\u0005M\u0015aD7bW\u0016D\u0015m\u001d5Ue&,7+\u001a;\u0016\t\u0005U5\u0011\u001f\u000b\r\u0003/\u001b\u0019pa>\u0004~\u0012\u0005A1\u0001\t\u0007\u0003+\nIja<\u0007\u000f\u0005m\u0015q\u0007\u0001\u0002\u001e\nY\u0001*Y:i)JLWmU3u+\u0011\ty*!*\u0014\t\u0005e\u0015\u0011\u0015\t\u00059\u0001\t\u0019\u000bE\u0002\u0011\u0003K#aAEAM\u0005\u0004\u0019\u0002BCAU\u00033\u0013)\u0019!C\u0005\u000b\u00061!-\u001b;nCBD!\"!,\u0002\u001a\n\u0005\t\u0015!\u0003G\u0003\u001d\u0011\u0017\u000e^7ba\u0002BA\"!\u0002\u0002\u001a\n\u0015\r\u0011\"\u0001\u0005\u0003c+\"!a-\u0011\u000bU\t),!)\n\u0007\u0005]fAA\u0003BeJ\f\u0017\u0010C\u0006\u0002<\u0006e%\u0011!Q\u0001\n\u0005M\u0016AB3mK6\u001c\b\u0005\u0003\u0006\u0002@\u0006e%Q1A\u0005\n\u0015\u000bQa]5{KBB!\"a1\u0002\u001a\n\u0005\t\u0015!\u0003G\u0003\u0019\u0019\u0018N_31A!9\u0001(!'\u0005\u0002\u0005\u001dG\u0003CAe\u0003\u0017\fi-a4\u0011\r\u0005U\u0013\u0011TAR\u0011\u001d\tI+!2A\u0002\u0019C\u0001\"!\u0002\u0002F\u0002\u0007\u00111\u0017\u0005\b\u0003\u007f\u000b)\r1\u0001G\u0011\u0019!\u0015\u0011\u0014C!\u000b\"A\u0011Q[AM\t\u0003\n9.\u0001\u0003hKR\u0004Dc\u00023\u0002Z\u0006u\u0017\u0011\u001d\u0005\t\u00037\f\u0019\u000e1\u0001\u0002$\u0006\u00191.Z=\t\u000f\u0005}\u00171\u001ba\u0001\r\u0006!\u0001.Y:i\u0011\u00191\u00181\u001ba\u0001\r\"A\u0011Q]AM\t\u0003\n9/\u0001\u0005va\u0012\fG/\u001a31)!\t\t+!;\u0002l\u00065\b\u0002CAn\u0003G\u0004\r!a)\t\u000f\u0005}\u00171\u001da\u0001\r\"1a/a9A\u0002\u0019C\u0011\"a\n\u0002\u001a\u0012\u0005#!!=\u0015\r\u0005\u0005\u00161_BH\u0011\u001di\u0017q\u001ea\u0001\u0003k\u0004b!!\u0016\u0002x\u0006\rfACA}\u0003o\t\t#a\u000e\u0002|\nYA*Z1g\u0011\u0006\u001c\bnU3u+\u0011\tiPa\u0001\u0014\t\u0005]\u0018q \t\u00059\u0001\u0011\t\u0001E\u0002\u0011\u0005\u0007!aAEA|\u0005\u0004\u0019\u0002b\u0002\u001d\u0002x\u0012\u0005!q\u0001\u000b\u0003\u0005\u0013\u0001b!!\u0016\u0002x\n\u0005\u0001\"CAp\u0003o4\t!a\u000eFS\u0019\t9Pa\u0004\u0003B\u001a9!\u0011CA\u001c\u0001\tM!\u0001\u0003%bg\"\u001cV\r^\u0019\u0016\t\tU!1D\n\u0005\u0005\u001f\u00119\u0002\u0005\u0004\u0002V\u0005](\u0011\u0004\t\u0004!\tmAA\u0002\n\u0003\u0010\t\u00071\u0003C\u0007\u0002\\\n=!Q1A\u0005\u0002\u0005]\"qD\u000b\u0003\u00053A1Ba\t\u0003\u0010\t\u0005\t\u0015!\u0003\u0003\u001a\u0005!1.Z=!\u00111\tyNa\u0004\u0003\u0006\u0004%\t!a\u000eF\u0011)\u0011ICa\u0004\u0003\u0002\u0003\u0006IAR\u0001\u0006Q\u0006\u001c\b\u000e\t\u0005\bq\t=A\u0011\u0001B\u0017)\u0019\u0011yC!\r\u00034A1\u0011Q\u000bB\b\u00053A\u0001\"a7\u0003,\u0001\u0007!\u0011\u0004\u0005\b\u0003?\u0014Y\u00031\u0001G\u0011\u0019!%q\u0002C!\u000b\"A\u0011Q\u001bB\b\t\u0003\u0012I\u0004F\u0004e\u0005w\u0011iDa\u0010\t\u0011\u0005m'q\u0007a\u0001\u00053Aq!a8\u00038\u0001\u0007a\t\u0003\u0004w\u0005o\u0001\rA\u0012\u0005\be\n=A\u0011\tB\")\u0015!'Q\tB%\u0011\u001di'\u0011\ta\u0001\u0005\u000f\u0002B\u0001\b\u0001\u0003\u001a!1aO!\u0011A\u0002\u0019C\u0001\"!:\u0003\u0010\u0011\u0005#Q\n\u000b\t\u0005\u000f\u0012yE!\u0015\u0003T!A\u00111\u001cB&\u0001\u0004\u0011I\u0002C\u0004\u0002`\n-\u0003\u0019\u0001$\t\rY\u0014Y\u00051\u0001G\u0011%\t9Ca\u0004\u0005B\t\u00119\u0006\u0006\u0004\u0003H\te#1\f\u0005\b[\nU\u0003\u0019\u0001B\f\u0011\u00191(Q\u000ba\u0001\r\"I\u0011q\u0005B\b\t\u0003\u0012!q\f\u000b\u000b\u0005\u000f\u0012\tGa\u0019\u0003f\t-\u0004bB7\u0003^\u0001\u0007!q\t\u0005\u0007m\nu\u0003\u0019\u0001$\t\u0011\t\u001d$Q\fa\u0001\u0005S\naAY;gM\u0016\u0014\b#B\u000b\u00026\n\u001d\u0003b\u0002B7\u0005;\u0002\rAR\u0001\b_\u001a47/\u001a;1\u0011%\u0011\tHa\u0004\u0005B\t\u0011\u0019(\u0001\u0006j]R,'o]3diB\"\"Ba\u0012\u0003v\t]$\u0011\u0010B>\u0011\u001di'q\u000ea\u0001\u0005\u000fBaA\u001eB8\u0001\u00041\u0005\u0002\u0003B4\u0005_\u0002\rA!\u001b\t\u000f\t5$q\u000ea\u0001\r\"I!q\u0010B\b\t\u0003\u0012!\u0011Q\u0001\u0006I&4g\r\r\u000b\u000b\u0005\u000f\u0012\u0019I!\"\u0003\b\n%\u0005bB7\u0003~\u0001\u0007!q\t\u0005\u0007m\nu\u0004\u0019\u0001$\t\u0011\t\u001d$Q\u0010a\u0001\u0005SBqA!\u001c\u0003~\u0001\u0007a\t\u0003\u0005\u0003\u000e\n=A\u0011\tBH\u0003!\u0011X-\\8wK\u0012\u0004D\u0003\u0003B$\u0005#\u0013\u0019J!&\t\u0011\u0005m'1\u0012a\u0001\u00053Aq!a8\u0003\f\u0002\u0007a\t\u0003\u0004w\u0005\u0017\u0003\rA\u0012\u0005\t\u00053\u0013y\u0001\"\u0015\u0003\u001c\u00069a-\u001b7uKJ\u0004D\u0003\u0004B$\u0005;\u0013\u0019Ka*\u0003*\n-\u0006\u0002\u0003BP\u0005/\u0003\rA!)\u0002\u0003A\u0004R!\u0006/\u0003\u001a\u0011DqA!*\u0003\u0018\u0002\u0007A-\u0001\u0004oK\u001e\fG/\u001a\u0005\u0007m\n]\u0005\u0019\u0001$\t\u0011\t\u001d$q\u0013a\u0001\u0005SBqA!\u001c\u0003\u0018\u0002\u0007a\tC\u0004N\u0005\u001f!\tEa,\u0016\u0005\tE\u0006\u0003\u0002\u0007Q\u00053Aqa\u0015B\b\t\u0003\u0012),\u0006\u0003\u00038\n}Fc\u0001,\u0003:\"9!La-A\u0002\tm\u0006CB\u000b]\u00053\u0011i\fE\u0002\u0011\u0005\u007f#a\u0001\u0019BZ\u0005\u0004\u0019b\u0001\u0003Bb\u0003o\u0001!A!2\u0003#!\u000b7\u000f[*fi\u000e{G\u000e\\5tS>t\u0017'\u0006\u0003\u0003H\n57\u0003\u0002Ba\u0005\u0013\u0004b!!\u0016\u0002x\n-\u0007c\u0001\t\u0003N\u00121!C!1C\u0002MAA\"a8\u0003B\n\u0015\r\u0011\"\u0001\u00028\u0015C!B!\u000b\u0003B\n\u0005\t\u0015!\u0003G\u0011-\u0011)N!1\u0003\u0006\u0004%\tAa6\u0002\u0005-\u001cXC\u0001Bm!\u0015a\"1\u001cBf\u0013\r\u0011iN\u0001\u0002\b\u0019&\u001cHoU3u\u0011-\u0011\tO!1\u0003\u0002\u0003\u0006IA!7\u0002\u0007-\u001c\b\u0005C\u00049\u0005\u0003$\tA!:\u0015\r\t\u001d(\u0011\u001eBv!\u0019\t)F!1\u0003L\"9\u0011q\u001cBr\u0001\u00041\u0005\u0002\u0003Bk\u0005G\u0004\rA!7\t\r\u0011\u0013\t\r\"\u0011F\u0011!\t)N!1\u0005B\tEHc\u00023\u0003t\nU(q\u001f\u0005\t\u00037\u0014y\u000f1\u0001\u0003L\"9\u0011q\u001cBx\u0001\u00041\u0005B\u0002<\u0003p\u0002\u0007a\tC\u0004s\u0005\u0003$\tEa?\u0015\u000b\u0011\u0014ip!\u0001\t\u000f5\u0014I\u00101\u0001\u0003\u0000B!A\u0004\u0001Bf\u0011\u00191(\u0011 a\u0001\r\"A\u0011Q\u001dBa\t\u0003\u001a)\u0001\u0006\u0005\u0003\u0000\u000e\u001d1\u0011BB\u0006\u0011!\tYna\u0001A\u0002\t-\u0007bBAp\u0007\u0007\u0001\rA\u0012\u0005\u0007m\u000e\r\u0001\u0019\u0001$\t\u0011\u0005\u001d\"\u0011\u0019C!\u0007\u001f!bAa@\u0004\u0012\rM\u0001bB7\u0004\u000e\u0001\u0007!\u0011\u001a\u0005\u0007m\u000e5\u0001\u0019\u0001$\t\u0011\u0005\u001d\"\u0011\u0019C!\u0007/!\"Ba@\u0004\u001a\rm1QDB\u0011\u0011\u001di7Q\u0003a\u0001\u0005\u007fDaA^B\u000b\u0001\u00041\u0005\u0002\u0003B4\u0007+\u0001\raa\b\u0011\u000bU\t)La@\t\u000f\t54Q\u0003a\u0001\r\"I!\u0011\u000fBa\t\u0003\u00121Q\u0005\u000b\u000b\u0005\u007f\u001c9c!\u000b\u0004,\r5\u0002bB7\u0004$\u0001\u0007!q \u0005\u0007m\u000e\r\u0002\u0019\u0001$\t\u0011\t\u001d41\u0005a\u0001\u0007?AqA!\u001c\u0004$\u0001\u0007a\tC\u0005\u0003\u0000\t\u0005G\u0011\t\u0002\u00042QQ!q`B\u001a\u0007k\u00199d!\u000f\t\u000f5\u001cy\u00031\u0001\u0003\u0000\"1aoa\fA\u0002\u0019C\u0001Ba\u001a\u00040\u0001\u00071q\u0004\u0005\b\u0005[\u001ay\u00031\u0001G\u0011!\u0011iI!1\u0005B\ruB\u0003\u0003B\u0000\u0007\u007f\u0019\tea\u0011\t\u0011\u0005m71\ba\u0001\u0005\u0017Dq!a8\u0004<\u0001\u0007a\t\u0003\u0004w\u0007w\u0001\rA\u0012\u0005\t\u00053\u0013\t\r\"\u0015\u0004HQa!q`B%\u0007\u001b\u001aye!\u0015\u0004T!A!qTB#\u0001\u0004\u0019Y\u0005E\u0003\u00169\n-G\rC\u0004\u0003&\u000e\u0015\u0003\u0019\u00013\t\rY\u001c)\u00051\u0001G\u0011!\u00119g!\u0012A\u0002\r}\u0001b\u0002B7\u0007\u000b\u0002\rA\u0012\u0005\b\u001b\n\u0005G\u0011IB,+\t\u0019I\u0006\u0005\u0003\r!\n-\u0007bB*\u0003B\u0012\u00053QL\u000b\u0005\u0007?\u001a9\u0007F\u0002W\u0007CBqAWB.\u0001\u0004\u0019\u0019\u0007\u0005\u0004\u00169\n-7Q\r\t\u0004!\r\u001dDA\u00021\u0004\\\t\u00071\u0003\u0003\u0005\u0004l\t\u0005G\u0011BB7\u0003-9(/\u001b;f\u001f\nTWm\u0019;\u0015\u0007Y\u001by\u0007\u0003\u0005\u0004r\r%\u0004\u0019AB:\u0003\ryW\u000f\u001e\t\u0005\u0007k\u001aY(\u0004\u0002\u0004x)!1\u0011PAA\u0003\tIw.\u0003\u0003\u0004~\r]$AE(cU\u0016\u001cGoT;uaV$8\u000b\u001e:fC6D\u0001b!!\u0003B\u0012%11Q\u0001\u000be\u0016\fGm\u00142kK\u000e$Hc\u0001,\u0004\u0006\"A1qQB@\u0001\u0004\u0019I)\u0001\u0002j]B!1QOBF\u0013\u0011\u0019iia\u001e\u0003#=\u0013'.Z2u\u0013:\u0004X\u000f^*ue\u0016\fW\u000e\u0003\u0004w\u0003_\u0004\rA\u0012\u0005\n\u0003O\tI\n\"\u0011\u0003\u0007'#\"\"!)\u0004\u0016\u000e]5\u0011TBN\u0011\u001di7\u0011\u0013a\u0001\u0003CCaA^BI\u0001\u00041\u0005\u0002\u0003B4\u0007#\u0003\r!a-\t\u000f\t54\u0011\u0013a\u0001\r\"I!\u0011OAM\t\u0003\u00121q\u0014\u000b\u000b\u0003C\u001b\tka)\u0004&\u000e\u001d\u0006bB7\u0004\u001e\u0002\u0007\u0011\u0011\u0015\u0005\u0007m\u000eu\u0005\u0019\u0001$\t\u0011\t\u001d4Q\u0014a\u0001\u0003gCqA!\u001c\u0004\u001e\u0002\u0007a\tC\u0005\u0003\u0000\u0005eE\u0011\t\u0002\u0004,RQ\u0011\u0011UBW\u0007_\u001b\tla-\t\u000f5\u001cI\u000b1\u0001\u0002\"\"1ao!+A\u0002\u0019C\u0001Ba\u001a\u0004*\u0002\u0007\u00111\u0017\u0005\b\u0005[\u001aI\u000b1\u0001G\u0011!\u0011i)!'\u0005B\r]F\u0003CAQ\u0007s\u001bYl!0\t\u0011\u0005m7Q\u0017a\u0001\u0003GCq!a8\u00046\u0002\u0007a\t\u0003\u0004w\u0007k\u0003\rA\u0012\u0005\be\u0006eE\u0011IBa)\u0015!71YBc\u0011\u001di7q\u0018a\u0001\u0003CCaA^B`\u0001\u00041\u0005\u0002\u0003BM\u00033#\tf!3\u0015\u0019\u0005\u000561ZBh\u0007#\u001c\u0019n!6\t\u0011\t}5q\u0019a\u0001\u0007\u001b\u0004R!\u0006/\u0002$\u0012DqA!*\u0004H\u0002\u0007A\r\u0003\u0004w\u0007\u000f\u0004\rA\u0012\u0005\t\u0005O\u001a9\r1\u0001\u00024\"9!QNBd\u0001\u00041\u0005bB'\u0002\u001a\u0012\u00053\u0011\\\u000b\u0003\u00077\u0004R\u0001HBo\u0003GK1aa8\u0003\u00051!&/[3Ji\u0016\u0014\u0018\r^8s\u0011\u001d\u0019\u0016\u0011\u0014C!\u0007G,Ba!:\u0004nR\u0019aka:\t\u000fi\u001b\t\u000f1\u0001\u0004jB1Q\u0003XAR\u0007W\u00042\u0001EBw\t\u0019\u00017\u0011\u001db\u0001'A\u0019\u0001c!=\u0005\rI\tyI1\u0001\u0014\u0011\u001d\u0019)0a$A\u0002\u0019\u000bQ\u0001[1tQBB\u0001b!?\u0002\u0010\u0002\u000711`\u0001\u0006K2,W\u000e\r\t\u00059\u0001\u0019y\u000fC\u0004\u0004\u0000\u0006=\u0005\u0019\u0001$\u0002\u000b!\f7\u000f[\u0019\t\u000fy\fy\t1\u0001\u0004|\"1a/a$A\u0002\u0019C\u0001\u0002b\u0002\u00028\u0011%A\u0011B\u0001\u000bEV4g-\u001a:TSj,Gc\u0001$\u0005\f!1A\t\"\u0002A\u0002\u0019CC\u0001\"\u0002\u0005\u0010A\u0019Q\u0003\"\u0005\n\u0007\u0011MaA\u0001\u0004j]2Lg.\u001a\u0005\t\t/\t9\u0004\"\u0003\u0005\u001a\u0005Ya.\u001e7m)>,U\u000e\u001d;z+\u0011!Y\u0002\"\t\u0015\t\u0011uA1\u0005\t\u00059\u0001!y\u0002E\u0002\u0011\tC!aA\u0005C\u000b\u0005\u0004\u0019\u0002\u0002\u0003C\u0013\t+\u0001\r\u0001\"\b\u0002\u0003MDC\u0001\"\u0006\u0005\u0010!AA1FA\u001c\t\u0013!i#\u0001\u0005lK\u0016\u0004()\u001b;t)\u00151Eq\u0006C\u0019\u0011\u001d\tI\u000b\"\u000bA\u0002\u0019Cq\u0001b\r\u0005*\u0001\u0007a)\u0001\u0003lK\u0016\u0004\b\"\u0003C\u001c\u0003o\u0001K\u0011\u0002C\u001d\u0003=)hn]5h]\u0016$7i\\7qCJ,G#\u00023\u0005<\u0011}\u0002b\u0002C\u001f\tk\u0001\rAR\u0001\u0002S\"9A\u0011\tC\u001b\u0001\u00041\u0015!\u00016)\t\u0011UBq\u0002\u0004\b\t\u000f\n9\u0004\u0002C%\u0005I\u0019VM]5bY&T\u0018\r^5p]B\u0013x\u000e_=\u0016\r\u0011-CQ\fC?'\u0015!)\u0005\"\u00145!\r)BqJ\u0005\u0004\t#2!AB!osJ+g\rC\u0006\u0005V\u0011\u0015#\u00111A\u0005\n\u0011]\u0013\u0001B8sS\u001e,\"\u0001\"\u0017\u0011\tq\u0001A1\f\t\u0004!\u0011uCA\u0002\n\u0005F\t\u00071\u0003C\u0006\u0005b\u0011\u0015#\u00111A\u0005\n\u0011\r\u0014\u0001C8sS\u001e|F%Z9\u0015\u0007Y#)\u0007\u0003\u0006\u0005h\u0011}\u0013\u0011!a\u0001\t3\n1\u0001\u001f\u00132\u0011-!Y\u0007\"\u0012\u0003\u0002\u0003\u0006K\u0001\"\u0017\u0002\u000b=\u0014\u0018n\u001a\u0011)\t\u0011%Dq\u000e\t\u0004+\u0011E\u0014b\u0001C:\r\tIAO]1og&,g\u000e\u001e\u0005\bq\u0011\u0015C\u0011\u0001C<)\u0011!I\b\"!\u0011\u0011\u0005UCQ\tC.\tw\u00022\u0001\u0005C?\t\u001d!y\b\"\u0012C\u0002M\u0011\u0011A\u0011\u0005\t\t+\")\b1\u0001\u0005Z!A11\u000eC#\t\u0013!)\tF\u0002W\t\u000fC\u0001b!\u001d\u0005\u0004\u0002\u000711\u000f\u0005\t\u0007\u0003#)\u0005\"\u0003\u0005\fR\u0019a\u000b\"$\t\u0011\r\u001dE\u0011\u0012a\u0001\u0007\u0013C\u0001\"!\u001e\u0005F\u0011%A\u0011\u0013\u000b\u0003\t\u001bB\u0003\u0002\"\u0012\u0005\u0016\u0012mEQ\u0014\t\u0004+\u0011]\u0015b\u0001CM\r\t\u00012+\u001a:jC24VM]:j_:,\u0016\nR\u0001\u0006m\u0006dW/\u001a\u0010\u0002\u0005!Q\u0011QOA\u001c\u0003\u0003%I!a\u001e\t\rY\f)\u00031\u0001G\u0011!\t9\u0003\u0001C\u0001\u0005\u0011\u0015F#C\u0015\u0005(\u0012%F1\u0016CX\u0011\u0019iG1\u0015a\u0001S!1a\u000fb)A\u0002\u0019C\u0001Ba\u001a\u0005$\u0002\u0007AQ\u0016\t\u0005+\u0005U\u0016\u0006C\u0004\u0003n\u0011\r\u0006\u0019\u0001$\t\u0011\tE\u0004\u0001\"\u0001\u0003\tg#\u0012\"\u000bC[\to#I\fb/\t\r5$\t\f1\u0001*\u0011\u00191H\u0011\u0017a\u0001\r\"A!q\rCY\u0001\u0004!i\u000bC\u0004\u0003n\u0011E\u0006\u0019\u0001$\t\u0011\t}\u0004\u0001\"\u0001\u0003\t\u007f#\u0012\"\u000bCa\t\u0007$)\rb2\t\r5$i\f1\u0001*\u0011\u00191HQ\u0018a\u0001\r\"A!q\rC_\u0001\u0004!i\u000bC\u0004\u0003n\u0011u\u0006\u0019\u0001$\t\u000f\u0011-\u0007\u0001\"\u0001\u0005N\u00061A%\\5okN$2!\u000bCh\u0011\u0019AG\u0011\u001aa\u0001\u001f!9A1\u001b\u0001\u0005B\u0011U\u0017A\u00024jYR,'\u000fF\u0002*\t/D\u0001Ba(\u0005R\u0002\u0007A\u0011\u001c\t\u0005+q{A\rC\u0004\u0005^\u0002!\t\u0005b8\u0002\u0013\u0019LG\u000e^3s\u001d>$HcA\u0015\u0005b\"A!q\u0014Cn\u0001\u0004!I\u000eC\u0004\u0003\u001a\u0002!\t\u0002\":\u0015\u0017%\"9\u000f\";\u0005l\u00125Hq\u001e\u0005\t\u0005?#\u0019\u000f1\u0001\u0005Z\"9!Q\u0015Cr\u0001\u0004!\u0007B\u0002<\u0005d\u0002\u0007a\t\u0003\u0005\u0003h\u0011\r\b\u0019\u0001CW\u0011\u001d\u0011i\u0007b9A\u0002\u0019Cq\u0001b=\u0001\t#!)0\u0001\u0007fY\u0016l\u0007*Y:i\u0007>$W\rF\u0002G\toDq!a7\u0005r\u0002\u0007q\u0002C\u0004\u0005|\u0002!)\u0002\"@\u0002\u000f%l\u0007O]8wKR\u0019a\tb@\t\u000f\u0015\u0005A\u0011 a\u0001\r\u0006)\u0001nY8eK\"AQQ\u0001\u0001\u0005\u0002\u0011)9!A\u0006d_6\u0004X\u000f^3ICNDGc\u0001$\u0006\n!9\u00111\\C\u0002\u0001\u0004y\u0001bBAk\u0001\u0011EQQ\u0002\u000b\bI\u0016=Q\u0011CC\n\u0011\u001d\tY.b\u0003A\u0002=Aq!a8\u0006\f\u0001\u0007a\t\u0003\u0004w\u000b\u0017\u0001\rA\u0012\u0005\b\u0003K\u0004A\u0011AC\f)\u001dIS\u0011DC\u000e\u000b;Aq!a7\u0006\u0016\u0001\u0007q\u0002C\u0004\u0002`\u0016U\u0001\u0019\u0001$\t\rY,)\u00021\u0001G\u0011\u001d\u0011i\t\u0001C\t\u000bC!r!KC\u0012\u000bK)9\u0003C\u0004\u0002\\\u0016}\u0001\u0019A\b\t\u000f\u0005}Wq\u0004a\u0001\r\"1a/b\bA\u0002\u0019Cq!b\u000b\u0001\t#!\t*\u0001\u0007xe&$XMU3qY\u0006\u001cW\rC\u0004\u00060\u0001!\t%\"\r\u0002\u000bQ|7+\u001a;\u0016\t\u0015MR\u0011H\u000b\u0003\u000bk\u0001B\u0001H\u000f\u00068A\u0019\u0001#\"\u000f\u0005\u0011\u0011}TQ\u0006b\u0001\u000bw\t\"a\u0004\r)\u000f\u0001)y$\"\u0012\u0006JA\u0019Q#\"\u0011\n\u0007\u0015\rcAA\u000beKB\u0014XmY1uK\u0012Le\u000e[3sSR\fgnY3\"\u0005\u0015\u001d\u0013a\u0015+iK\u0002JW\u000e\u001d7f[\u0016tG/\u0019;j_:\u0004C-\u001a;bS2\u001c\be\u001c4!S6lW\u000f^1cY\u0016\u0004\u0003.Y:iAM,Go\u001d\u0011nC.,\u0007%\u001b8iKJLG/\u001b8hA\u0019\u0014x.\u001c\u0011uQ\u0016l\u0007%\u001e8xSN,g&\t\u0002\u0006L\u00051!GL\u00192]ABs\u0001\u0001CK\t7#i\n")
public class HashSet<A>
extends AbstractSet<A>
implements scala.collection.immutable.Set<A>,
CustomParallelizable<A, ParHashSet<A>>,
Serializable {
    public static final long serialVersionUID = 2L;

    public static <A> CanBuildFrom<HashSet<?>, A, HashSet<A>> canBuildFrom() {
        return HashSet$.MODULE$.canBuildFrom();
    }

    public static <A> Object setCanBuildFrom() {
        return HashSet$.MODULE$.setCanBuildFrom();
    }

    @Override
    public Combiner<A, ParHashSet<A>> parCombiner() {
        return CustomParallelizable$class.parCombiner(this);
    }

    @Override
    public scala.collection.immutable.Set<A> seq() {
        return Set$class.seq(this);
    }

    @Override
    public GenericCompanion<HashSet> companion() {
        return HashSet$.MODULE$;
    }

    @Override
    public ParHashSet<A> par() {
        return ParHashSet$.MODULE$.fromTrie(this);
    }

    @Override
    public int size() {
        return 0;
    }

    @Override
    public HashSet<A> empty() {
        return (HashSet)HashSet$.MODULE$.empty();
    }

    @Override
    public Iterator<A> iterator() {
        return Iterator$.MODULE$.empty();
    }

    @Override
    public <U> void foreach(Function1<A, U> f) {
    }

    @Override
    public boolean contains(A e) {
        return this.get0(e, this.computeHash(e), 0);
    }

    @Override
    public boolean subsetOf(GenSet<A> that) {
        boolean bl;
        if (that instanceof HashSet) {
            HashSet hashSet = (HashSet)that;
            bl = this.subsetOf0(hashSet, 0);
        } else {
            bl = GenSetLike$class.subsetOf(this, that);
        }
        return bl;
    }

    public boolean subsetOf0(HashSet<A> that, int level) {
        return true;
    }

    @Override
    public HashSet<A> $plus(A e) {
        return this.updated0(e, this.computeHash(e), 0);
    }

    @Override
    public HashSet<A> $plus(A elem1, A elem2, Seq<A> elems) {
        return (HashSet)((AbstractSet)((HashSet)this.$plus((Object)elem1)).$plus((Object)elem2)).$plus$plus(elems);
    }

    @Override
    public HashSet<A> union(GenSet<A> that) {
        HashSet hashSet;
        if (that instanceof HashSet) {
            HashSet hashSet2 = (HashSet)that;
            int n = this.size() + hashSet2.size();
            HashSet$ hashSet$ = HashSet$.MODULE$;
            int n2 = n + 6;
            Predef$ predef$ = Predef$.MODULE$;
            HashSet[] buffer = new HashSet[RichInt$.MODULE$.min$extension(n2, 224)];
            HashSet hashSet3 = this.union0(hashSet2, 0, buffer, 0);
            HashSet$ hashSet$2 = HashSet$.MODULE$;
            hashSet = hashSet3 == null ? (HashSet)hashSet$2.empty() : hashSet3;
        } else {
            hashSet = (HashSet)SetLike$class.union(this, that);
        }
        return hashSet;
    }

    @Override
    public HashSet<A> intersect(GenSet<A> that) {
        HashSet hashSet;
        if (that instanceof HashSet) {
            HashSet hashSet2 = (HashSet)that;
            int n = this.size();
            Predef$ predef$ = Predef$.MODULE$;
            int n2 = RichInt$.MODULE$.min$extension(n, hashSet2.size());
            HashSet$ hashSet$ = HashSet$.MODULE$;
            int n3 = n2 + 6;
            Predef$ predef$2 = Predef$.MODULE$;
            HashSet[] buffer = new HashSet[RichInt$.MODULE$.min$extension(n3, 224)];
            HashSet hashSet3 = this.intersect0(hashSet2, 0, buffer, 0);
            HashSet$ hashSet$2 = HashSet$.MODULE$;
            hashSet = hashSet3 == null ? (HashSet)hashSet$2.empty() : hashSet3;
        } else {
            hashSet = (HashSet)GenSetLike$class.intersect(this, that);
        }
        return hashSet;
    }

    @Override
    public HashSet<A> diff(GenSet<A> that) {
        HashSet hashSet;
        if (that instanceof HashSet) {
            HashSet hashSet2 = (HashSet)that;
            int n = this.size();
            HashSet$ hashSet$ = HashSet$.MODULE$;
            int n2 = n + 6;
            Predef$ predef$ = Predef$.MODULE$;
            HashSet[] buffer = new HashSet[RichInt$.MODULE$.min$extension(n2, 224)];
            HashSet hashSet3 = this.diff0(hashSet2, 0, buffer, 0);
            HashSet$ hashSet$2 = HashSet$.MODULE$;
            hashSet = hashSet3 == null ? (HashSet)hashSet$2.empty() : hashSet3;
        } else {
            hashSet = (HashSet)SetLike$class.diff(this, that);
        }
        return hashSet;
    }

    public HashSet<A> union0(LeafHashSet<A> that, int level) {
        return that;
    }

    public HashSet<A> union0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
        return that;
    }

    public HashSet<A> intersect0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
        return null;
    }

    public HashSet<A> diff0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
        return null;
    }

    @Override
    public HashSet<A> $minus(A e) {
        HashSet hashSet = this.removed0(e, this.computeHash(e), 0);
        HashSet$ hashSet$ = HashSet$.MODULE$;
        return hashSet == null ? (HashSet)hashSet$.empty() : hashSet;
    }

    @Override
    public HashSet<A> filter(Function1<A, Object> p) {
        int n = this.size();
        HashSet$ hashSet$ = HashSet$.MODULE$;
        int n2 = n + 6;
        Predef$ predef$ = Predef$.MODULE$;
        HashSet[] buffer = new HashSet[RichInt$.MODULE$.min$extension(n2, 224)];
        HashSet hashSet = this.filter0(p, false, 0, buffer, 0);
        HashSet$ hashSet$2 = HashSet$.MODULE$;
        return hashSet == null ? (HashSet)hashSet$2.empty() : hashSet;
    }

    @Override
    public HashSet<A> filterNot(Function1<A, Object> p) {
        int n = this.size();
        HashSet$ hashSet$ = HashSet$.MODULE$;
        int n2 = n + 6;
        Predef$ predef$ = Predef$.MODULE$;
        HashSet[] buffer = new HashSet[RichInt$.MODULE$.min$extension(n2, 224)];
        HashSet hashSet = this.filter0(p, true, 0, buffer, 0);
        HashSet$ hashSet$2 = HashSet$.MODULE$;
        return hashSet == null ? (HashSet)hashSet$2.empty() : hashSet;
    }

    public HashSet<A> filter0(Function1<A, Object> p, boolean negate, int level, HashSet<A>[] buffer, int offset0) {
        return null;
    }

    public int elemHashCode(A key) {
        return ScalaRunTime$.MODULE$.hash(key);
    }

    public final int improve(int hcode) {
        int h = hcode + ~(hcode << 9);
        h ^= h >>> 14;
        h += h << 4;
        return h ^ h >>> 10;
    }

    public int computeHash(A key) {
        return this.improve(this.elemHashCode(key));
    }

    public boolean get0(A key, int hash, int level) {
        return false;
    }

    public HashSet<A> updated0(A key, int hash, int level) {
        return new HashSet1<A>(key, hash);
    }

    public HashSet<A> removed0(A key, int hash, int level) {
        return this;
    }

    public Object writeReplace() {
        return new SerializationProxy(this);
    }

    @Override
    public <B> scala.collection.immutable.Set<B> toSet() {
        return this;
    }

    public HashSet() {
        Traversable$class.$init$(this);
        Iterable$class.$init$(this);
        Set$class.$init$(this);
        CustomParallelizable$class.$init$(this);
    }

    public static class HashSet1<A>
    extends LeafHashSet<A> {
        private final A key;
        private final int hash;

        public A key() {
            return this.key;
        }

        @Override
        public int hash() {
            return this.hash;
        }

        @Override
        public int size() {
            return 1;
        }

        @Override
        public boolean get0(A key, int hash, int level) {
            A a;
            return hash == this.hash() && (key == (a = this.key()) ? true : (key == null ? false : (key instanceof Number ? BoxesRunTime.equalsNumObject((Number)key, a) : (key instanceof Character ? BoxesRunTime.equalsCharObject((Character)key, a) : key.equals(a)))));
        }

        @Override
        public boolean subsetOf0(HashSet<A> that, int level) {
            return that.get0(this.key(), this.hash(), level);
        }

        @Override
        public HashSet<A> updated0(A key, int hash, int level) {
            A a;
            return hash == this.hash() && (key == (a = this.key()) ? true : (key == null ? false : (key instanceof Number ? BoxesRunTime.equalsNumObject((Number)key, a) : (key instanceof Character ? BoxesRunTime.equalsCharObject((Character)key, a) : key.equals(a))))) ? this : (hash != this.hash() ? HashSet$.MODULE$.scala$collection$immutable$HashSet$$makeHashTrieSet(this.hash(), this, hash, new HashSet1<A>(key, hash), level) : new HashSetCollision1(hash, ((ListSet)((ListSet)ListSet$.MODULE$.empty()).$plus((Object)this.key())).$plus((Object)key)));
        }

        @Override
        public HashSet<A> union0(LeafHashSet<A> that, int level) {
            block5: {
                HashSet hashSet;
                block3: {
                    block4: {
                        block2: {
                            if (that.hash() == this.hash()) break block2;
                            hashSet = HashSet$.MODULE$.scala$collection$immutable$HashSet$$makeHashTrieSet(this.hash(), this, that.hash(), that, level);
                            break block3;
                        }
                        if (!(that instanceof HashSet1)) break block4;
                        HashSet1 hashSet1 = (HashSet1)that;
                        A a = hashSet1.key();
                        A a2 = this.key();
                        hashSet = (a2 == a ? true : (a2 == null ? false : (a2 instanceof Number ? BoxesRunTime.equalsNumObject((Number)a2, a) : (a2 instanceof Character ? BoxesRunTime.equalsCharObject((Character)a2, a) : a2.equals(a))))) ? this : new HashSetCollision1(this.hash(), ((ListSet)((ListSet)ListSet$.MODULE$.empty()).$plus((Object)this.key())).$plus((Object)hashSet1.key()));
                        break block3;
                    }
                    if (!(that instanceof HashSetCollision1)) break block5;
                    HashSetCollision1 hashSetCollision1 = (HashSetCollision1)that;
                    Object ks1 = hashSetCollision1.ks().$plus((Object)this.key());
                    hashSet = ((ListSet)ks1).size() == hashSetCollision1.ks().size() ? hashSetCollision1 : new HashSetCollision1(this.hash(), ks1);
                }
                return hashSet;
            }
            throw new MatchError(that);
        }

        @Override
        public HashSet<A> union0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
            return that.union0(this, level);
        }

        @Override
        public HashSet<A> intersect0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
            return that.get0(this.key(), this.hash(), level) ? this : null;
        }

        @Override
        public HashSet<A> diff0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
            return that.get0(this.key(), this.hash(), level) ? null : this;
        }

        @Override
        public HashSet<A> removed0(A key, int hash, int level) {
            A a;
            return hash == this.hash() && (key == (a = this.key()) ? true : (key == null ? false : (key instanceof Number ? BoxesRunTime.equalsNumObject((Number)key, a) : (key instanceof Character ? BoxesRunTime.equalsCharObject((Character)key, a) : key.equals(a))))) ? null : this;
        }

        @Override
        public HashSet<A> filter0(Function1<A, Object> p, boolean negate, int level, HashSet<A>[] buffer, int offset0) {
            return negate ^ BoxesRunTime.unboxToBoolean(p.apply(this.key())) ? this : null;
        }

        @Override
        public Iterator<A> iterator() {
            return Iterator$.MODULE$.apply(Predef$.MODULE$.genericWrapArray(new Object[]{this.key()}));
        }

        @Override
        public <U> void foreach(Function1<A, U> f) {
            f.apply(this.key());
        }

        public HashSet1(A key, int hash) {
            this.key = key;
            this.hash = hash;
        }
    }

    public static abstract class LeafHashSet<A>
    extends HashSet<A> {
        public abstract int hash();
    }

    public static class HashTrieSet<A>
    extends HashSet<A> {
        private final int bitmap;
        private final HashSet<A>[] elems;
        private final int size0;

        private int bitmap() {
            return this.bitmap;
        }

        public HashSet<A>[] elems() {
            return this.elems;
        }

        private int size0() {
            return this.size0;
        }

        @Override
        public int size() {
            return this.size0();
        }

        @Override
        public boolean get0(A key, int hash, int level) {
            boolean bl;
            int index = hash >>> level & 0x1F;
            int mask = 1 << index;
            if (this.bitmap() == -1) {
                bl = this.elems()[index & 0x1F].get0(key, hash, level + 5);
            } else if ((this.bitmap() & mask) != 0) {
                int offset = Integer.bitCount(this.bitmap() & mask - 1);
                bl = this.elems()[offset].get0(key, hash, level + 5);
            } else {
                bl = false;
            }
            return bl;
        }

        @Override
        public HashSet<A> updated0(A key, int hash, int level) {
            HashTrieSet hashTrieSet;
            int index = hash >>> level & 0x1F;
            int mask = 1 << index;
            int offset = Integer.bitCount(this.bitmap() & mask - 1);
            if ((this.bitmap() & mask) != 0) {
                HashSet<A> subNew;
                HashSet<A> sub = this.elems()[offset];
                if (sub == (subNew = sub.updated0(key, hash, level + 5))) {
                    hashTrieSet = this;
                } else {
                    HashSet[] elemsNew = new HashSet[this.elems().length];
                    Array$.MODULE$.copy(this.elems(), 0, elemsNew, 0, this.elems().length);
                    elemsNew[offset] = subNew;
                    hashTrieSet = new HashTrieSet<A>(this.bitmap(), elemsNew, this.size() + (subNew.size() - sub.size()));
                }
            } else {
                HashSet[] elemsNew = new HashSet[this.elems().length + 1];
                Array$.MODULE$.copy(this.elems(), 0, elemsNew, 0, offset);
                elemsNew[offset] = new HashSet1<A>(key, hash);
                Array$.MODULE$.copy(this.elems(), offset, elemsNew, offset + 1, this.elems().length - offset);
                int bitmapNew = this.bitmap() | mask;
                hashTrieSet = new HashTrieSet<A>(bitmapNew, elemsNew, this.size() + 1);
            }
            return hashTrieSet;
        }

        @Override
        public HashSet<A> union0(LeafHashSet<A> that, int level) {
            HashTrieSet hashTrieSet;
            int index = that.hash() >>> level & 0x1F;
            int mask = 1 << index;
            int offset = Integer.bitCount(this.bitmap() & mask - 1);
            if ((this.bitmap() & mask) != 0) {
                HashSet<A> sub1;
                HashSet<A> sub = this.elems()[offset];
                if (sub == (sub1 = sub.union0(that, level + 5))) {
                    hashTrieSet = this;
                } else {
                    HashSet[] elems1 = new HashSet[this.elems().length];
                    Array$.MODULE$.copy(this.elems(), 0, elems1, 0, this.elems().length);
                    elems1[offset] = sub1;
                    hashTrieSet = new HashTrieSet<A>(this.bitmap(), elems1, this.size() + (sub1.size() - sub.size()));
                }
            } else {
                HashSet[] elems1 = new HashSet[this.elems().length + 1];
                Array$.MODULE$.copy(this.elems(), 0, elems1, 0, offset);
                elems1[offset] = that;
                Array$.MODULE$.copy(this.elems(), offset, elems1, offset + 1, this.elems().length - offset);
                int bitmap1 = this.bitmap() | mask;
                hashTrieSet = new HashTrieSet<A>(bitmap1, elems1, this.size() + that.size());
            }
            return hashTrieSet;
        }

        @Override
        public HashSet<A> union0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
            HashSet hashSet;
            if (that == this) {
                hashSet = this;
            } else if (that instanceof LeafHashSet) {
                LeafHashSet leafHashSet = (LeafHashSet)that;
                hashSet = this.union0(leafHashSet, level);
            } else if (that instanceof HashTrieSet) {
                HashTrieSet hashTrieSet;
                HashTrieSet hashTrieSet2 = (HashTrieSet)that;
                HashSet<A>[] a = this.elems();
                int abm = this.bitmap();
                int ai = 0;
                HashSet<A>[] b = hashTrieSet2.elems();
                int bbm = hashTrieSet2.bitmap();
                int bi = 0;
                int offset = offset0;
                int rs = 0;
                while ((abm | bbm) != 0) {
                    int alsb = abm ^ abm & abm - 1;
                    int blsb = bbm ^ bbm & bbm - 1;
                    if (alsb == blsb) {
                        HashSet<A> sub1 = a[ai].union0(b[bi], level + 5, buffer, offset);
                        rs += sub1.size();
                        buffer[offset] = sub1;
                        ++offset;
                        abm &= ~alsb;
                        ++ai;
                        bbm &= ~blsb;
                        ++bi;
                        continue;
                    }
                    int n = blsb - 1;
                    int n2 = alsb - 1;
                    HashSet$ hashSet$ = HashSet$.MODULE$;
                    if (n2 < n ^ n2 < 0 ^ n < 0) {
                        HashSet<A> sub1 = a[ai];
                        rs += sub1.size();
                        buffer[offset] = sub1;
                        ++offset;
                        abm &= ~alsb;
                        ++ai;
                        continue;
                    }
                    HashSet<A> sub1 = b[bi];
                    rs += sub1.size();
                    buffer[offset] = sub1;
                    ++offset;
                    bbm &= ~blsb;
                    ++bi;
                }
                if (rs == this.size()) {
                    hashTrieSet = this;
                } else if (rs == hashTrieSet2.size()) {
                    hashTrieSet = hashTrieSet2;
                } else {
                    int length = offset - offset0;
                    HashSet[] elems = new HashSet[length];
                    System.arraycopy(buffer, offset0, elems, 0, length);
                    hashTrieSet = new HashTrieSet<A>(this.bitmap() | hashTrieSet2.bitmap(), elems, rs);
                }
                hashSet = hashTrieSet;
            } else {
                hashSet = this;
            }
            return hashSet;
        }

        @Override
        public HashSet<A> intersect0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
            HashSet hashSet;
            if (that == this) {
                hashSet = this;
            } else if (that instanceof LeafHashSet) {
                LeafHashSet leafHashSet = (LeafHashSet)that;
                hashSet = leafHashSet.intersect0(this, level, buffer, offset0);
            } else if (that instanceof HashTrieSet) {
                HashSet hashSet2;
                HashTrieSet hashTrieSet = (HashTrieSet)that;
                HashSet<A>[] a = this.elems();
                int abm = this.bitmap();
                int ai = 0;
                HashSet<A>[] b = hashTrieSet.elems();
                int bbm = hashTrieSet.bitmap();
                int bi = 0;
                if ((abm & bbm) == 0) {
                    return null;
                }
                int offset = offset0;
                int rs = 0;
                int rbm = 0;
                while ((abm & bbm) != 0) {
                    int alsb = abm ^ abm & abm - 1;
                    int blsb = bbm ^ bbm & bbm - 1;
                    if (alsb == blsb) {
                        HashSet<A> sub1 = a[ai].intersect0(b[bi], level + 5, buffer, offset);
                        if (sub1 != null) {
                            rs += sub1.size();
                            rbm |= alsb;
                            buffer[offset] = sub1;
                            ++offset;
                        }
                        abm &= ~alsb;
                        ++ai;
                        bbm &= ~blsb;
                        ++bi;
                        continue;
                    }
                    int n = blsb - 1;
                    int n2 = alsb - 1;
                    HashSet$ hashSet$ = HashSet$.MODULE$;
                    if (n2 < n ^ n2 < 0 ^ n < 0) {
                        abm &= ~alsb;
                        ++ai;
                        continue;
                    }
                    bbm &= ~blsb;
                    ++bi;
                }
                if (rbm == 0) {
                    hashSet2 = null;
                } else if (rs == this.size0()) {
                    hashSet2 = this;
                } else if (rs == hashTrieSet.size0()) {
                    hashSet2 = hashTrieSet;
                } else {
                    int length = offset - offset0;
                    if (length == 1 && !(buffer[offset0] instanceof HashTrieSet)) {
                        hashSet2 = buffer[offset0];
                    } else {
                        HashSet[] elems = new HashSet[length];
                        System.arraycopy(buffer, offset0, elems, 0, length);
                        hashSet2 = new HashTrieSet<A>(rbm, elems, rs);
                    }
                }
                hashSet = hashSet2;
            } else {
                hashSet = null;
            }
            return hashSet;
        }

        @Override
        public HashSet<A> diff0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
            HashSet hashSet;
            if (that == this) {
                hashSet = null;
            } else if (that instanceof HashSet1) {
                HashSet1 hashSet1 = (HashSet1)that;
                hashSet = this.removed0(hashSet1.key(), hashSet1.hash(), level);
            } else if (that instanceof HashTrieSet) {
                HashSet hashSet2;
                HashTrieSet hashTrieSet = (HashTrieSet)that;
                HashSet<A>[] a = this.elems();
                int abm = this.bitmap();
                int ai = 0;
                HashSet<A>[] b = hashTrieSet.elems();
                int bbm = hashTrieSet.bitmap();
                int bi = 0;
                int offset = offset0;
                int rs = 0;
                int rbm = 0;
                while (abm != 0) {
                    int alsb = abm ^ abm & abm - 1;
                    int blsb = bbm ^ bbm & bbm - 1;
                    if (alsb == blsb) {
                        HashSet<A> sub1 = a[ai].diff0(b[bi], level + 5, buffer, offset);
                        if (sub1 != null) {
                            rs += sub1.size();
                            rbm |= alsb;
                            buffer[offset] = sub1;
                            ++offset;
                        }
                        abm &= ~alsb;
                        ++ai;
                        bbm &= ~blsb;
                        ++bi;
                        continue;
                    }
                    int n = blsb - 1;
                    int n2 = alsb - 1;
                    HashSet$ hashSet$ = HashSet$.MODULE$;
                    if (n2 < n ^ n2 < 0 ^ n < 0) {
                        HashSet<A> sub1 = a[ai];
                        rs += sub1.size();
                        rbm |= alsb;
                        buffer[offset] = sub1;
                        ++offset;
                        abm &= ~alsb;
                        ++ai;
                        continue;
                    }
                    bbm &= ~blsb;
                    ++bi;
                }
                if (rbm == 0) {
                    hashSet2 = null;
                } else if (rs == this.size0()) {
                    hashSet2 = this;
                } else {
                    int length = offset - offset0;
                    if (length == 1 && !(buffer[offset0] instanceof HashTrieSet)) {
                        hashSet2 = buffer[offset0];
                    } else {
                        HashSet[] elems = new HashSet[length];
                        System.arraycopy(buffer, offset0, elems, 0, length);
                        hashSet2 = new HashTrieSet<A>(rbm, elems, rs);
                    }
                }
                hashSet = hashSet2;
            } else if (that instanceof HashSetCollision1) {
                HashSetCollision1 hashSetCollision1 = (HashSetCollision1)that;
                hashSet = this.removeAll$1(this, hashSetCollision1.ks(), level, hashSetCollision1);
            } else {
                hashSet = this;
            }
            return hashSet;
        }

        @Override
        public HashSet<A> removed0(A key, int hash, int level) {
            HashSet hashSet;
            int index = hash >>> level & 0x1F;
            int mask = 1 << index;
            int offset = Integer.bitCount(this.bitmap() & mask - 1);
            if ((this.bitmap() & mask) != 0) {
                HashSet<A> subNew;
                HashSet<A> sub = this.elems()[offset];
                if (sub == (subNew = sub.removed0(key, hash, level + 5))) {
                    hashSet = this;
                } else if (subNew == null) {
                    int bitmapNew = this.bitmap() ^ mask;
                    if (bitmapNew != 0) {
                        HashSet[] elemsNew = new HashSet[this.elems().length - 1];
                        Array$.MODULE$.copy(this.elems(), 0, elemsNew, 0, offset);
                        Array$.MODULE$.copy(this.elems(), offset + 1, elemsNew, offset, this.elems().length - offset - 1);
                        int sizeNew = this.size() - sub.size();
                        hashSet = elemsNew.length == 1 && !(elemsNew[0] instanceof HashTrieSet) ? elemsNew[0] : new HashTrieSet<A>(bitmapNew, elemsNew, sizeNew);
                    } else {
                        hashSet = null;
                    }
                } else if (this.elems().length == 1 && !(subNew instanceof HashTrieSet)) {
                    hashSet = subNew;
                } else {
                    HashSet[] elemsNew = new HashSet[this.elems().length];
                    Array$.MODULE$.copy(this.elems(), 0, elemsNew, 0, this.elems().length);
                    elemsNew[offset] = subNew;
                    int sizeNew = this.size() + (subNew.size() - sub.size());
                    hashSet = new HashTrieSet<A>(this.bitmap(), elemsNew, sizeNew);
                }
            } else {
                hashSet = this;
            }
            return hashSet;
        }

        /*
         * Enabled force condition propagation
         * Lifted jumps to return sites
         */
        @Override
        public boolean subsetOf0(HashSet<A> that, int level) {
            if (that == this) {
                return true;
            }
            if (!(that instanceof HashTrieSet)) return false;
            HashTrieSet hashTrieSet = (HashTrieSet)that;
            if (this.size0() > hashTrieSet.size0()) return false;
            int abm = this.bitmap();
            HashSet<A>[] a = this.elems();
            int ai = 0;
            HashSet<A>[] b = hashTrieSet.elems();
            int bbm = hashTrieSet.bitmap();
            int bi = 0;
            if ((abm & bbm) != abm) return false;
            while (abm != 0) {
                int alsb = abm ^ abm & abm - 1;
                int blsb = bbm ^ bbm & bbm - 1;
                if (alsb == blsb) {
                    if (!a[ai].subsetOf0(b[bi], level + 5)) return false;
                    abm &= ~alsb;
                    ++ai;
                }
                bbm &= ~blsb;
                ++bi;
            }
            return true;
        }

        @Override
        public HashSet<A> filter0(Function1<A, Object> p, boolean negate, int level, HashSet<A>[] buffer, int offset0) {
            HashSet hashSet;
            int offset = offset0;
            int rs = 0;
            int kept = 0;
            for (int i = 0; i < this.elems().length; ++i) {
                HashSet<A> result2 = this.elems()[i].filter0(p, negate, level + 5, buffer, offset);
                if (result2 == null) continue;
                buffer[offset] = result2;
                ++offset;
                rs += result2.size();
                kept |= 1 << i;
            }
            if (offset == offset0) {
                hashSet = null;
            } else if (rs == this.size0()) {
                hashSet = this;
            } else if (offset == offset0 + 1 && !(buffer[offset0] instanceof HashTrieSet)) {
                hashSet = buffer[offset0];
            } else {
                int length = offset - offset0;
                HashSet[] elems1 = new HashSet[length];
                System.arraycopy(buffer, offset0, elems1, 0, length);
                int bitmap1 = length == this.elems().length ? this.bitmap() : HashSet$.MODULE$.scala$collection$immutable$HashSet$$keepBits(this.bitmap(), kept);
                hashSet = new HashTrieSet<A>(bitmap1, elems1, rs);
            }
            return hashSet;
        }

        @Override
        public TrieIterator<A> iterator() {
            return new TrieIterator<A>(this){

                public final A getElem(Object cc) {
                    return ((HashSet1)cc).key();
                }
            };
        }

        @Override
        public <U> void foreach(Function1<A, U> f) {
            for (int i = 0; i < this.elems().length; ++i) {
                this.elems()[i].foreach(f);
            }
        }

        private final HashSet removeAll$1(HashSet s2, ListSet r, int level$4, HashSetCollision1 x4$1) {
            while (!((ListSet)r).isEmpty() && s2 != null) {
                HashSet hashSet = s2.removed0(((ListSet)r).head(), x4$1.hash(), level$4);
                r = ((ListSet)r).tail();
                s2 = hashSet;
            }
            return s2;
        }

        public HashTrieSet(int bitmap, HashSet<A>[] elems, int size0) {
            this.bitmap = bitmap;
            this.elems = elems;
            this.size0 = size0;
            Predef$.MODULE$.assert(Integer.bitCount(bitmap) == elems.length);
        }
    }

    public static class HashSetCollision1<A>
    extends LeafHashSet<A> {
        private final int hash;
        private final ListSet<A> ks;

        @Override
        public int hash() {
            return this.hash;
        }

        public ListSet<A> ks() {
            return this.ks;
        }

        @Override
        public int size() {
            return this.ks().size();
        }

        @Override
        public boolean get0(A key, int hash, int level) {
            return hash == this.hash() ? this.ks().contains(key) : false;
        }

        @Override
        public boolean subsetOf0(HashSet<A> that, int level) {
            return this.ks().forall((Function1<A, Object>)((Object)new Serializable(this, that, level){
                public static final long serialVersionUID = 0L;
                private final /* synthetic */ HashSetCollision1 $outer;
                private final HashSet that$1;
                private final int level$1;

                public final boolean apply(A key) {
                    return this.that$1.get0(key, this.$outer.hash(), this.level$1);
                }
                {
                    if ($outer == null) {
                        throw null;
                    }
                    this.$outer = $outer;
                    this.that$1 = that$1;
                    this.level$1 = level$1;
                }
            }));
        }

        @Override
        public HashSet<A> updated0(A key, int hash, int level) {
            return hash == this.hash() ? new HashSetCollision1<A>(hash, this.ks().$plus((Object)key)) : HashSet$.MODULE$.scala$collection$immutable$HashSet$$makeHashTrieSet(this.hash(), this, hash, new HashSet1<A>(key, hash), level);
        }

        @Override
        public HashSet<A> union0(LeafHashSet<A> that, int level) {
            block5: {
                HashSet hashSet;
                block3: {
                    block4: {
                        block2: {
                            if (that.hash() == this.hash()) break block2;
                            hashSet = HashSet$.MODULE$.scala$collection$immutable$HashSet$$makeHashTrieSet(this.hash(), this, that.hash(), that, level);
                            break block3;
                        }
                        if (!(that instanceof HashSet1)) break block4;
                        HashSet1 hashSet1 = (HashSet1)that;
                        Object ks1 = this.ks().$plus(hashSet1.key());
                        hashSet = ((ListSet)ks1).size() == this.ks().size() ? this : new HashSetCollision1<A>(this.hash(), ks1);
                        break block3;
                    }
                    if (!(that instanceof HashSetCollision1)) break block5;
                    HashSetCollision1 hashSetCollision1 = (HashSetCollision1)that;
                    Set ks1 = this.ks().$plus$plus((GenTraversableOnce)hashSetCollision1.ks());
                    int n = ((ListSet)ks1).size();
                    hashSet = n == this.ks().size() ? this : (n == hashSetCollision1.ks().size() ? hashSetCollision1 : new HashSetCollision1<A>(this.hash(), ks1));
                }
                return hashSet;
            }
            throw new MatchError(that);
        }

        @Override
        public HashSet<A> union0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
            HashSet hashSet;
            if (that instanceof LeafHashSet) {
                LeafHashSet leafHashSet = (LeafHashSet)that;
                hashSet = this.union0(leafHashSet, level);
            } else if (that instanceof HashTrieSet) {
                HashTrieSet hashTrieSet = (HashTrieSet)that;
                hashSet = hashTrieSet.union0(this, level);
            } else {
                hashSet = this;
            }
            return hashSet;
        }

        @Override
        public HashSet<A> intersect0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
            ListSet ks1 = (ListSet)this.ks().filter((Function1)((Object)new Serializable(this, that, level){
                public static final long serialVersionUID = 0L;
                private final /* synthetic */ HashSetCollision1 $outer;
                private final HashSet that$2;
                private final int level$2;

                public final boolean apply(A x$1) {
                    return this.that$2.get0(x$1, this.$outer.hash(), this.level$2);
                }
                {
                    if ($outer == null) {
                        throw null;
                    }
                    this.$outer = $outer;
                    this.that$2 = that$2;
                    this.level$2 = level$2;
                }
            }));
            int n = ks1.size();
            HashSet hashSet = 0 == n ? null : (n == this.size() ? this : (n == that.size() ? that : (1 == n ? new HashSet1(ks1.head(), this.hash()) : new HashSetCollision1<A>(this.hash(), ks1))));
            return hashSet;
        }

        @Override
        public HashSet<A> diff0(HashSet<A> that, int level, HashSet<A>[] buffer, int offset0) {
            ListSet ks1 = (ListSet)this.ks().filterNot((Function1)((Object)new Serializable((HashSetCollision1)this, that, level){
                public static final long serialVersionUID = 0L;
                private final /* synthetic */ HashSetCollision1 $outer;
                private final HashSet that$3;
                private final int level$3;

                public final boolean apply(A x$2) {
                    return this.that$3.get0(x$2, this.$outer.hash(), this.level$3);
                }
                {
                    if ($outer == null) {
                        throw null;
                    }
                    this.$outer = $outer;
                    this.that$3 = that$3;
                    this.level$3 = level$3;
                }
            }));
            int n = ks1.size();
            LeafHashSet leafHashSet = 0 == n ? null : (n == this.size() ? this : (1 == n ? new HashSet1(ks1.head(), this.hash()) : new HashSetCollision1<A>(this.hash(), ks1)));
            return leafHashSet;
        }

        @Override
        public HashSet<A> removed0(A key, int hash, int level) {
            LeafHashSet leafHashSet;
            if (hash == this.hash()) {
                Object ks1 = this.ks().$minus((Object)key);
                int n = ((ListSet)ks1).size();
                switch (n) {
                    default: {
                        if (n == this.ks().size()) {
                            leafHashSet = this;
                            break;
                        }
                        leafHashSet = new HashSetCollision1<A>(hash, ks1);
                        break;
                    }
                    case 1: {
                        leafHashSet = new HashSet1(((ListSet)ks1).head(), hash);
                        break;
                    }
                    case 0: {
                        leafHashSet = null;
                        break;
                    }
                }
            } else {
                leafHashSet = this;
            }
            return leafHashSet;
        }

        @Override
        public HashSet<A> filter0(Function1<A, Object> p, boolean negate, int level, HashSet<A>[] buffer, int offset0) {
            LeafHashSet leafHashSet;
            ListSet ks1 = negate ? (ListSet)this.ks().filterNot((Function1)p) : (ListSet)this.ks().filter((Function1)p);
            int n = ks1.size();
            switch (n) {
                default: {
                    if (n == this.ks().size()) {
                        leafHashSet = this;
                        break;
                    }
                    leafHashSet = new HashSetCollision1<A>(this.hash(), ks1);
                    break;
                }
                case 1: {
                    leafHashSet = new HashSet1(ks1.head(), this.hash());
                    break;
                }
                case 0: {
                    leafHashSet = null;
                }
            }
            return leafHashSet;
        }

        @Override
        public Iterator<A> iterator() {
            return this.ks().iterator();
        }

        @Override
        public <U> void foreach(Function1<A, U> f) {
            this.ks().foreach(f);
        }

        private void writeObject(ObjectOutputStream out) {
            throw package$.MODULE$.error("cannot serialize an immutable.HashSet where all items have the same 32-bit hash code");
        }

        private void readObject(ObjectInputStream in) {
            throw package$.MODULE$.error("cannot deserialize an immutable.HashSet where all items have the same 32-bit hash code");
        }

        public HashSetCollision1(int hash, ListSet<A> ks) {
            this.hash = hash;
            this.ks = ks;
        }
    }

    public static class SerializationProxy<A, B>
    implements Serializable {
        public static final long serialVersionUID = 2L;
        private transient HashSet<A> scala$collection$immutable$HashSet$SerializationProxy$$orig;

        public HashSet<A> scala$collection$immutable$HashSet$SerializationProxy$$orig() {
            return this.scala$collection$immutable$HashSet$SerializationProxy$$orig;
        }

        public void scala$collection$immutable$HashSet$SerializationProxy$$orig_$eq(HashSet<A> x$1) {
            this.scala$collection$immutable$HashSet$SerializationProxy$$orig = x$1;
        }

        private void writeObject(ObjectOutputStream out) {
            int s2 = this.scala$collection$immutable$HashSet$SerializationProxy$$orig().size();
            out.writeInt(s2);
            this.scala$collection$immutable$HashSet$SerializationProxy$$orig().foreach(new Serializable(this, out){
                public static final long serialVersionUID = 0L;
                private final ObjectOutputStream out$1;

                public final void apply(A e) {
                    this.out$1.writeObject(e);
                }
                {
                    this.out$1 = out$1;
                }
            });
        }

        /*
         * Enabled force condition propagation
         * Lifted jumps to return sites
         */
        private void readObject(ObjectInputStream in) {
            HashSet$ hashSet$ = HashSet$.MODULE$;
            this.scala$collection$immutable$HashSet$SerializationProxy$$orig_$eq(HashSet$EmptyHashSet$.MODULE$);
            int s2 = in.readInt();
            Predef$ predef$ = Predef$.MODULE$;
            Range$ range$ = Range$.MODULE$;
            Range range2 = new Range(0, s2, 1);
            if (range2.isEmpty()) return;
            int i1 = range2.start();
            while (true) {
                Object e1 = in.readObject();
                this.scala$collection$immutable$HashSet$SerializationProxy$$orig_$eq((HashSet<A>)this.scala$collection$immutable$HashSet$SerializationProxy$$orig().$plus(e1));
                if (i1 == range2.lastElement()) {
                    return;
                }
                i1 += range2.step();
            }
        }

        private Object readResolve() {
            return this.scala$collection$immutable$HashSet$SerializationProxy$$orig();
        }

        public SerializationProxy(HashSet<A> orig) {
            this.scala$collection$immutable$HashSet$SerializationProxy$$orig = orig;
        }
    }
}

