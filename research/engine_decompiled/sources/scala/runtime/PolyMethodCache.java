...
public final class PolyMethodCache
extends MethodCache {
    ...
    private Method findInternal(Class<?> forReceiver) {
        ...
        block2: {
            while (true) {
                ...
                if (forReceiver == this_.receiver) {
                    method = this_.method;
                    break block2;
                }
                MethodCache methodCache = this_.next;
                if (!(methodCache instanceof PolyMethodCache)) break;
                PolyMethodCache this_ = polyMethodCache = (PolyMethodCache)methodCache;
            }
            method = this_.next.find(forReceiver);
        }
        return method;
    }
    ...
    public MethodCache add(Class<?> forReceiver, Method forMethod) {
        return this.complexity < 160 ? new PolyMethodCache(this, forReceiver, forMethod, this.complexity + 1) : new MegaMethodCache(forMethod.getName(), forMethod.getParameterTypes());
    }

    public PolyMethodCache(MethodCache next2, Class<?> receiver, Method method, int complexity) {
        this.next = next2;
