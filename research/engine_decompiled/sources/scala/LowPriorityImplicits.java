...
public abstract class LowPriorityImplicits {
    ...
    public <T> WrappedArray<T> genericWrapArray(Object xs) {
        return xs == null ? null : WrappedArray$.MODULE$.make(xs);
    }

    public <T> WrappedArray<T> wrapRefArray(T[] xs) {
        return xs == null ? null : (xs.length == 0 ? WrappedArray$.MODULE$.empty() : new WrappedArray.ofRef<T>(xs));
    }

    public WrappedArray<Object> wrapIntArray(int[] xs) {
        return xs != null ? new WrappedArray.ofInt(xs) : null;
    }

    public WrappedArray<Object> wrapDoubleArray(double[] xs) {
        return xs != null ? new WrappedArray.ofDouble(xs) : null;
    }

    public WrappedArray<Object> wrapLongArray(long[] xs) {
        return xs != null ? new WrappedArray.ofLong(xs) : null;
    }

    public WrappedArray<Object> wrapFloatArray(float[] xs) {
        return xs != null ? new WrappedArray.ofFloat(xs) : null;
    }

    public WrappedArray<Object> wrapCharArray(char[] xs) {
        return xs != null ? new WrappedArray.ofChar(xs) : null;
    }

    public WrappedArray<Object> wrapByteArray(byte[] xs) {
        return xs != null ? new WrappedArray.ofByte(xs) : null;
    }

    public WrappedArray<Object> wrapShortArray(short[] xs) {
        return xs != null ? new WrappedArray.ofShort(xs) : null;
    }

    public WrappedArray<Object> wrapBooleanArray(boolean[] xs) {
        return xs != null ? new WrappedArray.ofBoolean(xs) : null;
    }

    public WrappedArray<BoxedUnit> wrapUnitArray(BoxedUnit[] xs) {
        return xs != null ? new WrappedArray.ofUnit(xs) : null;
    }

    public WrappedString wrapString(String s2) {
        return s2 != null ? new WrappedString(s2) : null;
    }

    public String unwrapString(WrappedString ws) {
        return ws != null ? ws.self() : null;
    }

    public <T> CanBuildFrom<String, T, IndexedSeq<T>> fallbackStringCanBuildFrom() {
        return new CanBuildFrom<String, T, IndexedSeq<T>>(this){

            public Builder<T, IndexedSeq<T>> apply(String from2) {
