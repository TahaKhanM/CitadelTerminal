...
extends Executor {
    ...
    public boolean batchable(Runnable var1);
    ...
    BlockContext {
        ...
        public void scala$concurrent$BatchingExecutor$Batch$$parentBlockContext_$eq(BlockContext x$1) {
            this.scala$concurrent$BatchingExecutor$Batch$$parentBlockContext = x$1;
        }
        ...
        public void run() {
            ...
            BlockContext$.MODULE$.withBlockContext(this, new Serializable(this, prevBlockContext){
                ...
                public void apply$mcV$sp() {
                    try {
                        ...
                        this.processBatch$1(this.$outer.initial());
                        return;
                    }
                    finally {
                ...
                private final void processBatch$1(List batch) {
                    while (true) {
                        ...
                        if (!(batch instanceof $colon$colon)) {
                            throw new MatchError(batch);
                        }
                        $colon$colon $colon$colon = ($colon$colon)batch;
                        this.$outer.scala$concurrent$BatchingExecutor$Batch$$$outer().scala$concurrent$BatchingExecutor$$_tasksLocal().set($colon$colon.tl$1());
                        try {
                            ((Runnable)$colon$colon.head()).run();
                            batch = this.$outer.scala$concurrent$BatchingExecutor$Batch$$$outer().scala$concurrent$BatchingExecutor$$_tasksLocal().get();
                        }
                        catch (Throwable throwable) {
                            List<Runnable> remaining = this.$outer.scala$concurrent$BatchingExecutor$Batch$$$outer().scala$concurrent$BatchingExecutor$$_tasksLocal().get();
                            this.$outer.scala$concurrent$BatchingExecutor$Batch$$$outer().scala$concurrent$BatchingExecutor$$_tasksLocal().set(Nil$.MODULE$);
                            this.$outer.scala$concurrent$BatchingExecutor$Batch$$$outer().unbatchedExecute(new Batch(this.$outer.scala$concurrent$BatchingExecutor$Batch$$$outer(), remaining));
                            throw throwable;
                        }
                    }
                }
                {
                    ...
                    this.$outer = $outer;
                    this.prevBlockContext$1 = prevBlockContext$1;
                }
            });
        }
        ...
        public <T> T blockOn(Function0<T> thunk, CanAwait permission) {
            ...
            Predef$.MODULE$.require(this.scala$concurrent$BatchingExecutor$Batch$$parentBlockContext() != null);
            return this.scala$concurrent$BatchingExecutor$Batch$$parentBlockContext().blockOn(thunk, permission);
        }
