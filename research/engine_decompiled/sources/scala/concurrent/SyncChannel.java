...
public class SyncChannel<A> {
    ...
    public void write(A data) {
        ...
        SyncChannel syncChannel = this;
        synchronized (syncChannel) {
            BoxedUnit boxedUnit;
            if (this.pendingReads().isEmpty()) {
                List<Tuple2<A, SyncVar<Object>>> list2 = this.pendingWrites();
                this.pendingWrites_$eq(((List)List$.MODULE$.apply(Predef$.MODULE$.wrapRefArray((Object[])new Tuple2[]{new Tuple2(data, writeReq)}))).$colon$colon$colon(list2));
    ...
    public A read() {
        ...
        SyncChannel syncChannel = this;
        synchronized (syncChannel) {
            Tuple2<A, SyncVar<Object>> tuple2;
            block6: {
                BoxedUnit boxedUnit;
                if (this.pendingWrites().isEmpty()) {
                    List<SyncVar<A>> list2 = this.pendingReads();
                    this.pendingReads_$eq(((List)List$.MODULE$.apply(Predef$.MODULE$.wrapRefArray((Object[])new SyncVar[]{readReq}))).$colon$colon$colon(list2));
                    boxedUnit = BoxedUnit.UNIT;
                } else {
                    tuple2 = this.pendingWrites().head();
                    if (tuple2 == null) break block6;
                    Tuple2<A, SyncVar<Object>> tuple22 = new Tuple2<A, SyncVar<Object>>(tuple2._1(), tuple2._2());
                    A data = tuple22._1();
                    SyncVar<Object> writeReq = tuple22._2();
