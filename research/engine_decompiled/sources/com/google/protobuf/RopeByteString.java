...
extends ByteString {
    ...
    static ByteString concatenate(ByteString left, ByteString right) {
        ...
        if (newLength >= minLengthByDepth[newDepth = Math.max(left.getTreeDepth(), right.getTreeDepth()) + 1]) {
    ...
    private static ByteString concatenateBytes(ByteString left, ByteString right) {
        ...
        byte[] bytes2 = new byte[leftSize + rightSize];
        left.copyTo(bytes2, 0, 0, leftSize);
        right.copyTo(bytes2, 0, leftSize, rightSize);
    ...
    public List<ByteBuffer> asReadOnlyByteBufferList() {
        ...
        while (pieces.hasNext()) {
            ...
            result2.add(byteString.asReadOnlyByteBuffer());
        }
        return result2;
    }
    ...
    extends InputStream {
        ...
        private int readSkipInternal(byte[] b, int offset, int length) {
            int bytesRemaining;
            int count2;
            for (bytesRemaining = length; bytesRemaining > 0; bytesRemaining -= count2) {
                ...
                count2 = Math.min(currentPieceRemaining, bytesRemaining);
                if (b != null) {
                    this.currentPiece.copyTo(b, this.currentPieceIndex, offset, count2);
                    offset += count2;
                }
                this.currentPieceIndex += count2;
            }
    ...
    implements Iterator<ByteString.LeafByteString> {
        ...
        private ByteString.LeafByteString getNextNonEmptyLeaf() {
            ByteString.LeafByteString result2;
            do {
        ...
        public ByteString.LeafByteString next() {
            ...
            ByteString.LeafByteString result2 = this.next;
