...
extends ByteString.LeafByteString {
    ...
    public ByteString substring(int beginIndex, int endIndex) {
        try {
            ByteBuffer slice2 = this.slice(beginIndex, endIndex);
            return new NioByteString(slice2);
        }
        catch (ArrayIndexOutOfBoundsException e) {
            throw e;
        }
    ...
    protected void copyToInternal(byte[] target, int sourceOffset, int targetOffset, int numberToCopy) {
        ...
        slice2.position(sourceOffset);
        slice2.get(target, targetOffset, numberToCopy);
    }
    ...
    protected String toStringInternal(Charset charset) {
        ...
        byte[] bytes2;
        if (this.buffer.hasArray()) {
            bytes2 = this.buffer.array();
            offset = this.buffer.arrayOffset() + this.buffer.position();
            length = this.buffer.remaining();
        } else {
            bytes2 = this.toByteArray();
            offset = 0;
            length = bytes2.length;
        }
        return new String(bytes2, offset, length, charset);
    }
    ...
    private ByteBuffer slice(int beginIndex, int endIndex) {
        ...
        ByteBuffer slice2 = this.buffer.slice();
        slice2.position(beginIndex - this.buffer.position());
        slice2.limit(endIndex - this.buffer.position());
        return slice2;
    }
}
