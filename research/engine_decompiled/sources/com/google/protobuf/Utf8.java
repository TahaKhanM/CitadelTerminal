...
final class Utf8 {
    ...
    public static boolean isValidUtf8(byte[] bytes2, int index, int limit) {
        return processor.isValidUtf8(bytes2, index, limit);
    }

    public static int partialIsValidUtf8(int state, byte[] bytes2, int index, int limit) {
        return processor.partialIsValidUtf8(state, bytes2, index, limit);
    }
    ...
    private static int incompleteStateFor(byte[] bytes2, int index, int limit) {
        byte byte1 = bytes2[index - 1];
        switch (limit - index) {
            case 0: {
                return Utf8.incompleteStateFor(byte1);
            }
            case 1: {
                return Utf8.incompleteStateFor(byte1, bytes2[index]);
            }
            case 2: {
                return Utf8.incompleteStateFor(byte1, (int)bytes2[index], (int)bytes2[index + 1]);
            }
        }
    ...
    static String decodeUtf8(ByteBuffer buffer, int index, int size2) throws InvalidProtocolBufferException {
        return processor.decodeUtf8(buffer, index, size2);
    }

    static String decodeUtf8(byte[] bytes2, int index, int size2) throws InvalidProtocolBufferException {
        return processor.decodeUtf8(bytes2, index, size2);
    }
    ...
    extends Processor {
        ...
        @Override
        int partialIsValidUtf8(int state, byte[] bytes2, int index, int limit) {
            if ((index | limit | bytes2.length - limit) < 0) {
                throw new ArrayIndexOutOfBoundsException(String.format("Array length=%d, index=%d, limit=%d", bytes2.length, index, limit));
            }
            long offset = index;
            ...
            if (state != 0) {
                ...
                if (byte1 < -32) {
                    if (byte1 < -62 || UnsafeUtil.getByte(bytes2, offset++) > -65) {
                        return -1;
                    }
                } else if (byte1 < -16) {
                    byte byte2 = (byte)(~(state >> 8));
                    if (byte2 == 0) {
                        byte2 = UnsafeUtil.getByte(bytes2, offset++);
                        if (offset >= offsetLimit) {
                            return Utf8.incompleteStateFor(byte1, byte2);
                        }
                    }
                    if (byte2 > -65 || byte1 == -32 && byte2 < -96 || byte1 == -19 && byte2 >= -96 || UnsafeUtil.getByte(bytes2, offset++) > -65) {
                        return -1;
                    }
                } else {
                    byte byte2 = (byte)(~(state >> 8));
                    ...
                    if (byte3 == 0) {
                        byte3 = UnsafeUtil.getByte(bytes2, offset++);
                        if (offset >= offsetLimit) {
                            return Utf8.incompleteStateFor(byte1, byte2, byte3);
                        }
                    }
                    if (byte2 > -65 || (byte1 << 28) + (byte2 - -112) >> 30 != 0 || byte3 > -65 || UnsafeUtil.getByte(bytes2, offset++) > -65) {
                        return -1;
                    }
                }
            }
            return UnsafeProcessor.partialIsValidUtf8(bytes2, offset, (int)(offsetLimit - offset));
        }
        ...
        @Override
        String decodeUtf8(byte[] bytes2, int index, int size2) throws InvalidProtocolBufferException {
            byte b;
            int offset;
            if ((index | size2 | bytes2.length - index - size2) < 0) {
                throw new ArrayIndexOutOfBoundsException(String.format("buffer length=%d, index=%d, size=%d", bytes2.length, index, size2));
            }
            int limit = offset + size2;
            char[] resultArr = new char[size2];
            int resultPos = 0;
            for (offset = index; offset < limit && DecodeUtil.isOneByte(b = UnsafeUtil.getByte(bytes2, (long)offset)); ++offset) {
                DecodeUtil.handleOneByte(b, resultArr, resultPos++);
            }
            while (offset < limit) {
                byte byte1;
                if (DecodeUtil.isOneByte(byte1 = UnsafeUtil.getByte(bytes2, (long)offset++))) {
                    byte b2;
                    DecodeUtil.handleOneByte(byte1, resultArr, resultPos++);
                    while (offset < limit && DecodeUtil.isOneByte(b2 = UnsafeUtil.getByte(bytes2, (long)offset))) {
                        ++offset;
                ...
                if (DecodeUtil.isTwoBytes(byte1)) {
                    ...
                    DecodeUtil.handleTwoBytes(byte1, UnsafeUtil.getByte(bytes2, (long)offset++), resultArr, resultPos++);
                    continue;
                }
                if (DecodeUtil.isThreeBytes(byte1)) {
                    ...
                    DecodeUtil.handleThreeBytes(byte1, UnsafeUtil.getByte(bytes2, (long)offset++), UnsafeUtil.getByte(bytes2, (long)offset++), resultArr, resultPos++);
                    continue;
                }
                if (offset >= limit - 2) {
                    throw InvalidProtocolBufferException.invalidUtf8();
                }
                DecodeUtil.handleFourBytes(byte1, UnsafeUtil.getByte(bytes2, (long)offset++), UnsafeUtil.getByte(bytes2, (long)offset++), UnsafeUtil.getByte(bytes2, (long)offset++), resultArr, resultPos++);
                ++resultPos;
            }
        ...
        @Override
        String decodeUtf8Direct(ByteBuffer buffer, int index, int size2) throws InvalidProtocolBufferException {
            byte b;
            long address;
            if ((index | size2 | buffer.limit() - index - size2) < 0) {
                throw new ArrayIndexOutOfBoundsException(String.format("buffer limit=%d, index=%d, limit=%d", buffer.limit(), index, size2));
            }
        ...
        private static int unsafeEstimateConsecutiveAscii(byte[] bytes2, long offset, int maxChars) {
            if (maxChars < 16) {
                return 0;
            }
            for (int i = 0; i < maxChars; ++i) {
                if (UnsafeUtil.getByte(bytes2, offset++) >= 0) continue;
                return i;
            }
            return maxChars;
        }
        ...
        private static int partialIsValidUtf8(byte[] bytes2, long offset, int remaining) {
            int skipped = UnsafeProcessor.unsafeEstimateConsecutiveAscii(bytes2, offset, remaining);
            remaining -= skipped;
            offset += (long)skipped;
            while (true) {
                byte byte2;
                int byte1 = 0;
                while (remaining > 0) {
                    byte by2 = UnsafeUtil.getByte(bytes2, offset++);
                    byte1 = by2;
                    if (by2 < 0) break;
                    --remaining;
                }
                ...
                if (byte1 < -32) {
                    ...
                    --remaining;
                    if (byte1 >= -62 && UnsafeUtil.getByte(bytes2, offset++) <= -65) continue;
                    return -1;
                }
                if (byte1 < -16) {
                    if (remaining < 2) {
                        return UnsafeProcessor.unsafeIncompleteStateFor(bytes2, byte1, offset, remaining);
                    }
                    remaining -= 2;
                    if (!((byte2 = UnsafeUtil.getByte(bytes2, offset++)) > -65 || byte1 == -32 && byte2 < -96 || byte1 == -19 && byte2 >= -96) && UnsafeUtil.getByte(bytes2, offset++) <= -65) continue;
                    return -1;
                }
                if (remaining < 3) {
                    return UnsafeProcessor.unsafeIncompleteStateFor(bytes2, byte1, offset, remaining);
                ...
                remaining -= 3;
                if ((byte2 = UnsafeUtil.getByte(bytes2, offset++)) > -65 || (byte1 << 28) + (byte2 - -112) >> 30 != 0 || UnsafeUtil.getByte(bytes2, offset++) > -65 || UnsafeUtil.getByte(bytes2, offset++) > -65) break;
            }
            return -1;
        }
        ...
        private static int partialIsValidUtf8(long address, int remaining) {
            ...
            while (true) {
                ...
                int byte1 = 0;
                while (remaining > 0) {
                    byte by2 = UnsafeUtil.getByte(address++);
                    byte1 = by2;
                    if (by2 < 0) break;
                    --remaining;
                }
        ...
        private static int unsafeIncompleteStateFor(byte[] bytes2, int byte1, long offset, int remaining) {
            switch (remaining) {
                case 0: {
                    return Utf8.incompleteStateFor(byte1);
                }
                case 1: {
                    return Utf8.incompleteStateFor(byte1, UnsafeUtil.getByte(bytes2, offset));
                }
                case 2: {
                    return Utf8.incompleteStateFor(byte1, UnsafeUtil.getByte(bytes2, offset), UnsafeUtil.getByte(bytes2, offset + 1L));
                }
            }
            throw new AssertionError();
        }
    ...
    extends Processor {
        ...
        int partialIsValidUtf8(int state, byte[] bytes2, int index, int limit) {
            if (state != 0) {
                ...
                if (byte1 < -32) {
                    ...
                    if (byte2 > -65 || byte1 == -32 && byte2 < -96 || byte1 == -19 && byte2 >= -96 || bytes2[index++] > -65) {
                        return -1;
                    }
                } else {
                    byte byte2 = (byte)(~(state >> 8));
                    ...
                    if (byte2 > -65 || (byte1 << 28) + (byte2 - -112) >> 30 != 0 || byte3 > -65 || bytes2[index++] > -65) {
                        return -1;
                    }
                }
            }
            return SafeProcessor.partialIsValidUtf8(bytes2, index, limit);
        }
        ...
        String decodeUtf8(byte[] bytes2, int index, int size2) throws InvalidProtocolBufferException {
            ...
            char[] resultArr = new char[size2];
            int resultPos = 0;
            for (offset = index; offset < limit && DecodeUtil.isOneByte(b = bytes2[offset]); ++offset) {
                DecodeUtil.handleOneByte(b, resultArr, resultPos++);
            }
            while (offset < limit) {
                byte byte1;
                if (DecodeUtil.isOneByte(byte1 = bytes2[offset++])) {
                    byte b2;
                    DecodeUtil.handleOneByte(byte1, resultArr, resultPos++);
                    while (offset < limit && DecodeUtil.isOneByte(b2 = bytes2[offset])) {
                        ++offset;
                        DecodeUtil.handleOneByte(b2, resultArr, resultPos++);
                    }
                    continue;
                ...
                if (DecodeUtil.isTwoBytes(byte1)) {
                    ...
                    DecodeUtil.handleTwoBytes(byte1, bytes2[offset++], resultArr, resultPos++);
                    continue;
                }
                if (DecodeUtil.isThreeBytes(byte1)) {
                    if (offset >= limit - 1) {
                    ...
                    DecodeUtil.handleThreeBytes(byte1, bytes2[offset++], bytes2[offset++], resultArr, resultPos++);
                    continue;
                }
                if (offset >= limit - 2) {
                    throw InvalidProtocolBufferException.invalidUtf8();
                }
                DecodeUtil.handleFourBytes(byte1, bytes2[offset++], bytes2[offset++], bytes2[offset++], resultArr, resultPos++);
                ++resultPos;
            }
        ...
        @Override
        String decodeUtf8Direct(ByteBuffer buffer, int index, int size2) throws InvalidProtocolBufferException {
            return this.decodeUtf8Default(buffer, index, size2);
        }
        ...
        private static int partialIsValidUtf8(byte[] bytes2, int index, int limit) {
            while (index < limit && bytes2[index] >= 0) {
                ++index;
            }
            return index >= limit ? 0 : SafeProcessor.partialIsValidUtf8NonAscii(bytes2, index, limit);
        }

        private static int partialIsValidUtf8NonAscii(byte[] bytes2, int index, int limit) {
            while (true) {
                byte byte2;
                byte byte1;
                if (index >= limit) {
                ...
                if (byte1 < -16) {
                    if (index >= limit - 1) {
                        return Utf8.incompleteStateFor(bytes2, index, limit);
                    }
                    if (!((byte2 = bytes2[index++]) > -65 || byte1 == -32 && byte2 < -96 || byte1 == -19 && byte2 >= -96) && bytes2[index++] <= -65) continue;
                    return -1;
                }
                if (index >= limit - 2) {
                    return Utf8.incompleteStateFor(bytes2, index, limit);
    ...
    static abstract class Processor {
        ...
        final boolean isValidUtf8(byte[] bytes2, int index, int limit) {
            return this.partialIsValidUtf8(0, bytes2, index, limit) == 0;
        }
        ...
        final String decodeUtf8(ByteBuffer buffer, int index, int size2) throws InvalidProtocolBufferException {
            if (buffer.hasArray()) {
                int offset = buffer.arrayOffset();
                return this.decodeUtf8(buffer.array(), offset + index, size2);
            }
            if (buffer.isDirect()) {
                return this.decodeUtf8Direct(buffer, index, size2);
            }
            return this.decodeUtf8Default(buffer, index, size2);
        }
        ...
        final String decodeUtf8Default(ByteBuffer buffer, int index, int size2) throws InvalidProtocolBufferException {
            byte b;
            int offset;
            if ((index | size2 | buffer.limit() - index - size2) < 0) {
                throw new ArrayIndexOutOfBoundsException(String.format("buffer limit=%d, index=%d, limit=%d", buffer.limit(), index, size2));
