...
implements Predicate<Character> {
    ...
    public String trimFrom(CharSequence sequence2) {
        ...
        return sequence2.subSequence(first, last2 + 1).toString();
    }
    ...
    public String trimTrailingFrom(CharSequence sequence2) {
        int len = sequence2.length();
        for (int last2 = len - 1; last2 >= 0; --last2) {
            if (this.matches(sequence2.charAt(last2))) continue;
            return sequence2.subSequence(0, last2 + 1).toString();
        }
        return "";
    }
    ...
    public String collapseFrom(CharSequence sequence2, char replacement) {
        ...
        for (int i = 0; i < len; ++i) {
            ...
            if (!(c != replacement || i != len - 1 && this.matches(sequence2.charAt(i + 1)))) {
                ++i;
                continue;
            }
            StringBuilder builder = new StringBuilder(len).append(sequence2, 0, i).append(replacement);
            return this.finishCollapseFrom(sequence2, i + 1, len, replacement, builder, true);
        }
    ...
    public String trimAndCollapseFrom(CharSequence sequence2, char replacement) {
        ...
        while (last2 > first && this.matches(sequence2.charAt(last2))) {
            --last2;
        }
        return first == 0 && last2 == len - 1 ? this.collapseFrom(sequence2, replacement) : this.finishCollapseFrom(sequence2, first, last2 + 1, replacement, new StringBuilder(last2 + 1 - first), false);
    }

    private String finishCollapseFrom(CharSequence sequence2, int start, int end, char replacement, StringBuilder builder, boolean inMatchingGroup) {
