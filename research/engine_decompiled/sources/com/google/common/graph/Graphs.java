...
public final class Graphs {
    ...
    public static <N> MutableGraph<N> copyOf(Graph<N> graph) {
        MutableGraph copy2 = GraphBuilder.from(graph).expectedNodeCount(graph.nodes().size()).build();
        for (N n : graph.nodes()) {
            copy2.addNode(n);
        }
    ...
    public static <N, V> MutableValueGraph<N, V> copyOf(ValueGraph<N, V> graph) {
        MutableValueGraph copy2 = ValueGraphBuilder.from(graph).expectedNodeCount(graph.nodes().size()).build();
        for (Object n : graph.nodes()) {
            copy2.addNode(n);
        }
        for (EndpointPair endpointPair : graph.edges()) {
            copy2.putEdgeValue(endpointPair.nodeU(), endpointPair.nodeV(), graph.edgeValue(endpointPair.nodeU(), endpointPair.nodeV()));
        }
        return copy2;
    }
