...
public final class InetAddresses {
    ...
    private static byte[] textToNumericFormatV4(String ipString) {
        ...
        try {
            for (String octet : IPV4_SPLITTER.split(ipString)) {
                bytes2[i++] = InetAddresses.parseOctet(octet);
            }
        }
        catch (NumberFormatException ex) {
            return null;
        }
        return (byte[])(i == 4 ? bytes2 : null);
    }
    ...
    public static String toAddrString(InetAddress ip) {
        ...
        byte[] bytes2 = ip.getAddress();
        int[] hextets = new int[8];
        for (int i = 0; i < hextets.length; ++i) {
            hextets[i] = Ints.fromBytes((byte)0, (byte)0, bytes2[2 * i], bytes2[2 * i + 1]);
        }
    ...
    public static TeredoInfo getTeredoInfo(Inet6Address ip) {
        ...
        int flags = ByteStreams.newDataInput(bytes2, 8).readShort() & 0xFFFF;
    ...
    public static InetAddress fromLittleEndianByteArray(byte[] addr) throws UnknownHostException {
        byte[] reversed2 = new byte[addr.length];
        for (int i = 0; i < addr.length; ++i) {
            reversed2[i] = addr[addr.length - i - 1];
        }
        return InetAddress.getByAddress(reversed2);
    }
    ...
    private static IllegalArgumentException formatIllegalArgumentException(String format2, Object ... args) {
        return new IllegalArgumentException(String.format(Locale.ROOT, format2, args));
    }
