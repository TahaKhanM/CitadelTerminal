...
public final class Quantiles {
    ...
    private static void selectInPlace(int required, double[] array, int from2, int to2) {
        if (required == from2) {
            int min2 = from2;
            for (int index = from2 + 1; index <= to2; ++index) {
                if (!(array[min2] > array[index])) continue;
                min2 = index;
            }
            if (min2 != from2) {
                Quantiles.swap(array, min2, from2);
            }
            return;
        }
        while (to2 > from2) {
            int partitionPoint = Quantiles.partition(array, from2, to2);
            if (partitionPoint >= required) {
                to2 = partitionPoint - 1;
            }
            if (partitionPoint > required) continue;
            from2 = partitionPoint + 1;
        }
    }

    private static int partition(double[] array, int from2, int to2) {
        Quantiles.movePivotToStartOfSlice(array, from2, to2);
        double pivot = array[from2];
        int partitionPoint = to2;
        for (int i = to2; i > from2; --i) {
            if (!(array[i] > pivot)) continue;
            Quantiles.swap(array, partitionPoint, i);
            --partitionPoint;
        }
        Quantiles.swap(array, from2, partitionPoint);
        return partitionPoint;
    }

    private static void movePivotToStartOfSlice(double[] array, int from2, int to2) {
        boolean toLessThanFrom;
        int mid = from2 + to2 >>> 1;
        boolean toLessThanMid = array[to2] < array[mid];
        boolean midLessThanFrom = array[mid] < array[from2];
        boolean bl = toLessThanFrom = array[to2] < array[from2];
        if (toLessThanMid == midLessThanFrom) {
            Quantiles.swap(array, mid, from2);
        } else if (toLessThanMid != toLessThanFrom) {
            Quantiles.swap(array, from2, to2);
        }
    }

    private static void selectAllInPlace(int[] allRequired, int requiredFrom, int requiredTo, double[] array, int from2, int to2) {
        int requiredAbove;
        int requiredBelow;
        int requiredChosen = Quantiles.chooseNextSelection(allRequired, requiredFrom, requiredTo, from2, to2);
        int required = allRequired[requiredChosen];
        Quantiles.selectInPlace(required, array, from2, to2);
        ...
        if (requiredBelow >= requiredFrom) {
            Quantiles.selectAllInPlace(allRequired, requiredFrom, requiredBelow, array, from2, required - 1);
        }
    ...
    private static int chooseNextSelection(int[] allRequired, int requiredFrom, int requiredTo, int from2, int to2) {
        if (requiredFrom == requiredTo) {
            return requiredFrom;
        }
        int centerFloor = from2 + to2 >>> 1;
        ...
        if (from2 + to2 - allRequired[low] - allRequired[high] > 0) {
            return high;
        }
        return low;
    }
