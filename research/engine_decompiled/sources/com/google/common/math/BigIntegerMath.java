...
public final class BigIntegerMath {
    ...
    public static BigInteger factorial(int n) {
        ...
        long product2 = LongMath.factorials[startingNumber - 1];
        int shift = Long.numberOfTrailingZeros(product2);
        int productBits = LongMath.log2(product2 >>= shift, RoundingMode.FLOOR) + 1;
        int bits2 = LongMath.log2(startingNumber, RoundingMode.FLOOR) + 1;
        int nextPowerOfTwo = 1 << bits2 - 1;
        for (long num = (long)startingNumber; num <= (long)n; ++num) {
            ...
            shift += tz;
            int normalizedBits = bits2 - tz;
            if (normalizedBits + productBits >= 64) {
                bignums.add(BigInteger.valueOf(product2));
                product2 = 1L;
                productBits = 0;
            }
            productBits = LongMath.log2(product2 *= normalizedNum, RoundingMode.FLOOR) + 1;
        }
        if (product2 > 1L) {
            bignums.add(BigInteger.valueOf(product2));
        }
    ...
    public static BigInteger binomial(int n, int k) {
        ...
        int numeratorBits = bits2 = LongMath.log2(n, RoundingMode.CEILING);
        for (int i = 1; i < k; ++i) {
            int p = n - i;
            int q = i + 1;
            if (numeratorBits + bits2 >= 63) {
