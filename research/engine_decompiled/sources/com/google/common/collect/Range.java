...
Serializable {
    ...
    public static <C extends Comparable<?>> Range<C> encloseAll(Iterable<C> values) {
        ...
        Comparable max2 = min2 = (Comparable)Preconditions.checkNotNull(valueIterator.next());
        while (valueIterator.hasNext()) {
            Comparable value = (Comparable)Preconditions.checkNotNull(valueIterator.next());
            min2 = Ordering.natural().min(min2, value);
            max2 = Ordering.natural().max(max2, value);
        }
        return Range.closed(min2, max2);
    }
    ...
    @Override
    @Deprecated
    public boolean apply(C input2) {
        return this.contains(input2);
    }
