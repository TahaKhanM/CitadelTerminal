...
public abstract class TreeTraverser<T> {
    ...
    implements PeekingIterator<T> {
        ...
        @Override
        public T next() {
            Object result2 = this.queue.remove();
            Iterables.addAll(this.queue, TreeTraverser.this.children(result2));
            return result2;
        }
    }
    ...
    extends UnmodifiableIterator<T> {
        ...
        public T next() {
            ...
            Object result2 = Preconditions.checkNotNull(itr.next());
            if (!itr.hasNext()) {
                this.stack.removeLast();
            }
            if ((childItr = TreeTraverser.this.children(result2).iterator()).hasNext()) {
