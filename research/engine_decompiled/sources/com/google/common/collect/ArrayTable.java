...
implements Serializable {
    ...
    private ArrayTable(ArrayTable<R, C, V> table) {
        ...
        Object[][] copy2 = new Object[this.rowList.size()][this.columnList.size()];
        this.array = copy2;
        this.eraseAll();
        for (int i = 0; i < this.rowList.size(); ++i) {
            System.arraycopy(table.array[i], 0, copy2[i], 0, table.array[i].length);
        }
    }
    ...
    public V[][] toArray(Class<V> valueClass) {
        Object[][] copy2 = (Object[][])Array.newInstance(valueClass, this.rowList.size(), this.columnList.size());
        for (int i = 0; i < this.rowList.size(); ++i) {
            System.arraycopy(this.array[i], 0, copy2[i], 0, this.array[i].length);
        }
        return copy2;
    }
    ...
    public Map<C, Map<R, V>> columnMap() {
        ColumnMap map2 = this.columnMap;
        return map2 == null ? (this.columnMap = new ColumnMap()) : map2;
    }
    ...
    public Map<R, Map<C, V>> rowMap() {
        RowMap map2 = this.rowMap;
        return map2 == null ? (this.rowMap = new RowMap()) : map2;
    }
    ...
    extends Maps.IteratorBasedAbstractMap<K, V> {
        ...
        K getKey(int index) {
            return (K)((ImmutableSet)this.keyIndex.keySet()).asList().get(index);
        }
