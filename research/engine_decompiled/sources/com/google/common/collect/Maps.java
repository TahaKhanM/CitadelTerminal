...
public final class Maps {
    ...
    @Beta
    public static <K extends Enum<K>, V> ImmutableMap<K, V> immutableEnumMap(Map<K, ? extends V> map2) {
        if (map2 instanceof ImmutableEnumMap) {
            ImmutableEnumMap result2 = (ImmutableEnumMap)map2;
            return result2;
        }
        if (map2.isEmpty()) {
    ...
    private static <K, V> Map<K, V> unmodifiableMap(Map<K, ? extends V> map2) {
    ...
    private static <K, V> Map<K, V> filterFiltered(AbstractFilteredMap<K, V> map2, Predicate<? super Map.Entry<K, V>> entryPredicate) {
        return new FilteredEntryMap(map2.unfiltered, Predicates.and(map2.predicate, entryPredicate));
    }
    ...
    private static <K, V> NavigableMap<K, V> filterFiltered(FilteredEntryNavigableMap<K, V> map2, Predicate<? super Map.Entry<K, V>> entryPredicate) {
        Predicate<? super Map.Entry<K, V>> predicate = Predicates.and(((FilteredEntryNavigableMap)map2).entryPredicate, entryPredicate);
        return new FilteredEntryNavigableMap(((FilteredEntryNavigableMap)map2).unfiltered, predicate);
    }
    ...
    @GwtIncompatible
    public static <K, V> NavigableMap<K, V> unmodifiableNavigableMap(NavigableMap<K, ? extends V> map2) {
        Preconditions.checkNotNull(map2);
        if (map2 instanceof UnmodifiableNavigableMap) {
            NavigableMap<K, ? extends V> result2 = map2;
            return result2;
        }
    ...
    static <V> V safeGet(Map<?, V> map2, @Nullable Object key) {
        Preconditions.checkNotNull(map2);
        try {
            return map2.get(key);
        }
    ...
    static <E> ImmutableMap<E, Integer> indexMap(Collection<E> list2) {
        ImmutableMap.Builder<E, Integer> builder = new ImmutableMap.Builder<E, Integer>(list2.size());
        int i = 0;
        for (E e : list2) {
            builder.put(e, i++);
        }
    ...
    @Beta
    @GwtIncompatible
    public static <K extends Comparable<? super K>, V> NavigableMap<K, V> subMap(NavigableMap<K, V> map2, Range<K> range2) {
        if (map2.comparator() != null && map2.comparator() != Ordering.natural() && range2.hasLowerBound() && range2.hasUpperBound()) {
            Preconditions.checkArgument(map2.comparator().compare(range2.lowerEndpoint(), range2.upperEndpoint()) <= 0, "map is using a custom comparator which is inconsistent with the natural ordering.");
        }
        if (range2.hasLowerBound() && range2.hasUpperBound()) {
            return map2.subMap(range2.lowerEndpoint(), range2.lowerBoundType() == BoundType.CLOSED, range2.upperEndpoint(), range2.upperBoundType() == BoundType.CLOSED);
        }
        if (range2.hasLowerBound()) {
            return map2.tailMap(range2.lowerEndpoint(), range2.lowerBoundType() == BoundType.CLOSED);
        }
        if (range2.hasUpperBound()) {
            return map2.headMap(range2.upperEndpoint(), range2.upperBoundType() == BoundType.CLOSED);
        }
        return Preconditions.checkNotNull(map2);
    }
    ...
    implements SortedSet<K> {
        SortedKeySet(SortedMap<K, V> map2) {
            super(map2);
        }
    ...
    Serializable {
        ...
        public NavigableMap<K, V> descendingMap() {
            ...
            return result2 == null ? (this.descendingMap = new UnmodifiableNavigableMap<K, V>(this.delegate.descendingMap(), this)) : result2;
        }
    ...
    extends AbstractNavigableMap<K, V> {
        ...
        @Override
        public NavigableMap<K, V> headMap(K toKey, boolean inclusive2) {
            return Maps.filterEntries(this.unfiltered.headMap(toKey, inclusive2), this.entryPredicate);
        }

        @Override
        public NavigableMap<K, V> tailMap(K fromKey, boolean inclusive2) {
            return Maps.filterEntries(this.unfiltered.tailMap(fromKey, inclusive2), this.entryPredicate);
        }
    }
    ...
    implements Serializable {
        ...
        private static <X, Y> Y convert(BiMap<X, Y> bimap, X input2) {
            Object output = bimap.get(input2);
            Preconditions.checkArgument(output != null, "No non-null mapping present for input: %s", input2);
            return (Y)output;
        }
