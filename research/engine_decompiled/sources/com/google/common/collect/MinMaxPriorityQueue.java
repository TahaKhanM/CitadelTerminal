...
extends AbstractQueue<E> {
    ...
    static int initialQueueSize(int configuredExpectedSize, int maximumSize, Iterable<?> initialContents) {
        int result2;
        int n = result2 = configuredExpectedSize == -1 ? 11 : configuredExpectedSize;
        if (initialContents instanceof Collection) {
            int initialSize = ((Collection)initialContents).size();
            result2 = Math.max(result2, initialSize);
        }
        return MinMaxPriorityQueue.capAtMaximumSize(result2, maximumSize);
    }
