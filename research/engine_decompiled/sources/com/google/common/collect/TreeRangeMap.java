...
implements RangeMap<K, V> {
    ...
    private static final RangeMap EMPTY_SUB_RANGE_MAP = new RangeMap(){
        ...
        public void put(Range range2, Object value) {
            Preconditions.checkNotNull(range2);
            throw new IllegalArgumentException("Cannot insert range " + range2 + " into an empty subRangeMap");
        }
        ...
        public void remove(Range range2) {
            Preconditions.checkNotNull(range2);
        }
        ...
        public RangeMap subRangeMap(Range range2) {
            Preconditions.checkNotNull(range2);
            return this;
        }
    };
    ...
    public void put(Range<K> range2, V value) {
        if (!range2.isEmpty()) {
            ...
            this.remove(range2);
            this.entriesByLowerBound.put(range2.lowerBound, new RangeMapEntry<K, V>(range2, value));
        }
    }
    ...
    implements RangeMap<K, V> {
        ...
        @Override
        public void put(Range<K> range2, V value) {
            Preconditions.checkArgument(this.subRange.encloses(range2), "Cannot put range %s into a subRangeMap(%s)", range2, this.subRange);
            TreeRangeMap.this.put(range2, value);
        }
        ...
        public void putAll(RangeMap<K, V> rangeMap) {
            ...
            Range span2 = rangeMap.span();
            Preconditions.checkArgument(this.subRange.encloses(span2), "Cannot putAll rangeMap with span %s into a subRangeMap(%s)", span2, this.subRange);
        ...
        public void remove(Range<K> range2) {
            if (range2.isConnected(this.subRange)) {
                TreeRangeMap.this.remove(range2.intersection(this.subRange));
            }
        }

        @Override
        public RangeMap<K, V> subRangeMap(Range<K> range2) {
            if (!range2.isConnected(this.subRange)) {
                return TreeRangeMap.this.emptySubRangeMap();
            }
            return TreeRangeMap.this.subRangeMap(range2.intersection(this.subRange));
        }
        ...
        extends AbstractMap<Range<K>, V> {
            ...
            public V remove(Object key) {
                ...
                if (value != null) {
                    Range range2 = (Range)key;
                    TreeRangeMap.this.remove(range2);
                    return value;
                }
                return null;
            }
    ...
    extends Maps.IteratorBasedAbstractMap<Range<K>, V> {
        ...
        public V get(@Nullable Object key) {
            if (key instanceof Range) {
                Range range2 = (Range)key;
                RangeMapEntry rangeMapEntry = (RangeMapEntry)TreeRangeMap.this.entriesByLowerBound.get(range2.lowerBound);
                if (rangeMapEntry != null && ((Range)rangeMapEntry.getKey()).equals(range2)) {
    ...
    extends AbstractMapEntry<Range<K>, V> {
        ...
        RangeMapEntry(Range<K> range2, V value) {
            this.range = range2;
            this.value = value;
        }
