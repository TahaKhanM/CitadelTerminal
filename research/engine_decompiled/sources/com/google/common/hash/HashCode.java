...
public abstract class HashCode {
    ...
    public static HashCode fromBytes(byte[] bytes2) {
        Preconditions.checkArgument(bytes2.length >= 1, "A HashCode must contain at least 1 byte.");
        return HashCode.fromBytesNoCopy((byte[])bytes2.clone());
    }

    static HashCode fromBytesNoCopy(byte[] bytes2) {
        return new BytesHashCode(bytes2);
    }

    public static HashCode fromString(String string2) {
        Preconditions.checkArgument(string2.length() >= 2, "input string (%s) must have at least 2 characters", (Object)string2);
        Preconditions.checkArgument(string2.length() % 2 == 0, "input string (%s) must have an even number of characters", (Object)string2);
        byte[] bytes2 = new byte[string2.length() / 2];
        for (int i = 0; i < string2.length(); i += 2) {
            int ch1 = HashCode.decode(string2.charAt(i)) << 4;
            int ch2 = HashCode.decode(string2.charAt(i + 1));
            bytes2[i / 2] = (byte)(ch1 + ch2);
        }
    ...
    public final int hashCode() {
        ...
        byte[] bytes2 = this.getBytesInternal();
        int val = bytes2[0] & 0xFF;
        for (int i = 1; i < bytes2.length; ++i) {
            val |= (bytes2[i] & 0xFF) << i * 8;
        }
        return val;
    }
    ...
    public final String toString() {
        byte[] bytes2 = this.getBytesInternal();
        StringBuilder sb = new StringBuilder(2 * bytes2.length);
        for (byte b : bytes2) {
    ...
    implements Serializable {
        ...
        BytesHashCode(byte[] bytes2) {
            this.bytes = Preconditions.checkNotNull(bytes2);
        }
