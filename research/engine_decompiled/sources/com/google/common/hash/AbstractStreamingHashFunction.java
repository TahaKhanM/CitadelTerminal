...
implements HashFunction {
    ...
    public HashCode hashUnencodedChars(CharSequence input2) {
        return this.newHasher().putUnencodedChars(input2).hash();
    }
    ...
    public HashCode hashInt(int input2) {
        return this.newHasher().putInt(input2).hash();
    }

    @Override
    public HashCode hashLong(long input2) {
        return this.newHasher().putLong(input2).hash();
    }

    @Override
    public HashCode hashBytes(byte[] input2) {
        return this.newHasher().putBytes(input2).hash();
    }

    @Override
    public HashCode hashBytes(byte[] input2, int off, int len) {
        return this.newHasher().putBytes(input2, off, len).hash();
    }
    ...
    extends AbstractHasher {
        ...
        @Override
        public final Hasher putBytes(byte[] bytes2) {
            return this.putBytes(bytes2, 0, bytes2.length);
        }

        @Override
        public final Hasher putBytes(byte[] bytes2, int off, int len) {
            return this.putBytes(ByteBuffer.wrap(bytes2, off, len).order(ByteOrder.LITTLE_ENDIAN));
        }
