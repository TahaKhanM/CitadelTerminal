...
public final class Hashing {
    ...
    public static HashFunction goodFastHash(int minimumBits) {
        int bits2 = Hashing.checkPositiveAndMakeMultipleOf32(minimumBits);
        if (bits2 == 32) {
        ...
        int hashFunctionsNeeded = (bits2 + 127) / 128;
    ...
    public static int consistentHash(long input2, int buckets) {
        int next2;
        Preconditions.checkArgument(buckets > 0, "buckets must be positive: %s", buckets);
        LinearCongruentialGenerator generator = new LinearCongruentialGenerator(input2);
        int candidate = 0;
        while ((next2 = (int)((double)(candidate + 1) / generator.nextDouble())) >= 0 && next2 < buckets) {
            candidate = next2;
        }
        return candidate;
    }
    ...
    public static HashCode combineOrdered(Iterable<HashCode> hashCodes) {
        ...
        int bits2 = iterator2.next().bits();
        byte[] resultBytes = new byte[bits2 / 8];
    ...
    static int checkPositiveAndMakeMultipleOf32(int bits2) {
        Preconditions.checkArgument(bits2 > 0, "Number of bits must be positive");
        return bits2 + 31 & 0xFFFFFFE0;
    }

    public static HashFunction concatenating(HashFunction first, HashFunction second, HashFunction ... rest) {
        ArrayList<HashFunction> list2 = new ArrayList<HashFunction>();
        list2.add(first);
        list2.add(second);
        for (HashFunction hashFunc : rest) {
            list2.add(hashFunc);
        }
        return new ConcatenatedHashFunction(list2.toArray(new HashFunction[0]));
    }
    ...
    public static HashFunction concatenating(Iterable<HashFunction> hashFunctions) {
        ...
        for (HashFunction hashFunction : hashFunctions) {
            list2.add(hashFunction);
        }
        Preconditions.checkArgument(list2.size() > 0, "number of hash functions (%s) must be > 0", list2.size());
        return new ConcatenatedHashFunction(list2.toArray(new HashFunction[0]));
    }
    ...
    extends AbstractCompositeHashFunction {
        ...
        HashCode makeHash(Hasher[] hashers) {
            byte[] bytes2 = new byte[this.bits / 8];
            int i = 0;
            for (Hasher hasher : hashers) {
                HashCode newHash = hasher.hash();
                i += newHash.writeBytesTo(bytes2, i, newHash.bits() / 8);
            }
            return HashCode.fromBytesNoCopy(bytes2);
        }
