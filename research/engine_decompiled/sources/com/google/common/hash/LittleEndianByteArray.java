...
final class LittleEndianByteArray {
    ...
    static long load64(byte[] input2, int offset) {
        assert (input2.length >= offset + 8);
        return byteArray.getLongLittleEndian(input2, offset);
    }

    static long load64Safely(byte[] input2, int offset, int length) {
        long result2 = 0L;
        int limit = Math.min(length, 8);
        for (int i = 0; i < limit; ++i) {
            result2 |= ((long)input2[offset + i] & 0xFFL) << i * 8;
        }
        return result2;
    }
