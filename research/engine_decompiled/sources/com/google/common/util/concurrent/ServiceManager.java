...
public final class ServiceManager {
    ...
    public ServiceManager(Iterable<? extends Service> services) {
        ImmutableList<Service> copy2 = ImmutableList.copyOf(services);
        if (copy2.isEmpty()) {
            ...
            copy2 = ImmutableList.of(new NoOpService());
        }
        this.state = new ServiceManagerState(copy2);
        this.services = copy2;
    ...
    extends Service.Listener {
        ...
        public void stopping(Service.State from2) {
            ServiceManagerState state = (ServiceManagerState)this.state.get();
            if (state != null) {
                state.transitionService(this.service, from2, Service.State.STOPPING);
            }
        }
        ...
        public void terminated(Service.State from2) {
            ...
            if (state != null) {
                if (!(this.service instanceof NoOpService)) {
                    logger.log(Level.FINE, "Service {0} has terminated. Previous state was: {1}", new Object[]{this.service, from2});
                }
                state.transitionService(this.service, from2, Service.State.TERMINATED);
            }
        }
        ...
        public void failed(Service.State from2, Throwable failure) {
            ...
            if (state != null) {
                ...
                if (log) {
                    logger.log(Level.SEVERE, "Service " + this.service + " has failed in the " + (Object)((Object)from2) + " state.", failure);
                }
                state.transitionService(this.service, from2, Service.State.FAILED);
            }
        }
    }
    ...
    private static final class ServiceManagerState {
         ...
         * WARNING - Removed try catching itself - possible behaviour change.
         */
        void transitionService(Service service, Service.State from2, Service.State to2) {
            Preconditions.checkNotNull(service);
            Preconditions.checkArgument(from2 != to2);
            this.monitor.enter();
            try {
                ...
                Preconditions.checkState(this.servicesByState.remove((Object)from2, service), "Service %s not at the expected location in the state map %s", (Object)service, (Object)from2);
                Preconditions.checkState(this.servicesByState.put(to2, service), "Service %s in the state map unexpectedly at %s", (Object)service, (Object)to2);
                Stopwatch stopwatch = this.startupTimers.get(service);
                if (stopwatch == null) {
