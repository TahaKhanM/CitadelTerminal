...
implements ListenableFuture<V> {
    ...
    private static void complete(AbstractFuture<?> future) {
        ...
        block0: while (true) {
            ...
            future.afterDone();
            next2 = super.clearListeners(next2);
            future = null;
            while (next2 != null) {
                Listener curr = next2;
                next2 = next2.next;
    ...
    private void releaseWaiters() {
        Waiter head2;
        while (!ATOMIC_HELPER.casWaiters(this, head2 = this.waiters, Waiter.TOMBSTONE)) {
        }
        Waiter currentWaiter = head2;
    ...
    private Listener clearListeners(Listener onto) {
        Listener head2;
        while (!ATOMIC_HELPER.casListeners(this, head2 = this.listeners, Listener.TOMBSTONE)) {
        }
    ...
    extends AtomicHelper {
         ...
         */
        @Override
        boolean casWaiters(AbstractFuture<?> future, Waiter expect, Waiter update2) {
            AbstractFuture<?> abstractFuture = future;
            synchronized (abstractFuture) {
                if (((AbstractFuture)future).waiters == expect) {
                    ((AbstractFuture)future).waiters = update2;
                    return true;
                }
                return false;
            }
         ...
         */
        @Override
        boolean casListeners(AbstractFuture<?> future, Listener expect, Listener update2) {
            AbstractFuture<?> abstractFuture = future;
            synchronized (abstractFuture) {
                if (((AbstractFuture)future).listeners == expect) {
                    ((AbstractFuture)future).listeners = update2;
                    return true;
                }
                return false;
            }
         ...
         */
        @Override
        boolean casValue(AbstractFuture<?> future, Object expect, Object update2) {
            AbstractFuture<?> abstractFuture = future;
            synchronized (abstractFuture) {
                if (((AbstractFuture)future).value == expect) {
                    ((AbstractFuture)future).value = update2;
                    return true;
                }
                return false;
            }
    ...
    extends AtomicHelper {
        ...
        @Override
        boolean casWaiters(AbstractFuture<?> future, Waiter expect, Waiter update2) {
            return this.waitersUpdater.compareAndSet(future, expect, update2);
        }

        @Override
        boolean casListeners(AbstractFuture<?> future, Listener expect, Listener update2) {
            return this.listenersUpdater.compareAndSet(future, expect, update2);
        }

        @Override
        boolean casValue(AbstractFuture<?> future, Object expect, Object update2) {
            return this.valueUpdater.compareAndSet(future, expect, update2);
        }
    }
    ...
    extends AtomicHelper {
        ...
        @Override
        boolean casWaiters(AbstractFuture<?> future, Waiter expect, Waiter update2) {
            return UNSAFE.compareAndSwapObject(future, WAITERS_OFFSET, expect, update2);
        }

        @Override
        boolean casListeners(AbstractFuture<?> future, Listener expect, Listener update2) {
            return UNSAFE.compareAndSwapObject(future, LISTENERS_OFFSET, expect, update2);
        }

        @Override
        boolean casValue(AbstractFuture<?> future, Object expect, Object update2) {
            return UNSAFE.compareAndSwapObject(future, VALUE_OFFSET, expect, update2);
        }
    ...
    implements Runnable {
        ...
        SetFuture(AbstractFuture<V> owner2, ListenableFuture<? extends V> future) {
            this.owner = owner2;
            this.future = future;
        }
    ...
    private static final class Waiter {
        ...
        void setNext(Waiter next2) {
            ATOMIC_HELPER.putNext(this, next2);
        }
