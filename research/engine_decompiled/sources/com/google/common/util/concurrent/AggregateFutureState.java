...
abstract class AggregateFutureState {
    ...
    extends AtomicHelper {
         ...
         */
        @Override
        void compareAndSetSeenExceptions(AggregateFutureState state, Set<Throwable> expect, Set<Throwable> update2) {
            AggregateFutureState aggregateFutureState = state;
            synchronized (aggregateFutureState) {
                if (state.seenExceptions == expect) {
                    state.seenExceptions = update2;
                }
            }
        }
    ...
    extends AtomicHelper {
        ...
        @Override
        void compareAndSetSeenExceptions(AggregateFutureState state, Set<Throwable> expect, Set<Throwable> update2) {
            this.seenExceptionsUpdater.compareAndSet(state, expect, update2);
        }
