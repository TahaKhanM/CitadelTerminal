...
implements Serializable {
    ...
    public final Class<? super T> getRawType() {
        Class rawType;
        Class result2 = rawType = (Class)this.getRawTypes().iterator().next();
        return result2;
    }
    ...
    private boolean isSubtypeOfParameterizedType(ParameterizedType supertype) {
        ...
        TypeVariable<Class<?>>[] typeParams2 = matchedClass.getTypeParameters();
        Type[] toTypeArgs = supertype.getActualTypeArguments();
        for (int i = 0; i < typeParams2.length; ++i) {
            if (super.is(toTypeArgs[i])) continue;
            return false;
        }
    ...
    private ImmutableSet<Class<? super T>> getRawTypes() {
        ...
        ImmutableCollection result2 = builder.build();
        return result2;
    }
    ...
    static <T> TypeToken<? extends T> toGenericType(Class<T> cls) {
        ...
        Type[] typeParams2 = cls.getTypeParameters();
        ...
        if (typeParams2.length > 0 || ownerType != null && ownerType != cls.getEnclosingClass()) {
            TypeToken<?> type2 = TypeToken.of(Types.newParameterizedTypeWithOwner(ownerType, cls, typeParams2));
            return type2;
        }
    ...
    private static abstract class TypeCollector<K> {
        ...
        ImmutableList<K> collectTypes(Iterable<? extends K> types) {
            ...
            for (K type : types) {
                this.collectTypes(type, map2);
            }
            return TypeCollector.sortKeysByValue(map2, Ordering.natural().reverse());
        }

        @CanIgnoreReturnValue
        private int collectTypes(K type, Map<? super K, Integer> map2) {
            Integer existing = map2.get(type);
            if (existing != null) {
                return existing;
            }
