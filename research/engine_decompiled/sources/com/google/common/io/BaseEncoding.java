...
public abstract class BaseEncoding {
    ...
    public String encode(byte[] bytes2) {
        return this.encode(bytes2, 0, bytes2.length);
    }

    public final String encode(byte[] bytes2, int off, int len) {
        Preconditions.checkPositionIndexes(off, off + len, bytes2.length);
        StringBuilder result2 = new StringBuilder(this.maxEncodedSize(len));
        try {
            this.encodeTo(result2, bytes2, off, len);
        }
    ...
    extends BaseEncoding {
        ...
        @Override
        int maxEncodedSize(int bytes2) {
            int unseparatedSize = this.delegate.maxEncodedSize(bytes2);
        ...
        @Override
        void encodeTo(Appendable target, byte[] bytes2, int off, int len) throws IOException {
            this.delegate.encodeTo(SeparatedBaseEncoding.separatingAppendable(target, this.separator, this.afterEveryChars), bytes2, off, len);
        }
    ...
    extends StandardBaseEncoding {
        ...
        void encodeTo(Appendable target, byte[] bytes2, int off, int len) throws IOException {
            ...
            for (int remaining = len; remaining >= 3; remaining -= 3) {
                int chunk = (bytes2[i++] & 0xFF) << 16 | (bytes2[i++] & 0xFF) << 8 | bytes2[i++] & 0xFF;
            ...
            if (i < off + len) {
                this.encodeChunkTo(target, bytes2, i, off + len - i);
            }
        }
    ...
    extends BaseEncoding {
        ...
        @Override
        int maxEncodedSize(int bytes2) {
            return this.alphabet.charsPerChunk * IntMath.divide(bytes2, this.alphabet.bytesPerChunk, RoundingMode.CEILING);
        }
        ...
        void encodeTo(Appendable target, byte[] bytes2, int off, int len) throws IOException {
            ...
            for (int i = 0; i < len; i += this.alphabet.bytesPerChunk) {
                this.encodeChunkTo(target, bytes2, off + i, Math.min(this.alphabet.bytesPerChunk, len - i));
            }
        }

        void encodeChunkTo(Appendable target, byte[] bytes2, int off, int len) throws IOException {
            int bitsProcessed;
            Preconditions.checkNotNull(target);
            Preconditions.checkPositionIndexes(off, off + len, bytes2.length);
            ...
            long bitBuffer = 0L;
            for (int i = 0; i < len; ++i) {
                bitBuffer |= (long)(bytes2[off + i] & 0xFF);
                bitBuffer <<= 8;
            }
    ...
    extends CharMatcher {
        ...
        char encode(int bits2) {
            return this.chars[bits2];
        }
