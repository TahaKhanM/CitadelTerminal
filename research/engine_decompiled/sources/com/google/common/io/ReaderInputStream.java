...
extends InputStream {
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        ...
        block0: while (true) {
            ...
            while (true) {
                if ((result2 = this.doneFlushing ? CoderResult.UNDERFLOW : (doneEncoding ? this.encoder.flush(this.byteBuffer) : this.encoder.encode(this.charBuffer, this.byteBuffer, this.endOfInput))).isOverflow()) {
                ...
                if (result2.isError()) break block0;
            }
            break;
        }
    ...
    private static CharBuffer grow(CharBuffer buf) {
        char[] copy2 = Arrays.copyOf(buf.array(), buf.capacity() * 2);
        CharBuffer bigger = CharBuffer.wrap(copy2);
