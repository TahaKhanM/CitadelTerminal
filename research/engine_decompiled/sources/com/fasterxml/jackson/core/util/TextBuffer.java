...
public final class TextBuffer {
    ...
    public void append(char[] c, int start, int len) {
        ...
        this._resultArray = null;
        char[] curr = this._currentSegment;
        int max2 = curr.length - this._currentSize;
        if (max2 >= len) {
        ...
        if (max2 > 0) {
            System.arraycopy(c, start, curr, this._currentSize, max2);
            start += max2;
            len -= max2;
        }
        do {
    ...
    public void append(String str, int offset, int len) {
        ...
        if (max2 > 0) {
            str.getChars(offset, offset + max2, curr, this._currentSize);
            len -= max2;
            offset += max2;
        }
        do {
    ...
    private char[] buildResultArray() {
        ...
        if (this._inputStart >= 0) {
            ...
            result2 = this._charArray(this._inputLen);
            System.arraycopy(this._inputBuffer, this._inputStart, result2, 0, this._inputLen);
        } else {
            int size2 = this.size();
            if (size2 < 1) {
                return NO_CHARS;
