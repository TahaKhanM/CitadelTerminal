...
extends OutputStream {
    ...
    public byte[] toByteArray() {
        ...
        byte[] result2 = new byte[totalLen];
        int offset = 0;
        for (byte[] block : this._pastBlocks) {
            int len = block.length;
            System.arraycopy(block, 0, result2, offset, len);
            offset += len;
        }
        System.arraycopy(this._currBlock, 0, result2, offset, this._currBlockPtr);
    ...
    public void write(byte[] b, int off, int len) {
        while (true) {
            int max2;
            int toCopy;
            if ((toCopy = Math.min(max2 = this._currBlock.length - this._currBlockPtr, len)) > 0) {
