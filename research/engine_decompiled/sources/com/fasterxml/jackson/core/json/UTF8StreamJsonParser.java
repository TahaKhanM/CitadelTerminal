...
extends ParserBase {
    ...
    public int releaseBuffered(OutputStream out) throws IOException {
        ...
        out.write(this._inputBuffer, origPtr, count2);
        return count2;
    }
    ...
    protected boolean loadMore() throws IOException {
        ...
        if (this._inputStream != null) {
            int count2 = this._inputStream.read(this._inputBuffer, 0, this._inputBuffer.length);
            if (count2 > 0) {
                this._inputPtr = 0;
    ...
    protected boolean _loadToHaveAtLeast(int minAvailable) throws IOException {
        ...
        while (this._inputEnd < minAvailable) {
            int count2 = this._inputStream.read(this._inputBuffer, this._inputEnd, this._inputBuffer.length - this._inputEnd);
            if (count2 < 1) {
                this._closeInput();
                if (count2 == 0) {
    ...
    protected int _readBinary(Base64Variant b64variant, OutputStream out, byte[] buffer) throws IOException, JsonParseException {
        ...
        while (true) {
            ...
            if ((bits2 = b64variant.decodeBase64Char(ch = this._inputBuffer[this._inputPtr++] & 0xFF)) < 0) {
    ...
    protected Name _parseFieldName(int i) throws IOException, JsonParseException {
        ...
        byte[] input2 = this._inputBuffer;
        if (codes[q = input2[this._inputPtr++] & 0xFF] == 0) {
            if (codes[i = input2[this._inputPtr++] & 0xFF] == 0) {
                q = q << 8 | i;
                if (codes[i = input2[this._inputPtr++] & 0xFF] == 0) {
                    q = q << 8 | i;
                    if (codes[i = input2[this._inputPtr++] & 0xFF] == 0) {
    ...
    protected void _finishString() throws IOException, JsonParseException {
        ...
        int max2 = Math.min(this._inputEnd, ptr + outBuf.length);
    ...
    private void _finishString2(char[] outBuf, int outPtr) throws IOException, JsonParseException {
        ...
        while (true) {
            ...
            block15: {
                ...
                int max2 = Math.min(this._inputEnd, ptr + (outBuf.length - outPtr));
                while (ptr < max2) {
    ...
    protected void _skipString() throws IOException, JsonParseException {
        ...
        block6: while (true) {
            ...
            block10: {
                int max2;
                if ((ptr = this._inputPtr) >= (max2 = this._inputEnd)) {
                    this.loadMoreGuaranteed();
                    ptr = this._inputPtr;
                    max2 = this._inputEnd;
                }
    ...
    protected JsonToken _handleApostropheValue() throws IOException, JsonParseException {
        ...
        block6: while (true) {
            ...
            if ((max22 = this._inputPtr + (outBuf.length - outPtr)) < (max2 = this._inputEnd)) {
                max2 = max22;
            }
            while (true) {
                if (this._inputPtr >= max2) continue block6;
