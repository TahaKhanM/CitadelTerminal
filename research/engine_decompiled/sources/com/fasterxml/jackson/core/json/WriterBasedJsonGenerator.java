...
extends JsonGeneratorImpl {
    ...
    private void _writeLongString(String text) throws IOException, JsonGenerationException {
        ...
        do {
            int max2;
            segmentLen = offset + (max2 = this._outputEnd) > textLen ? textLen - offset : max2;
    ...
    private void _prependOrWriteCharacterEscape(char ch, int escCode) throws IOException, JsonGenerationException {
        ...
        if (this._currentEscape == null) {
            escape2 = this._characterEscapes.getEscapeSequence(ch).getValue();
        } else {
            escape2 = this._currentEscape.getValue();
            this._currentEscape = null;
        }
        int len = escape2.length();
        if (this._outputTail >= len) {
            int ptr;
            this._outputHead = ptr = this._outputTail - len;
            escape2.getChars(0, len, this._outputBuffer, ptr);
            return;
        }
        this._outputHead = this._outputTail;
        this._writer.write(escape2);
    }
    ...
    private int _prependOrWriteCharacterEscape(char[] buffer, int ptr, int end, char ch, int escCode) throws IOException, JsonGenerationException {
        ...
        int len = escape2.length();
        if (ptr >= len && ptr < end) {
            escape2.getChars(0, len, buffer, ptr -= len);
        } else {
            this._writer.write(escape2);
        }
        return ptr;
    ...
    private void _appendCharacterEscape(char ch, int escCode) throws IOException, JsonGenerationException {
        ...
        escape2.getChars(0, len, this._outputBuffer, this._outputTail);
        this._outputTail += len;
    }
