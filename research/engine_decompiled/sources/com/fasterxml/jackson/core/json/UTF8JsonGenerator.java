...
extends JsonGeneratorImpl {
    ...
    public void writeRaw(SerializableString text) throws IOException, JsonGenerationException {
        byte[] raw2 = text.asUnquotedUTF8();
        if (raw2.length > 0) {
            this._writeBytes(raw2);
        }
    }
    ...
    public int writeBinary(Base64Variant b64variant, InputStream data, int dataLength) throws IOException, JsonGenerationException {
        ...
        try {
            if (dataLength < 0) {
                bytes2 = this._writeBinary(b64variant, data, encodingBuffer);
            } else {
    ...
    protected final void _verifyValueWrite(String typeMsg) throws IOException, JsonGenerationException {
        ...
        if (this._cfgPrettyPrinter == null) {
            ...
            switch (status) {
                ...
                case 3: {
                    byte[] raw2;
                    if (this._rootValueSeparator != null && (raw2 = this._rootValueSeparator.asUnquotedUTF8()).length > 0) {
                        this._writeBytes(raw2);
                    }
                    return;
                }
    ...
    private final void _writeBytes(byte[] bytes2) throws IOException {
        int len = bytes2.length;
        if (this._outputTail + len > this._outputEnd) {
            this._flushBuffer();
            if (len > 512) {
                this._outputStream.write(bytes2, 0, len);
                return;
            }
        }
        System.arraycopy(bytes2, 0, this._outputBuffer, this._outputTail, len);
        this._outputTail += len;
    }

    private final void _writeBytes(byte[] bytes2, int offset, int len) throws IOException {
        if (this._outputTail + len > this._outputEnd) {
            this._flushBuffer();
            if (len > 512) {
                this._outputStream.write(bytes2, offset, len);
                return;
            }
        }
        System.arraycopy(bytes2, offset, this._outputBuffer, this._outputTail, len);
        this._outputTail += len;
    }
    ...
    private final void _writeStringSegment2(char[] cbuf, int offset, int end) throws IOException, JsonGenerationException {
        ...
        while (offset < end) {
            ...
            if ((ch = cbuf[offset++]) <= '\u007f') {
                ...
                int escape2 = escCodes[ch];
                if (escape2 > 0) {
                    outputBuffer[outputPtr++] = 92;
                    outputBuffer[outputPtr++] = (byte)escape2;
                    continue;
                }
    ...
    private int _writeCustomEscape(byte[] outputBuffer, int outputPtr, SerializableString esc, int remainingChars) throws IOException, JsonGenerationException {
        byte[] raw2 = esc.asUnquotedUTF8();
        int len = raw2.length;
        if (len > 6) {
            return this._handleLongCustomEscape(outputBuffer, outputPtr, this._outputEnd, raw2, remainingChars);
        }
        System.arraycopy(raw2, 0, outputBuffer, outputPtr, len);
        return outputPtr + len;
    }

    private int _handleLongCustomEscape(byte[] outputBuffer, int outputPtr, int outputEnd, byte[] raw2, int remainingChars) throws IOException, JsonGenerationException {
        int len = raw2.length;
        if (outputPtr + len > outputEnd) {
            ...
            if (len > outputBuffer.length) {
                this._outputStream.write(raw2, 0, len);
                return outputPtr;
            }
            System.arraycopy(raw2, 0, outputBuffer, outputPtr, len);
            outputPtr += len;
    ...
    protected void _writeBinary(Base64Variant b64variant, byte[] input2, int inputPtr, int inputEnd) throws IOException, JsonGenerationException {
        ...
        while (inputPtr <= safeInputEnd) {
            ...
            int b24 = input2[inputPtr++] << 8;
            b24 |= input2[inputPtr++] & 0xFF;
            b24 = b24 << 8 | input2[inputPtr++] & 0xFF;
        ...
        if (inputLeft > 0) {
            ...
            int b24 = input2[inputPtr++] << 16;
            if (inputLeft == 2) {
                b24 |= (input2[inputPtr++] & 0xFF) << 8;
            }
    ...
    private int _readMore(InputStream in, byte[] readBuffer, int inputPtr, int inputEnd, int maxRead) throws IOException {
        ...
        do {
            if ((count2 = in.read(readBuffer, inputEnd, maxRead - inputEnd)) >= 0) continue;
            return inputEnd;
        } while ((inputEnd += count2) < 3);
        return inputEnd;
    }
