...
extends BaseReader {
    ...
    private boolean loadMore(int available) throws IOException {
        ...
        if (available > 0) {
            ...
            this._ptr = 0;
            int n = count2 = this._in == null ? -1 : this._in.read(this._buffer);
            if (count2 < 1) {
                this._length = 0;
                if (count2 < 0) {
        ...
        while (this._length < 4) {
            int n = count2 = this._in == null ? -1 : this._in.read(this._buffer, this._length, this._buffer.length - this._length);
            if (count2 < 1) {
                if (count2 < 0) {
                    if (this._managedBuffers) {
                        this.freeBuffers();
