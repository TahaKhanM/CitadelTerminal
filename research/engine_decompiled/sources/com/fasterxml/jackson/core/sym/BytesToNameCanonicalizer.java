...
public final class BytesToNameCanonicalizer {
    ...
    private int findBestBucket() {
        ...
        for (int i = 0; i < len; ++i) {
            int count2 = buckets[i].length();
            if (count2 >= bestCount) continue;
            if (count2 == 1) {
                return i;
            }
            bestCount = count2;
    ...
    private static final class TableInfo {
        ...
        public TableInfo(int count2, int mainHashMask, int[] mainHash, Name[] mainNames, Bucket[] collList, int collCount, int collEnd, int longestCollisionList) {
            this.count = count2;
