...
public class PeriodFormatter {
    ...
    public int parseInto(ReadWritablePeriod readWritablePeriod, String string2, int n) {
        this.checkParser();
        this.checkPeriod(readWritablePeriod);
        return this.getParser().parseInto(readWritablePeriod, string2, n, this.iLocale);
    }

    public Period parsePeriod(String string2) {
        this.checkParser();
        return this.parseMutablePeriod(string2).toPeriod();
    }

    public MutablePeriod parseMutablePeriod(String string2) {
        this.checkParser();
        MutablePeriod mutablePeriod = new MutablePeriod(0L, this.iParseType);
        int n = this.getParser().parseInto(mutablePeriod, string2, 0, this.iLocale);
        if (n >= 0) {
            if (n >= string2.length()) {
