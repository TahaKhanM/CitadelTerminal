...
public class DateTimeFormatter {
    ...
    public int parseInto(ReadWritableInstant readWritableInstant, String string2, int n) {
        ...
        int n3 = internalParser.parseInto(dateTimeParserBucket, string2, n);
    ...
    public long parseMillis(String string2) {
        ...
        return dateTimeParserBucket.doParseMillis(internalParser, string2);
    }
    ...
    public LocalDateTime parseLocalDateTime(String string2) {
        ...
        InternalParser internalParser = this.requireParser();
        int n = internalParser.parseInto(dateTimeParserBucket = new DateTimeParserBucket(0L, chronology = this.selectChronology(null).withUTC(), this.iLocale, this.iPivotYear, this.iDefaultYear), string2, 0);
        if (n >= 0) {
            if (n >= string2.length()) {
    ...
    public DateTime parseDateTime(String string2) {
        ...
        DateTimeParserBucket dateTimeParserBucket;
        InternalParser internalParser = this.requireParser();
        int n = internalParser.parseInto(dateTimeParserBucket = new DateTimeParserBucket(0L, chronology = this.selectChronology(null), this.iLocale, this.iPivotYear, this.iDefaultYear), string2, 0);
        if (n >= 0) {
            if (n >= string2.length()) {
                long l = dateTimeParserBucket.computeMillis(true, string2);
                if (this.iOffsetParsed && dateTimeParserBucket.getOffsetInteger() != null) {
