...
ISSLChannel {
    ...
    public SSLSocketChannel2(SocketChannel channel, SSLEngine sslEngine, ExecutorService exec2, SelectionKey key) throws IOException {
        if (channel == null || sslEngine == null || exec2 == null) {
    ...
    public boolean connect(SocketAddress remote2) throws IOException {
        return this.socketChannel.connect(remote2);
    }
    ...
    private int transfereTo(ByteBuffer from2, ByteBuffer to2) {
        int toremain;
        int fremain = from2.remaining();
        if (fremain > (toremain = to2.remaining())) {
