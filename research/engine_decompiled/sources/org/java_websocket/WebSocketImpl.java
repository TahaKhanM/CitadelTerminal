...
implements WebSocket {
    ...
    public synchronized void close(int code, String message, boolean remote2) {
        if (this.readyState != ReadyState.CLOSING && this.readyState != ReadyState.CLOSED) {
            if (this.readyState == ReadyState.OPEN) {
                ...
                this.flushAndClose(code, message, remote2);
            } else if (code == -3) {
                assert (remote2);
    ...
    public synchronized void closeConnection(int code, String message, boolean remote2) {
        ...
        try {
            this.wsl.onWebsocketClose(this, code, message, remote2);
        }
        catch (RuntimeException e) {
            this.wsl.onWebsocketError(this, e);
        }
    ...
    protected void closeConnection(int code, boolean remote2) {
        this.closeConnection(code, "", remote2);
    }
    ...
    public synchronized void flushAndClose(int code, String message, boolean remote2) {
        if (this.flushandclosestate) {
            return;
        }
        this.closecode = code;
        this.closemessage = message;
        this.closedremotely = remote2;
        this.flushandclosestate = true;
        this.wsl.onWriteDemand(this);
        try {
            this.wsl.onWebsocketClosing(this, code, message, remote2);
        }
        catch (RuntimeException e) {
    ...
    @Override
    public void send(ByteBuffer bytes2) {
        if (bytes2 == null) {
            throw new IllegalArgumentException("Cannot send 'null' data to a WebSocketImpl.");
        }
        this.send(this.draft.createFrames(bytes2, this.role == Role.CLIENT));
    }

    @Override
    public void send(byte[] bytes2) {
        this.send(ByteBuffer.wrap(bytes2));
    }
    ...
    public void startHandshake(ClientHandshakeBuilder handshakedata) throws InvalidHandshakeException {
        ...
        try {
            ...
            this.wsl.onWebsocketError(this, e);
            throw new InvalidHandshakeException("rejected because of " + String.valueOf(e));
        }
