...
implements Runnable {
    ...
    private void doAccept(SelectionKey key, Iterator<SelectionKey> i) throws IOException, InterruptedException {
        ...
        WebSocket w = this.wsf.createWebSocket((WebSocketAdapter)this, (List)this.drafts);
        ((WebSocketImpl)w).setSelectionKey(channel.register(this.selector, 1, w));
        try {
            ((WebSocketImpl)w).setChannel(this.wsf.wrapChannel(channel, ((WebSocketImpl)w).getSelectionKey()));
            i.remove();
            this.allocateBuffers(w);
        }
        catch (IOException ex) {
            if (((WebSocketImpl)w).getSelectionKey() != null) {
                ((WebSocketImpl)w).getSelectionKey().cancel();
            }
            this.handleIOException(((WebSocketImpl)w).getSelectionKey(), null, ex);
        }
    }
    ...
    private void handleFatal(WebSocket conn, Exception e) {
        ...
        try {
            ...
            this.log.error("Error during shutdown", e1);
            ...
            System.err.println("Fatal error in web socket lib 3 during try catch exiting now");
            System.exit(1);
        }
        System.err.println("Fatal error in web socket lib 3 exiting now");
        System.exit(1);
    }
     ...
     */
    @Override
    public final void onWebsocketClose(WebSocket conn, int code, String reason, boolean remote2) {
        this.selector.wakeup();
        try {
            if (this.removeConnection(conn)) {
                this.onClose(conn, code, reason, remote2);
            }
        }
        finally {
            try {
    ...
    protected boolean removeConnection(WebSocket ws) {
        ...
        synchronized (collection) {
            if (this.connections.contains(ws)) {
                ...
                this.log.trace("Removing connection which is not in the connections collection! Possible no handshake recieved! {}", (Object)ws);
            }
        }
    ...
    @Override
    public void onWebsocketClosing(WebSocket conn, int code, String reason, boolean remote2) {
        this.onClosing(conn, code, reason, remote2);
    }

    public void onCloseInitiated(WebSocket conn, int code, String reason) {
    }

    public void onClosing(WebSocket conn, int code, String reason, boolean remote2) {
    }
