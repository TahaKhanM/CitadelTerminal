...
ISSLChannel {
    ...
    public synchronized int read(ByteBuffer dst) throws IOException {
        ...
        if (bytesRead > 0 || this.peerNetData.hasRemaining()) {
            ...
            if (this.peerNetData.hasRemaining()) {
                ...
                try {
                    result2 = this.engine.unwrap(this.peerNetData, this.peerAppData);
                }
                catch (SSLException e) {
                    this.log.error("SSLExcpetion during unwrap", e);
                    throw e;
                }
                ...
                throw new IllegalStateException("Invalid SSL status: " + String.valueOf((Object)result2.getStatus()));
            }
        } else if (bytesRead < 0) {
    ...
    public synchronized int write(ByteBuffer output) throws IOException {
        ...
        block6: while (output.hasRemaining()) {
            ...
            SSLEngineResult result2 = this.engine.wrap(output, this.myNetData);
            switch (result2.getStatus()) {
                case OK: {
                    this.myNetData.flip();
        ...
        block27: while (!handshakeComplete) {
            switch (handshakeStatus) {
                ...
                case NEED_WRAP: {
                    ...
                    this.myNetData.clear();
                    try {
                        result2 = this.engine.wrap(this.myAppData, this.myNetData);
                        handshakeStatus = result2.getHandshakeStatus();
                    }
                    catch (SSLException sslException) {
                        this.engine.closeOutbound();
