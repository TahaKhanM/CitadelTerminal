...
Sortable {
    ...
    public void filter(Filter filter2) throws NoTestsRemainException {
        if (this.getTest() instanceof Filterable) {
            ...
            TestSuite filtered2 = new TestSuite(suite.getName());
            int n = suite.testCount();
            for (int i = 0; i < n; ++i) {
                Test test = suite.testAt(i);
                if (!filter2.shouldRun(JUnit38ClassRunner.makeDescription(test))) continue;
                filtered2.addTest(test);
            }
            this.setTest(filtered2);
        }
    }
