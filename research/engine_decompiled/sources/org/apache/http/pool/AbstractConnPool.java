...
ConnPoolControl<T> {
    ...
    public void setDefaultMaxPerRoute(int max2) {
        Args.positive(max2, "Max per route value");
        this.lock.lock();
        try {
            this.defaultMaxPerRoute = max2;
        }
        finally {
            this.lock.unlock();
        }
     ...
     */
    @Override
    public void setMaxPerRoute(T route, int max2) {
        Args.notNull(route, "Route");
        Args.positive(max2, "Max per route value");
        this.lock.lock();
        try {
            this.maxPerRoute.put(route, max2);
        }
        finally {
            this.lock.unlock();
        }
