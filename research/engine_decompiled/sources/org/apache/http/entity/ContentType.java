...
implements Serializable {
    ...
    ContentType(String mimeType, Charset charset, NameValuePair[] params2) {
        this.mimeType = mimeType;
        this.charset = charset;
        this.params = params2;
    }
    ...
    public String getParameter(String name) {
        ...
        for (NameValuePair param2 : this.params) {
            if (!param2.getName().equalsIgnoreCase(name)) continue;
    ...
    private static boolean valid(String s2) {
    ...
    private static ContentType create(String mimeType, NameValuePair[] params2, boolean strict) {
        Charset charset = null;
        for (NameValuePair param2 : params2) {
            if (!param2.getName().equalsIgnoreCase("charset")) continue;
            String s2 = param2.getValue();
            if (TextUtils.isBlank(s2)) break;
            try {
        ...
        return new ContentType(mimeType, charset, params2 != null && params2.length > 0 ? params2 : null);
    }

    public static ContentType create(String mimeType, NameValuePair ... params2) throws UnsupportedCharsetException {
        ...
        return ContentType.create(mimeType, params2, true);
    }

    public static ContentType parse(String s2) throws ParseException, UnsupportedCharsetException {
        Args.notNull(s2, "Content type");
        CharArrayBuffer buf = new CharArrayBuffer(s2.length());
        buf.append(s2);
        ...
        throw new ParseException("Invalid content type: " + s2);
    }
    ...
    static {
        ContentType[] contentTypes = new ContentType[]{APPLICATION_ATOM_XML, APPLICATION_FORM_URLENCODED, APPLICATION_JSON, APPLICATION_SVG_XML, APPLICATION_XHTML_XML, APPLICATION_XML, MULTIPART_FORM_DATA, TEXT_HTML, TEXT_PLAIN, TEXT_XML};
