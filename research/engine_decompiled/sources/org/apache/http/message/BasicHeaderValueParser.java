...
implements HeaderValueParser {
    ...
    public HeaderElement parseHeaderElement(CharArrayBuffer buffer, ParserCursor cursor) {
        ...
        if (!cursor.atEnd() && (ch = buffer.charAt(cursor.getPos() - 1)) != ',') {
            params2 = this.parseParameters(buffer, cursor);
        }
        return this.createHeaderElement(nvp.getName(), nvp.getValue(), params2);
    }

    protected HeaderElement createHeaderElement(String name, String value, NameValuePair[] params2) {
        return new BasicHeaderElement(name, value, params2);
    }
    ...
    public NameValuePair[] parseParameters(CharArrayBuffer buffer, ParserCursor cursor) {
        ...
        while (!cursor.atEnd()) {
            NameValuePair param2 = this.parseNameValuePair(buffer, cursor);
            params2.add(param2);
