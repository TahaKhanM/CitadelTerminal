...
implements Idn {
    ...
    protected String decode(String s2) {
        ...
        int i = 0;
        int bias = 72;
        StringBuilder output = new StringBuilder(input2.length());
        int lastdelim = input2.lastIndexOf(45);
        if (lastdelim != -1) {
            output.append(input2.subSequence(0, lastdelim));
            input2 = input2.substring(lastdelim + 1);
        }
