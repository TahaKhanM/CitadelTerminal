...
public class URIBuilder {
    ...
    public URIBuilder(String string2) throws URISyntaxException {
        this.digestURI(new URI(string2));
    }
    ...
    private String encodeUrlForm(List<NameValuePair> params2) {
        return URLEncodedUtils.format(params2, this.charset != null ? this.charset : Consts.UTF_8);
    }
    ...
    public URIBuilder addParameter(String param2, String value) {
        if (this.queryParams == null) {
            this.queryParams = new ArrayList<NameValuePair>();
        }
        this.queryParams.add(new BasicNameValuePair(param2, value));
        this.encodedQuery = null;
        this.encodedSchemeSpecificPart = null;
        this.query = null;
        return this;
    ...
    public URIBuilder setParameter(String param2, String value) {
        if (this.queryParams == null) {
            this.queryParams = new ArrayList<NameValuePair>();
        }
        if (!this.queryParams.isEmpty()) {
            ...
            while (it.hasNext()) {
                ...
                if (!nvp.getName().equals(param2)) continue;
                it.remove();
            }
        }
        this.queryParams.add(new BasicNameValuePair(param2, value));
    ...
    private static String normalizePath(String path) {
        ...
        for (n = 0; n < s2.length() && s2.charAt(n) == '/'; ++n) {
        }
        if (n > 1) {
            s2 = s2.substring(n - 1);
        }
