...
implements CookieSpec {
    ...
    public List<Header> formatCookies(List<Cookie> cookies) {
        ...
        for (int n = 0; n < sortedCookies.size(); ++n) {
            ...
            if (s2 == null) continue;
            buffer.append('=');
            if (this.containsSpecialChar(s2)) {
                buffer.append('\"');
    ...
    boolean containsSpecialChar(CharSequence s2) {
        return this.containsChars(s2, SPECIAL_CHARS);
    }

    boolean containsChars(CharSequence s2, BitSet chars) {
        for (int i = 0; i < s2.length(); ++i) {
            char ch = s2.charAt(i);
