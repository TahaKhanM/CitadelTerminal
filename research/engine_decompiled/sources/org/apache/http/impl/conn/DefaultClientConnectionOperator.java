...
implements ClientConnectionOperator {
    ...
    @Override
    public void openConnection(OperatedClientConnection conn, HttpHost target, InetAddress local, HttpContext context, HttpParams params2) throws IOException {
        ...
        for (int i = 0; i < addresses.length; ++i) {
            ...
            block8: {
                ...
                boolean last2 = i == addresses.length - 1;
                Socket sock = sf.createSocket(params2);
                ...
                try {
                    Socket connsock = sf.connectSocket(sock, remoteAddress, localAddress, params2);
                    if (sock != connsock) {
                        sock = connsock;
                        conn.opening(sock, target);
                    }
                    this.prepareSocket(sock, context, params2);
                    conn.openCompleted(sf.isSecure(sock), params2);
                    return;
                }
    ...
    @Override
    public void updateSecureConnection(OperatedClientConnection conn, HttpHost target, HttpContext context, HttpParams params2) throws IOException {
        Args.notNull(conn, "Connection");
        Args.notNull(target, "Target host");
        ...
        Socket sock = lsf.createLayeredSocket(conn.getSocket(), target.getHostName(), schm.resolvePort(target.getPort()), params2);
        this.prepareSocket(sock, context, params2);
        conn.update(sock, target, lsf.isSecure(sock), params2);
    }

    protected void prepareSocket(Socket sock, HttpContext context, HttpParams params2) throws IOException {
        sock.setTcpNoDelay(HttpConnectionParams.getTcpNoDelay(params2));
        sock.setSoTimeout(HttpConnectionParams.getSoTimeout(params2));
