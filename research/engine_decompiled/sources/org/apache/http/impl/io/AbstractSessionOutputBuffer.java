...
BufferInfo {
    ...
    protected void init(OutputStream outstream, int buffersize, HttpParams params2) {
        ...
        this.metrics = this.createTransportMetrics();
        CodingErrorAction a1 = (CodingErrorAction)params2.getParameter("http.malformed.input.action");
        this.onMalformedCharAction = a1 != null ? a1 : CodingErrorAction.REPORT;
        CodingErrorAction a2 = (CodingErrorAction)params2.getParameter("http.unmappable.input.action");
    ...
    public void writeLine(String s2) throws IOException {
        ...
        if (s2.length() > 0) {
            if (this.ascii) {
                ...
                CharBuffer cbuf = CharBuffer.wrap(s2);
    ...
    private void writeEncoded(CharBuffer cbuf) throws IOException {
        ...
        while (cbuf.hasRemaining()) {
            result2 = this.encoder.encode(cbuf, this.bbuf, true);
            this.handleEncodingResult(result2);
        }
        result2 = this.encoder.flush(this.bbuf);
        this.handleEncodingResult(result2);
        this.bbuf.clear();
    }

    private void handleEncodingResult(CoderResult result2) throws IOException {
        if (result2.isError()) {
