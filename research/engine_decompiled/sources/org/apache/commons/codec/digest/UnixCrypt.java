...
public class UnixCrypt {
    ...
    private static int dEncrypt(int el, int r, int s2, int e0, int e1, int[] sArr) {
        int v = r ^ r >>> 16;
        int u = v & e0;
        u = u ^ u << 16 ^ r ^ sArr[s2];
        int t = (v &= e1) ^ v << 16 ^ r ^ sArr[s2 + 1];
        t = t >>> 4 | t << 28;
    ...
    private static int[] desSetKey(byte[] key) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            int s2 = SKB[0][(c &= 0xFFFFFFF) & 0x3F] | SKB[1][c >>> 6 & 3 | c >>> 7 & 0x3C] | SKB[2][c >>> 13 & 0xF | c >>> 14 & 0x30] | SKB[3][c >>> 20 & 1 | c >>> 21 & 6 | c >>> 22 & 0x38];
            int t = SKB[4][(d &= 0xFFFFFFF) & 0x3F] | SKB[5][d >>> 7 & 3 | d >>> 8 & 0x3C] | SKB[6][d >>> 15 & 0x3F] | SKB[7][d >>> 21 & 0xF | d >>> 22 & 0x30];
            schedule[j++] = t << 16 | s2 & 0xFFFF;
            s2 = s2 >>> 16 | t & 0xFFFF0000;
            s2 = s2 << 4 | s2 >>> 28;
            schedule[j++] = s2;
        }
        return schedule;
