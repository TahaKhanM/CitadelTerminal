...
implements StringEncoder {
    ...
    private static char[] transcodeRemaining(char prev, char curr, char next2, char aNext) {
        if (curr == 'E' && next2 == 'V') {
            return CHARS_AF;
        }
        ...
        if (!(curr != 'H' || Nysiis.isVowel(prev) && Nysiis.isVowel(next2))) {
    ...
    public String nysiis(String str) {
        ...
        for (int i = 1; i < len; ++i) {
            char next2 = i < len - 1 ? chars[i + 1] : (char)' ';
            char aNext = i < len - 2 ? chars[i + 2] : (char)' ';
            char[] transcoded = Nysiis.transcodeRemaining(chars[i - 1], chars[i], next2, aNext);
