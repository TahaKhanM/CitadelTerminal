...
implements StringEncoder {
    ...
    private boolean isVowel(StringBuilder string2, int index) {
        return VOWELS.indexOf(string2.charAt(index)) >= 0;
    }

    private boolean isPreviousChar(StringBuilder string2, int index, char c) {
        boolean matches = false;
        if (index > 0 && index < string2.length()) {
            matches = string2.charAt(index - 1) == c;
        }
        return matches;
    }

    private boolean isNextChar(StringBuilder string2, int index, char c) {
        boolean matches = false;
        if (index >= 0 && index < string2.length() - 1) {
            matches = string2.charAt(index + 1) == c;
        }
        return matches;
    }

    private boolean regionMatch(StringBuilder string2, int index, String test) {
        boolean matches = false;
        if (index >= 0 && index + test.length() - 1 < string2.length()) {
            String substring = string2.substring(index, index + test.length());
