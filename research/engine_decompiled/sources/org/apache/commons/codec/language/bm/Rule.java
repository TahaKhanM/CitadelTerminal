...
public class Rule {
    public static final RPattern ALL_STRINGS_RMATCHER = new RPattern(){
        ...
        @Override
        public boolean isMatch(CharSequence input2) {
            return true;
        }
    };
    ...
    private static boolean contains(CharSequence chars, char input2) {
        for (int i = 0; i < chars.length(); ++i) {
            if (chars.charAt(i) != input2) continue;
            return true;
        }
        return false;
    }
    ...
    private static boolean endsWith(CharSequence input2, CharSequence suffix) {
        if (suffix.length() > input2.length()) {
            return false;
        }
        int i = input2.length() - 1;
        for (int j = suffix.length() - 1; j >= 0; --j) {
            if (input2.charAt(i) != suffix.charAt(j)) {
                return false;
            }
            --i;
        }
    ...
    private static PhonemeExpr parsePhonemeExpr(String ph) {
        if (ph.startsWith("(")) {
            ...
            String body2 = ph.substring(1, ph.length() - 1);
            for (String part : body2.split("[|]")) {
    ...
    private static Map<String, List<Rule>> parseRules(Scanner scanner, final String location) {
        HashMap<String, List<Rule>> lines2 = new HashMap<String, List<Rule>>();
        int currentLine = 0;
        ...
        while (scanner.hasNextLine()) {
            ...
            if (line.startsWith(HASH_INCLUDE)) {
                ...
                lines2.putAll(Rule.parseRules(Rule.createScanner(incl), location + "->" + incl));
                continue;
            }
            ...
            try {
                ...
                ArrayList<2> rules = (ArrayList<2>)lines2.get(patternKey);
                if (rules == null) {
                    rules = new ArrayList<2>();
                    lines2.put(patternKey, rules);
                }
                rules.add(r);
            }
    ...
    private static RPattern pattern(final String regex) {
        boolean endsWith2;
        boolean startsWith2 = regex.startsWith("^");
        final String content = regex.substring(startsWith2 ? 1 : 0, (endsWith2 = regex.endsWith("$")) ? regex.length() - 1 : regex.length());
        ...
        if (!boxes) {
            if (startsWith2 && endsWith2) {
                ...
                return new RPattern(){
                    ...
                    @Override
                    public boolean isMatch(CharSequence input2) {
                        return input2.equals(content);
                    }
                };
            }
            if ((startsWith2 || endsWith2) && content.length() == 0) {
            ...
            if (startsWith2) {
                return new RPattern(){
                    ...
                    @Override
                    public boolean isMatch(CharSequence input2) {
                        return Rule.startsWith(input2, content);
                    }
                };
            }
            if (endsWith2) {
                return new RPattern(){
                    ...
                    @Override
                    public boolean isMatch(CharSequence input2) {
                        return Rule.endsWith(input2, content);
                    }
                };
            }
        } else {
            ...
            if (startsWithBox && endsWithBox && !(boxContent = content.substring(1, content.length() - 1)).contains("[")) {
                ...
                if (startsWith2 && endsWith2) {
                    return new RPattern(){
                        ...
                        @Override
                        public boolean isMatch(CharSequence input2) {
                            return input2.length() == 1 && Rule.contains(bContent, input2.charAt(0)) == shouldMatch;
                        }
                    };
                }
                if (startsWith2) {
                    return new RPattern(){
                        ...
                        @Override
                        public boolean isMatch(CharSequence input2) {
                            return input2.length() > 0 && Rule.contains(bContent, input2.charAt(0)) == shouldMatch;
                        }
                    };
                }
                if (endsWith2) {
                    return new RPattern(){
                        ...
                        @Override
                        public boolean isMatch(CharSequence input2) {
                            return input2.length() > 0 && Rule.contains(bContent, input2.charAt(input2.length() - 1)) == shouldMatch;
                        }
                    };
                }
            }
        ...
        return new RPattern(){
            ...
            @Override
            public boolean isMatch(CharSequence input2) {
                Matcher matcher = this.pattern.matcher(input2);
                return matcher.find();
            }
        };
    }

    private static boolean startsWith(CharSequence input2, CharSequence prefix) {
        if (prefix.length() > input2.length()) {
            return false;
        }
        for (int i = 0; i < prefix.length(); ++i) {
            if (input2.charAt(i) == prefix.charAt(i)) continue;
            return false;
        }
        return true;
    }
    ...
    public boolean patternAndContextMatches(CharSequence input2, int i) {
        if (i < 0) {
            throw new IndexOutOfBoundsException("Can not match pattern at negative indexes");
        }
        int patternLength = this.pattern.length();
        int ipl = i + patternLength;
        if (ipl > input2.length()) {
            return false;
        }
        if (!input2.subSequence(i, ipl).equals(this.pattern)) {
            return false;
        }
        if (!this.rContext.isMatch(input2.subSequence(ipl, input2.length()))) {
            return false;
        }
        return this.lContext.isMatch(input2.subSequence(0, i));
    }
    ...
    static {
        for (NameType s2 : NameType.values()) {
            ...
            for (RuleType rt : RuleType.values()) {
                ...
                Languages ls = Languages.getInstance(s2);
                for (String l : ls.getLanguages()) {
                    try {
                        rs.put(l, Rule.parseRules(Rule.createScanner(s2, rt, l), Rule.createResourceName(s2, rt, l)));
                    }
                    catch (IllegalStateException e) {
                        throw new IllegalStateException("Problem processing " + Rule.createResourceName(s2, rt, l), e);
                    }
                }
                if (!rt.equals((Object)RuleType.RULES)) {
                    rs.put("common", Rule.parseRules(Rule.createScanner(s2, rt, "common"), Rule.createResourceName(s2, rt, "common")));
                }
