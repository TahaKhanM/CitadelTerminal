...
StringDecoder {
    ...
    protected byte[] doEncoding(byte[] bytes2) {
        ...
        byte[] data = QuotedPrintableCodec.encodeQuotedPrintable(PRINTABLE_CHARS, bytes2);
    ...
    @Override
    protected byte[] doDecoding(byte[] bytes2) throws DecoderException {
        if (bytes2 == null) {
            return null;
        }
        ...
        if (hasUnderscores) {
            byte[] tmp = new byte[bytes2.length];
