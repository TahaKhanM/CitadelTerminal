...
abstract class RFC1522Codec {
    ...
    protected String decodeText(String text) throws DecoderException, UnsupportedEncodingException {
        ...
        int from2 = 2;
        int to2 = text.indexOf(63, from2);
        if (to2 == terminator) {
            throw new DecoderException("RFC 1522 violation: charset token not found");
        }
        String charset = text.substring(from2, to2);
        if (charset.equals("")) {
            throw new DecoderException("RFC 1522 violation: charset not specified");
        }
        from2 = to2 + 1;
        if ((to2 = text.indexOf(63, from2)) == terminator) {
            throw new DecoderException("RFC 1522 violation: encoding token not found");
        }
        String encoding = text.substring(from2, to2);
        if (!this.getEncoding().equalsIgnoreCase(encoding)) {
            throw new DecoderException("This codec cannot decode " + encoding + " encoded content");
        }
        from2 = to2 + 1;
        to2 = text.indexOf(63, from2);
        byte[] data = StringUtils.getBytesUsAscii(text.substring(from2, to2));
