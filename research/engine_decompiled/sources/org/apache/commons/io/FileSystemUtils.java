...
public class FileSystemUtils {
    ...
    long freeSpaceWindows(String path, long timeout) throws IOException {
        ...
        List<String> lines2 = this.performCommand(cmdAttribs, Integer.MAX_VALUE, timeout);
    ...
    long freeSpaceUnix(String path, boolean kb, boolean posix, long timeout) throws IOException {
        ...
        List<String> lines2 = this.performCommand(cmdAttribs, 3, timeout);
        if (lines2.size() < 2) {
            throw new IOException("Command line '" + DF + "' did not return info as expected for path '" + path + "'- response was " + lines2);
        }
        String line2 = lines2.get(1);
        StringTokenizer tok = new StringTokenizer(line2, " ");
        if (tok.countTokens() < 4) {
            if (tok.countTokens() != 1 || lines2.size() < 3) throw new IOException("Command line '" + DF + "' did not return data as expected for path '" + path + "'- check path is valid");
    ...
    List<String> performCommand(String[] cmdAttribs, int max2, long timeout) throws IOException {
