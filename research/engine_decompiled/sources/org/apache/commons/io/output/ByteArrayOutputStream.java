...
extends OutputStream {
    ...
    public synchronized void reset() {
        ...
        if (this.reuseBuffers) {
            ...
            int size2 = this.buffers.get(0).length;
            this.buffers.clear();
            this.needNewBuffer(size2);
    ...
    public static InputStream toBufferedInputStream(InputStream input2) throws IOException {
        return ByteArrayOutputStream.toBufferedInputStream(input2, 1024);
    }

    public static InputStream toBufferedInputStream(InputStream input2, int size2) throws IOException {
        ByteArrayOutputStream output = new ByteArrayOutputStream(size2);
        output.write(input2);
    ...
    public synchronized InputStream toInputStream() {
        ...
        ArrayList<ByteArrayInputStream> list2 = new ArrayList<ByteArrayInputStream>(this.buffers.size());
        for (byte[] buf : this.buffers) {
            int c = Math.min(buf.length, remaining);
            list2.add(new ByteArrayInputStream(buf, 0, c));
            if ((remaining -= c) != 0) continue;
            break;
        }
        this.reuseBuffers = false;
        return new SequenceInputStream(Collections.enumeration(list2));
    }
