...
implements Serializable {
    ...
    public void initialize() throws Exception {
        ...
        FileEntry[] children2 = this.doListFiles(this.rootEntry.getFile(), this.rootEntry);
        this.rootEntry.setChildren(children2);
    }
    ...
    private void checkAndNotify(FileEntry parent, FileEntry[] previous, File[] files2) {
        int c = 0;
        FileEntry[] current = files2.length > 0 ? new FileEntry[files2.length] : FileEntry.EMPTY_ENTRIES;
        for (FileEntry entry : previous) {
            while (c < files2.length && this.comparator.compare(entry.getFile(), files2[c]) > 0) {
                current[c] = this.createFileEntry(parent, files2[c]);
                this.doCreate(current[c]);
                ++c;
            }
            if (c < files2.length && this.comparator.compare(entry.getFile(), files2[c]) == 0) {
                this.doMatch(entry, files2[c]);
                this.checkAndNotify(entry, entry.getChildren(), this.listFiles(files2[c]));
                current[c] = entry;
                ++c;
                continue;
            }
    ...
    private FileEntry createFileEntry(FileEntry parent, File file) {
        ...
        entry.refresh(file);
        FileEntry[] children2 = this.doListFiles(file, entry);
        entry.setChildren(children2);
        return entry;
    }

    private FileEntry[] doListFiles(File file, FileEntry entry) {
        File[] files2 = this.listFiles(file);
        FileEntry[] children2 = files2.length > 0 ? new FileEntry[files2.length] : FileEntry.EMPTY_ENTRIES;
        for (int i = 0; i < files2.length; ++i) {
            children2[i] = this.createFileEntry(entry, files2[i]);
        }
        return children2;
    }
    ...
    private void doCreate(FileEntry entry) {
        ...
        for (FileEntry aChildren : children2 = entry.getChildren()) {
    ...
    private File[] listFiles(File file) {
        File[] children2 = null;
        if (file.isDirectory()) {
            File[] fileArray = children2 = this.fileFilter == null ? file.listFiles() : file.listFiles(this.fileFilter);
        }
        if (children2 == null) {
            children2 = FileUtils.EMPTY_FILE_ARRAY;
        }
        if (this.comparator != null && children2.length > 1) {
            Arrays.sort(children2, this.comparator);
        }
        return children2;
    }
