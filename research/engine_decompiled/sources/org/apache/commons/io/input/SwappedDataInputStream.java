...
implements DataInput {
    public SwappedDataInputStream(InputStream input2) {
        super(input2);
    }
    ...
    public void readFully(byte[] data, int offset, int length) throws IOException, EOFException {
        int count2;
        for (int remaining = length; remaining > 0; remaining -= count2) {
            int location = offset + length - remaining;
            count2 = this.read(data, location, remaining);
            if (-1 != count2) continue;
    ...
    @Override
    public int skipBytes(int count2) throws IOException, EOFException {
        return (int)this.in.skip(count2);
    }
}
