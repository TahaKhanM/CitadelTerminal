"""v17 extra-turret — v13 + forward cover at [6,8]/[21,8].

Addresses the v12-vs-v12 Nash tie (39-39 HP) at T3: each side breaches
once at (7,6)/(20,21) on BOTTOM_LEFT/RIGHT edge. These edge tiles sit at
distance 3.0 from [7,9]/[20,9] — covered by upgraded range 3.5 but one
upgraded Turret is not enough throughput to kill every Scout in the
edge-passing wave.

v17 adds a SECOND Turret covering (7,6)/(20,6) from [6,8]/[21,8] at base
range 2.5 (distance 2.24). Combined with upgraded [7,9]/[20,9] this
doubles damage throughput at the mirror-breach tile. The extra 4 SP at
T0 pushes two tail wall_line walls to T1, a non-critical delay since
enemy MP at T0 cannot mount a wave that exploits them.

Preserves §5.2 (second-ring [11,5]/[16,5]) and §5.6 (reactive fix).
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _build_defense(self, gs):
        ...
        forward_cover = [[6, 8], [21, 8]]
        inner_corners = [[1, 13], [26, 13]]
        outer_corners = [[0, 13], [27, 13]]

        # Opening: 12 v12-core turrets + 2 forward_cover = 28 SP of
        # Turrets. Walls then fill remaining 12 SP at T0; tail of
        # wall_line slips to T1.
        gs.attempt_spawn(TURRET, center_turrets)
        gs.attempt_spawn(TURRET, sidelane_deep)
        gs.attempt_spawn(TURRET, forward_cover)
        ...
        all_turrets = (center_turrets + sidelane_deep + forward_cover
                       + outer_turrets + inner_corners + outer_corners)
        for loc in all_turrets:
