"""codex_v102_lowcenter_antibanana: v100 plus low-center anti-banana turrets.
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _build_defense(self, gs):
        ...
        if not lattice and t >= 20:
            gs.attempt_spawn(TURRET, [[12, 8], [15, 8]])
        if not lattice and t >= 24:
            gs.attempt_upgrade([[12, 8], [15, 8]])
        if not lattice and t >= 28:
            gs.attempt_spawn(TURRET, [[9, 5], [18, 5]])
        if not lattice and t >= 32:
            gs.attempt_upgrade([[9, 5], [18, 5]])
        if not lattice and t >= 34:
            gs.attempt_spawn(TURRET, [[10, 4], [17, 4]])
        if not lattice and t >= 38:
            gs.attempt_upgrade([[10, 4], [17, 4]])
