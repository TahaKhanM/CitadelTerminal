"""codex_v37_fast_caravan: fast Scout pressure with priority y=13 caravan.

Parent: v21_quadrant_adaptive_support / v21_temporal_phase_gate.
Tag: "quadrant-adaptive-support".
Hypothesis: the edge-caravan adversary punishes slow structure grinding. Use
Scout pressure as the default, and reserve Demolishers for future explicit
turtle detection rather than `enemy_front >= 10`.
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _priority_caravan(self, gs):
        """Make sure our Scouts actually receive high-Y shield.

        This runs before the old defense schedule from T8 onward. It deliberately
        sacrifices some late wall completion if necessary; the replay loss shows
        unshielded center offense cannot race a shielded edge-caravan opponent.
        """
        t = gs.turn_number
        if t >= 8:
    ...
    def _attack_phase(self, gs):
        ...
        scout_thresh = 6 * scout_cost if counter else 8 * scout_cost

        enemy_front = self._count_enemy_front(gs)

        if False and not deferring and enemy_front >= 10 and mp >= 4 * demo_cost:
