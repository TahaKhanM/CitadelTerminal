"""codex_v30_mobile_edge: support-scout counter plus mobile edge defense.

Parent: v21_quadrant_adaptive_support / v21_temporal_phase_gate.
Tag: "quadrant-adaptive-support".
Hypothesis: after switching back to Scouts, the remaining failure is the edge
collapse. Use Interceptors on the low-edge lanes during enemy MP spikes rather
than trying to solve a pathing problem with fragile stationary patches.
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _pressure_response(self, gs):
        ...
        interceptor_cost = gs.type_cost(INTERCEPTOR)[MP]
        if enemy_mp >= 10 * scout_cost or gs.turn_number >= 34:
