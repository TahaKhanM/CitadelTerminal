"""codex_v35_fast_scout_edge: fast Scout pressure plus narrow edge reserve.
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _pressure_response(self, gs):
        ...
        interceptor_cost = gs.type_cost(INTERCEPTOR)[MP]
        if 42 <= gs.turn_number <= 53 and enemy_mp >= 8 * scout_cost:
            for spawn in ([7, 6], [20, 6]):
                if gs.get_resource(MP) - interceptor_cost < 8 * scout_cost:
                    break
