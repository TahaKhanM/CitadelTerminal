# v4_deepplug — variant of lostkids_v3 targeting the deep-edge breach
# weakness seen in ranked replays (Ranked Replays/lostkids_v3_codex/
# lostkids_v3_codex_loss_python-algo_1322_*.replay and _1425_*.replay):
# opponent breached at tile (11, 2) 23-24 times. Lostkids' canonical
# defense has NO turret in range of (11,2): nearest is (12,8) at distance
# 7, way out of upgraded turret range 3.5. So scouts that pass through
# y=5 walk freely down column 11 to (11,2).
#
# Change vs lostkids_v3:
#   - Phase 8 added to defense-order.json: place base turrets at (11,4)
#     and (16,4), then upgrade both. Range 3.5 (upgraded) covers
#     (10,2)-(13,2) for the left turret and (15,2)-(18,2) for the right
#     turret — directly plugging the (11,2)/(16,2) deep-breach hotspots.
#   - All other v3 logic kept as-is (anti-demo, anti-scout streak,
#     scout-success demo pivot).
#
# Original lostkids_v3 context (kept for reference):
#
...
#
# Changes vs lostkids_v2 (inherited from v3):
...
class AlgoStrategy(gamelib.AlgoCore):
    def __init__(self):
        ...
        random.seed(seed)
        gamelib.debug_write('v4_deepplug random seed: {}'.format(seed))

    def on_game_start(self, config):
        gamelib.debug_write('Configuring v4_deepplug...')
        self.config = config
    ...
    def on_turn(self, turn_state):
        ...
        try:
            ...
            gamelib.debug_write(
                "[v4_deepplug] watchdog fired after {}s".format(TURN_WATCHDOG_SECONDS)
            )
            used_fallback = True
        except Exception as exc:
            gamelib.debug_write(
                "[v4_deepplug] on_turn exception: {!r}\n{}".format(
        ...
        if used_fallback:
            try:
                ...
                gamelib.debug_write(
                    "[v4_deepplug] safe-fallback also failed: {!r}\n{}".format(
    ...
    def on_action_frame(self, turn_string):
        try:
            ...
            gamelib.debug_write(
                "[v4_deepplug] on_action_frame exception: {!r}".format(exc)
            )
