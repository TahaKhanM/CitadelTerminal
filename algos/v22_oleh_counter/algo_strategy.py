"""v22_oleh_counter: v21 + anti-scout-rush (pre-commit corner Turrets + Interceptor reserve).
Parent: v21_temporal_phase_gate. Tag: "anti-scout-rush-oleh".
Hypothesis: oleh-v2's 22-28 Scout corner rushes (7/9 ranked losses) breach our defense
where we have no coverage; pre-commit 4 upgraded Turrets at corners [3,12],[4,12],[23,12],[24,12]
and hold 2 Interceptors in reserve for their peak wave.
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        # v22: anti-scout-rush detection — count enemy mobiles per action-frame.
        self.opp_is_scout_rush = False
        self.enemy_mobile_peak_this_turn = 0
        self.enemy_mobile_peak_prev_turn = 0
    ...
    def on_turn(self, turn_state):
        ...
        # v22: update scout-rush detection before defense spend.
        self._update_scout_rush_flag(gs)

        self._anti_corner_defense(gs)  # v22: pre-commit corner Turrets
        self._build_defense(gs)
        self._spend_hoard(gs)
        self._reactive_defense(gs)
        self._pressure_response(gs)
        self._scout_rush_interceptor_reserve(gs)  # v22: peak-wave tank
    ...
    def _update_scout_rush_flag(self, gs):
        """Roll per-turn enemy-mobile peak; mark opp as scout-rush if prev peak >= 12."""
        # Carry last turn's accumulated peak into prev, reset current.
        self.enemy_mobile_peak_prev_turn = self.enemy_mobile_peak_this_turn
        self.enemy_mobile_peak_this_turn = 0
        if self.enemy_mobile_peak_prev_turn >= 12:
            self.opp_is_scout_rush = True

    def _anti_corner_defense(self, gs):
        """v22: pre-commit 4 upgraded corner Turrets by turn 3. Budgeted across t=0..3."""
        t = gs.turn_number
        corner_turrets = [[3, 12], [4, 12], [23, 12], [24, 12]]
        # Spawn (staggered so we don't starve the v13 opening on t=0).
        if t >= 0:
            gs.attempt_spawn(TURRET, [[3, 12], [24, 12]])
        if t >= 1:
            gs.attempt_spawn(TURRET, [[4, 12], [23, 12]])
        # Upgrade progressively by turn 3.
        if t >= 2:
            gs.attempt_upgrade([[3, 12], [24, 12]])
        if t >= 3:
            gs.attempt_upgrade([[4, 12], [23, 12]])

    def _scout_rush_interceptor_reserve(self, gs):
        """v22: hold 2 Interceptors in reserve for peak Scout waves."""
        if not self.opp_is_scout_rush:
            return
        if gs.turn_number % 3 != 0:
            return
        if gs.get_resource(MP, 1) < 12:
            return
        interceptor_cost = gs.type_cost(INTERCEPTOR)[MP]
        for spawn in ([6, 8], [21, 8]):
    ...
    def on_action_frame(self, turn_string):
        ...
        # v22: count enemy mobile units on the board this frame; track turn peak.
        # p2Units slots: [0]Wall [1]Support [2]Turret [3]Scout [4]Demo [5]Interceptor
        p2 = state.get("p2Units") or []
        if len(p2) >= 6:
            enemy_mobiles = len(p2[3]) + len(p2[4]) + len(p2[5])
            if enemy_mobiles > self.enemy_mobile_peak_this_turn:
                self.enemy_mobile_peak_this_turn = enemy_mobiles
