# v4_layered — variant #4 of 4 v4 attempts. Inherits lostkids_v3 base
# (defense + offense) and ADDS a SECOND DEFENSIVE RING at y=4 to create
# a kill zone for any scout that breaks through the y=8 front line.
#
# Thesis: lostkids_v3's defense is single-front. Once scouts get past
# y=8 they walk freely to deep tiles like (11,2). Phase 8 adds 4
# upgraded turrets at (10,4), (11,4), (16,4), (17,4) — all interior,
# all open. Range 2.5 base / 3.5 upgraded covers the deep approach.
#
# Original lostkids_v3 commentary follows:
...
class AlgoStrategy(gamelib.AlgoCore):
    def __init__(self):
        ...
        random.seed(seed)
        gamelib.debug_write('v4_layered random seed: {}'.format(seed))

    def on_game_start(self, config):
        gamelib.debug_write('Configuring v4_layered...')
        self.config = config
    ...
    def on_turn(self, turn_state):
        ...
        try:
            ...
            gamelib.debug_write(
                "[v4_layered] watchdog fired after {}s".format(TURN_WATCHDOG_SECONDS)
            )
            used_fallback = True
        except Exception as exc:
            gamelib.debug_write(
                "[v4_layered] on_turn exception: {!r}\n{}".format(
        ...
        if used_fallback:
            try:
                ...
                gamelib.debug_write(
                    "[v4_layered] safe-fallback also failed: {!r}\n{}".format(
    ...
    def on_action_frame(self, turn_string):
        try:
            ...
            gamelib.debug_write(
                "[v4_layered] on_action_frame exception: {!r}".format(exc)
            )
