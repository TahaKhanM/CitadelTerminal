# v7_pathaware — path-aware scout spawn on top of v5_antiinterceptor

Targets the offensive failure mode observed in
`Ranked Replays/v4_smartdemo/v4_smartdemo_loss_python-algo-v5_1663_15310311.replay`,
where 17/19/21-scout waves at the canonical dual-spawn (23,9)+(24,10) ALL
DIED across 8 attack rounds (0 breaches) because the opponent's defense
heavily covered that path.

The fix replaces the fixed first/second spawn in `_execute_scout_attack`
with a path-aware pick: `_best_spawn(...)` scores 5 candidate edge tiles by
estimated total turret damage on their path to the enemy edge and picks
the lowest. The secondary spawn is one tile inboard on the same edge.

Everything else (early defensive interceptors, central turret at (13,11),
smartdemo cadence gated by `enemy_recent_demos < 2`, anti-scout / anti-demo
hardening) is inherited verbatim from v5_antiinterceptor.

---

# v4_smartdemo (parent docstring) — opponent-gated demolisher cadence on top of lostkids_v3
