# v7_pathaware — fork of v5_antiinterceptor with PATH-AWARE scout spawn.
# Targets the offensive failure mode in
#   Ranked Replays/v4_smartdemo/v4_smartdemo_loss_python-algo-v5_1663_15310311.replay
# where 17/19/21 scouts at the canonical dual-spawn (23,9)+(24,10) ALL DIED,
# scoring 0 breaches across 8 attack rounds because opp's defense covered
# that path heavily.
#
# Fix: in `_execute_scout_attack`, after the misdirection-clear branch, the
# canonical fixed (4,9)+(3,10) spawn is REPLACED with `_best_spawn(...)`
# which evaluates 5 candidate edge tiles ([2,11], [3,10], [4,9], [5,8],
# [6,7] mirrored for the opposite side) and picks the one with LOWEST
# estimated total turret damage on its path to the enemy edge. The
# secondary spawn is one tile away on the same edge, so both groups funnel
# into approximately the same path.
#
# Everything else (early interceptors, central turret, smartdemo cadence,
# anti-scout, anti-demo) is inherited unchanged from v5_antiinterceptor.
#
# v5_antiinterceptor — fork of v4_smartdemo with early-game anti-interceptor
# defense. Loss vs algo-v6 (1737 elo) showed opp sent 5 interceptors T0-T5
# that walked through our undefended middle and breached for 5 HP. Fix:
# pre-emptively spawn 1 defensive interceptor at (13,0) on T0-T4 + add a
# central turret at (13,11) at the start of Phase 2 of defense-order.
#
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def execute_turn_strategy(self, game_state):
        # NEW v5: pre-emptive defensive interceptor for early-game (T0-T5).
        # Loss vs algo-v6 (1737) showed opp sends interceptors T0-T5 that
        # walk through undefended middle and breach. Spawn 1 intc at (13,0)
        # to engage them mid-map. Cost: 1 MP/turn for 5 turns = 5 MP total.
    ...
    def _execute_scout_attack(self, game_state):
        ...
        # v7: PATH-AWARE primary spawn. The v4_smartdemo loss replay showed
        # 17/19/21 scouts at the canonical (4,9)+(3,10) spawn ALL DIED across
        # 8 attack rounds because opp's defense covered that path heavily.
        # _best_spawn evaluates 5 candidate edge tiles and picks the one with
        # LOWEST estimated total turret damage on its path to enemy edge.
        ...
        if best_spawn is None:
            best_spawn = [4, 9] if attack_side == "right" else [game_state.ARENA_SIZE - 5, 9]
        first_loc = list(best_spawn)
        # Secondary spawn one tile away (offset for natural funnel into the
        # same path as the primary). For 'right' attack-side the edge runs
        # diagonally toward x=13, so x-1, y+1 stays on/near the BOTTOM_LEFT
        # edge. For 'left' attack-side mirror it: x+1, y+1.
        if attack_side == "right":
            second_loc = [max(first_loc[0] - 1, 0), min(first_loc[1] + 1, 13)]
        else:
            second_loc = [min(first_loc[0] + 1, 27), min(first_loc[1] + 1, 13)]
        # Verify second_loc is in bounds and not blocked; fall back to first_loc.
        if (not game_state.game_map.in_arena_bounds(second_loc)
                or game_state.contains_stationary_unit(second_loc)):
            second_loc = first_loc
