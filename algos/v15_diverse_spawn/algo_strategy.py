"""v15 diverse-spawn — v13 + offensive spawn diversification.

Addresses §5.4 of REPLAY_ANALYSIS_2026-04-23_v12.md:
  §5.4 _best_spawn candidate set expanded from [13,0]/[14,0] to include
       the sidelane-edge tiles [3,10] (BOTTOM_LEFT edge) and [24,10]
       (BOTTOM_RIGHT edge). Against opponents with center-stacked
       defense, the least-damage spawn may be off-center. v12 always
       picks centre, so this breaks mirror symmetry: v15 can find lower-
       damage paths that v12 never evaluates.

Inherits §5.2 (second-ring [11,5]/[16,5]) and §5.6 (reactive fix).
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _attack_phase(self, gs):
        ...
        # §5.4 expanded spawn candidates — centre push + sidelane-edge
        # entries. [3,10] and [24,10] sit on our BOTTOM_LEFT/RIGHT edges.
        scout_opts = [[13, 0], [14, 0], [3, 10], [24, 10]]
        demo_opts = [[13, 0], [14, 0]]

        if enemy_front >= 10 and mp >= 4 * demo_cost:
            spawn_opts = [s for s in demo_opts
        ...
        if mp >= 10 * scout_cost:
            spawn_opts = [s for s in scout_opts
                          if gs.game_map.in_arena_bounds(s)
                          and not gs.contains_stationary_unit(s)]
            spawn = self._best_spawn(gs, spawn_opts)
            if spawn is not None:
