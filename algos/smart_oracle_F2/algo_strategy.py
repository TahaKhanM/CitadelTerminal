...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        try:
            ...
            gamelib.debug_write(f"[smart_oracle_F2] sim_rs path={p}")
        except Exception as e:
            gamelib.debug_write(f"[smart_oracle_F2] sim_rs probe failed: {e!r}")
        ...
        try:
            ...
            gamelib.debug_write(f"[smart_oracle_F2] opp_model: {stats}")
        except Exception as e:
            gamelib.debug_write(f"[smart_oracle_F2] opp_model init failed: {e!r}")
            self.opp_model = OpponentModel(rng_seed=42)

        gamelib.debug_write("[smart_oracle_F2] on_game_start complete")
    ...
    def on_turn(self, turn_state):
        ...
        try:
            ...
            if cur_turn != self._opp_classifier_turn_advanced_for:
                try:
                    ...
                    gamelib.debug_write(
                        f"[smart_oracle_F2] opp_classifier advance failed: {e!r}"
                    )
            ...
            try:
                ...
                gamelib.debug_write(
                    f"[smart_oracle_F2] opp_classifier classify failed: {e!r}"
                )
            ...
            gamelib.debug_write(
                f"[smart_oracle_F2] turn={gs.turn_number} "
            ...
            gamelib.debug_write(
                f"[smart_oracle_F2] WATCHDOG fired @ {TURN_WATCHDOG_S}s - "
                f"using safe fallback"
            )
            used_fallback = True
        except Exception as e:
            gamelib.debug_write(
                f"[smart_oracle_F2] on_turn exception: {e!r}\n{traceback.format_exc()}"
            )
            used_fallback = True
        finally:
            disarm()
        ...
        if used_fallback:
            ...
            gamelib.debug_write(
                f"[smart_oracle_F2] fallback count={self._fallback_count}/{self._turn_count}"
            )
