"""v14 supports — v13 + back-row Supports for Scout shielding/economy.

Addresses §5.5 of REPLAY_ANALYSIS_2026-04-23_v12.md:
  §5.5 Place 4 back-row Supports at Y=13. In competition config this
       gives 10.1 shield/Scout each (4 * 10.1 = 40 shield/Scout,
       Scout HP 15 → 55 effective). In the base-game engine used by
       the local test harness Supports instead generate +1 SP base /
       +2 MP upgraded per turn — either way, strong positive EV.
       Placements: [11,13]/[13,13]/[14,13]/[16,13] — off the
       [12,12]/[15,12] gap paths so they don't block our own Scouts.

Inherits §5.2 (second-ring [11,5]/[16,5]) and §5.6 (reactive fix) from
v13. Preserves v12's T0 opening and _best_spawn scorer.
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _spend_hoard(self, gs):
        """Convert idle SP into second-ring defences + back-row Supports.

        Second-ring turrets (§5.2) protect the dominant breach tiles.
        Back-row Supports (§5.5) at Y=13 shield Scouts in competition
        and generate resources in the local base-game engine.
        """
        t = gs.turn_number
        if t < 5:
            return
        ...
        # §5.5 Supports placed off the [12,12]/[15,12] gap paths so
        # our own Scouts can still exit through the wall gaps.
        supports = [[13, 13], [14, 13], [11, 13], [16, 13]]
