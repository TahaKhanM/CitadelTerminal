"""v20_hybrid_turtle_cracker: v13 + classifier-dispatched offense (turtle->Demolisher,
mixed->Scout burst). Parent: v13_second_ring. Tag: "classifier-dispatch".
Hypothesis: different opponents need different archetypes; turret-stacks die to
Demos (+8 HP vs +1 HP baseline); mixed opponents still fall to Scout burst.
"""
import gamelib
import json


# Support-train config copied from v20_demo_train. 4 upgraded Supports at Y=13
# flanking the two gaps in v13's y=12 wall (x=12 and x=15). Each upgraded
# Support gives 1 + 0.7*13 = 10.1 shield -> 4 Supports ~= 40 shield per Demo.
TRAIN_SUPPORTS = [[11, 13], [13, 13], [14, 13], [16, 13]]
TRAIN_PLACE_TURN = 2   # start placing Supports at T2 (only if turtle archetype)
TRAIN_UPGRADE_TURN = 3  # upgrade once placed

# Classifier parameters.
CLASSIFY_TURN = 5        # lock archetype at T5
TURTLE_TURRET_THRESHOLD = 6  # >=6 enemy 'DF' structures in y>=14 => turtle
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.scored_on = []
        self.path_cache = {}
        # Classifier state: set once at CLASSIFY_TURN, cached for the game.
        # Values: None (unclassified), "turtle", or "mixed".
    ...
    def on_turn(self, turn_state):
        ...
        # Lock archetype at CLASSIFY_TURN (once). Before that, act as "mixed"
        # (v13 default offense) which matches v13 baseline pre-lock behavior.
        if self.opponent_archetype is None and gs.turn_number >= CLASSIFY_TURN:
            self.opponent_archetype = self._classify_opponent(gs)

        self._build_defense(gs)
        self._spend_hoard(gs)
        # Only build the Support train if we've classified as turtle.
        if self.opponent_archetype == "turtle":
    ...
    def _classify_opponent(self, gs):
        """Count enemy Turret ('DF') structures in y>=14 (opponent's half).

        TURRET shorthand is 'DF' per the pre-rename live config (runtime
        shorthands are still FF/EF/DF/PI/EI/SI). We read TURRET dynamically
        from config at on_game_start, so this works regardless of the
        shorthand string in use.

        Returns "turtle" if >= TURTLE_TURRET_THRESHOLD turrets, else "mixed".
        """
        n_turrets = 0
        for x in range(0, 28):
            for y in range(14, 28):
                ...
                for u in gs.game_map[x, y]:
                    if u.player_index == 1 and u.stationary and u.unit_type == TURRET:
                        n_turrets += 1
        if n_turrets >= TURTLE_TURRET_THRESHOLD:
            return "turtle"
        return "mixed"

    def _support_train(self, gs):
        """Place & upgrade the 4-Support train at Y=13 (turtle branch only).

        Scheduled from T2; attempt_spawn/upgrade silently no-op on SP
        shortfall. Re-attempted every turn so late SP surplus still
        completes the train. Copied verbatim from v20_demo_train.
        """
        t = gs.turn_number
    ...
    def _attack_phase(self, gs):
        """Classifier-dispatched offense.

        - turtle (>=6 enemy turrets at T5): Demolisher-primary offense, copy
          of v20_demo_train._attack_phase. Demos (range 4.5) outrange base
          Turrets (2.5) so from y=10 they chip safely; the Support train
          gives ~40 shield per Demo.
        - mixed (or pre-T5): v13's default Scout-burst logic (with big-front
          Demolisher trigger on enemy_front >= 10).
        """
        ...
        if self.opponent_archetype == "turtle":
            # --- v20_demo_train offense (Demolisher primary, Scout fallback) ---
            if mp >= 4 * demo_cost:
        ...
        # --- v13 default offense (Scout burst + big-front Demolisher trigger) ---
