...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _execute_scout_attack(self, game_state):
        ...
        # v5: conservative path-aware scout. Default to canonical Lostkids
        # (4,9)+(3,10), only switch if alternative tile is >= 30% less damaged.
        ...
        if attack_side == "right":
            canonical_first = [4, 9]
            canonical_second = [3, 10]
            alt_cands = [[5, 8], [13, 0], [14, 0]]
            target = game_state.game_map.TOP_RIGHT
        else:
            canonical_first = [game_state.ARENA_SIZE - 4 - 1, 9]
            canonical_second = [game_state.ARENA_SIZE - 3 - 1, 10]
            alt_cands = [[game_state.ARENA_SIZE - 5 - 1, 8], [14, 0], [13, 0]]
            target = game_state.game_map.TOP_LEFT
        canonical_dmg = self._path_damage_v5(game_state, canonical_first, target)
        first_loc = canonical_first
        best_dmg = canonical_dmg
        for alt in alt_cands:
            if not game_state.game_map.in_arena_bounds(alt):
                continue
            if game_state.contains_stationary_unit(alt):
                continue
            alt_dmg = self._path_damage_v5(game_state, alt, target)
            if alt_dmg < canonical_dmg * 0.7:
                if alt_dmg < best_dmg:
                    first_loc = alt
                    best_dmg = alt_dmg
        second_loc = canonical_second
        if game_state.contains_stationary_unit(canonical_second) or canonical_second == first_loc:
    ...
    def _path_damage_v5(self, gs, spawn, target_edge):
