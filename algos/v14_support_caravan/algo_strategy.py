"""v14 = v13_second_ring + Support-caravan offense.

Hypothesis (from ADVANCED_STRATEGIES_PROMPT §5.1):
  None of v1-v13 used Supports. An upgraded Support at Y=13 grants 10.1
  shield per Scout at range 7. Four back-row upgraded Supports at
  [12..15, 13] give +40 shield/Scout → each Scout effectively 55 HP,
  letting a 10-Scout wave punch through mid-tier defenses.

Cost plan: 4 × (4 SP base + 4 SP upgrade) = 32 SP total, scheduled over
T8-T11 so it does NOT compete with v13's opening (40 SP through T7). We
do NOT touch offensive spawn logic — existing Scout / Demolisher triggers
get the shields "for free".

Everything else inherits verbatim from v13_second_ring:
  §5.2 second-ring Turrets at [11,5]/[16,5]
  §5.6 reactive defense with inward-tile preference
  deep sidelane [7,9]/[20,9], _best_spawn path scorer, _pressure_response
"""
import gamelib
import json


# Support-caravan config. Module-level so it's greppable & evolvable.
# IMPORTANT: v13's wall line at y=12 has gaps at x=12 and x=15 — our own
# Scouts exit the board through those gaps. Placing Supports at (12,13) or
# (15,13) would seal them and cause the 39-39 stalemate observed in the
# v14-vs-v13 mirror. Flank the gaps instead: (11,13)(16,13) inner pair,
# (10,13)(17,13) outer pair. Shield/unit at y=13 is the same 10.1 regardless
# of x (see UNITS_REFERENCE.md).
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _support_caravan(self, gs):
        """Place & upgrade 4 Supports at Y=13.

        Schedule: inner pair at T8, upgrade T9; outer pair T10, upgrade T11.
        attempt_spawn/upgrade silently no-op when SP is short, which lets
        the outer pair defer if an earlier turn drained SP. We re-attempt
        every subsequent turn so late-game SP surplus still funds the
        caravan.
        """
        t = gs.turn_number
