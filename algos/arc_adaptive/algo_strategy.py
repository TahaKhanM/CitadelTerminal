...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        # phalanx_v2 deltas: demolisher specialist.
        # Trigger strength = 10: fire demos at any meaningful corner defense.
        # ATTACK_MP_THRESHOLD = 12: fire scout/demo at minimal MP.
        # DEMOLISHER_MIN_MP = 9: 3 demos = 9 MP, our minimum demo wave.
        # Lookback = 1: pivot to demos after a single failed scout wave.
        DEMOLISHER_TRIGGER_STRENGTH = 10
        ...
        # arc_adaptive: opponent profile tracking. Built during T0-T8 from
        # on_action_frame counts. Profile = "rusher" / "demoer" / "hoarder"
        # / "balanced". Each profile maps to per-turn threshold overrides.
        self.opp_total_scouts = 0
        self.opp_total_demos = 0
        self.opp_total_interceptors = 0
    ...
    def _classify_opp(self, t):
        """Classify opponent profile. Lock-in by T8."""
        if self.profile_locked:
            return self.profile
        if t < 8:
            # Pre-lock: provisional read.
            if self.opp_total_demos >= 2 and self.opp_first_attack_turn <= 6:
                return "demoer"
            if self.opp_total_scouts >= 6 and self.opp_first_attack_turn <= 6:
                return "rusher"
            return "balanced"
        # Lock at T8.
        self.profile_locked = True
        if self.opp_total_demos >= 3:
            self.profile = "demoer"
        elif self.opp_total_scouts >= 8 and self.opp_first_attack_turn <= 8:
            self.profile = "rusher"
        elif self.opp_first_attack_turn == -1 or self.opp_first_attack_turn >= 12:
            self.profile = "hoarder"
        else:
    ...
    def evaluate_next_turn_strategy(self, game_state):
        self.my_MP = game_state.get_resource(MP, 0)
        # arc_adaptive: profile-driven threshold overrides.
        profile = self._classify_opp(game_state.turn_number)
        # rusher → don't attack early, build defense. raise threshold.
        # hoarder → attack aggressively while they're hoarding. lower threshold.
        # demoer → switch to demos earlier (but defense is JSON-driven, can't easily change here).
        # balanced → default.
        if profile == "rusher" and game_state.turn_number < 12:
            adj_threshold = ATTACK_MP_THRESHOLD + 4
        elif profile == "hoarder":
            adj_threshold = max(8, ATTACK_MP_THRESHOLD - 3)
        else:
            adj_threshold = ATTACK_MP_THRESHOLD
        if self.my_MP < adj_threshold:
    ...
    def on_action_frame(self, turn_string):
        try:
            ...
            if state["turnInfo"][0] == 1 and state["turnInfo"][2] == 0:
                ...
                # arc_adaptive: lifetime tallies for profile classification.
                e_ints = sum(1 for s in spawns
                             if len(s) >= 4 and s[3] == 2 and s[1] == 5)
                self.opp_total_demos += e_demos
                self.opp_total_scouts += e_scouts
                self.opp_total_interceptors += e_ints
                if self.opp_first_attack_turn == -1:
                    if e_demos > 0 or e_scouts > 0 or e_ints > 0:
                        try:
