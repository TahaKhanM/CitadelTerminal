        ...
        if self.action_predictor is not None and self._posterior is not None:
            try:
                try:
                    ...
                    opp_mp = 0.0
                # Athena v5: k=15 (was k=3) to give beam search a much
                # wider opponent-response set. Each (candidate, opp_action)
                # pair is one sim — increasing k linearly increases the
                # work pumped into sim_rs. At k=15 with ~200 candidates
                # we'll have ~3,000 sims/turn vs the previous ~15.
                opp_actions = self.action_predictor.top_k(
                    {"mp": opp_mp, "turn": self.turn_count},
                    self._posterior, k=15,
                )
            except Exception as exc:  # noqa: BLE001
