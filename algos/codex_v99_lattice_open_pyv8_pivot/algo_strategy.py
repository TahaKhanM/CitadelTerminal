"""codex_v99_lattice_open_pyv8_pivot: v73 opening with non-lattice pyv8 pivot."""
import gamelib
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_turn(self, turn_state):
        ...
        cap_line = [[x, 13] for x in list(range(0, 13)) + list(range(15, 28))]
        for i in range(0, len(cap_line), 4):
            if t >= 12 + i // 4:
                gs.attempt_spawn(WALL, cap_line[i:i + 4])

        if t >= 16:
            gs.attempt_spawn(TURRET, [[13, 12], [14, 12], [4, 12], [23, 12]])
        if t >= 20:
            gs.attempt_upgrade([[13, 12], [14, 12], [4, 12], [23, 12]])

        if t >= 10 and not self._enemy_lattice(gs):
            self._attack_pyv8(gs)
        else:
            self._attack_lattice(gs)

        gs.submit_turn()

    def _attack_lattice(self, gs):
        t = gs.turn_number
    ...
    def _attack_pyv8(self, gs):
        ...
        if t >= 17 and mp >= interceptor_cost:
            gs.attempt_spawn(INTERCEPTOR, [7, 6] if t < 46 else [9, 4], 1)
            mp = gs.get_resource(MP)
