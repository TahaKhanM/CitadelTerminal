"""v20_flank_edge: v20_sidelane + forced edge-corner spawns to bypass central Support Caravan.
Parent: v20_sidelane -> v13_second_ring. Tag: "flank-edge-bypass".
Hypothesis: Support shield range=7 leaves x=0,27 uncovered; edge-path attacks take 0
shield support, so Caravan's shielding does nothing.
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.scored_on = []
        self.path_cache = {}
        # Flank-edge: alternate left/right whenever _pick_spawn_side returns
        # "center" (balanced opponent). Start on "left" at turn 0; flip after
        # each attack call so bursts alternate edges across turns.
        self.last_flank_side = "right"  # so first forced flank flips to "left"
    ...
    def _spawn_options_for_side(self, gs, side, role):
        ...
        Flank-edge: for "attack", bias hard toward the deep corners ([0,13] /
        [27,13]) so mobile units hug the edge and bypass any central Support
        Caravan (shield range 7 does not reach x=0 or x=27 at y=13).
        Returned options are filtered to in-bounds and unoccupied.
        """
        if side == "left":
            if role == "interceptor":
                opts = [[6, 7], [7, 6], [3, 10]]
            else:  # attack — deep-edge first
                opts = [[0, 13], [1, 12], [2, 11], [3, 10], [6, 7], [7, 6]]
        elif side == "right":
            if role == "interceptor":
                opts = [[21, 7], [20, 6], [24, 10]]
            else:  # attack — deep-edge first
                opts = [[27, 13], [26, 12], [25, 11], [24, 10], [21, 7], [20, 6]]
        else:  # center
            if role == "interceptor":
                opts = [[13, 0]]
            else:
    ...
    def _resolve_flank_side(self, gs):
        """Force a flank — never return 'center' for attacks.

        If _pick_spawn_side says "left"/"right", honor it. If it says
        "center" (balanced opponent), alternate by flipping last_flank_side.
        """
        side = self._pick_spawn_side(gs)
        if side == "center":
            side = "left" if self.last_flank_side == "right" else "right"
        return side
    ...
    def _attack_phase(self, gs):
        ...
        side = self._resolve_flank_side(gs)

        if enemy_front >= 10 and mp >= 4 * demo_cost:
            spawn_opts = self._spawn_options_for_side(gs, side, "attack")
            # Fallback to the opposite flank if this one is fully blocked;
            # only fall back to center as a last resort.
            if not spawn_opts:
                other = "right" if side == "left" else "left"
                spawn_opts = self._spawn_options_for_side(gs, other, "attack")
                if spawn_opts:
                    side = other
            if not spawn_opts:
            ...
            if spawn is not None:
                ...
                self.last_flank_side = side
                return

        # Flank paths are longer, so hoard a bit more MP before committing.
        if mp >= 11 * scout_cost:
            spawn_opts = self._spawn_options_for_side(gs, side, "attack")
            if not spawn_opts:
                other = "right" if side == "left" else "left"
