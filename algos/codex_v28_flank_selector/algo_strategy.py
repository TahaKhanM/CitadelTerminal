"""codex_v28_flank_selector: v25 plus flank-aware attack spawn selection.

Parent: v21_quadrant_adaptive_support / v21_temporal_phase_gate.
Tag: "quadrant-adaptive-support".
Hypothesis: the edge-caravan loss punishes center-only attacks. Keep v25's
burst timing, but let both Demolishers and Scouts choose among center and
flank deploy tiles using the existing path-damage scorer.
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _attack_phase(self, gs):
        ...
        if not deferring and enemy_front >= 10 and mp >= 4 * demo_cost:
            spawn_opts = [[13, 0], [14, 0], [3, 10], [24, 10], [6, 7], [21, 7]]
