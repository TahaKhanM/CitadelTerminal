...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _execute_demo_attack(self, game_state):
        ...
        # v3c: scout front-runners (speed 1 vs demo 0.5) walk ahead to
        # draw turret fire and chip walls. Reserve 30% of MP for them.
        scout_reserve_mp = min(int(my_mp * 0.30), 6)
        demo_mp_avail = my_mp - scout_reserve_mp
        demo_count = min(int(demo_mp_avail // demo_cost), 8)
        ...
        if scout_count > 0:
            # v3c: spawn scouts at SAME tile so they pace ahead of slower demos.
            game_state.attempt_spawn(SCOUT, spawn, scout_count)
