"""codex_v72_edge_caravan_wave24_patch4: wave24 with fourth low-lane patch.

Variant of codex_v68 that adds [9,4]/[18,4] turrets after the observed
fourth-lane leak.
"""
import gamelib
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_turn(self, turn_state):
        ...
        if t >= 28:
            second_edge = [[6, 7], [21, 7]]
            gs.attempt_spawn(TURRET, second_edge)
            if t >= 30:
                gs.attempt_upgrade(second_edge)
        if t >= 34:
            deep_edge = [[8, 5], [19, 5]]
            gs.attempt_spawn(TURRET, deep_edge)
            if t >= 36:
                gs.attempt_upgrade(deep_edge)
        if t >= 40:
            fourth_edge = [[9, 4], [18, 4]]
            gs.attempt_spawn(TURRET, fourth_edge)
            if t >= 42:
                gs.attempt_upgrade(fourth_edge)
        ...
        if t >= 10 and t % 2 == 0 and mp >= 24 * scout_cost:
        ...
        elif t >= 25 and mp >= 28 * scout_cost:
