...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def evaluate_next_turn_strategy(self, game_state):
        ...
        if self.my_MP < ATTACK_MP_THRESHOLD:
            ...
            return
        # v6: side rotation to defeat reactive defense.
        # Force-switch sides if we've attacked the same side 3+ times in a row
        # AND the strength gap is small (<25). Reactive defenders compound
        # turrets at the breach side; rotating evens the load.
        ls = self.compute_enemy_left_edge_defense_strength(game_state)
        rs = self.compute_enemy_right_edge_defense_strength(game_state)
        natural = "right" if ls > rs else "left"
        if (self.consecutive_same_side_attacks >= 3
                and self.last_attack_side == natural
                and abs(ls - rs) < 25):
            attack_side = "left" if natural == "right" else "right"
        else:
            attack_side = natural
        if attack_side == "right":
            for location in EDGE_BLOCK_LOCATIONS_RIGHT:
