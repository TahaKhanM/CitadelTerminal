# v6_smartspawn — fork of v5_antiinterceptor that fixes the misdirection-clear
# attack failure mode observed in ranked game 15310316 (loss to python-algo @
# 1765). v5/v4_smartdemo's `_execute_scout_attack` calls
# `ping_one_batch(game_state, [23,9])` (or [4,9]) when the opponent's edge
# is "misdirecting" — this dumps ALL ~17-22 scouts at a SINGLE tile. When
# the opponent's defense at the misdirection corner is heavy, the entire
# wave dies for 0 breaches.
#
# Concrete data from game 15310316:
#   - T22: 17 scouts at (23,9) → 0 breaches
#   - T28: 19 scouts at (23,9) → 0 breaches
#   - T33: 7 demos at (23,9) → 0 breaches
#   - T37: 21 scouts at (23,9) → 0 breaches
#   - T41: 22 scouts at (23,9) → 17 breaches (finally!)
# 86 MP burned for 0 result across 4 attacks; opp's late-game scout floods
# (21+23+25 scouts) finished us off.
#
# v6 fix: track each `ping_one_batch` attack's breach count via
# `misdir_ping_breaches`. If the latest TWO entries are < 5 each, skip the
# misdirection ping and fall back to the canonical dual-spawn at (4,9)+(3,10)
# (mirrored to (23,9)+(24,10) for left attacks). The dual-spawn forces the
# scouts to split paths — even if the corner is hard, the second group's
# offset path increases the chance of breaching.
#
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        # v6: track misdirection-ping success. If recent pings scored < 5 breaches,
        # stop trusting the misdirection-clear logic — opp likely has heavy defense
        # at the misdirection corner.
        self.misdir_ping_breaches = []  # breach counts from each ping_one_batch attack
    ...
    def _execute_scout_attack(self, game_state):
        ...
        # v6: skip misdirection ping if recent pings have failed.
        ...
        if attack_side == "left" and self.is_enemy_left_edge_misdirecting(game_state) and not self.enemy_left_edge_blocked:
            if not recent_misdir_failures:
                ...
                return
            # else fall through to canonical dual-spawn
