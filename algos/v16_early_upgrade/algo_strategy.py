"""v16 early-upgrade — v13 + T0 upgrade of sidelane_deep.

Addresses the deterministic local-engine symmetry: v12 self-play ties
at HP 39-39, with each side's only breach at (7,6)/(20,21) on T3 —
both tiles inside range-3.5 coverage of upgraded [7,9]/[20,9] but
OUTSIDE base range-2.5. By upgrading sidelane_deep at T0 (funded by
deferring inner_corners to T1), v16 kills the T3-breacher Scout at
(7,6) that v12 cannot, while v12 still takes the same breach — v16
wins by 1 HP.

Also inherits §5.2 (second-ring [11,5]/[16,5]) and §5.6 (reactive fix).
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _build_defense(self, gs):
        ...
        # v16 opening: drop inner_corners from T0 (4 SP saved), use for
        # T0 upgrade of sidelane_deep [7,9]/[20,9]. Upgraded sidelane
        # (range 3.5) covers the (7,6)/(20,21) BOTTOM-edge tiles that
        # decide every v12 self-play at T3 in local config.
        gs.attempt_spawn(TURRET, center_turrets)
        gs.attempt_spawn(TURRET, sidelane_deep)
        gs.attempt_spawn(TURRET, outer_turrets)
        gs.attempt_upgrade(sidelane_deep)  # T0 upgrade — 8 SP freed
