"""codex_v43_edge_caravan_pathpick: edge caravan with path-aware spawn choice.

Variant of codex_v38 that chooses the lowest-turret-damage spawn path from
center and side-edge options instead of hard alternation.
"""
import gamelib
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_turn(self, turn_state):
        ...
        if t >= 10 and t % 2 == 0 and mp >= 8 * scout_cost:
            spawn = self._best_spawn(gs, [[13, 0], [14, 0], [3, 10], [24, 10]])
            if spawn is not None:
                gs.attempt_spawn(SCOUT, spawn, 1000)
        elif t >= 14 and t % 5 in (1, 4) and mp >= 2 * demo_cost:
            spawn = self._best_spawn(gs, [[24, 10], [3, 10], [13, 0], [14, 0]])
            if spawn is not None:
    ...
    def _best_spawn(self, gs, options):
        ...
        for opt in options:
            if not gs.game_map.in_arena_bounds(opt) or gs.contains_stationary_unit(opt):
                continue
