"""codex_v93_priority_shieldlane: v86 with prioritized banana-lane Supports.
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _build_defense(self, gs):
        ...
        if lattice and t >= 18:
            gs.attempt_spawn(SUPPORT, [[7, 8], [8, 7]])
        if lattice and t >= 22:
            gs.attempt_upgrade([[7, 8], [8, 7]])
        if lattice and t >= 28:
            gs.attempt_spawn(SUPPORT, [[9, 6]])
        if lattice and t >= 32:
            gs.attempt_upgrade([[9, 6]])
