...
class GameState:
    ...
    def __init__(self, config, serialized_string):
        ...
        self._build_stack = []
        self._deploy_stack = []
        # athena path cache: per-GameState memoization of find_path_to_edge.
        # Key = (start_x, start_y, target_edge). Caller invalidates with
        # _invalidate_path_cache() after any attempt_spawn / attempt_remove
        # that could change the walkable grid. SimCore bulk-evaluates many
        # plans on identical structure layouts, which is where this pays.
        self._path_cache = {}
    ...
    def find_path_to_edge(self, start_location, target_edge=None):
        ...
        cache_key = (start_location[0], start_location[1], target_edge)
        cached = self._path_cache.get(cache_key)
        if cached is not None:
            return [list(p) for p in cached]

        end_points = self.game_map.get_edge_locations(target_edge)
        path = self._shortest_path_finder.navigate_multiple_endpoints(start_location, end_points, self)
        if path is not None:
            self._path_cache[cache_key] = tuple(tuple(p) for p in path)
        return path

    def _invalidate_path_cache(self):
        """Clear the find_path_to_edge memoization. Call after any structural mutation."""
