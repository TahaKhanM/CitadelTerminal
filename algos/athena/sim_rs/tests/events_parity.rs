...
//!
//! After the UID-intern refactor the keys are u32 instead of String. The
//! Python reference (which still emits string UIDs like "S1", "T2") maps to
//! the same canonical order because both sides use ascending numeric order —
//! we simply pick u32 values whose relative order matches the Python test
//! fixture (S1=1 < S2=2, etc).
//!
//! Engine citations:
...
use sim_rs::events::{bucket_keys, canonicalize_bucket, BucketKey, EventEntry};

// Shorthand u32 UIDs chosen so numeric order matches the Python fixture's
// lex order (S1<S2, V1<V2, M1<M2<M3, A<B).
const S1: u32 = 1;
const S2: u32 = 2;
const T1: u32 = 11;
const T2: u32 = 12;
...
fn shield_canonical_order() {
    ...
    let mut entries = vec![
        EventEntry::Shield {
            ...
            src_uid: S2, tgt_uid: T1, pid: 1,
        }, // E1
        EventEntry::Shield {
            src_xy: (0, 0), tgt_xy: (1, 1),
            amount: 5.0, type_id: 2,
            src_uid: S1, tgt_uid: T2, pid: 1,
        }, // E2
        EventEntry::Shield {
            src_xy: (0, 0), tgt_xy: (1, 1),
            amount: 5.0, type_id: 2,
            src_uid: S1, tgt_uid: T1, pid: 1,
        }, // E3
        EventEntry::Shield {
            src_xy: (0, 0), tgt_xy: (1, 1),
    ...
    let summaries: Vec<(u32, u32, f32)> = entries
        .iter()
        .map(|e| match e {
            EventEntry::Shield { src_uid, tgt_uid, amount, .. } => (*src_uid, *tgt_uid, *amount),
...
fn damage_canonical_order() {
    ...
    let mut entries = vec![
        EventEntry::Damage { xy: (5, 5), amount: 10.0, type_id: 3,
            victim_uid: V2, pid: 1 }, // D1
        EventEntry::Damage { xy: (4, 4), amount: 10.0, type_id: 3,
            victim_uid: V1, pid: 1 }, // D2
        EventEntry::Damage { xy: (4, 4), amount: 5.0,  type_id: 3,
            victim_uid: V1, pid: 1 }, // D3
        EventEntry::Damage { xy: (3, 3), amount: 10.0, type_id: 3,
            victim_uid: V1, pid: 1 }, // D4
    ];

    canonicalize_bucket("damage", &mut entries);

    let summaries: Vec<(u32, f32, (i32, i32))> = entries
        .iter()
        .map(|e| match e {
            EventEntry::Damage { victim_uid, amount, xy, .. } => (*victim_uid, *amount, *xy),
            _ => unreachable!(),
        })
        .collect();
...
fn death_canonical_order() {
    ...
    let mut entries = vec![
        EventEntry::Death { xy: (1, 1), type_id: 3, uid: M3, pid: 1, removed: false }, // DE1
        EventEntry::Death { xy: (2, 2), type_id: 3, uid: M1, pid: 1, removed: true  }, // DE2
        EventEntry::Death { xy: (2, 2), type_id: 3, uid: M1, pid: 1, removed: false }, // DE3
        EventEntry::Death { xy: (3, 3), type_id: 3, uid: M2, pid: 1, removed: false }, // DE4
    ];

    canonicalize_bucket("death", &mut entries);

    let summaries: Vec<(u32, bool)> = entries
        .iter()
        .map(|e| match e {
            EventEntry::Death { uid, removed, .. } => (*uid, *removed),
            _ => unreachable!(),
        })
        .collect();
    ...
    assert_eq!(
        summaries,
        vec![(M1, false), (M1, true), (M2, false), (M3, false),]
    );
}
...
fn self_destruct_canonical_order_and_target_sort() {
    ...
    let summaries: Vec<(u32, (i32, i32), f32, Vec<(i32, i32)>)> = entries
        .iter()
        .map(|e| match e {
    ...
    assert_eq!(
        summaries,
        vec![
            (A, (1, 1), 5.0, vec![(2, 2), (3, 3)]),
            (A, (1, 1), 5.0, vec![(2, 2), (3, 3)]),
            (A, (1, 1), 5.0, vec![(2, 2), (9, 9)]),
            (B, (5, 5), 2.0, vec![(6, 6), (7, 7)]),
        ]
    );
}
...
fn float_bit_equality_f32_roundtrip() {
    ...
    let keys = bucket_keys("shield", &[k_a, k_b]);
