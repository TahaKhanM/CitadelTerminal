...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _attack_phase(self, gs):
        ...
        # v2 LADDER FIX: Demolisher line was firing too eagerly on the live
        # ladder (vs python-funnel: 205 demos died for 0 breaches). Demos
        # die at (12,12) gap exit (5 HP vs 20 dmg/frame upgraded turret).
        # DISABLED: demos rarely useful in v2's horizontal-line topology.
        ...
        if mp >= 9 * scout_cost:
            ...
            # Pick the spawn via lowest-damage path (proven reliable for v1
            # mirror match where sim is brittle).
            spawn = self._best_spawn(gs, spawn_opts)
            if spawn is None:
                return

            # v2 LADDER FIX: HOARD check via simulator. If sim predicts 0
            # breaches at our chosen spawn AND opp has strong defense
            # (corner OR center), save MP for bigger wave.
            best_br, _ = self._simulate_attack(gs, spawn, SCOUT, n_scouts)
            opp_max_corner_for_hoard = max(
                self._compute_opp_corner_strength_v2(gs, 'right_zone'),
                self._compute_opp_corner_strength_v2(gs, 'left_zone'),
            )
            opp_center_strength = 0.0
            for cx in range(10, 18):
                for cy in (14, 15, 16):
                    if not gs.game_map.in_arena_bounds([cx, cy]):
                        continue
                    unit = gs.contains_stationary_unit([cx, cy])
                    if unit and unit.unit_type == TURRET:
                        opp_center_strength += 5.0 if unit.upgraded else 1.5
            opp_strong = (opp_max_corner_for_hoard >= 25 or
                          opp_center_strength >= 30)
            if best_br == 0 and n_scouts < 16 and opp_strong:
                if not hasattr(self, '_hoard_count'):
                    self._hoard_count = 0
                self._hoard_count += 1
                if self._hoard_count < 4:
                    return  # Skip — hoard MP for bigger wave
            if hasattr(self, '_hoard_count'):
                self._hoard_count = 0

            opp_max_corner = opp_max_corner_for_hoard
            # Multi-prong: try splits if we have multiple options and many scouts
            if n_scouts >= 13 and len(spawn_opts) >= 2:
                second = self._second_best_spawn(gs, spawn_opts, spawn)
                if second is not None:
                    splits = [(0.65, 0.35), (0.5, 0.5), (0.7, 0.3)]
                    best_split_br = best_br
                    best_split = None
