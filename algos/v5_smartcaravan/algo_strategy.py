# v5_smartcaravan — hybrid of v4_smartdemo offense gating + v4_caravan
# Y=13 support placement. Combines the two strongest v4 offensive elements
# in one defense-order.
#
# From v4_smartdemo (offense): scout-success backstop + every-3rd-attack
# demolisher cadence GATED by `enemy_recent_demos < 2`. Won 17-3 vs
# baseline LostKids and 10-0 vs v13_second_ring.
#
# From v4_caravan (shielding): Phase-5 supports relocated from Y=5/6 to
# Y=13 (the deploy edge), maximizing per-Scout shield via the upgraded
# formula (1 + 0.7*Y). Won 14-6 vs baseline but lost 0-10 vs v13.
#
# Hypothesis: v13's edge over caravan came from caravan's offense being
# too vanilla. The smartdemo gate (which closes vs v13) plus the
# higher-shield caravan placement should give us BOTH offensive boosts
# without re-introducing the v13 weakness.
#
# Original v4_smartdemo / lostkids_v3 docstring follows.
#
