"""codex_v27_edge_priority: v25 burst cadence plus priority edge keeps.

Parent: v21_quadrant_adaptive_support / v21_temporal_phase_gate.
Tag: "quadrant-adaptive-support".
Hypothesis: the ranked v21 loss exposes a real low-edge structural weakness.
Build upgraded low-edge turrets before adaptive Supports so shielded edge waves
cannot cascade through (7,6)/(20,6)/(6,7)/(21,7)-style tiles.
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _edge_keep(self, gs):
        """Pre-build the low-edge hardpoints missing in the v21 ranked loss.

        The failure tiles are either on or just above the deploy edges. Exact
        tile occupation is valuable here: even if a Turret is eventually killed,
        it blocks the clean edge route and forces Scouts to spend more frames
        under upgraded-Turret fire.
        """
        t = gs.turn_number
        if t < 8:
            return

        edge_primary = [[7, 6], [20, 6]]
        edge_secondary = [[6, 7], [21, 7]]
        edge_outer = [[4, 9], [23, 9]]
        edge_walls = [[5, 8], [22, 8], [8, 7], [19, 7]]

        gs.attempt_spawn(TURRET, edge_primary)
        if t >= 10:
            gs.attempt_upgrade(edge_primary)

        if t >= 12:
            gs.attempt_spawn(TURRET, edge_secondary)
        if t >= 14:
            gs.attempt_upgrade(edge_secondary)

        if t >= 16:
            gs.attempt_spawn(WALL, edge_walls)
        if t >= 18:
            gs.attempt_upgrade(edge_walls)

        if t >= 20:
            gs.attempt_spawn(TURRET, edge_outer)
        if t >= 22:
            gs.attempt_upgrade(edge_outer)
