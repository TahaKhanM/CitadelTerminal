...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _execute_defense(self, game_state):
        ...
        # v4b: predictive interceptor screen when enemy hoards >= 10 MP.
        # Interceptors at (6,7)/(21,7) walk into the V-funnel path and
        # collide with incoming scouts (20 dmg/frame vs mobile = 1-shots Scout).
        if self.enemy_MP >= 10 and game_state.turn_number >= 6:
            ic_cost = game_state.type_cost(INTERCEPTOR)[MP]
            my_mp = game_state.get_resource(MP, 0)
            if my_mp >= ic_cost:
                # Spawn 1 interceptor on the side enemy is more likely to attack
                # (heuristic: weaker defense = where they spawned-and-saved
                # offensive MP). We don't have great signals; pick by recent
                # scored_on locations.
                left_scored = sum(1 for loc in self.scored_on_locations[-6:] if loc[0] < 14)
                right_scored = sum(1 for loc in self.scored_on_locations[-6:] if loc[0] >= 14)
                if left_scored >= right_scored:
                    spawn = [6, 7]
                else:
                    spawn = [21, 7]
                if (game_state.game_map.in_arena_bounds(spawn)
                        and not game_state.contains_stationary_unit(spawn)):
