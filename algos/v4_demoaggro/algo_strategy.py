...
class AlgoStrategy(gamelib.AlgoCore):
    def __init__(self):
        ...
        random.seed(seed)
        gamelib.debug_write('v4_demoaggro random seed: {}'.format(seed))

    def on_game_start(self, config):
        gamelib.debug_write('Configuring v4_demoaggro...')
        self.config = config
        ...
        # v4_demoaggro: counter of attack decisions (defend turns excluded).
        # Every 3rd attack (counter % 3 == 2) becomes a demolisher wave.
        self.attack_count = 0
    ...
    def evaluate_next_turn_strategy(self, game_state):
        ...
        # v4_demoaggro: every 3rd attack is a proactive demolisher wave.
        # Backstop (scouts_failing + losing_hp_race) still applies and can
        # also force demos if it's not already a demo turn.
        force_demo_cycle = (self.attack_count % 3) == 2
        if force_demo_cycle and self.my_MP >= DEMOLISHER_MIN_MP:
            self.turn_strategy = f"attack_demo_{attack_side}"
        elif scouts_failing and losing_hp_race and self.my_MP >= DEMOLISHER_MIN_MP:
            self.turn_strategy = f"attack_demo_{attack_side}"
        else:
            self.turn_strategy = f"attack_{attack_side}"

        # Increment attack counter (only counts attack-mode decisions, not defends).
        self.attack_count += 1
