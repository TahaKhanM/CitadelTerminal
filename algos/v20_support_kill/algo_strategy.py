"""v20_support_kill: v13 + anti-caravan Demolisher attack targeting enemy Supports directly.
Parent: v13_second_ring. Tag: "support-kill".
Hypothesis: killing the Caravan's Supports neutralizes v14's shield engine; once they're
dead, subsequent Scouts/Demos do full damage.
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _detect_caravan(self, gs):
        """Count enemy Supports (shorthand 'EF') in opponent territory y>=14."""
        n = 0
        for y in range(14, 28):
            for x in range(0, 28):
                ...
                for u in gs.game_map[x, y]:
                    if u.player_index == 1 and u.unit_type == 'EF':
                        n += 1
        return n >= 3
    ...
    def _attack_phase(self, gs):
        ...
        # Anti-caravan: 5 Demos + tank Interceptor regardless of front density.
        if self._detect_caravan(gs) and mp >= 5 * demo_cost:
            spawn = [13, 0]
            if gs.game_map.in_arena_bounds(spawn) and not gs.contains_stationary_unit(spawn):
                gs.attempt_spawn(DEMOLISHER, spawn, 5)
                for tank_loc in ([10, 3], [17, 3]):
                    if (gs.get_resource(MP) >= int_cost
                            and gs.game_map.in_arena_bounds(tank_loc)
                            and not gs.contains_stationary_unit(tank_loc)):
                        gs.attempt_spawn(INTERCEPTOR, tank_loc)
                        break
                return
