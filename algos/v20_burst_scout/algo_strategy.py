"""v20_burst_scout: v13 + mass-Scout burst (threshold 15x, or 18x if caravan detected).
Parent: v13_second_ring. Tag: "burst-anti-caravan".
Hypothesis: Support shield regen is capped per-frame; massing 15+ Scouts single frame
outpaces shield and forces breach damage through.
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _attack_phase(self, gs):
        ...
        # v20: raise Scout-burst threshold to 15x (18x if enemy Support caravan on path).
        # Fallback dump if MP >= 25 (near-decay limit) even without caravan detection.
        spawn_opts = [[13, 0], [14, 0]]
        spawn_opts = [s for s in spawn_opts
                      if gs.game_map.in_arena_bounds(s)
                      and not gs.contains_stationary_unit(s)]
        spawn = self._best_spawn(gs, spawn_opts)
        if spawn is not None:
            caravan = self._caravan_on_path(gs, spawn)
            threshold = (18 if caravan else 15) * scout_cost
            if mp >= threshold or mp >= 25:
                gs.attempt_spawn(SCOUT, spawn, 1000)

    def _caravan_on_path(self, gs, spawn):
        """Return True if >=3 enemy Supports ('EF') lie within Manhattan 7 of any path tile."""
        ...
        for x in range(28):
            for y in range(14, 28):
                ...
                for u in gs.game_map[x, y]:
                    # Live-server shorthand for Support is 'EF' (pre-rename code).
