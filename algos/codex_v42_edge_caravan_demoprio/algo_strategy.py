"""codex_v42_edge_caravan_demoprio: edge caravan with Demolisher priority.

Variant of codex_v38 that gives scheduled flank Demolishers priority over
Scout waves on overlapping turns, probing whether edge structure removal is
worth more than cadence.
"""
import gamelib
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_turn(self, turn_state):
        ...
        if t >= 14 and t % 5 in (1, 4) and mp >= 2 * demo_cost:
            spawn = [24, 10] if t % 10 < 5 else [3, 10]
            gs.attempt_spawn(DEMOLISHER, spawn, 1000)
        elif t >= 10 and t % 2 == 0 and mp >= 8 * scout_cost:
