"""codex_v31_timed_edge: support-scout counter plus timed edge reserve.

Parent: v21_quadrant_adaptive_support / v21_temporal_phase_gate.
Tag: "quadrant-adaptive-support".
Hypothesis: permanent side Interceptors delay the loss but starve offense.
Use a narrow two-lane reserve during the first edge-collapse window, then
release MP back into the Scout counterpunch.
"""
import gamelib
import json
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def _pressure_response(self, gs):
        ...
        interceptor_cost = gs.type_cost(INTERCEPTOR)[MP]
        if 36 <= gs.turn_number <= 50 and enemy_mp >= 8 * scout_cost:
